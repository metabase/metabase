...
function setup({
  ...
  return renderWithProviders(
    <>
      <Route path={CONVERSATIONS_PATH} component={ConversationsPage} />
      <Route
        path={`${CONVERSATIONS_PATH}/:conversationId`}
        component={() => <div data-testid="conversation-detail-page" />}
      />
    </>,
    {
      withRouter: true,
...
describe("ConversationsPage", () => {
  describe("table", () => {
    it("renders the user, profile label and IP address of each conversation", async () => {
      setup();

      await screen.findByText("Bobby Tables");
    ...
    it("opens the conversation detail page when a row is clicked", async () => {
      ...
      await userEvent.click(await screen.findByText("Bobby Tables"));
  ...
  describe("pagination", () => {
    it("requests and renders the next page of conversations", async () => {
      ...
      expect(await screen.findByText("10.0.0.0")).toBeInTheDocument();
      await userEvent.click(screen.getByLabelText("Next page"));

      await assertRequestedWithParams({ offset: "25", limit: "25" });
      expect(await screen.findByText("10.0.0.25")).toBeInTheDocument();
      expect(screen.queryByText("10.0.0.0")).not.toBeInTheDocument();
    });
  });
});
