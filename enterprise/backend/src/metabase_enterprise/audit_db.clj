...
(defn- install-database!
  ...
  - This uses a weird ID because some tests were hardcoded to look for database with ID = 2, and inserting an extra db
    without an app-chosen id throws that off since the IDs are sequential."
  [engine id]
  ;; guard against manual deletion the audit-db entry, but not removing the audit-db permissions.
  ...
  (let [audit-db (t2/select-one Database :is_audit true)]
    (cond
      ...
      (u/prog1 ::installed
        ...
        (install-database! mdb.env/db-type (default-audit-db-id)))
