...
(defn- dirty-where
  ...
  sync is off), and ledger scoping — the given worktree's, or with a nil `worktree-id` the main app's
  (rows with no worktree reference), so one worktree's unpushed changes never dirty another's flows."
  [worktree-id]
