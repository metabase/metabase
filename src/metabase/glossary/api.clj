...
(api.macros/defendpoint :get "/"
  ...
  {:data (t2/hydrate (glossary.db/select-glossary-entries-matching-search search) :creator)})
...
(api.macros/defendpoint :post "/"
  ...
  (let [glossary (glossary.db/insert-glossary!
                  {:term       term
...
(api.macros/defendpoint :put "/:id"
  ...
  [{:keys [id]} :- [:map {:closed true} [:id ms/PositiveInt]]
   ...
   {:keys [term definition]} :- [:map {:closed true}
                                 ...
                                 [:definition ms/NonBlankString]]]
  (api/check-data-analyst)
  (let [previous-glossary (api/check-404 (glossary.db/select-one-glossary {:id id}))]
    (glossary.db/update-glossaries! {:id id} {:term term :definition definition})
    (let [glossary (glossary.db/select-one-glossary {:id id})]
...
(api.macros/defendpoint :delete "/:id"
  ...
  [{:keys [id]} :- [:map {:closed true} [:id ms/PositiveInt]]]
  (api/check-data-analyst)
  (let [glossary (api/check-404 (glossary.db/select-one-glossary {:id id}))]
