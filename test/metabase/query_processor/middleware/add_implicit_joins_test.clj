  ...
  (let [query (-> (lib/query
    ...
    (is (=? (lib.tu.macros/$ids orders
              {:source-table $$orders
               ...
               :order-by     [[:asc $id]]
               :limit        2})
            (-> (#'qp.add-implicit-joins/add-implicit-joins-to-stage query [:stages 0] (lib/query-stage query 0))
                lib/->legacy-MBQL
                :query
                :source-query)))))
    ...
    (is (=? (lib.tu.macros/mbql-query venues
              {:source-table $$venues
               ...
               :joins        [{:source-table $$categories
                               ...
                               :condition    [:= [:field %category-id {}] &CATEGORIES__via__CATEGORY_ID.categories.id]
                               :strategy     :left-join
                               :fields       :none
