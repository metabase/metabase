(ns ^:mb/driver-tests metabase.data-studio.api.table-test
  "Tests for /api/data-studio/table endpoints."
  (:require
...
(deftest ^:parallel non-admins-cant-trigger-bulk-sync-test
  (testing "Non-admins should not be allowed to trigger sync"
    (is (= "You don't have permissions to do that."
           (mt/user-http-request :rasta :post 403 "data-studio/table/sync-schema" {:database_ids [(mt/id)]})))))
...
(deftest ^:parallel non-admins-cant-trigger-bulk-rescan-values-test
  (testing "Non-admins should not be allowed to trigger rescan values"
    (is (= "You don't have permissions to do that."
           (mt/user-http-request :rasta :post 403 "data-studio/table/rescan-values" {:database_ids [(mt/id)]})))))
...
(deftest ^:parallel non-admins-cant-trigger-bulk-discard-values-test
  (testing "Non-admins should not be allowed to trigger discard values"
    (is (= "You don't have permissions to do that."
           (mt/user-http-request :rasta :post 403 "data-studio/table/discard-values" {:database_ids [(mt/id)]})))))

(deftest bulk-discard-values-test
  (mt/with-empty-h2-app-db!
    (testing "POST /api/data-studio/table/discard-values"
      (mt/with-temp
...
(deftest selection-basic-test
  (testing "POST /api/data-studio/table/selection"
    (mt/with-temp [:model/Database {db-id :id} {}
...
(deftest selection-with-remapping-test
  (testing "POST /api/data-studio/table/selection with remapping dependencies"
    (mt/with-temp [:model/Database  {db-id :id}    {}
...
(deftest selection-recursive-upstream-test
  (testing "POST /api/data-studio/table/selection with recursive upstream dependencies"
...
(deftest selection-recursive-downstream-test
  (testing "POST /api/data-studio/table/selection with recursive downstream dependencies"
