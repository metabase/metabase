...
describe("StaticQuestion - multiple guest questions", () => {
  ...
  it("should keep distinct questions when multiple guest StaticQuestions mount under one provider", async () => {
    ...
    // Mount a second guest question under the same provider/store. Each card
    // writes the shared Redux guest token; without prop-token precedence the
    // first card re-queries with the second card's JWT and both show Bravo.
    rerender(
      <div>
        <StaticQuestion token={tokens[0]} />
