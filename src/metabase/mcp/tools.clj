...
;; TODO: Clean up defenterprise seams after agent API is migrated to OSS
...
(defenterprise generate-manifest
  "Generate tools manifest from agent API endpoint metadata.
   OSS impl returns an empty manifest since agent API is not available."
  metabase-enterprise.mcp.tools
  []
...
(defenterprise deliver-agent-api-response
  "Dispatch to agent API routes and deliver response to promise.
   OSS impl returns 402 since agent API requires a premium token."
  metabase-enterprise.mcp.tools
  [result _method _path _token-scopes _body]
  (deliver result {:status 402 :body {:message "Agent API requires a premium token with the :agent-api feature."}}))
