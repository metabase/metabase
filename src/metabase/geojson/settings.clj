...
(defn builtin-geojson
  "Built-in GeoJSON settings, if the [[default-maps-enabled]] Setting is set."
  []
...
(defn invalid-location-msg
  "Error message for invalid GeoJSON, in the user locale."
  []
...
(defn valid-geojson-url?
  "Whether a `url` points to valid GeoJSON."
  [url]
  (or (io/resource url)
      (valid-url? url)))
...
(defn- validate-geojson
  ...
  [geojson]
  (when-not (mr/validate ::custom-geojson geojson)
