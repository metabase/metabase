...
(deftest define-queue-test
  ...
  (let [queue-name (keyword "queue" (str "define-test-" (gensym)))
    ...
    (testing "Define queue creates a queue"
      (q.backend/define-queue! :mq.queue.backend/memory queue-name)
      (is (contains? @q.memory/*queues* queue-name))
      (is (= 0 (q.backend/queue-length :mq.queue.backend/memory queue-name))))
    ...
    (testing "Close queue"
      (q.backend/close-queue! :mq.queue.backend/memory queue-name)
...
(deftest ^:parallel publish-test
  (let [queue-name (keyword "queue" (str "publish-test-" (gensym)))]
    ...
    (testing "Publish adds message to queue"
      (q.backend/publish! :mq.queue.backend/memory queue-name ["test-message"])
      (is (= 1 (q.backend/queue-length :mq.queue.backend/memory queue-name)))

      (q.backend/publish! :mq.queue.backend/memory queue-name ["test-message2"])
      (is (= 2 (q.backend/queue-length :mq.queue.backend/memory queue-name))))

    (testing "Publish to non-existent queue throws"
      (is (thrown-with-msg? ExceptionInfo #"Queue not defined"
                            (q.backend/publish! :mq.queue.backend/memory :non-existent-queue ["test-message"]))))))

(deftest ^:parallel queue-length-test
  (testing "Queue length on non-existent queue throws"
    (is (thrown-with-msg? ExceptionInfo #"Queue not defined"
                          (q.backend/queue-length :mq.queue.backend/memory :queue/non-existent-queue))))
  ...
  (let [queue-name (keyword "queue" (str "length-test-" (gensym)))]
    (testing "Queue length returns correct count"
      ...
      (q.backend/publish! :mq.queue.backend/memory queue-name ["msg1"])
      (q.backend/publish! :mq.queue.backend/memory queue-name ["msg2"])
      (is (= 2 (q.backend/queue-length :mq.queue.backend/memory queue-name))))))
...
(deftest listen-and-close-queue-test
  (let [queue-name (keyword "queue" (str "listen-test-" (gensym)))
    ...
    (testing "Listen creates queue and registers handler"
      (q.backend/listen! :mq.queue.backend/memory queue-name)
      (is (contains? @q.memory/*queues* queue-name)))
...
(deftest message-successful-test
  (let [queue-name :test-queue
    ...
    (testing "Message successful tracks the message"
      (q.backend/message-successful! :mq.queue.backend/memory queue-name message-id)
    ...
    (testing "Multiple successful messages are tracked"
      (q.backend/message-successful! :mq.queue.backend/memory queue-name "msg-456")
      (q.backend/message-successful! :mq.queue.backend/memory queue-name "msg-789")
...
(deftest message-failed-test
  (let [queue-name :test-queue
    ...
    (testing "Message failed tracks the message"
      (q.backend/message-failed! :mq.queue.backend/memory queue-name message-id)
    ...
    (testing "Multiple failed messages are tracked"
      (q.backend/message-failed! :mq.queue.backend/memory queue-name "failed-456")
      (q.backend/message-failed! :mq.queue.backend/memory queue-name "failed-789")
