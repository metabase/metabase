...
(defn retry-configuration->diehard-map
  ...
  (-> retry-conf
      (select-keys
       [:on-retry :max-duration-ms :listener :max-retries :on-complete :retry-on :on-success :abort-on :retry-if :on-abort :backoff-ms :jitter-factor :jitter-ms :fallback :abort-if :circuit-breaker :on-failure :delay-ms :on-failed-attempt :retry-when :abort-when :policy :on-retries-exceeded])
      (assoc :backoff-ms (cond-> [initial-interval-millis max-interval-millis]
