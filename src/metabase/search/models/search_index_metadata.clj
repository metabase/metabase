...
(defn delete-index!
  ...
  [engine version index-name]
  (t2/delete! :model/SearchIndexMetadata :engine engine :version version :lang_code (i18n-be/site-locale-string) :index_name (name index-name)))

(defn active-pending!
  "If there is 'pending' index, make it 'active'. Return the name of the active index, regardless."
  [engine version]
  (t2/with-transaction [_conn]
    (when (t2/exists? :model/SearchIndexMetadata :engine engine :version version :lang_code (i18n-be/site-locale-string) :status :pending)
      (t2/delete! :model/SearchIndexMetadata :engine engine :version version :lang_code (i18n-be/site-locale-string) :status :retired)
      (t2/update! :model/SearchIndexMetadata {:engine engine :version version :lang_code (i18n-be/site-locale-string) :status :active} {:status :retired})
      (t2/update! :model/SearchIndexMetadata {:engine engine :version version :lang_code (i18n-be/site-locale-string) :status :pending} {:status :active}))
    (t2/select-one-fn :index_name :model/SearchIndexMetadata :engine engine :version version :lang_code (i18n-be/site-locale-string) :status :active)))
