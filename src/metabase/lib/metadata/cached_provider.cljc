...
(defn- metadatas-for-table-ids
  [cache uncached-provider {metadata-type :lib/type, table-ids :table-id, :as metadata-spec}]
  (let [cache-key   (fn [table-id]
                      [::spec (assoc metadata-spec :table-id #{table-id})])
    ...
    (when (seq missing-ids)
      (let [newly-fetched (lib.metadata.protocols/metadatas uncached-provider (assoc metadata-spec :table-id missing-ids))
...
(mu/defn- metadatas
  [cache uncached-provider {metadata-type :lib/type, id-set :id, name-set :name, table-ids :table-id, :as metadata-spec} :- ::lib.metadata.protocols/metadata-spec]
  (cond
    (or id-set name-set)
