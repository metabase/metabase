...
(s/defn ^:always-validate wrap-inner-query
  ...
  {:style/indent 0}
  [query :- qi/Query]
  {:database (db/sel-1-field :db_id Table, :id (:source-table query))
   :type     :query
   :query    query})
  ...
  (let [table-name (format-name table-name)]
    (or (db/sel-1-id Table, :db_id db-id, :name table-name)
        (throw (Exception. (format "No Table '%s' found for Database %d.\nFound: %s" table-name db-id
                                   (u/pprint-to-str (db/sel :id->field [Table :name], :db_id db-id, :active true))))))))
  ...
  (let [field-name (format-name field-name)]
    (or (db/sel-1-id Field, :active true, :table_id table-id, :name field-name, :parent_id parent-id)
        (throw (Exception. (format "Couldn't find Field %s for Table %d.\nFound: %s"
                                   ...
                                   table-id
                                   (u/pprint-to-str (db/sel :id->field [Field :name], :active true, :table_id table-id))))))))
               ...
               (let [table-name (:table-name table-definition)
                     table      (delay (or  (i/metabase-instance table-definition db)
                                            (throw (Exception. (format "Table '%s' not loaded from definiton:\n%s\nFound:\n%s"
                                                                       ...
                                                                       (u/pprint-to-str (db/sel [Table :schema :name], :db_id (:id db))))))))]
