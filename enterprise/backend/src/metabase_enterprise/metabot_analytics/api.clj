...
(def ^:private ReviewLabel
  [:enum "ok" "friction" "failed"])

(def ^:private ConversationReview
  "The automatic quality review of a conversation. `answers` holds the raw judgments the label was derived from."
  [:map
   [:label      ReviewLabel]
...
(def ^:private ConversationSummary
  ...
  [:map
   ...
   [:user                        [:maybe UserInfo]]
   [:review_label                [:maybe ReviewLabel]]
   [:review_issues               [:sequential :string]]
...
(def ^:private ListConversationsQueryParams
  ...
  [:map {:closed true}
   ...
   [:has_issues {:optional true} [:maybe ms/BooleanValue]]])
...
(def ^:private ConversationDetail
  ...
  [:map
   ...
   [:review                      [:maybe ConversationReview]]])
...
(api.macros/defendpoint :get "/conversations" :- ListConversationsResponse
  ...
  [_route-params
   {:keys [sort_by sort_dir user_id group_id tenant_id date has_issues]} :- ListConversationsQueryParams]
  (api/check-superuser)
