...
(deftest mbql-query-reads-the-workspace-table-test
  (testing "An MBQL query on a remapped table compiles to SQL naming the workspace table -- the Table metadata it
    ...
    (mt/with-premium-features #{:workspaces}
      (mt/with-temp [:model/Workspace {ws-id :id} {:name "ws-mw", :creator_id (mt/user->id :crowberto)}]
...
(deftest mbql-query-reads-the-canonical-table-with-remapping-suppressed-test
  (testing "with table remapping suppressed -- the path that shows someone the SQL they authored -- the same query
            compiles to the canonical table even inside a workspace"
    (mt/with-premium-features #{:workspaces}
      (mt/with-temp [:model/Workspace {ws-id :id} {:name "ws-mw", :creator_id (mt/user->id :crowberto)}]
        (let [mp                    (mt/metadata-provider)
...
(deftest mbql-query-reads-the-canonical-table-outside-a-workspace-test
  (testing "With workspaces on but no workspace bound, a query reads the canonical table -- as every query did
      ...
      (mt/with-temp [:model/Workspace {ws-id :id} {:name "ws-mw", :creator_id (mt/user->id :crowberto)}]
        (let [mp                    (mt/metadata-provider)
          ...
          (mt/with-temp [:model/WorkspaceTableRemapping _ {:db_id        (mt/id)
                                                           ...
                                                           :from_table   name
                                                           :to_schema    schema
                                                           :to_table     "ws_unbound_target"}]
            (clear-remappings-cache!)
            (is (nil? (workspaces/current-workspace-id)) "no workspace is bound here")
            (let [query    (lib/query mp (lib.metadata/table mp (mt/id :orders)))
                  compiled (qp.compile/compile query)]
              (is (not (re-find #"(?i)ws_unbound_target" (:query compiled)))
                  "an unbound read does not reach into a workspace's table"))))))))
