...
(api.macros/defendpoint :post "/"
  ...
  (card/check-run-permissions-for-query (dissoc query :query-permissions/perms))
...
(defn- check-allowed-to-modify-query
  ...
  (let [card-updates (m/update-existing card-updates :dataset_query card.metadata/normalize-dataset-query)]
    (when (api/column-will-change? :dataset_query card-before-updates card-updates)
      (card/check-run-permissions-for-query (dissoc (:dataset_query card-updates) :query-permissions/perms)))))
