...
(defn permissions-set
  ...
  (when-let [user-id (u/get-id user-or-id)]
    (-> #{(str "/member/" user-or-id "/")}
        (into (map perms/collection-readwrite-path (collection/user->personal-collection-and-descendant-ids user-or-id)))
        (into (map :object (db/query {:select [:p.object]
