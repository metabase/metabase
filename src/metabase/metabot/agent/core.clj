...
(mr/def ::content-block
  ...
  [:map {:closed true, :probe/id "src/metabase/metabot/agent/core.clj:99"}
...
(mr/def ::tool-call
  ...
  [:map {:closed true, :probe/id "src/metabase/metabot/agent/core.clj:108"}
   [:id :string]
   [:name :string]
   [:arguments :string]])
...
(mr/def ::user-message
  ...
  [:map {:closed true, :probe/id "src/metabase/metabot/agent/core.clj:115"}
   [:role [:= :user]]
   [:content ::content]])

(mr/def ::assistant-message
  "An assistant message with optional text and/or tool calls."
  [:map {:closed true, :probe/id "src/metabase/metabot/agent/core.clj:121"}
   [:role [:= :assistant]]
...
(mr/def ::system-message
  "A system message."
  [:map {:closed true, :probe/id "src/metabase/metabot/agent/core.clj:128"}
   [:role [:= :system]]
   [:content :string]])

(mr/def ::tool-message
  "A tool result message, referencing a previous tool call by ID."
  [:map {:closed true, :probe/id "src/metabase/metabot/agent/core.clj:134"}
   [:role [:= :tool]]
   [:tool_call_id :string]
   [:content :string]])
...
(mr/def ::tracking-opts
  ...
  [:map {:closed true, :probe/id "src/metabase/metabot/agent/core.clj:162"}
...
(mu/defn run-agent-loop
  ...
  [opts :- [:map {:closed true, :probe/id "src/metabase/metabot/agent/core.clj:690"}
            [:messages ::messages]
