...
(defn- migrate-legacy-pivot-viz-settings
  "Adds a prefix to all field ref-based pivot table settings. New column name-based settings would be used instead."
  [viz-settings]
  (-> viz-settings
...
(defn- update-legacy-card-pivot-viz-settings
  ...
  (let [update-one! (fn [{id :id viz-settings :visualization_settings}]
                      (let [viz-settings         (json/parse-string viz-settings)
                            updated-viz-settings (update-viz-settings-fn viz-settings)]
                        (when (not= viz-settings updated-viz-settings)
                          (t2/query-one {:update :report_card
                                         :set    {:visualization_settings (json/generate-string updated-viz-settings)}
                                         :where  [:= :id id]}))))]
...
(defn- update-legacy-dashboard-card-pivot-viz-settings
  "Updates dashboard card visualization settings by applying `update-viz-settings-fn`."
  [update-viz-settings-fn]
  (let [update-one! (fn [{id :id viz-settings :visualization_settings}]
                      (let [viz-settings         (json/parse-string viz-settings)
                            updated-viz-settings (update-viz-settings-fn viz-settings)]
...
(define-reversible-migration MigrateLegacyDashboardCardPivotVizSettings
  (update-legacy-dashboard-card-pivot-viz-settings migrate-legacy-pivot-viz-settings)
  (update-legacy-dashboard-card-pivot-viz-settings rollback-legacy-pivot-viz-settings))
