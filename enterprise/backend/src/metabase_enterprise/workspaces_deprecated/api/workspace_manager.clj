(ns metabase-enterprise.workspaces-deprecated.api.workspace-manager
  "EE API endpoints for managing workspaces, served under `/api/ee/workspace-manager`.
   ...
   [[metabase-enterprise.workspaces-deprecated.core]] and permission predicates live on
   `:model/WorkspaceDeprecated` and `:model/WorkspaceDatabaseDeprecated` (see `mi/can-read?`/`can-write?`/`can-create?`)."
  (:require
   [medley.core :as m]
   [metabase-enterprise.workspaces-deprecated.config :as ws.config]
...
(api.macros/defendpoint :get "/:id" :- WorkspaceResponse
  ...
  (api/read-check :model/WorkspaceDeprecated id)
...
(api.macros/defendpoint :post "/" :- WorkspaceResponse
  ...
  (api/create-check :model/WorkspaceDeprecated params)
  (present-workspace
   (ws/create-workspace!
...
(api.macros/defendpoint :put "/:id" :- WorkspaceResponse
  ...
  (api/write-check :model/WorkspaceDeprecated id)
  (when (:name params)
    (t2/update! :model/WorkspaceDeprecated :id id {:name (:name params)}))
  (present-workspace (api/check-404 (ws/get-workspace id))))
