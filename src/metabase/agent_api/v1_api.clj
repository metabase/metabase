(ns metabase.agent-api.v1-api
  "The v1 Agent API endpoints: the first generation of tools, one endpoint per verb.

   They assemble content through the models directly — a dashboard's cards, a card's patch — and mirror the
   REST endpoints' permission pre-checks by hand where the domain functions do not carry them.

   The v2 tools in [[metabase.agent-api.api]] replace them; both generations are served under `/api/agent`
   by the same route table."
  (:require
   [clojure.string :as str]
   [metabase.agent-api.query :as agent-api.query]
   [metabase.agent-api.search :as agent-api.search]
   [metabase.agent-api.settings :as agent-api.settings]
   [metabase.agent-api.v1-dashcards :as v1-dashcards]
...
(api.macros/defendpoint :post "/v1/search" :- ::search-response
  ...
  [_route-params
   ...
   :- [:map
       [:term_queries
        {:optional true
         :tool/description (str "Keyword search queries as an array of strings, for example "
                                "[\"orders\", \"revenue\"].")}
        [:maybe [:or [:sequential ms/NonBlankString] ms/NonBlankString]]]
       [:semantic_queries
        {:optional true
         :tool/description (str "Natural-language search queries as an array of strings, for example "
                                "[\"how much revenue did we make\"].")}
        [:maybe [:or [:sequential ms/NonBlankString] ms/NonBlankString]]]]]
  (let [results (metabot-search/search
                 {:term-queries     (or (agent-api.search/coerce-queries term-queries) [])
                  :semantic-queries (or (agent-api.search/coerce-queries semantic-queries) [])
  ...
  {:scope metabot/agent-query-construct
   :tool  {:name "construct_query"
           ...
           (str "Construct a Metabase MBQL 5 query as JSON. Pass the body "
                ...
                "\n"
                "Workflow: search for the content you need and read its tables and columns "
                "first, so you have the exact database, schema, table, and column NAMES (not "
                "numeric ids). Never invent identifiers.\n"
                "\n"
...
(defn- decode-continuation-token
  "Decode a base64-encoded continuation token into {:query ... :pagination ...}.
   ...
   the embedded query happens in [[metabase.agent-api.query/check-source-permissions!]] — a token
...
(defn- prepare-combined-query
  ...
  [query]
  (assoc (agent-api.query/prepare query)
...
(defn- initial-page-state
  ...
  (cond
    ...
    (let [{:keys [query pagination]} (decode-continuation-token (:continuation_token body))]
      ...
      (agent-api.query/check-shape! query)
      (agent-api.query/check-source-permissions! query)
    ...
    (let [query (decode-base64-json-map (:query body))]
      ...
      (agent-api.query/check-shape! query)
      {:query       query
       :total-limit (clamp-total-limit (agent-api.query/own-limit query))
       :page        1})
...
(mr/def ::execute-query-request
  ...
  [:map
   [:query
    {:tool/description (str "A base64-encoded query string returned by /v1/construct-query. Do not "
                            "construct this value manually.")}
    ms/NonBlankString]])
...
(api.macros/defendpoint :post "/v1/execute-sql"
  ...
  (let [raw-query {:database database_id
    ...
    ;; shape instead of a raw 500.
    (qp.streaming/streaming-response [rff :api]
      (qp/process-query (agent-api.query/prepare raw-query) rff))))
...
(api.macros/defendpoint :put "/v1/dashboard/:id" :- ::update-dashboard-response
  ...
  (let [current-dash (api/write-check :model/Dashboard id)
        ...
        result       (t2/with-transaction [_conn]
                       ...
                       (when (seq mutations)
                         (v1-dashcards/apply-mutations! id mutations)))]
...
(def ^{:arglists '([request respond raise])} routes
  "The v1 half of the `/api/agent/` route table. Mounted by [[metabase.agent-api.api/routes]]."
