...
(mu/defn- batch-fetch-query-metadata*
  ...
  (let [source-table-ids       (into #{}
        ...
        fk-target-card-ids     (into #{} (comp (mapcat :fields)
                                               (keep :fk_target_card_id))
                                     source-tables)
        fk-target-card-ids     (into #{} (remove source-card-ids)
                                     fk-target-card-ids)
        fk-target-cards        (schema.table/batch-fetch-card-query-metadatas fk-target-card-ids
                                                                              {:include-database? false})
        tables                 (concat source-tables fk-target-tables fk-target-cards)
