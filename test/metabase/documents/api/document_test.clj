...
(deftest post-document-records-recent-item-test
  (testing "POST /api/document puts the new document in the creator's recents"
    (mt/with-temporary-setting-values [synchronous-batch-updates true]
      (mt/with-model-cleanup [:model/Document]
        (let [document (mt/user-http-request :crowberto
          ...
          (is (some? (recent-view (mt/user->id :crowberto) (:id document)))
              "creating a document should record a recent item")
          (testing "without counting as a read: the view count stays at zero"
            (is (zero? (:view_count (t2/select-one :model/Document :id (:id document)))))))))))
