   ...
   [:items [:sequential {:min 1 :description "The content to fetch — up to 10 items, mixed types allowed (e.g. a dashboard and its questions in one call)."}
            [:map {:closed true}
             ...
             [:fields {:optional true}
              [:maybe [:sequential [:string {:min 1 :description "Dot-paths picked from this type's detailed projection (see the metabase://catalogs/fields resource), item-relative inside arrays. Mutually exclusive with response_format and include."}]]]]]]]
