...
describe("scenarios > question > filter", () => {
  ...
  it("should offer case expression in the auto-complete suggestions", () => {
    ...
    H.enterCustomColumnDetails({ formula: "c" });
  ...
  it("should provide accurate auto-complete custom-expression suggestions based on the aggregated column name (metabase#14776)", () => {
    ...
    // eslint-disable-next-line no-unscoped-text-selectors -- deprecated usage
    cy.findByText("Custom Expression").click();
    H.enterCustomColumnDetails({ formula: "su" });
  ...
  it("should filter using IsNull() and IsEmpty()", () => {
    ...
    H.enterCustomColumnDetails({ formula: "NOT IsEmpty([Reviewer])" });
    cy.get("@formula").blur();
  ...
  it("should work on twice summarized questions and preserve both summaries (metabase#15620)", () => {
    ...
    // filter
    H.getNotebookStep("filter").should("contain", "Category is Gizmo");

    // summarize 1
    H.getNotebookStep("summarize", { stage: 0, index: 0 }).should(
      "contain",
      "Created At: Month",
    );

    // summarize 2
    H.getNotebookStep("summarize", { stage: 1, index: 0 }).should(
      "contain",
      "Average of Count",
    );
  });
