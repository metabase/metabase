...
(api.macros/defendpoint :get "/clients" :- [:sequential OAuthClientResponse]
  ...
  [_route-params
   {:keys [registration-type]} :- [:map
                                   [:registration-type {:optional true} [:maybe ms/NonBlankString]]]]
  (let [clients (if registration-type
                  (t2/select :model/OAuthClient :registration_type registration-type)
