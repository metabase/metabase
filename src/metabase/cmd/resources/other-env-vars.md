...
### `MB_SESSION_HASH_SECRET_KEY`

Type: string<br>
Default: `null`

When set, session keys are stored in the application database as an HMAC signed with this secret, so a valid session cannot be created or used with database access alone. Requirement: minimum 16 characters. Setting or changing this value logs out all active sessions.
