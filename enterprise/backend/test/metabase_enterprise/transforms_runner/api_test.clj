(ns ^:mb/driver-tests ^:mb/transforms-python-test metabase-enterprise.transforms-runner.api-test
  "Tests for /api/ee/advanced-transforms endpoints and runner-based transform CRUD."
  (:require
        ...
        (mt/with-db-perm-for-group! (perms-group/all-users) (mt/id) :perms/transforms :yes
          ...
          (testing "returns existing library"
            ...
            (is (=? {:source "def my_function():\n    return 42"
                     ...
                     :created_at some?
                     :updated_at some?}
                    (mt/user-http-request :lucky :get 200 "ee/advanced-transforms/library/common"))))
          (testing "rejects invalid paths"
            (is (= "Invalid library path. Only 'common' is currently supported."
                   (:message (mt/user-http-request :lucky :get 400 "ee/advanced-transforms/library/invalid-path"))))))))))

(deftest put-library-path-test
  (testing "PUT /api/ee/advanced-transforms/library/:path"
    (mt/with-premium-features #{:transforms-python :transforms}
      (testing "requires transform permissions"
        (is (= "You don't have permissions to do that."
               (mt/user-http-request :rasta :put 403 "ee/advanced-transforms/library/common"
        ...
        (mt/with-db-perm-for-group! (perms-group/all-users) (mt/id) :perms/transforms :yes
          (testing "validates request body"
            (testing "requires source field"
              (is (=? {:errors
                       {:source "string"}}
                      (mt/user-http-request :lucky :put 400 "ee/advanced-transforms/library/common"
                                            {}))))
            (testing "allows empty string"
              ...
              (is (=? {:source ""
                       ...
                       :created_at some?
                       :updated_at some?}
                      (mt/user-http-request :lucky :put 200 "ee/advanced-transforms/library/common"
                                            {:source ""})))))
          ...
          (testing "rejects invalid paths"
            (is (= "Invalid library path. Only 'common' is currently supported."
                   (:message (mt/user-http-request :lucky :put 400 "ee/advanced-transforms/library/invalid-path"
                                                   {:source "def test(): pass"}))))))))))

(deftest get-javascript-library-path-test
  (testing "GET /api/ee/advanced-transforms/library/:path?type=javascript"
    (mt/with-premium-features #{:transforms-python :transforms}
      (mt/with-data-analyst-role! (mt/user->id :lucky)
        (mt/with-db-perm-for-group! (perms-group/all-users) (mt/id) :perms/transforms :yes
...
(deftest put-javascript-library-path-test
  (testing "PUT /api/ee/advanced-transforms/library/:path with type=javascript"
    (mt/with-premium-features #{:transforms-python :transforms}
      (mt/with-data-analyst-role! (mt/user->id :lucky)
        (mt/with-db-perm-for-group! (perms-group/all-users) (mt/id) :perms/transforms :yes
...
(deftest test-run-test
  (mt/with-premium-features #{:transforms :transforms-python}
    (let [program ["import pandas as pd"
          ...
          {:keys [error logs output]} (mt/user-http-request :crowberto :post 200 "ee/advanced-transforms/test-run" body)]
      (is (nil? error))
...
(defn- test-run [& {:keys [program user features source-tables extra-opts]
  ...
  (let [body (merge {:source_tables source-tables, :code (str/join "\n" program)} extra-opts)]
    (mt/with-premium-features features
      (mt/user-http-request-full-response user :post "ee/advanced-transforms/test-run" body))))
