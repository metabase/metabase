...
(expect
  ...
  (#'db.config/parse-connection-string "postgres://localhost/toms_cool_db"))

;; parse connection string using alternate `postgreql` URI schema
(expect
  {:type :postgres, :user nil, :password nil, :host "localhost", :port nil, :dbname "toms_cool_db" }
  (#'db.config/parse-connection-string "postgresql://localhost/toms_cool_db"))
...
(expect
  ...
  (tu.log/suppress-output
    (#'db.config/parse-connection-string (str "postgres://tom:1234@localhost:5432/toms_cool_db"
                                        "?ssl=true&sslfactory=org.postgresql.ssl.NonValidatingFactory"))))
...
(expect
  {:type :postgres, :user "tom", :password "1234", :host "localhost", :port "5432", :dbname "toms_cool_db",
   :ssl "true", :sslfactory "org.postgresql.ssl.NonValidatingFactory"}
  (tu.log/suppress-output
    (#'db.config/parse-connection-string (str "jdbc:postgres://tom:1234@localhost:5432/toms_cool_db"
                                        "?ssl=true&sslfactory=org.postgresql.ssl.NonValidatingFactory"))))
