(ns metabase.collection-items.core
  "The items of a Collection: the union-all query behind every \"what is in this folder\" listing, and the
  post-processing that turns its rows into the records the API returns.

  One query serves every child type — cards, models, metrics, dashboards, documents, pulses, snippets, timelines,
  transforms, published tables, and sub-collections — because a listing has to sort, page, and count across all of
  them at once. Each type contributes a `collection-children-query` method whose `SELECT` is padded out to the shared
  column list, and a `post-process-collection-children` method that hydrates its own rows.

  Permission filtering is inside the query: every method restricts to the `visible_collection_ids` CTE, so a caller
  only ever sees the items in collections they can read.

  This lives below the REST layer so the callers that are not REST — notably the agent API's `browse_collection`
  tool — list a collection's items through the same code path the app does."
  (:require
...
(def valid-pinned-state-values
  "Valid values for the `?pinned_state` param accepted by the collection items endpoints."
  #{"all" "is_pinned" "is_not_pinned"})

(def valid-sort-columns
  "Columns a listing can be sorted by."
  #{"name" "last_edited_at" "last_edited_by" "model" "description"})

(def valid-sort-directions
  "Directions a listing can be sorted in."
  #{"asc" "desc"})

(defn normalize-sort-choice
  "The keyword form of a snake_case sort column or direction: `\"last_edited_at\"` -> `:last-edited-at`."
  [w]
...
(def CollectionChildrenOptions
  "Options [[collection-children]] takes."
  [:map
...
(defn- post-process-card-like
  ...
  (let [threshold              (collection-items.settings/can-run-adhoc-query-check-threshold)
