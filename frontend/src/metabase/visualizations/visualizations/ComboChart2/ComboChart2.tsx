...
import {
  ComboChart2 as ComboChart2View,
  ComboChart2Props as ComboChart2ViewProps,
} from "metabase/visualizations/shared/components/ComboChart2";
import {
  getGroupedDataset,
  getSeries,
  trimData,
...
const RowChartRenderer = ExplicitSize({
  ...
  selector: false,
})((props: ComboChart2ViewProps<GroupedDatum>) => (
  <RowChartContainer>
    <ComboChart2View {...props} />
  </RowChartContainer>
));

interface ComboChart2Props {
  className: string;
  width: number;
  height: number;
...
ComboChart2.uiName = t`Combo Chart 2`;
ComboChart2.identifier = "combo2";
ComboChart2.iconName = "sum";
ComboChart2.noun = t`combo chart`;

ComboChart2.noHeader = true;
ComboChart2.minSize = getMinSize("row");
ComboChart2.defaultSize = getDefaultSize("row");

ComboChart2.settings = {
  ...ROW_CHART_SETTINGS,
  ...GRAPH_DATA_SETTINGS,
};

ComboChart2.isSensible = ({ cols, rows }: DatasetData) => {
  return (
    rows.length > 1 &&
    cols.length >= 2 &&
...
ComboChart2.isLiveResizable = (series: any[]) => {
  const totalRows = series.reduce((sum, s) => sum + s.data.rows.length, 0);
  return totalRows < 10;
};

ComboChart2.settings["graph.metrics"] = {
  ...ComboChart2.settings["graph.metrics"],
  title: t`X-axis`,
};
ComboChart2.settings["graph.dimensions"] = {
  ...ComboChart2.settings["graph.dimensions"],
  title: t`Y-axis`,
};
 ...
 */
ComboChart2.transformSeries = (originalMultipleSeries: any) => {
