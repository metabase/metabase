...
cy.describeWithSnowplow("scenarios > admin > settings", () => {
  beforeEach(() => {
    cy.resetSnowplow();
    cy.restore();
    cy.signInAsAdmin();
  ...
  it("should surface an error when validation for any field fails (metabase#4506)", () => {
    ...
    cy.undoToast().contains(/^Error: Invalid site URL/);
  });
  ...
  it("should check for working https before enabling a redirect", () => {
    ...
    cy.contains("Site URL")
      ...
      .click();
    cy.popover().contains("https://").click({ force: true });

    cy.wait("@httpsCheck");
    // eslint-disable-next-line no-unscoped-text-selectors -- deprecated usage
    cy.contains("Redirect to HTTPS").parent().parent().contains("Disabled");

    cy.restore(); // avoid leaving https site url
  });
  ...
  it("should correctly apply the globalized date formats (metabase#11394) and update the formatting", () => {
    ...
    cy.popover().findByText("2018/1/31").click({ force: true });
  ...
  it(
    ...
    { tags: "@OSS" },
    () => {
      cy.isEE && cy.setTokenFeatures("all");

      const lastItem = cy.isOSS ? "Cloud" : "Appearance";
...
cy.describeEE("scenarios > admin > settings (EE)", () => {
  beforeEach(() => {
    cy.restore();
    cy.signInAsAdmin();
...
describe("scenarios > admin > settings > email settings", () => {
  ...
  it("should be able to save and clear email settings", () => {
    ...
    cy.main().findByText("Clear").click();
  ...
  it("should show an error if test email fails", () => {
    ...
    cy.main().findByText("Send test email").click();
    cy.findAllByText(
...
describe("scenarios > admin > localization", () => {
  function setFirstWeekDayTo(day) {
    cy.updateSetting("start-of-week", day.toLowerCase());
  }
  ...
  it("should correctly apply start of the week to a bar chart (metabase#13516)", () => {
    ...
    cy.echartsContainer().get("text").contains("April 25, 2022");
  });
  ...
  it("should use currency settings for number columns with style set to currency (metabase#10787)", () => {
    ...
    cy.undoToast().findByText("Changes saved").should("be.visible");
  ...
  it("should use fix up clj unit testsdate and time styling settings in the date filter widget (metabase#9151, metabase#12472)", () => {
    ...
    cy.popover().findByText("2018/1/31").click();
    ...
    cy.tableHeaderClick("Created At");
...
describe("notifications", { tags: "@external" }, () => {
  beforeEach(() => {
    ...
    cy.request({
      ...
      url: `${cy.WEBHOOK_TEST_HOST}/api/session/${cy.WEBHOOK_TEST_SESSION_ID}/requests`,
      method: "DELETE",
    }).then(response => {
  ...
  it("Should allow you to create and edit Notifications", () => {
    ...
    cy.modal().within(() => {
      ...
      cy.findByLabelText("Webhook URL").clear().type(cy.WEBHOOK_TEST_HOST);
      cy.button("Send a test").click();
      ...
      cy.findByLabelText("Webhook URL").clear().type(cy.WEBHOOK_TEST_URL);
      cy.button("Send a test").click();
...
describe("admin > settings > updates", () => {
  ...
  it("should change release notes based on the selected update channel", () => {
    ...
    cy.popover().findByText("Beta releases").click();
    ...
    cy.popover().findByText("Nightly builds").click();
