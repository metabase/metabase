  ...
  useEffect(() => {
    ...
    async function fetchUserAndSettings() {
      try {
        ...
        // subscribers when they are invalidated. Neither call refetches; both are fulfilled.
        //
        // That subscription is also the one way this design breaks: an invalidation would
        // refetch, and both requests fail here — `current-user` 401s, and `session-properties`,
        // mounted without `+auth`, answers anonymously and silently drops the authenticated
        // settings. Nothing in the iframe's tree dispatches a mutation today; the first one
        // added has to reckon with this.
