...
(expect
  ...
  (tu/with-temp Card [card {:dataset_query {:database (id)
    ...
    (process-dependencies-event {:topic :card-create
                                 :item  card})
    (set (db/sel [Dependency :dependent_on_model :dependent_on_id], :model "Card", :model_id (:id card)))))
...
(expect
  ...
  (tu/with-temp Card [card {:dataset_query {:database (id)
    ...
    (process-dependencies-event {:topic :card-create
                                 :item  card})
    (db/sel [Dependency :dependent_on_model :dependent_on_id], :model "Card", :model_id (:id card))))
...
(expect
  ...
  (tu/with-temp* [Database [{database-id :id}]
    ...
    (process-dependencies-event {:topic :metric-create
                                 :item  metric})
    (set (db/sel [Dependency :dependent_on_model :dependent_on_id], :model "Metric", :model_id (:id metric)))))
