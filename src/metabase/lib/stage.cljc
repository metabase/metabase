...
(defmethod lib.metadata.calculation/metadata-method ::stage
  [_query _stage-number _stage _options]
...
(mu/defn- fields-columns :- [:maybe [:sequential ::lib.metadata.calculation/column-metadata-with-source]]
  ...
  (when-let [{fields :fields} (lib.util/query-stage query stage-number)]
    (-> (for [[tag :as ref-clause] fields
              :let                 [col (lib.metadata.calculation/metadata query stage-number ref-clause options)]]
          (cond-> col
...
(mu/defn- expressions-metadata :- [:maybe ::lib.metadata.calculation/visible-columns]
  [query                                      :- ::lib.schema/query
   stage-number                               :- :int
   {:keys [include-late-exprs?], :as options} :- [:merge
  ...
  (not-empty
   (for [[clause col] (map vector
                           ...
                           (lib.expression/expressions-metadata query stage-number options))
     ...
     (-> col
         (assoc :lib/source :source/expressions, :lib/source-column-alias ((some-fn :name :lib/source-column-alias) col))
  ...
  (let [source-columns (previous-stage-or-source-visible-columns query stage-number options)]
    (concat
     ...
     (when include-expressions?
       (expressions-metadata query stage-number options))
    ...
    (into []
          ...
          [existing-columns
           ...
           (when include-implicitly-joinable?
             (lib.metadata.calculation/implicitly-joinable-columns query stage-number existing-columns options))])))
...
(mu/defmethod lib.metadata.calculation/returned-columns-method ::stage :- ::lib.metadata.calculation/returned-columns
  [query stage-number _stage {:keys [include-remaps?], :as options}]
  (or
   ...
   (let [summary-cols (summary-columns query stage-number options)
         ...
         cols (cond
                ...
                :else
                ;; there is no `:fields` or summary columns (aggregtions or breakouts) which means we return all the
                ;; visible columns from the source or previous stage plus all the expressions. We return only the
                ;; `:fields` from any joins
                (let [ ;; we don't want to include all visible joined columns, so calculate that separately
                  ...
                  (concat
                   source-cols
                   (expressions-metadata query stage-number (assoc options :include-late-exprs? true))
     ...
     (into []
           (comp (lib.field.util/add-source-and-desired-aliases-xform query)
                 ;; we need to update `:name` to be the deduplicated name here, otherwise viz settings will
                 ;; break (see longer explanation
                 ;; in [[metabase.lib.stage-test/returned-columns-deduplicate-names-test]]). Only do this if this
                 ;; is the last stage of the query, just like the QP does! Otherwise we might accidentally break
                 ;; something else that incorrectly relies on the notoriously unreliable `:name` key.
