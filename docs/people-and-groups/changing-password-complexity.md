...
The settings above can be used independently, so it's fine to use only one or the other. By default Metabase uses complexity = `strong-enough` (minimum 15 characters). The following options are available for complexity choice:
