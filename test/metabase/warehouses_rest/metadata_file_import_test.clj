(ns metabase.warehouses-rest.metadata-file-import-test
  ...
  Pipeline driven by MB_TABLE_METADATA_PATH and MB_FIELD_VALUES_PATH env vars; sync-state is
  orthogonal — the loader imports whenever the env vars point at valid files."
  (:require
...
(deftest field-values-without-metadata-throws-test
  (testing "MB_FIELD_VALUES_PATH without MB_TABLE_METADATA_PATH throws — field-id map requires the metadata file"
    (let [fv-path (write-temp-json! {:field_values []})]
      (with-env {:mb-field-values-path fv-path}
        (is (thrown-with-msg? Exception #"(?i)field_values.*without|field ids cannot be resolved"
                              (metadata-file-import/initialize-from-env!)))))))
