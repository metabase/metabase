(ns metabase.warehouse-index-manager.introspection
  ...
  `pg_get_indexdef` definition string. The latter is the most faithful
  representation of what's actually in the database.

  Lifted from `metabase-enterprise.transform-optimizer.index-introspection`."
  (:require
...
(defn- normalise-pairs
  "Accept either a single `[schema table]` pair or a sequence of them."
  [table-pairs]
  (cond
...
(defn fetch-indexes
  "Look up full index detail for the given `[[schema table] …]` pairs
  against `database`. A single `[schema table]` pair is also accepted as
  a convenience. Returns a vector of index info maps.

  Postgres only. Other drivers return `nil` and emit a warning — callers
  should treat that as `driver_not_supported`."
  [driver database table-pairs]
  (let [pairs (normalise-pairs table-pairs)]
    (cond
      (not (isa? driver/hierarchy driver :postgres))
      (do (log/warnf "index-introspection only supports :postgres (got %s)" driver)
          nil)
...
(defn group-by-table
  "Turn the flat seq returned by `fetch-indexes` into a
  `{[schema table] [indexes…]}` map, normalising schema and table to
  lowercase (Postgres unquoted-identifier semantics)."
  [indexes]
  (->> indexes
