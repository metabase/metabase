...
(defn- remove-remapped-columns
  "Fields with remapped_from attribute are not actual fields that exists in the table,
  they were added to :cols by [[metabase.query-processor.middleware.add-dimension-projections/remap-results]] middleware.

  We don't want to preserve these fields in the `results_metadata` because some pre-processing middleware like `add-implicit-clauses`
  or `resolve-joins` assume all fields in `results_metadata` are fields exists in DB (see #23449)"
  [metadata]
  (->> (remove :remapped_from metadata)
       ;; # we don't need the `remapped_from` after this, so let's just remove it
       (map #(dissoc % :remapped_from))))
   ...
   (fn [{final-base-type :base_type, :as final-col} {our-base-type :base_type, :as insights-col}]
     (merge
      (select-keys final-col [:id :description :display_name :semantic_type :fk_target_field_id
                              ...
                              :coercion_strategy :visibility_type :remapped_from])
      insights-col
...
(defn- insights-xform [orig-metadata record! rf]
  (qp.reducible/combine-additional-reducing-fns
   ...
   (fn combine [result {:keys [metadata insights]}]
     (let [metadata (-> (get-in result [:data :cols])
                        (merge-final-column-metadata metadata)
                        remove-remapped-columns)]
       (record! metadata)
       (rf (cond-> result
             (map? result)
             (update :data             assoc
