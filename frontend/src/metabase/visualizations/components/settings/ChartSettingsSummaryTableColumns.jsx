...
const convertValueToState = (
  stateSerialized: SummaryTableSettings,
  cols: Column[],
  columns,
): State => {
  ...
  const columnNames = cols.reduce(
    (o, col) => ({ ...o, [col.name]: getFriendlyName(col) }),
    {},
  );

  const items = [
    ...groupsSources.map(n => createDraggableColumn(n, columnNames[n])),
    columnSourceItem,
    ...columnsSource.map(n => createDraggableColumn(n, columnNames[n])),
    valueSourceItem,
    ...valuesSources.map(n => createDraggableColumn(n, columnNames[n])),
    unusedSourceItem,
    ...unusedColumns.map(n => createDraggableColumn(n, columnNames[n])),
  ];
...
const createDraggableColumn = (
  columnName: string,
  displayName: string,
): DraggableItem => ({ columnName, displayName, isDraggable: true });
const createSectionItem = (displayName: string): DraggableItem => ({
  displayName,
  isDraggable: false,
  columnName: "undefined",
});
const groupSourceItem: DraggableItem = createSectionItem(
  t`Fields to use for the table rows`,
);
const columnSourceItem: DraggableItem = createSectionItem(
  t`Field to use for the table columns`,
);
const valueSourceItem: DraggableItem = createSectionItem(
  t`Fields to use for the table values`,
);
...
const convertStateToValue = (state: State): ValueSerializedSupertype => {
  ...
  const columnNameToMetadata = state.items
    ? createMetadata(oldMetadata, columnsPart)
    : oldMetadata;
  return { columnNameToMetadata, ...columnsPart };
};

const convertItemsToState = (
  items: DraggableItem[],
): ValueSerializedSupertype => {
  const {
    columnSourceItemIndex,
    valueSourceItemIndex,
    unusedSourceItemIndex,
  } = getBorderIndexes(items);
  const gs = items.slice(0, columnSourceItemIndex).map(p => p.columnName);
  const cs = items
    .slice(columnSourceItemIndex + 1, valueSourceItemIndex)
    .map(p => p.columnName);
  const vs = items
    .slice(valueSourceItemIndex + 1, unusedSourceItemIndex)
    .map(p => p.columnName);
...
> {
  ...
  onSortEnd = ({ oldIndex, newIndex }: ArrayMoveArg) =>
    moveItem(this.updateState)(this.state.items, { oldIndex, newIndex });
    ...
    return (
      <div>
        <div style={{ fontSize: "1rem" }}>
          <h2 className="text-bold text-paragraph mb2">{t`Customize this table`}</h2>
        </div>
...
const getItemTypeHelper = (items: DraggableItem[]): ItemTypeHelper => {
  ...
  const isColumnSourceColumn = index =>
    columnSourceItemIndex < index && index < valueSourceItemIndex;
  const isValueColumn = index =>
    valueSourceItemIndex < index && index < unusedSourceItemIndex;

  const isBorderElem = index =>
    columnSourceItemIndex === index ||
    valueSourceItemIndex === index ||
    unusedSourceItemIndex === index;

  const getSortableKey = index => {
    const pref = isBorderElem(index)
      ? "Border"
      : isGroupingColumn(index)
        ? "Group"
        : isColumnSourceColumn(index)
          ? "Column"
          : isValueColumn(index) ? "Value" : "Unused";
...
const getBorderIndexes = (items: DraggableItem[]) => {
  ...
  const valueSourceItemIndex = items.indexOf(
    valueSourceItem,
    columnSourceItemIndex,
  );
  const unusedSourceItemIndex = items.indexOf(
    unusedSourceItem,
    valueSourceItemIndex,
  );
...
export const emptyColumnMetadata: ColumnMetadata = {
  showTotals: true,
  isAscSortOrder: true,
};
...
const SortableList = SortableContainer(
  ({
    ...
    const updateMetadata = updateMetadataBuilder(
      columnNameToMetadata,
      updateState,
    );
    ...
    return (
      <ul>
        {items.map((value, index) => (
          <SortableItem
            class="no-select"
            key={itemTypeHelper.getSortableKey(index)}
            index={index}
            valueIndex={index}
            value={value}
            disabled={!isChanging && !value.isDraggable}
            columnMetadata={
              columnNameToMetadata[value.columnName] || emptyColumnMetadata
            }
            itemTypeHelper={itemTypeHelper}
...
const updateMetadataBuilder = (
  columnNameToMetadata,
  updateState: StateSuperType => Promise<*>,
) => (columnName: ?ColumnName) => (newMetadata: ColumnMetadata): void => {
  if (columnName) {
    const newColumnNameToMetadata = {
      ...columnNameToMetadata,
      [columnName]: newMetadata,
    };
...
const moveItem = (updateState: StateSuperType => Promise<*>) => async (
  items: DraggableItem[],
  { oldIndex, newIndex }: ArrayMoveArg,
): Promise<*> => {
...
type FatColumnArgType = {
  displayName: string,
  columnMetadata: ColumnMetadata,
  onRemove: void => void,
  onChange: ColumnMetadata => void,
};
const FatColumn = ({
  displayName,
  columnMetadata,
  ...
  const changeOrder = value =>
    onChange({ ...columnMetadata, isAscSortOrder: value });

  return (
    <div className={cx("my2 bordered shadowed  cursor-pointer bg-white")}>
      ...
      <div className="py1 px2 border-grey-1">
        <div className="flex align-center justify-between flex-no-shrink">
          ...
          <Toggle
            value={columnMetadata.showTotals === true}
            onChange={() =>
              onChange({
                ...columnMetadata,
                showTotals: !columnMetadata.showTotals,
              })
            }
          />
        </div>
...
const SectionHeader = ({ text }) => (
  <div style={{ fontSize: "1rem" }}>
    ...
    <h2 className="text-bold text-paragraph mb2 no-select">{text}</h2>
  </div>
);

const createMetadata = (
  metadata: {},
  { groupsSources, columnsSource }: ValueSerializedSupertype,
): {} => {
  const names = [...groupsSources, ...columnsSource];
  return names.reduce(
    (acc, name) => ({ ...acc, [name]: metadata[name] || emptyColumnMetadata }),
    {},
  );
};
