...
(defn- generate-mise-local!
  ...
  (let [wt-name    (worktree-name (root))
        ...
        headers    [(str "# Auto-generated by ./bin/mage -bot-dev-env")
                    (str "# Worktree: " wt-name " (slot " slot ")")
                    "# Re-run `./bin/mage -bot-dev-env` to update."]
...
(defn- start-processes!
  ...
  (let [env-map   (build-env slot full-config)
    ...
    (println (c/cyan "Stop with: ") (c/yellow "./bin/mage -bot-dev-env --down"))))
