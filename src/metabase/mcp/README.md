...
| `agent:author:write`      | the author toolset: `question_write`, `metric_write`, `dashboard_write`                                        |
...
| `dashboard_write`   | Create or update a dashboard — its cards, its tabs, its layout, its filters — through `ops`, an ordered list of named changes. Cards: `add_card`, `add_text`, `add_heading`, `add_link`, `add_iframe`, `add_action`, `duplicate_card`, `replace_card`, `move`, `resize`, `remove`, `set_series`, and `patch_dashcard` for a card's content. Tabs: `add_tab`, `rename_tab`, `move_tab`, `duplicate_tab`, `remove_tab`. Filters: `add_parameter`, `update_parameter`, `remove_parameter` (which names the subscriptions it breaks), `move_parameter`, `wire_parameter` — whose `autowire` maps every card on the tab that has the column, the server-side port of the editor's auto-wire — and `unwire_parameter`. The server holds the dashboard's current state, so an op names only what it changes and a list cannot delete a card it does not mention. Ops compile into one save: a bad op names its index and nothing is written. `validate_only` is the dry run. |
