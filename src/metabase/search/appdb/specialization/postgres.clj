...
(defmethod specialization/->db-type :postgres [t]
...
(defmethod specialization/base-query :postgres
  ...
  {:select select-items
   ...
   :join   [[[:to_tsquery [:inline tsv-language] [:lift (to-tsquery-expr search-term)]] :query]
            [:= [:inline 1] [:inline 1]]]
...
(defmethod specialization/text-score :postgres [_search-ctx]
