...
(def ^:private date-bucketing-units
  "Set of valid units for bucketing or comparing against a *date* [:ref ::field-or-expression."
  (conj lib.schema.temporal-bucketing/date-bucketing-units :default))

(def ^:private time-bucketing-units
  "Set of valid units for bucketing or comparing against a *time* [:ref ::field-or-expression."
  (conj lib.schema.temporal-bucketing/time-bucketing-units :default))

(def ^:private datetime-bucketing-units
  "Set of valid units for bucketing or comparing against a *datetime* [:ref ::field-or-expression."
  (conj lib.schema.temporal-bucketing/datetime-bucketing-units :default))

;;; Valid unit for *date* bucketing.
(mr/def ::unit.date
  [:or {:error/message "date bucketing unit"}
   [:= :default]
...
(mr/def ::unit.time
  [:or
   {:error/message "time bucketing unit"}
   [:= :default]
...
(def ^{:arglists '([s])} ^:deprecated can-parse-datetime?
...
(defclause relative-datetime
  n    [:or [:= :current] :int]
  unit (optional [:ref ::lib.schema.temporal-bucketing/unit.date-time.interval]))
...
;; types) are encountered. Unit is inferred by looking at the [:ref ::field-or-expression the timestamp is compared against. Implemented
...
(mr/def ::absolute-datetime
  [:multi
   ...
   [:invalid (helpers/clause
              ...
              "unit" [:ref ::lib.schema.temporal-bucketing/unit])]
   [:date (helpers/clause
           :absolute-datetime
           "date" [:ref ::lib.schema.literal/date]
           "unit" [:ref ::unit.date])]
   [:datetime (helpers/clause
               :absolute-datetime
               "datetime" [:ref ::lib.schema.literal/datetime]
               "unit"     [:ref ::lib.schema.temporal-bucketing/unit])]])
...
;; clearly a time (e.g. "08:00:00.000") and/or the [:ref ::field-or-expression derived from `:type/Time` and/or the unit was a
;; time-bucketing unit
;;
;; TODO - should we have a separate `date` type as well
(defclause ^:internal time
  time [:ref ::lib.schema.literal/time]
  unit [:ref ::unit.time])

(mr/def ::literal.date-or-datetime
  [:or
   absolute-datetime
...
(mr/def ::literal.time
  [:or
   time
   ;; time literals should get transformed into [[time]] clauses automatically by QP middleware.
   [:ref ::lib.schema.literal/time]])

(mr/def ::literal.temporal
  [:or [:ref ::literal.time] [:ref ::literal.date-or-datetime]])

(mr/def ::date-time-value
...
;;; filter clauses based on the [:ref ::field-or-expression in the clause.
(mr/def ::value.type-info
...
;; treat certain objects. For example, a string compared against a Postgres UUID [:ref ::field-or-expression needs to be parsed into a UUID
;; object, since text <-> UUID comparison doesn't work in Postgres. For this reason, raw literals in `:filter`
;; clauses are wrapped in `:value` clauses and given information about the type of the [:ref ::field-or-expression they will be compared to.
(defclause ^:internal value
  value    :any
  type-info [:maybe [:ref ::value.type-info]])
...
(defclause ^{:requires-features #{:expressions}} expression
  expression-name [:ref ::lib.schema.common/non-blank-string]
...
(mr/def ::field.binning.options
  (-> [:map
       [:strategy                   [:ref ::lib.schema.binning/binning-strategies]]
       [:num-bins {:optional true}  [:ref ::lib.schema.common/positive-int]]
...
(mr/def ::field.options
  (-> [:map
       [:base-type {:optional true} [:maybe [:ref ::lib.schema.common/base-type]]]
       ;;
       ;; replaces `fk->`
       ;;
       ;; `:source-field` is used to refer to a [:ref ::field-or-expression from a different Table you would like IMPLICITLY JOINED to the
       ;; source table.
       ;;
       ...
       [:source-field {:optional true} [:maybe [:ref ::lib.schema.id/field]]]
       ;;
       ;; `:temporal-unit` is used to specify DATE BUCKETING for a [:ref ::field-or-expression that represents a moment in time of some sort.
       ;;
       ;; There is no requirement that all `:type/Temporal` derived [:ref ::field-or-expressions specify a `:temporal-unit`, but for legacy
       ;; reasons `:field` clauses that refer to `:type/DateTime` [:ref ::field-or-expressions will be automatically "bucketed" in the
       ...
       ;; do any special datetime handling for plain `:field` clauses when their [:ref ::field-or-expression derives from `:type/DateTime`.
       [:temporal-unit {:optional true} [:maybe [:ref ::lib.schema.temporal-bucketing/unit]]]
       ;;
       ;; replaces `joined-field`
       ;;
       ;; `:join-alias` is used to refer to a [:ref ::field-or-expression from a different Table/nested query that you are EXPLICITLY
       ;; JOINING against.
       [:join-alias {:optional true} [:maybe [:ref ::lib.schema.common/non-blank-string]]]
       ;;
       ;; replaces `binning-strategy`
       ;;
       ;; Using binning requires the driver to support the `:binning` feature.
       [:binning {:optional true} [:maybe [:ref ::field.binning.options]]]]
...
(def ^{:clause-name :field, :added "0.39.0"} field
  ...
  (-> (helpers/clause
       :field
       "id-or-name" [:or [:ref ::lib.schema.id/field] [:ref ::lib.schema.common/non-blank-string]]
       "options"    [:maybe [:ref ::field.options]])
      require-base-type-for-field-name))

(def ^{:clause-name :field, :added "0.39.0"} field:id
  "Schema for a `:field` clause, with the added constraint that it must use an integer [:ref ::field-or-expression ID."
  [:and
   field
   [:fn
    {:error/message "Must be a :field with an integer [:ref ::field-or-expression ID."}
    (fn [[_ id-or-name]]
      (integer? id-or-name))]])

(mr/def ::field-or-expression
  ...
  [:multi
   {:dispatch (fn [x]
                (cond
                  ...
                  :else                           :else))}
   [:string            :string]
   [:string-expression [:ref ::expression.string]]
   [:value             value]
   [:else              [:ref ::field-or-expression]]])

(def numeric-functions
  "Functions that return numeric values. Should match [[[:ref ::expression.numeric]]."
...
(def ^:private boolean-functions
  "Functions that return boolean values. Should match [[[:ref ::expression.boolean]]."
  #{:and :or :not :< :<= :> :>= := :!=})
...
(mr/def ::expression.arg.datetime
  [:multi
   {:dispatch (fn [x]
                (cond
                  (is-clause? aggregations x)       :aggregation
...
(mr/def ::expression.arg.expression
  [:multi
   {:dispatch (fn [x]
                (cond
                  (number? x)                       :number
   ...
   [:number              number?]
   [:boolean             :boolean]
   [:boolean-expression  [:ref ::expression.boolean]]
   [:numeric-expression  [:ref ::expression.numeric]]
   [:datetime-expression [:ref ::expression.datetime]]
   [:string              :string]
   [:string-expression   [:ref ::expression.string]]
...
(mr/def ::expression.arg.numeric-or-interval
  [:or
   interval
   [:ref ::expression.arg.numeric]])

(mr/def ::positive-int-or-numeric-expression
  [:multi
   {:dispatch (fn [x]
                (if (number? x)
                  :number
                  :else))}
   [:number [:ref ::lib.schema.common/positive-int]]
   [:else  [:ref ::expression.arg.numeric]]])

(defclause ^{:requires-features #{:expressions}} coalesce
  a [:ref ::expression.arg.expression], b [:ref ::expression.arg.expression], more (rest [:ref ::expression.arg.expression]))

(defclause ^{:requires-features #{:expressions}} substring
  s [:ref ::expression.arg.string], start [:ref ::positive-int-or-numeric-expression], length (optional [:ref ::expression.arg.numeric]))

(defclause ^{:requires-features #{:expressions}} length
  s [:ref ::expression.arg.string])
...
(defclause ^{:requires-features #{:expressions}} replace
  s [:ref ::expression.arg.string], match :string, replacement :string)

(defclause ^{:requires-features #{:expressions}} concat
  a [:ref ::expression.arg.string], b [:ref ::expression.arg.string], more (rest [:ref ::expression.arg.string]))

(defclause ^{:requires-features #{:expressions :regex}} regex-match-first
  s [:ref ::expression.arg.string], pattern :string)

(defclause ^{:requires-features #{:expressions}} +
  x [:ref ::expression.arg.numeric-or-interval], y [:ref ::expression.arg.numeric-or-interval], more (rest [:ref ::expression.arg.numeric-or-interval]))

(defclause ^{:requires-features #{:expressions}} -
  x [:ref ::expression.arg.numeric], y [:ref ::expression.arg.numeric-or-interval], more (rest [:ref ::expression.arg.numeric-or-interval]))

(defclause ^{:requires-features #{:expressions}} /, x [:ref ::expression.arg.numeric], y [:ref ::expression.arg.numeric], more (rest [:ref ::expression.arg.numeric]))

(defclause ^{:requires-features #{:expressions}} *, x [:ref ::expression.arg.numeric], y [:ref ::expression.arg.numeric], more (rest [:ref ::expression.arg.numeric]))

(defclause ^{:requires-features #{:expressions}} floor
  x [:ref ::expression.arg.numeric])
...
(defclause ^{:requires-features #{:advanced-math-expressions}} power
  x [:ref ::expression.arg.numeric],  y [:ref ::expression.arg.numeric])
...
(defclause ^{:requires-features #{:datetime-diff}} datetime-diff
  datetime-x [:ref ::expression.arg.datetime]
  datetime-y [:ref ::expression.arg.datetime]
  unit       [:ref ::lib.schema.temporal-bucketing/unit.date-time.truncate])

(defclause ^{:requires-features #{:temporal-extract}} temporal-extract
  datetime [:ref ::expression.arg.datetime]
  unit     [:ref ::lib.schema.expression.temporal/temporal-extract.unit]
  mode     (optional [:ref ::lib.schema.expression.temporal/temporal-extract.week-mode])) ;; only for get-week

;; SUGAR CLAUSE: get-year, get-month... clauses are all sugars clause that will be rewritten as [:temporal-extract column :year]
(defclause ^{:requires-features #{:temporal-extract}} get-year
  date [:ref ::expression.arg.datetime])

(defclause ^{:requires-features #{:temporal-extract}} get-quarter
  date [:ref ::expression.arg.datetime])

(defclause ^{:requires-features #{:temporal-extract}} get-month
  date [:ref ::expression.arg.datetime])

(defclause ^{:requires-features #{:temporal-extract}} get-week
  date [:ref ::expression.arg.datetime]
  mode (optional [:ref ::lib.schema.expression.temporal/temporal-extract.week-mode]))

(defclause ^{:requires-features #{:temporal-extract}} get-day
  date [:ref ::expression.arg.datetime])

(defclause ^{:requires-features #{:temporal-extract}} get-day-of-week
  date [:ref ::expression.arg.datetime])

(defclause ^{:requires-features #{:temporal-extract}} get-hour
  datetime [:ref ::expression.arg.datetime])

(defclause ^{:requires-features #{:temporal-extract}} get-minute
  datetime [:ref ::expression.arg.datetime])

(defclause ^{:requires-features #{:temporal-extract}} get-second
  datetime [:ref ::expression.arg.datetime])
...
(defclause ^{:requires-features #{:date-arithmetics}} datetime-add
  datetime [:ref ::expression.arg.datetime]
  amount   [:ref ::expression.arg.numeric]
  unit     [:ref ::lib.schema.temporal-bucketing/unit.date-time.interval])
...
(defclause not, clause [:ref ::filter])

(mr/def ::field-or-relative-datetime
  [:multi
   {:dispatch (fn [x]
...
(mr/def ::equality-comparable
  [:maybe
   [:or
    ...
    number?
    :string
    [:ref ::literal.temporal]
    [:ref ::field-or-relative-datetime]
    [:ref ::expression.arg.expression]
    value]])

;;; Schema for things that make sense in a filter like `>` or `<`, i.e. things that can be sorted.
(mr/def ::order-comparable
  [:multi
   {:dispatch (fn [x]
...
;; For all of the non-compound [:ref ::filter clauses below the first arg is an implicit [:ref ::field-or-expression ID
...
(defclause =,  field [:ref ::equality-comparable], value-or-field [:ref ::equality-comparable], more-values-or-fields (rest [:ref ::equality-comparable]))
(defclause !=, field [:ref ::equality-comparable], value-or-field [:ref ::equality-comparable], more-values-or-fields (rest [:ref ::equality-comparable]))

(defclause <,  field [:ref ::order-comparable], value-or-field [:ref ::order-comparable])
(defclause >,  field [:ref ::order-comparable], value-or-field [:ref ::order-comparable])
(defclause <=, field [:ref ::order-comparable], value-or-field [:ref ::order-comparable])
(defclause >=, field [:ref ::order-comparable], value-or-field [:ref ::order-comparable])

;; :between is INCLUSIVE just like SQL !!!
(defclause between field [:ref ::order-comparable], min [:ref ::order-comparable], max [:ref ::order-comparable])

;; SUGAR CLAUSE: This is automatically written as a pair of `:between` clauses by the `:desugar` middleware.
(defclause inside
  lat-field [:ref ::order-comparable]
  lon-field [:ref ::order-comparable]
  lat-max   [:ref ::order-comparable]
  lon-min   [:ref ::order-comparable]
  lat-min   [:ref ::order-comparable]
  lon-max   [:ref ::order-comparable])

;; SUGAR CLAUSES: These are rewritten as `[:= <field> nil]` and `[:not= <field> nil]` respectively
(defclause is-null,  field [:ref ::field-or-expression])
(defclause not-null, field [:ref ::field-or-expression])

;; These are rewritten as `[:or [:= <field> nil] [:= <field> ""]]` and
;; `[:and [:not= <field> nil] [:not= <field> ""]]`
(defclause is-empty,  field [:ref ::field-or-expression])
(defclause not-empty, field [:ref ::field-or-expression])
...
(defclause starts-with
  field           [:ref ::expression.arg.string]
  string-or-field [:ref ::expression.arg.string]
  options         (optional [:ref ::string-filter-options]))
...
;; [:ref ::filter subclause. Syntactic sugar for specifying a specific time interval.
;;
;; Return rows where datetime [:ref ::field-or-expression 100's value is in the current month
;;
;;    [:time-interval [:field 100 nil] :current :month]
;;
;; Return rows where datetime [:ref ::field-or-expression 100's value is in the current month, including partial results for the
;; current day
;;
...
(defclause time-interval
  ...
  unit    [:ref ::lib.schema.temporal-bucketing/unit.date-time.interval]
  options (optional [:ref ::time-interval-options]))

;; A segment is a special `macro` that saves some pre-definied filter clause, e.g. [:segment 1]
;; this gets replaced by a normal [:ref ::filter clause in MBQL macroexpansion
;;
...
(defclause segment, segment-id [:or [:ref ::lib.schema.id/segment] [:ref ::lib.schema.common/non-blank-string]])

(mr/def ::expression.boolean
  (one-of and or not < <= > >= = !=))
  ...
  [:multi
   {:dispatch (fn [x]
                (cond
                  ...
                  :else                             :else))}
   [:datetime [:ref ::expression.datetime]]
   [:numeric  [:ref ::expression.numeric]]
   [:string   [:ref ::expression.string]]
   [:boolean  [:ref ::expression.boolean]]
   [:else    (one-of
...
(mr/def ::case.clauses
  [:sequential [:ref ::case.clause]])

(mr/def ::case.options
  [:map
   [:default {:optional true} [:ref ::expression.arg.expression]]])

(defclause ^{:requires-features #{:basic-aggregations}} case
  clauses [:ref ::case.clauses], options (optional [:ref ::case.options]))

(mr/def ::expression.numeric
...
;; For all of the 'normal' Aggregations below (excluding Metrics) fields are implicit [:ref ::field-or-expression IDs
...
(defclause ^{:requires-features #{:basic-aggregations}} count,     field (optional [:ref ::field-or-expression]))
(defclause ^{:requires-features #{:basic-aggregations}} cum-count, field (optional [:ref ::field-or-expression]))
...
(defclause ^{:requires-features #{:basic-aggregations}} avg,      field-or-expression [:ref ::field-or-expression-def])
(defclause ^{:requires-features #{:basic-aggregations}} cum-sum,  field-or-expression [:ref ::field-or-expression-def])
(defclause ^{:requires-features #{:basic-aggregations}} distinct, field-or-expression [:ref ::field-or-expression-def])
(defclause ^{:requires-features #{:basic-aggregations}} sum,      field-or-expression [:ref ::field-or-expression-def])
(defclause ^{:requires-features #{:basic-aggregations}} min,      field-or-expression [:ref ::field-or-expression-def])
(defclause ^{:requires-features #{:basic-aggregations}} max,      field-or-expression [:ref ::field-or-expression-def])

(defclause ^{:requires-features #{:basic-aggregations}} sum-where
  field-or-expression [:ref ::field-or-expression-def], pred [:ref ::filter])
...
(defclause ^{:requires-features #{:standard-deviation-aggregations}} stddev
  field-or-expression [:ref ::field-or-expression-def])
...
(defclause ^{:requires-features #{:percentile-aggregations}} percentile
  field-or-expression [:ref ::field-or-expression-def], percentile [:ref ::expression.arg.numeric])
...
(defclause metric
  metric-id [:or [:ref ::lib.schema.id/metric] [:ref ::lib.schema.common/non-blank-string]])
...
(mr/def ::aggregation-options.options
  [:map
   ;; name to use for this aggregation in the native query instead of the default name (e.g. `count`)
   [:name         {:optional true} [:ref ::lib.schema.common/non-blank-string]]
...
;;
;; [:ref ::field-or-expression ID is implicit in these clauses

(defclause asc,  field [:ref ::field-or-aggregation-ref])
(defclause desc, field [:ref ::field-or-aggregation-ref])
...
(mr/def ::query.source
  [:multi
   ...
   ;; queries).
   [:native [:ref ::query.inner.native]]
   [:mbql   [:ref ::query.inner.mbql]]])

(mr/def ::query.source.metadata
  [:map
   [:name         [:ref ::lib.schema.common/non-blank-string]]
...
(mr/def ::join
  [:and
   [:map
    ...
    ;;
    ;; The [:ref ::field-or-expressions to include in the results *if* a top-level `:fields` clause *is not* specified. This can be either
    ;; `:none`, `:all`, or a sequence of [:ref ::field-or-expression clauses.
    ;;
    ;; *  `:none`: no [:ref ::field-or-expressions from the joined table or nested query are included (unless indirectly included by
    ;;    breakouts or other clauses). This is the default, and what is used for automatically-generated joins.
    ;;
    ;; *  `:all`: will include all of the [:ref ::field-or-expressions from the joined table or query
    ;;
    ;; *  a sequence of [:ref ::field-or-expression clauses: include only the [:ref ::field-or-expressions specified. Valid clauses are the same as the top-level
    ...
    [:fields {:optional true} [:or
                               {:error/message "Valid [:ref ::join `:fields`: `:all`, `:none`, or a sequence of `:field` clauses that have `:join-alias`."}
                               [:enum :all :none]
                               [:ref ::fields]]]
    ;;
    ...
    ;;
    ;; Driver implementations: This is guaranteed to be present after pre-processing.
    [:alias {:optional true} [:ref ::lib.schema.common/non-blank-string]]
    ;;
    ...
    [:fk-field-id {:optional true} [:maybe [:ref ::lib.schema.id/field]]]
    ;;
    ...
    [:source-metadata {:optional true} [:maybe [:sequential [:ref ::query.source.metadata]]]]]
   [:fn
...
(mr/def ::fields
  [:schema
   {:error/message "Distinct, non-empty sequence of [:ref ::field-or-expression clauses"}
   (helpers/distinct (helpers/non-empty [:sequential [:ref ::field-or-expression]]))])

(mr/def ::page
  [:map
   [:page  [:ref ::lib.schema.common/positive-int]]
   [:items [:ref ::lib.schema.common/positive-int]]])

(mr/def ::query.inner.mbql
  [:and
   [:map
    [:source-query    {:optional true} [:ref ::query.source]]
    [:source-table    {:optional true} [:ref ::source-table]]
    ...
    [:limit           {:optional true} [:ref ::lib.schema.common/int-greater-than-or-equal-to-zero]]
    [:order-by        {:optional true} (helpers/distinct (helpers/non-empty [:sequential [:ref ::order-by]]))]
    ...
    ;; used to let power things like binning when used with [:ref ::field-or-expression Literals instead of normal [:ref ::field-or-expressions
    [:source-metadata {:optional true} [:maybe [:sequential [:ref ::query.source.metadata]]]]]
   ;;
   ;; CONSTRAINTS
   ;;
   ...
   [:fn
    {:error/message "[:ref ::field-or-expressions specified in `:breakout` should not be specified in `:fields`; this is implied."}
...
(defclause template-tag
  tag-name [:or
            ...
            [:map
             [:id [:ref ::lib.schema.common/non-blank-string]]]])

(defclause dimension
  target [:or [:ref ::field-or-expression] template-tag])
...
(mr/def ::parameter.target
  ...
  ;; `dimension` it comes from a [:ref ::field-or-expression filter template tag parameter (don't quote me on this -- working theory)
  [:or
   [:ref ::field-or-expression]
...
(mr/def ::parameter
  [:map
   ...
   [:id      {:optional true} [:ref ::lib.schema.common/non-blank-string]]
   ...
   ;; on the [[[:ref ::lib.schema.parameter/type]]
...
(mr/def ::query.middleware-options
  [:map
   ...
   ;; If true, this will override the `:max-results` and `:max-results-bare-rows` values in [[[:ref ::query.constraints]]].
...
(mr/def ::query.info
  [:map
   ...
   [:context                   {:optional true} [:maybe [:ref ::query.info.context]]]
   [:executed-by               {:optional true} [:maybe [:ref ::lib.schema.common/positive-int]]]
   [:action-id                 {:optional true} [:maybe [:ref ::lib.schema.common/positive-int]]]
   [:card-id                   {:optional true} [:maybe [:ref ::lib.schema.id/card]]]
   [:card-name                 {:optional true} [:maybe [:ref ::lib.schema.common/non-blank-string]]]
   [:dashboard-id              {:optional true} [:maybe [:ref ::lib.schema.common/positive-int]]]
   [:alias/escaped->original   {:optional true} [:maybe [:map-of :any :any]]]
   [:pulse-id                  {:optional true} [:maybe [:ref ::lib.schema.common/positive-int]]]
   ...
   [:query-hash                {:optional true} [:maybe #?(:clj bytes?
                                                           :cljs :any)]]])

;; To the reader: yes, this seems sort of hacky, but one of the goals of the Nested Query project was to minimize if
;; not completely eliminate any changes to the frontend. After experimenting with several possible ways to do this
...
(mr/def ::query
  (-> [:map
       ...
       [:native     {:optional true} [:ref ::query.native]]
       [:query      {:optional true} [:ref ::query.inner.mbql]]
       [:parameters {:optional true} [:ref ::parameter-list]]
       ;;
       ;; OPTIONS
       ;;
       ...
       ;;
       [:settings    {:optional true} [:maybe [:ref ::query.settings]]]
       [:constraints {:optional true} [:maybe [:ref ::query.constraints]]]
       [:middleware  {:optional true} [:maybe [:ref ::query.middleware-options]]]
       ;;
       ;; INFO
       ;;
       ;; Used when recording info about this run in the QueryExecution log; things like context query was ran in and
       ;; User who ran it
       [:info {:optional true} [:maybe [:ref ::query.info]]]]
      ;;
      ;; CONSTRAINTS
...
;;; all the stuff below can probably be deprecated and replaced with the keyword schema names instead.

(def ^:deprecated DatetimeUnit
  "Valid unit for *datetime* bucketing.

  DEPRECATED: use `:metabase.lib.schema.temporal-bucketing/unit` directly."
  ::lib.schema.temporal-bucketing/unit)

(def Field
  "Schema for either a `:field` clause (reference to a [:ref ::field-or-expression) or an `:expression`
  clause (reference to an expression)."
  ::field-or-expression)

(def FieldOrAggregationReference
  "Schema for any type of valid [:ref ::field-or-expression clause, or for an indexed reference to an aggregation clause."
  ::field-or-aggregation-ref)
...
(def Join
  ...
  In the top-level query, you can reference [:ref ::field-or-expressions from the joined table or nested query by including `:source-field`
...
(def ^:deprecated join-strategies
  "Valid values of the `:strategy` key in a join map.

  DEPRECATED: use [[lib.schema.join/join-strategies]] directly."
  lib.schema.join/join-strategies)
