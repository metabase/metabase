...
describe("issue 15542", () => {
  ...
  function openOrdersTable() {
    ...
    cy.navigationSidebar().findByText("Databases").click({
  ...
  it("should be possible to use the foreign key field display values immediately when changing the setting", () => {
    ...
    cy.visitAlias("@ORDERS_PRODUCT_ID_URL");

    select("Plain input box").click();
    cy.popover().findByText("A list of all values").click();

    select("Use original value").click();
    cy.popover().findByText("Use foreign key").click();
    cy.popover().findByText("Title").click();
    ...
    cy.popover().findByText("Use original value").click();
