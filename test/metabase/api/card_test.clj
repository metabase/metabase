...
(deftest permissions-X-parameters-rewrite-test
  (mt/with-temp* [PermissionsGroup [admin-pg {:name "Test Administrators"}]
                  ...
                  User [admin-user {:first_name "All-User" :email "mr.all@user.com"}]
                  PermissionsGroupMembership [_all-pgm {:user_id (:id admin-user) :group_id (:id admin-pg)}]

                  User [all-user {:first_name "Admin-User" :email "mr.admin@user.com"}]
                  PermissionsGroupMembership [_admin-pgm {:user_id (:id all-user) :group_id (:id all-pg)}]

                  Table [table {:name "table" :db_id (:id (mt/db))}]
                  Field [field {:name "field" :table_id (:id table)}]

                  Card [source-card {:database_id   (:id (mt/db))
                                     :table_id      (:id table)
                                     :dataset_query {:database (:id (mt/db))
                                                     :type     :query
                                                     :query    {:limit 5 :source-table (:id table)}}}]
                  Card [field-filter-card  {:dataset_query
                                            {:database (:id (mt/db)) ;; can use the usual db since we re-created permission entries
                                             :type     :native
                                             :native   {:query         "SELECT COUNT(*) FROM VENUES WHERE {{NAME}}"
                                                        :template-tags {"NAME" {:id           "name_param_id"
                                                                                :name         "NAME"
                                            ...
                                            :parameters [{:id     "name_param_id"
                                                          ...
                                                          :name   "Name"
                                                          :slug   "NAME"}]}]
                  Card [card        {:database_id   (:id (mt/db))
                                     :dataset_query {:database (:id (mt/db))
                                                     :type     :query
                                     ...
                                     :parameters    [{:name                 "Static Category",
                                                      ...
                                                      :values_source_type   "static-list",
                                                      :values_source_config {:values ["African" "American" "Asian"]}}
                                                     {:name                 "Static Category with label",
                                                      :slug                 "static_category_label",
                                                      :id                   "_STATIC_CATEGORY_LABEL_",
                                                      :type                 "category",
                                                      :values_source_type   "static-list",
    ...
    (is (= #{(str "/db/" (:id (mt/db)) "/")
             (str "/data/db/" (:id (mt/db)) "/")
             (str "/query/db/" (:id (mt/db)) "/")}
           (t2/select-fn-set :object :permissions {:where [:= :group_id (:id all-pg)]})))

    (is (= #{"/"}
           (t2/select-fn-set :object :permissions {:where [:= :group_id (:id admin-pg)]})))
    ...
    (is (= {:values          []
            :field_id        (:id field)
            :has_more_values false}
           (mt/user-http-request all-user :get 200 (param-values-url field-filter-card "name_param_id"))))

    (is (= {:values          ["African" "American" "Asian"]
            :has_more_values false} (mt/user-http-request all-user :get 200 (param-values-url card "_STATIC_CATEGORY_"))))

    (is (= {:values          [["African" "Af"] ["American" "Am"] ["Asian" "As"]]
            :has_more_values false} (mt/user-http-request all-user :get 200 (param-values-url card "_STATIC_CATEGORY_LABEL_"))))
