...
(mr/def ::column
  [:map
   ...
   ;; this gets added by the [[metabase.query-processor.middleware.annotate]] middleware and is generally present for
   ;; metadata for custom columns (ones that come from expressions in this stage of the query) if that metadata
   ;; originally came from the QP/results metadata (e.g. the JS version of metadata)
   [:expression-name    {:optional true} [:maybe ::lib.schema.common/non-blank-string]]
