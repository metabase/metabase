    ...
    (doseq [line lines]
      ...
      (when @in-report?
        (cond
          ...
          (let [clean-line (str/replace line #"^[0-9T:.Z-]+ +" "")]
            (swap! results conj {:type :summary :text (str/trim clean-line)}))
          ;; Failed count line (contains "test" and "failed" but not config text)
          (and (str/includes? line "failed")
               (str/includes? line "quarantined"))
          (let [clean-line (str/replace line #"^[0-9T:.Z-]+ +" "")]
            (swap! results conj {:type :failed-count :text (str/trim clean-line)}))
          ;; Package name with 📦
          (str/includes? line "📦")
...
(defn generate-report
  ...
  (let [{:keys [failed pending passed]} (categorize-checks checks)
    ...
    (println "---")
    (println)
    (println (format "_Generated by `bin/ci-failures.bb` at %s UTC_"
