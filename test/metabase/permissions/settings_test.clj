(ns metabase.permissions.settings-test
  (:require
   ...
   [java-time.api :as t]
   [metabase.permissions.settings :as perms.settings]
   [metabase.test :as mt]))
  ...
  (doseq [instance-creation-time [(t/local-date-time 2020) (t/local-date-time 2022)]
    ...
    (testing (str "instance-creation-time: " instance-creation-time
      ...
      (mt/with-current-user (mt/user->id :crowberto)
        (perms.settings/show-updated-permission-modal! modal-setting-value)
        (with-redefs [perms.settings/instance-create-time (constantly instance-creation-time)
                      perms.settings/v-fifty-migration-time (constantly fifty-migration-time)]
          (let [expected-modal-value (get ->expected
            ...
            (is (= expected-modal-value (perms.settings/show-updated-permission-modal)))))))))
...
(deftest show-updated-permission-banner-test
  (doseq [instance-creation-time [(t/local-date-time 2020) (t/local-date-time 2022)]
    ...
    (testing (str "instance-creation-time: " instance-creation-time
      ...
      (mt/with-current-user (mt/user->id :crowberto)
        (perms.settings/show-updated-permission-banner! banner-setting-value)
        (with-redefs [perms.settings/instance-create-time (constantly instance-creation-time)
                      perms.settings/v-fifty-migration-time (constantly fifty-migration-time)]
          (let [expected-banner-value (get ->expected
                                           {:new-admin? (t/after? instance-creation-time fifty-migration-time)
                                            :setting-value banner-setting-value})]
            (is (= expected-banner-value (perms.settings/show-updated-permission-banner)))))))))
