...
(def ^:private generic-sql-engines
  (delay (set (for [engine datasets/all-valid-engines
                    ...
                    ; bigquery, presto, athena and sparksql don't use the generic sql implementations of things like `field-avg-length`
                    :when  (not (contains? #{:bigquery :presto :sparksql :athena} engine))
