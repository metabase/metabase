(ns metabase.driver.sql.query-processor
  ...
  (:require [clojure.core.match :refer [match]]
            ...
            [metabase.driver.sql.query-processor.preprocess.add-references :as sql.qp.references]
...
;; TODO -- this should be added to the query as a key e.g. `:sql.qp/depth`
(def ^:dynamic ^:deprecated *nested-query-level*
...
;; NOCOMMIT
(defn NOCOMMIT-pretty-print-query [message query]
  (println message)
  (-> query
      ((requiring-resolve 'dev.debug-qp/add-names))
      ((requiring-resolve 'puget.printer/cprint) {:color-scheme {:nil [:bold :white]}}))
  query)

(defn mbql->honeysql
  "Build the HoneySQL form we will compile to SQL and execute."
  [driver {inner-query :query}]
  (u/prog1 (apply-clauses driver {} (NOCOMMIT-pretty-print-query "PREPROCESSED" (preprocess/preprocess driver inner-query)))
...
(defn mbql->native
  ...
  (NOCOMMIT-pretty-print-query "QUERY" inner-query)
