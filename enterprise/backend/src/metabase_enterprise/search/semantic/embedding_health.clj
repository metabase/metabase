(ns metabase-enterprise.search.semantic.embedding-health
...
(defn- probe-embedding-service
  ...
  (try
    ...
    (catch InterruptedException e
      (throw e))
    (catch Exception e
      (log/debugf "Embedding service health probe failed: %s" (ex-message e))
...
(defn- recover-circuit! []
  (try
    ...
    (catch InterruptedException e
      (throw e))
    (catch Exception e
      (log/debugf "Embedding service circuit recovery probe failed: %s" (ex-message e)))
    (finally
...
(defn request-circuit-recovery!
  ...
  (when (and (embedding/embedder-circuit-untrusted?)
    ...
    (try
      ...
      (catch Exception e
        (swap! recovery-state assoc :running? false)
        (log/errorf "Failed to schedule embedding service circuit recovery: %s" (ex-message e)))))
  nil)
