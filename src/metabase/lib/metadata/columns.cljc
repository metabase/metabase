(ns metabase.lib.metadata.columns
...
(mu/defn ^:private field-metadata :- [:maybe lib.metadata/FieldMetadata]
  [query        :- PipelineQuery
   stage-number :- :int
   field-id-or-name]
  (lib.metadata/field-metadata query stage-number nil field-id-or-name))
...
(mu/defn ^:private cols-for-fields :- [:sequential lib.metadata/FieldMetadata]
  [query        :- PipelineQuery
   stage-number :- :int]
...
(mu/defn ^:private cols-for-ags-and-breakouts :- [:sequential lib.metadata/FieldMetadata]
  [query        :- PipelineQuery
   stage-number :- :int]
...
(mu/defn ^:private source-table-default-metadata :- [:sequential lib.metadata/FieldMetadata]
...
(mu/defn ^:private fields-from-joins :- [:sequential lib.metadata/FieldMetadata]
...
(mu/defn ^:private default-fields :- [:sequential lib.metadata/FieldMetadata]
  [query        :- PipelineQuery
   stage-number :- :int]
  (let [{:keys [source-table joins], :as stage} (lib.util/query-stage query stage-number)]
    (concat (if-let [previous-stage-number (lib.util/previous-stage-number stage stage-number)]
              (let [previous-stage (lib.util/query-stage query previous-stage-number)]
                (some-> previous-stage lib.metadata/stage-metadata :columns))
              (when (integer? source-table)
                (source-table-default-metadata
                 (lib.metadata/table-metadata (lib.metadata/database-metadata query) source-table))))
            (fields-from-joins query stage-number joins))))

(mu/defn cols-for-mbql-query :- [:sequential lib.metadata/FieldMetadata]
...
(mu/defn ^:private stage-metadata :- lib.metadata/StageMetadata
  [query        :- PipelineQuery
   stage-number :- :int]
  (lib.metadata/stage-metadata (cols-for-mbql-query query stage-number)))

(defn populate-stage-metadatas
  "Attempt to update `stages` to add `:lib/stage-metadata` to every stage that does not already have it."
  [query]
  (reduce
   (fn [query stage-number]
     (let [stage (lib.util/query-stage query stage-number)]
       (if (:lib/stage-metadata stage)
         query
         (let [metadata (stage-metadata query stage-number)]
           (lib.util/update-query-stage query stage-number assoc :lib/stage-metadata metadata)))))
   query
