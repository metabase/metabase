...
(def ^:private ApplicationPermissionsGraph
  [:map {:closed true}
   [:revision :int]
   ;; not part of the graph itself; body-level revision-check bypass read by `perms/check-revision-numbers`
...
(def ^:private ApplicationPermissionsGraphUpdate
  "An incoming graph. Unlike the one [[graph]] returns, `:revision` is optional: a forced update never reads it, and
  on the non-forced path a missing revision is a 409 from `perms/check-revision-numbers` rather than a schema error."
  [:map {:closed true}
