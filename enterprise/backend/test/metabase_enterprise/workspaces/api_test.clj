...
(deftest workspace-crud-test
  (mt/with-premium-features #{:workspaces}
    (mt/with-model-cleanup [:model/Workspace]
      (let [ws (mt/user-http-request :crowberto :post 200 "ee/workspace" {:branch "alex/my-branch"})]
        ...
        (testing "GET /api/ee/workspace/:id"
          (is (=? {:id (:id ws), :branch "alex/my-branch"}
                  (mt/user-http-request :crowberto :get 200 (str "ee/workspace/" (:id ws))))))
        (testing "PUT /api/ee/workspace/:id"
          (is (=? {:id (:id ws), :branch "alex/renamed"}
                  (mt/user-http-request :crowberto :put 200 (str "ee/workspace/" (:id ws))
                                        {:branch "alex/renamed"}))))
...
(deftest workspace-admin-only-test
  (mt/with-premium-features #{:workspaces}
    (mt/with-temp [:model/Workspace ws {:branch "alex/admin-only"}]
      (testing "workspace endpoints are superuser-only"
        (mt/user-http-request :rasta :get 403 "ee/workspace")
        (mt/user-http-request :rasta :get 403 (str "ee/workspace/" (:id ws)))
        (mt/user-http-request :rasta :post 403 "ee/workspace" {:branch "alex/nope"})
        (mt/user-http-request :rasta :put 403 (str "ee/workspace/" (:id ws)) {:branch "alex/nope"})
        (mt/user-http-request :rasta :delete 403 (str "ee/workspace/" (:id ws)))))))

(deftest workspace-requires-feature-test
  (mt/with-premium-features #{}
    (testing "the /api/ee/workspace routes 402 without the :workspaces feature"
      (mt/user-http-request :crowberto :get 402 "ee/workspace"))))

(deftest set-current-workspace-test
  (mt/with-premium-features #{:workspaces}
    (mt/with-temp [:model/Workspace ws {:branch "My branch"}]
      (testing "an admin can set their own active workspace and read it back"
        (is (=? {:workspace_id (:id ws)}
                (mt/user-http-request :crowberto :put 200 (str "user/" (mt/user->id :crowberto))
                                      {:workspace_id (:id ws)})))
        (is (=? {:workspace_id (:id ws)}
                (mt/user-http-request :crowberto :get 200 "user/current"))))
      (testing "a non-admin cannot activate a workspace (read-check, 403)"
        (mt/user-http-request :rasta :put 403 (str "user/" (mt/user->id :rasta))
                              {:workspace_id (:id ws)}))
      (testing "setting a nonexistent workspace is a 400"
        (mt/user-http-request :crowberto :put 400 (str "user/" (mt/user->id :crowberto))
