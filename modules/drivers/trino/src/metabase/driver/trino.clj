(ns metabase.driver.trino
  "Trino driver. See https://trino.io/docs/current/ for complete docs."
...
(driver/register! :trino, :parent :sql)

;;; Trino API helpers

(def ^:private TrinoConnectionDetails
  {:host    su/NonBlankString
   :port    (s/cond-pre su/NonBlankString su/IntGreaterThanZero)
   :catalog su/NonBlankString
   s/Any    s/Any})

(defn- trino-type->base-type [field-type]
...
(s/defn ^:private details->uri
  [{:keys [ssl host port]} :- TrinoConnectionDetails, path]
  (str (if ssl "https" "http") "://" host ":" port
       path))

(defn- details->request [{:keys [user password catalog]}]
  (merge {:headers (merge {"X-Trino-Source" "metabase"
                           "X-Trino-User"   user}
                          (when catalog
                            {"X-Trino-Catalog" catalog})
                          (when-let [report-timezone (qp.timezone/report-timezone-id-if-supported :trino)]
                            {"X-Trino-Time-Zone" report-timezone}))}
         (when password
           {:basic-auth [user password]})))

(defn ^:private create-cancel-url [cancel-uri host port info-uri]
  ;; Replace the host in the cancel-uri with the host from the info-uri provided from the trino response- this doesn't
...
(defn- fetch-next-page [details uri]
  ...
  (let [{{:keys [columns data nextUri error]} :body} (http/get uri (assoc (details->request details) :as :json))]
    ...
    (let [parse-row (parse-row-fn columns)
          next-page (delay
                      (when nextUri
                        ;; Might not be the best way, but the pattern is that we poll Trino at intervals
                        (when (empty? data)
                          (Thread/sleep 100))
       ...
       :cols (lazy-seq
              (if (seq columns)
                (for [{col-name :name, col-type :type} columns]
                  {:name      col-name
                   :base_type (trino-type->base-type col-type)})
                (:cols @next-page)))})))
...
(def ^:private trino-metadata-sync-query-timeout
  (u/minutes->ms 2))
...
(defn- execute-query-for-sync
  ...
  (let [result-chan (a/promise-chan)
        query       (if (string? query)
                      query
                      (do
                        (assert (empty? (second query)) "Trino doesn't allow parameterized queries")
                        (first query)))]
    (execute-trino-query details query nil (fn [cols rows]
                                              (a/>!! result-chan {:cols cols, :rows rows})))
    (let [[val] (a/alts!! [result-chan (a/timeout trino-metadata-sync-query-timeout)])]
      (a/close! result-chan)
      val)))

;;; `:sql` driver implementation

(s/defmethod driver/can-connect? :trino
  [driver {:keys [catalog] :as details} :- TrinoConnectionDetails]
...
(defmethod driver/execute-reducible-query :trino
  [driver
   ...
   respond]
  (let [sql     (str "-- "
                     (qputil/query->remark :trino outer-query) "\n"
                     (binding [*param-splice-style* :paranoid]
                       (unprepare/unprepare driver (cons sql params))))
        details (merge (:details (qp.store/database))
                       settings)]
    (execute-trino-query details sql (context/canceled-chan context) respond)))

(defmethod driver/humanize-connection-error-message :trino
  [_ message]
...
(defmethod sql.qp/date [:trino :default]         [_ _ expr] expr)
(defmethod sql.qp/date [:trino :minute]          [_ _ expr] (hsql/call :date_trunc (hx/literal :minute) expr))
(defmethod sql.qp/date [:trino :minute-of-hour]  [_ _ expr] (hsql/call :minute expr))
(defmethod sql.qp/date [:trino :hour]            [_ _ expr] (hsql/call :date_trunc (hx/literal :hour) expr))
(defmethod sql.qp/date [:trino :hour-of-day]     [_ _ expr] (hsql/call :hour expr))
(defmethod sql.qp/date [:trino :day]             [_ _ expr] (hsql/call :date_trunc (hx/literal :day) expr))
(defmethod sql.qp/date [:trino :day-of-month]    [_ _ expr] (hsql/call :day expr))
(defmethod sql.qp/date [:trino :day-of-year]     [_ _ expr] (hsql/call :day_of_year expr))

(defmethod sql.qp/date [:trino :day-of-week]
  [_ _ expr]
  (sql.qp/adjust-day-of-week :trino (hsql/call :day_of_week expr)))

(defmethod sql.qp/date [:trino :week]
  [_ _ expr]
  (sql.qp/adjust-start-of-week :trino (partial hsql/call :date_trunc (hx/literal :week)) expr))

(defmethod sql.qp/date [:trino :month]           [_ _ expr] (hsql/call :date_trunc (hx/literal :month) expr))
(defmethod sql.qp/date [:trino :month-of-year]   [_ _ expr] (hsql/call :month expr))
(defmethod sql.qp/date [:trino :quarter]         [_ _ expr] (hsql/call :date_trunc (hx/literal :quarter) expr))
(defmethod sql.qp/date [:trino :quarter-of-year] [_ _ expr] (hsql/call :quarter expr))
(defmethod sql.qp/date [:trino :year]            [_ _ expr] (hsql/call :date_trunc (hx/literal :year) expr))
