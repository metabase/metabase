...
(deftest login-history-test
  (testing "GET /api/login-history/current"
    (mt/with-temp* [User         [user]
                    Session      [session {:id (str (java.util.UUID/randomUUID)), :user_id (u/the-id user)}]
      ...
      (is (= [{:timestamp          "2021-03-18T20:55:50.955232Z"
              ...
              {:timestamp          "2021-03-18T15:52:20.172351-04:00"
               ...
               :timezone           "ET"}]
             ;; filter out the most recent request, which will be created when we make this request
             (drop 1 (mt/user-http-request user :get 200 "login-history/current")))))))
