...
import { sendAgentRequest } from "metabase/metabot/state/actions";
import { metabotReducer } from "metabase/metabot/state/reducer";
...
describe("sendAgentRequest", () => {
  ...
  it("opens the QB code editor when receiving a qb code edit", async () => {
    ...
    const conversationId =
      store.getState().metabot.conversations.sql?.conversationId;
    ...
    expect(
      store.getState().metabot.reactions.suggestedCodeEdits.qb,
    ).toEqual(
