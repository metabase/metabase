...
(def ^:private metabot-search-models
  (sorted-set "card" "dashboard" "database" "dataset" "document" "measure" "metric" "segment" "table" "transform"))
...
(mu/defn ^{:tool-name  "search"
  ...
  [args :- search-schema]
  (do-search "search" (sorted-set "dashboard" "document" "measure" "metric" "model" "question" "segment" "table") {} args))
...
(mu/defn ^{:tool-name  "search"
  ...
  (let [allowed-types (sorted-set "dashboard" "document" "measure" "metric" "model" "question" "segment" "table")
        args          (cond-> args
