...
(deftest schema-context-field-values-respect-impersonation-test
  (testing "sample values in the LLM schema context come from the impersonated role, not the unrestricted cache"
    (mt/with-premium-features #{:advanced-permissions}
      ;; `build-schema-context` caches per-role `:advanced` FieldValues for *every* eligible venues field, not
      ;; just the one under test; without cleanup they leak into later tests that count `:advanced` rows.
