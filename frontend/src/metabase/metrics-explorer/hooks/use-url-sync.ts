...
export function useUrlSync(hash: string, search: string): void {
  ...
  useEffect(() => {
    if (projectionConfig && loadedSourceCount > 0 && activeTabId !== ALL_TAB_ID) {
      dispatch(fetchAllResults());
    }
  }, [projectionConfig, dimensionOverrides, activeTabId, loadedSourceCount, storedDimensionTabs, dispatch]);
}
