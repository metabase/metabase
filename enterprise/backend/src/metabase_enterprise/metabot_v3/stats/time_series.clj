...
(defn compute-correlations
  ...
  series-map is a map of series-name -> values sequence.
  ...
  (let [series-names (keys series-map)
    ...
    (vec
     (for [[name-a name-b] pairs
           :let [vals-a (get series-map name-a)
                 vals-b (get series-map name-b)
                 coef (dfn/pearsons-correlation vals-a vals-b)]]
       {:series_a name-a
        :series_b name-b
        :coefficient coef
  ...
  (let [series-stats (into {}
        ...
        correlations (when (and (:deep? opts) (> (count series-data) 1))
                       (compute-correlations
                        (into {} (for [[name {:keys [y_values]}] series-data]
                                   [name y_values]))))]
