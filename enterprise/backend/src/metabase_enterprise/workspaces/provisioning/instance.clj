...
(p/defprotocol+ InstanceProvisioner
  ...
  (create! [this workspace]
    "Create a child instance for `workspace`. Returns `{:id <string>, :url <string>}`.")
...
(mu/defn provision-instance! :- :nil
  "Provision a child instance for the workspace (blocking) and persist its
   `:instance_id`/`:instance_url` on the row. No-op when the workspace already
   has an instance, so retries are safe. Throws on failure — the caller records
   the failure on the workspace status."
  ([ws-id]
  ...
  ([ws-id :- pos-int?
   ...
   (let [workspace (t2/select-one :model/Workspace :id ws-id)]
     (when-not (:instance_id workspace)
       (let [{:keys [id url]} (create! provisioner workspace)]
