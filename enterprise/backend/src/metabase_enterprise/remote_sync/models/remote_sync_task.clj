...
(mu/defn create-sync-task!
  "Creates a new remote sync task. Pass `:worktree_id` in `additional-fields` to attach it to a worktree.
...
(defn most-recent-task
  ...
  ([worktree-id]
   (remote-sync.db/most-recent-task worktree-id)))
...
(defn last-version
  ...
  ([] (last-version nil))
  ([worktree-id]
   (:version (remote-sync.db/last-successful-task worktree-id))))
