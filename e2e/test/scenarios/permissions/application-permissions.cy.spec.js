...
cy.describeEE("scenarios > admin > permissions > application", () => {
  beforeEach(() => {
    cy.restore();
    cy.signInAsAdmin();
  ...
  describe("subscriptions permission", () => {
    describe("revoked", () => {
      beforeEach(() => {
        ...
        cy.modifyPermission("All Users", SUBSCRIPTIONS_INDEX, "No");
      ...
      it("revokes ability to create subscriptions and alerts and manage them", () => {
        ...
        cy.sharingMenuButton().should("be.disabled");
    ...
    describe("granted", () => {
      it("gives ability to create dashboard subscriptions and question alerts", () => {
        ...
        cy.visitDashboard(ORDERS_DASHBOARD_ID);
        cy.openSharingMenu(/subscriptions/i);
        cy.sidebar().findByText("Email this dashboard").should("exist");

        cy.log("Create a question alert");
        cy.visitQuestion(ORDERS_QUESTION_ID);
        cy.openSharingMenu(/alert/i);
        cy.modal().findByText("The wide world of alerts").should("be.visible");
      });
    });
  });
