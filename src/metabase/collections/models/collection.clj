...
(def ^:private CollectionVisibilityConfig
  [:map {:closed true, :probe/id "src/metabase/collections/models/collection.clj:833"}
...
(def ^:private UserScope
  [:map {:closed true, :probe/id "src/metabase/collections/models/collection.clj:842"}
...
(mu/defn unarchive-collection!
  ...
  [collection :- CollectionWithLocationAndIDOrRoot
   ...
   updates :- [:map {:closed true, :probe/id "src/metabase/collections/models/collection.clj:1708"}
               [:parent_id {:optional true} [:maybe ms/PositiveInt]]
               [:archived  {:optional true} :boolean]]]
...
(mu/defn archive-or-unarchive-collection!
  ...
  [collection :- CollectionWithLocationAndIDOrRoot
   ;; `updates` is a map *possibly* containing `parent_id`. This allows us to distinguish
   ;; between specifying a `nil` parent_id (move to the root) and not specifying a parent_id.
   updates :- [:map {:closed true, :probe/id "src/metabase/collections/models/collection.clj:1765"}
               [:parent_id {:optional true} [:maybe ms/PositiveInt]]
               [:archived  {:optional true} :boolean]]]
  (if (:archived updates)
