...
(defn- utc-date-time
  ...
  []
  (t/local-date-time (u.date/with-time-zone-same-instant (t/zoned-date-time) "UTC")))
...
(def ^{:arglists '([])} unique-prefix
  "Unique prefix for test datasets for this instance. Format is `<current-date-utc>_<hour>_<site-uuid>_`. See comments above.
  Example:
