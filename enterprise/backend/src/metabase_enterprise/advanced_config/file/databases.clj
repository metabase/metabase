...
(defn- init-sample-database!
  ...
  (if (warehouses.db/database-exists? {:is_sample true})
...
(defn- init-from-config-file!
  ...
  (cond
    ...
    (let [magic-request (format "DELETE_WITH_DEPENDENTS:%s" (:name database))]
      ...
      (when-let [existing-database-id (warehouses.db/select-one-database-pk {:engine (:engine database) :name (:name database)})]
        (log/info (u/format-color :blue "Deleting Database %s with ID %s" (:engine database) existing-database-id))
        (warehouses.db/delete-databases! {:id existing-database-id})))
    ...
    (do
      ...
      (if-let [existing-database-id (warehouses.db/select-one-database-pk {:engine (:engine database) :name (:name database)})]
        (if (:is_stub database)
          ...
          (let [database (cond-> database
            ...
            (warehouses.db/update-databases! {:id existing-database-id} (normalize-settings database))))
        (do
          (log/info (u/format-color :green "Creating new %s Database" (:engine database)))
          (let [db (warehouses.db/insert-database! (normalize-settings database))]
            (cond
              (:is_stub database)
