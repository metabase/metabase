  ...
  (let [remap-python-source #'ws.execute/remap-python-source]
    (testing "remaps to table ID when mapping has :id"
      (let [table-mapping {[1 "public" "orders"] {:db-id  1
            ...
            source        {:type          "python"
                           ...
                           :source-tables [{:alias "orders" :table {:database_id 1
                                                                    :schema      "public"
                                                                    :table       "orders"
                                                                    :table_id    123}}]}]
        (is (= {:type          "python"
                :body          "import pandas as pd"
                :source-tables [{:alias "orders" :table 456}]}
    ...
    (testing "remaps legacy integer to table ID when mapping has :id"
      (let [table-mapping {123 {:db-id  1
            ...
            source        {:type          "python"
                           :body          "import pandas as pd"
                           :source-tables [{:alias "orders" :table 123}]}]
        (is (= {:type          "python"
                :body          "import pandas as pd"
                :source-tables [{:alias "orders" :table 456}]}
               (remap-python-source table-mapping source)))))
      ...
      (let [table-mapping {[1 "public" "orders"] {:db-id  1
            ...
            source        {:type          "python"
                           ...
                           :source-tables [{:alias "orders" :table {:database_id 1
                                                                    ...
                                                                    :table       "orders"
                                                                    :table_id    123}}
                                           {:alias "customers" :table 789}]}]
        (is (= {:type          "python"
                :body          "import pandas as pd"
                :source-tables [{:alias "orders" :table 456}
                                {:alias "customers" :table 999}]}
               (remap-python-source table-mapping source)))))
