...
(deftest ^:parallel swap-source-in-query-card-join-card-implicit-joins-test
  (let [orders-query   (lib/query meta/metadata-provider (meta/table-metadata :orders))
        reviews-query  (lib/query meta/metadata-provider (meta/table-metadata :reviews))
        mp             (-> meta/metadata-provider
                           (lib.tu/metadata-provider-with-card-from-query 1 orders-query)
                           (lib.tu/metadata-provider-with-card-from-query 2 orders-query)
                           (lib.tu/metadata-provider-with-card-from-query 3 reviews-query))
        card-1         (lib.metadata/card mp 1)
        card-2         (lib.metadata/card mp 2)
        query          (as-> (lib/query mp card-1) q
                         (let [lhs (m/find-first #(= (:name %) "ID") (lib/join-condition-lhs-columns q card-2 nil nil))
                               rhs (m/find-first #(= (:name %) "ID") (lib/join-condition-rhs-columns q card-2 nil nil))]
                           (lib/join q (lib/join-clause card-2 [(lib/= lhs rhs)])))
                         (let [cats (filter #(= (:name %) "CATEGORY") (lib/filterable-columns q))]
                           (reduce (fn [q col] (lib/filter q (lib/not-null col))) q cats)))
        join-alias     (-> query :stages first :joins first :alias)
        upgraded-query (lib-be/upgrade-field-refs-in-query query)
        swapped-query  (lib-be/swap-source-in-query upgraded-query
                                                    {:type :card, :id 1}
                                                    {:type :card, :id 3})]
    (testing "should preserve implicit join refs when upgrading"
      (is (=? {:stages [{:source-card 1
                         :filters [[:not-null {} [:field {:source-field (meta/id :orders :product-id)
                                                          :source-field-name "PRODUCT_ID"}
                                                  (meta/id :products :category)]]
                                   [:not-null {} [:field {:source-field (meta/id :orders :product-id)
                                                          :source-field-join-alias join-alias}
                                                  (meta/id :products :category)]]]}]}
              upgraded-query)))
    (testing "should return an identical query if upgrade is not needed"
      (is (= upgraded-query (lib-be/upgrade-field-refs-in-query upgraded-query))))
    (testing "should only change source-field on the first filter, not the joined one"
      (is (=? {:stages [{:source-card 3
                         :filters [[:not-null {} [:field {:source-field (meta/id :reviews :product-id)}
                                                  (meta/id :products :category)]]
                                   [:not-null {} [:field {:source-field (meta/id :orders :product-id)
                                                          :source-field-join-alias join-alias}
                                                  (meta/id :products :category)]]]}]}
