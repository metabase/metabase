(ns metabase-enterprise.search.semantic.index
  (:require
   ...
   [java-time.api :as t]
   [metabase-enterprise.search.semantic.appdb-scoring :as appdb-scoring]
   ...
   [metabase-enterprise.search.semantic.scoring :as scoring]
...
(comment
  ((requiring-resolve 'metabase-enterprise.search.semantic.db.datasource/init-db!))
  (def db @@(requiring-resolve 'metabase-enterprise.search.semantic.db.datasource/data-source)))
...
(def ^:private index-table-name-pattern
  ...
  suffix appended by [[metabase-enterprise.search.semantic.pgvector-api/fresh-index]]),
...
(defn create-hnsw-index-if-not-exists!
  ...
  async build triggered by [[metabase-enterprise.search.semantic.settings/semantic-search-vector-strategy]]).
...
(comment
  (def pgvector ((requiring-resolve 'metabase-enterprise.search.semantic.env/get-pgvector-datasource!)))
  (def embedding-model (embedding/get-configured-model))
  (def index (default-index embedding-model))
