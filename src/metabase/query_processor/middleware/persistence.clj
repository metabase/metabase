(ns metabase.query-processor.middleware.persistence
  (:require [metabase.query-processor.middleware.permissions :as qp.perms]))
...
(defn substitute-persisted-query
  ...
  (if-let [substitute-query (and (not perms) (:persisted-info/native (:query query)))]
    (assoc-in query [:query :source-query] {:native substitute-query})
    query))
