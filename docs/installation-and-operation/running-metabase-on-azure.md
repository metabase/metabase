...
```
If you have trouble connecting, refer to the [postgres configuration instructions](https://www.metabase.com/docs/latest/installation-and-operation/configuring-application-database#postgres) as you may run into a problem with an `@` symbol in the username portion of the connection string.  Using a a combination of `MB_DB_CONNECTION_URI` with seperate `MB_DB_USER` and `MB_DB_PASSWORD` fields also works.
