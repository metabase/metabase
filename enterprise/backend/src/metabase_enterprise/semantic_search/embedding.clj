  ...
  (let [step (fn [{:keys [current-batch current-measure batches] :as acc} text]
               (let [text-measure (measure text)]
                 (cond
                   ...
                   ;; TODO (Chris 2026-06-29) -- BOT-1742: silently dropping an over-budget doc here is a poor
                   ;; default — it vanishes from the index with only a warning. Make the over-budget policy a
                   ;; per-call option (:truncate / :skip / :error, likely truncate-by-default eventually) and
                   ;; check what the ai-service path does, which bypasses create-batches entirely.
