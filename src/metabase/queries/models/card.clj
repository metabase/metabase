...
(mu/defn populate-query-fields
  ...
  ([{query :dataset_query, :as card} :- ::queries.schema/card.partial
...
(mu/defn template-tag-parameters :- ::parameters.schema/parameters
  ...
  [card :- [:maybe ::queries.schema/card.partial]]
...
(mu/defn- assert-valid-type
  ...
  [{query :dataset_query, card-type :type, source-card :source_card_id, :as _card} :- [:maybe ::queries.schema/card.partial]]
...
(mu/defn- assert-not-native-audit-db-query
  [{query :dataset_query, :as _card} :- [:maybe ::queries.schema/card.partial]]
  (when (and (seq query)
...
(defn- pre-insert [card]
  (let [card     (lib/normalize ::queries.schema/card.partial card)
        defaults {:parameters         []
                  :parameter_mappings []
...
(mu/defn model-supports-implicit-actions?
  ...
  [{query :dataset_query :as _card} :- ::queries.schema/card.partial]
  (and (seq query)
...
(mu/defn- upgrade-card-schema-to-latest :- ::queries.schema/card.partial
  [card :- :map]
  (-> (if (and (:id card)
...
(mu/defn- update-associated-parameters!
  ...
  [card-before :- ::queries.schema/card.partial
   card-after  :- ::queries.schema/card.partial]
