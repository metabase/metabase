...
Frontend tasks are executed using `pnpm`. All available tasks can be found in `package.json` under _scripts_.
...
```sh
$ USE_ESLINT=true pnpm build-hot
```
...
```
BETTER_SOURCE_MAPS=true pnpm dev
```
...
```
pnpm test-unit # Run all tests at once
pnpm test-unit-watch # Watch for file changes
```
...
use Metabase, don't forget to start the frontend as well (e.g. with `pnpm build-hot`).
...
- If you are running the frontend with `pnpm build-hot`, set the `MB_FRONTEND_DEV_PORT` environment variable: `MB_FRONTEND_DEV_PORT=8089 MB_EDITION=ee pnpm build-hot`
- If you are building the frontend statically with `pnpm build`, there is nothing different to do
