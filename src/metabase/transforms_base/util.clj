...
(defn mark-inline-index-requests-failed!
  ...
  (when (and (seq index-request-ids) (full-create-run? transform))
    (let [database   (t2/select-one :model/Database (transforms-base.i/target-db-id transform))
          methods    (driver/supported-index-methods (:engine database) database)
          unsettled? (fn [row] (not (contains? #{:succeeded :failed} (:status row))))
          inline-ids (->> (t2/select :model/TableIndex :id [:in index-request-ids])
                          (filter unsettled?)
                          (filter #(= :inline (get-in methods [(:kind (:structured %)) :lifecycle])))
                          (map :id))]
      (when (seq inline-ids)
        (t2/update! :model/TableIndex :id [:in inline-ids]
                    {:status :failed :error_message message :last_executed_at :%now})))))

(defn verify-managed-indexes!
  "Reconcile each index request against what's physically in the warehouse and set its `:status`.
  Only runs on full-create runs (same guard as [[apply-target-indexes!]]). Returns false when the warehouse
  couldn't be read -- affected rows are left `:running` for a future full-create run to re-verify, rather than
  treated as a hard failure (a transient/unreadable warehouse doesn't mean the index that was just applied is
  gone). Returns true otherwise, including when there was nothing to verify."
  [transform]
  (if-not (full-create-run? transform)
    true
        ...
        (let [database (t2/select-one :model/Database (transforms-base.i/target-db-id transform))
          ...
          (if-some [warehouse-indexes (reconcile/fetch-warehouse-indexes database schema table-name)]
            (let [present-keys (into #{} (map reconcile/match-key) warehouse-indexes)
              ...
              true)
            (do
              (log/warnf "verify-managed-indexes!: could not read indexes for %s.%s; leaving %d request(s) for a future run to re-verify"
