...
cy.describeEE("Inaccessible Onboarding checklist", () => {
  beforeEach(() => {
    cy.restore();
    cy.signInAsAdmin();
  ...
  it("should not render when embedded in an iframe", () => {
    cy.visitFullAppEmbeddingUrl({ url: "/", qs: {} });
    ...
    cy.visitFullAppEmbeddingUrl({ url: "/getting-started", qs: {} });
    cy.location("pathname").should("eq", "/");
  });

  it("should not render when the instance is whitelabelled", () => {
    cy.updateSetting("application-name", "Acme, corp.");
...
cy.describeWithSnowplow("Onboarding checklist events", () => {
  beforeEach(() => {
    cy.restore();
    cy.signInAsAdmin();
  ...
  describe("Onboarding checklist page", () => {
    it("should track each item when expanded", () => {
      ...
      cy.expectGoodSnowplowEvents(PAGE_VIEW);
