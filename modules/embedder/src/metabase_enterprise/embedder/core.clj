(ns metabase-enterprise.embedder.core
  "In-process text embedder: sentence-transformers/all-MiniLM-L6-v2 (384-dim, mean-pooled, L2-normalized)
  running inside the Metabase JVM via DJL + ONNX Runtime.
...
(defn warm-up!
  "Load the model and run a probe embedding, so the one-time init cost is paid at a chosen moment rather
  than on the first real indexing run.
