...
(defn load-extract!
  ...
  (serdes/with-cache
    (-> (extract/extract {:include-field-values true :include-metabot true})
        (storage/store! (storage.files/make-backend output-dir)))))
...
(defn add-to-baseline!
  ...
  []
  (storage/store! (into [] (extract/extract {:include-field-values true :include-metabot true})) (storage.files/make-backend source-dir)))
