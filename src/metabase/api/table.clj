...
(defendpoint GET "/"
  "Get all `Tables`."
  []
  (-> (db/sel Table, :active true, {:order-by [[:name :asc]]})
      (hydrate :db)
...
(defendpoint GET "/:id/fields"
  ...
  (let-404 [table (Table id)]
    (read-check table)
    (db/sel Field :table_id id, :visibility_type [not-in ["sensitive" "retired"]], {:order-by [[:name :asc]]})))
...
(defendpoint GET "/:id/fks"
  ...
  (let-404 [table (Table id)]
    (read-check table)
    (let [field-ids (db/sel-idField :table_id id :visibility_type [not= "retired"])]
      (for [origin-field (db/sel Field :fk_target_field_id [in field-ids])]
...
(defendpoint POST "/:id/reorder"
  ...
  (write-check Table id)
  (let [table-fields (db/sel Field :table_id id)]
