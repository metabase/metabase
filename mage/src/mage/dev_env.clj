...
(defn- generate-mise-local!
  ...
  (let [wt-name (worktree-name)
        lines   (cond-> [(str "# Auto-generated by ./bin/mage dev-env")
                         (str "# Worktree: " wt-name " (slot " slot ")")
                         (str "# Re-run `./bin/mage dev-env` to regenerate.")
                         ""
                         "[env]"
...
(defn- print-summary!
  ...
  (let [wt-name (worktree-name)
    ...
    (println (c/cyan "Tear down with: ") (c/yellow "./bin/mage dev-env --down"))))
