...
(api.macros/defendpoint :post "/"
  ...
  (dashboards.write/create-dashboard! dashboard))
...
(api.macros/defendpoint :post "/:from-dashboard-id/copy"
  ...
  (let [existing-dashboard (dashboards.read/get-dashboard from-dashboard-id)
...
(api.macros/defendpoint :get "/:id"
  ...
  (dashboards.read/with-dashboard-load-id dashboard-load-id
    (let [resolved-id (eid-translation/->id-or-404 :dashboard id)
          dashboard (dashboards.read/get-dashboard resolved-id)]
...
(api.macros/defendpoint :get "/:id/items"
  ...
  (let [dashboard  (api/read-check :model/Dashboard id)
    ...
    {:total  (count cards)
     :data   (collection-items/post-process-rows {}
...
(api.macros/defendpoint :put "/:id"
  ...
  [{:keys [id]} :- [:map
                    [:id ms/PositiveInt]]
   _query-params
   dash-updates :- dashboards.write/DashUpdates]
  (dashboards.write/update-dashboard! id dash-updates))
...
(api.macros/defendpoint :put "/:id/cards"
  ...
  [{:keys [id]} :- [:map
   ...
   _query-params
   {:keys [cards tabs]} :- [:map
                            [:cards (ms/maps-with-unique-key [:sequential dashboards.write/UpdatedDashboardCard] :id)]
                            [:tabs  {:optional true} [:maybe (ms/maps-with-unique-key
                                                              [:sequential dashboards.write/UpdatedDashboardTab] :id)]]]]
  (log/warn
   "DELETE /api/dashboard/:id/cards is deprecated. Use PUT /api/dashboard/:id instead.")
  (let [dashboard (dashboards.write/update-dashboard! id {:dashcards cards :tabs tabs})]
...
(api.macros/defendpoint :post "/:dashboard-id/dashcard/:dashcard-id/card/:card-id/query"
  ...
  (dashboards.read/with-dashboard-load-id dashboard_load_id
