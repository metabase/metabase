...
(mr/def ::patch-value
  "A single attribute value in a PATCH operation. Deliberately *not* `ms/BooleanValue`: its decoder maps everything
  that isn't `true`/\"true\" onto `false`, so a blank or junk value would coerce into a deactivation rather than be
  rejected. The strings clients send for booleans are parsed by the handler instead."
...
(defn- patch->boolean
  ...
  else is an error rather than a value: `Boolean/valueOf` reads every other string as `false`, which would quietly
  deactivate the user."
  [path value]
  (cond
    (boolean? value)
