...
3. Create a user account and add the user attribute you associated with your router database. Set the value as the key of one of your destination databases.
...
Here's a `curl` command to add a PostgreSQL database as a destination database. Here the database's `key` is defined by `name` (in this case, `Green PostgreSQL`).
