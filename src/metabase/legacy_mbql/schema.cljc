(ns metabase.legacy-mbql.schema
  "Schema for validating a *normalized* MBQL query. This is also the definitive grammar for legacy MBQL, wow!

  You can refer to schemas as follows:

  * General stuff like 'database ID' uses the namespace `legacy-mbql`, e.g. `:legacy-mbql/database-id`

  * Schemas for specific clauses use the namespace `legacy-mbql.clause`, e.g. `:legacy-mbql.clause/absolute-datetime`"
...
(mr/def :legacy-mbql/temporal-extract-unit
...
(mr/def :legacy-mbql/datetime-diff-unit
...
(defclause relative-datetime
  n    [:or [:= :current] :int]
  unit (optional [:ref :legacy-mbql/relative-datetime-unit]))
...
(mr/def :legacy-mbql.clause/absolute-datetime
...
(def ^:internal ^{:clause-name :absolute-datetime} absolute-datetime
  "Schema for an `:absolute-datetime` clause."
  [:ref :legacy-mbql.clause/absolute-datetime])
...
(defclause ^:internal time
  ...
  unit :legacy-mbql/time-unit)

(mr/def :legacy-mbql/date-or-datetime-literal
...
(mr/def :legacy-mbql/time-literal
...
(mr/def :legacy-mbql/temporal-literal
  "Schema for valid temporal literals."
  [:or
   {:error/message "temporal literal"}
   [:ref :legacy-mbql/date-or-datetime-literal]
   [:ref :legacy-mbql/time-literal]])

(mr/def :legacy-mbql/date-time-value
...
(mr/def :legacy-mbql/value-type-info
  [:map
...
(defclause ^:internal value
  value    :any
  type-info [:maybe :legacy-mbql/value-type-info])
...
(mr/def :legacy-mbql/field-options
  [:and
   [:map
...
(mr/def :legacy-mbql.clause/field
...
(mr/def :legacy-mbql/reference
  (one-of aggregation expression field))

(defclause ^{:added "0.50.0"} offset
  opts [:ref ::lib.schema.common/options]
  expr [:or [:ref :legacy-mbql/field-or-expression] [:ref :legacy-mbql/aggregation]]
...
(def ^:private StringExpression
  ...
  [:ref :legacy-mbql/string-expression])

(mr/def :legacy-mbql/string-expression-arg
  [:multi
   {:dispatch (fn [x]
                (cond
                  (string? x)                     :string
...
(def ^:private StringExpressionArg
  [:ref :legacy-mbql/string-expression-arg])
...
(def ^:private NumericExpressionArg
  [:ref :legacy-mbql/numeric-expression-arg])

(mr/def :legacy-mbql/datetime-expression-arg
  [:multi
   ...
   [:else                [:or [:ref :legacy-mbql/date-or-datetime-literal] Field]]])

(def ^:private DateTimeExpressionArg
  [:ref :legacy-mbql/datetime-expression-arg])

(mr/def :legacy-mbql/expression-arg
  [:multi
...
(def ^:private ExpressionArg
  [:ref :legacy-mbql/expression-arg])

(mr/def :legacy-mbql/addable
  [:or
...
(def ^:private Addable
  [:ref :legacy-mbql/addable])

(mr/def :legacy-mbql/int-greater-than-zero-or-numeric-expression
  [:multi
...
(def ^:private IntGreaterThanZeroOrNumericExpression
  [:ref :legacy-mbql/int-greater-than-zero-or-numeric-expression])
...
(defclause ^{:requires-features #{:datetime-diff}} datetime-diff
  ...
  unit       [:ref :legacy-mbql/datetime-diff-unit])

(defclause ^{:requires-features #{:temporal-extract}} temporal-extract
  datetime DateTimeExpressionArg
  unit     [:ref :legacy-mbql/temporal-extract-unit]
  mode     (optional [:ref :legacy-mbql/extract-week-mode])) ;; only for get-week and get-day-of-week
...
(defclause ^{:requires-features #{:temporal-extract}} ^:sugar get-week
  date DateTimeExpressionArg
  mode (optional [:ref :legacy-mbql/extract-week-mode]))
...
(mr/def :legacy-mbql/equality-comparable
  [:maybe
...
(def ^:private EqualityComparable
  ...
  [:ref :legacy-mbql/equality-comparable])

(mr/def :legacy-mbql/order-comparable
  [:multi
...
(def ^:private OrderComparable
  ...
  [:ref :legacy-mbql/order-comparable])
...
(defclause ^:sugar relative-time-interval
  col           Field
  value         :int
  bucket        [:ref :legacy-mbql/relative-datetime-unit]
  offset-value  :int
  offset-bucket [:ref :legacy-mbql/relative-datetime-unit])
...
(mr/def :legacy-mbql/boolean-expression
  (one-of
...
(mr/def :legacy-mbql/string-expression
...
(mr/def :legacy-mbql/field-or-expression
...
(defclause ^{:requires-features #{:basic-aggregations}} avg,      field-or-expression [:ref :legacy-mbql/field-or-expression])
(defclause ^{:requires-features #{:basic-aggregations}} cum-sum,  field-or-expression [:ref :legacy-mbql/field-or-expression])
(defclause ^{:requires-features #{:basic-aggregations}} distinct, field-or-expression [:ref :legacy-mbql/field-or-expression])
(defclause ^{:requires-features #{:basic-aggregations}} sum,      field-or-expression [:ref :legacy-mbql/field-or-expression])
(defclause ^{:requires-features #{:basic-aggregations}} min,      field-or-expression [:ref :legacy-mbql/field-or-expression])
(defclause ^{:requires-features #{:basic-aggregations}} max,      field-or-expression [:ref :legacy-mbql/field-or-expression])

(defclause ^{:requires-features #{:basic-aggregations}} sum-where
  field-or-expression [:ref :legacy-mbql/field-or-expression], pred Filter)
...
(defclause ^{:requires-features #{:standard-deviation-aggregations}} stddev
  field-or-expression [:ref :legacy-mbql/field-or-expression])
...
(defclause ^{:requires-features #{:percentile-aggregations}} percentile
  field-or-expression [:ref :legacy-mbql/field-or-expression], percentile NumericExpressionArg)
...
(mr/def :legacy-mbql/unnamed-aggregation
  [:multi
...
(mr/def :legacy-mbql/aggregation
  [:multi
   ...
   [:unnamed-aggregation :legacy-mbql/unnamed-aggregation]])
...
(defclause asc,  field [:ref :legacy-mbql/reference])
(defclause desc, field [:ref :legacy-mbql/reference])

(mr/def :legacy-mbql/order-by
  "Schema for an `order-by` clause subclause."
  (one-of asc desc))

(mr/def :legacy-mbql/order-bys
  "Schema for a non-empty sequence of distinct order-by clauses."
  (helpers/distinct [:sequential {:min 1} [:ref :legacy-mbql/order-by]]))
...
(mr/def :legacy-mbql/template-tag
  ...
  [:multi
   {:dispatch :type}
   [:dimension   [:ref :legacy-mbql/template-tag:field-filter]]
   [:snippet     [:ref :legacy-mbql/template-tag:snippet]]
   [:card        [:ref :legacy-mbql/template-tag:source-query]]
   [::mc/default [:ref :legacy-mbql/template-tag:raw-value]]])

(def ^:private TemplateTag
  "Alias for `:legacy-mbql/template-tag`; prefer that going forward."
  [:ref :legacy-mbql/template-tag])

(mr/def :legacy-mbql/template-tag-map
...
(def ^:private SourceQueryMetadata
  "Alias for `:legacy-mbql/source-query-metadata` -- prefer that instead."
  ;; TODO - there is a very similar schema in `metabase.analyze.query-results`; see if we can merge them
  [:ref :legacy-mbql/source-query-metadata])
...
(def ^:private Join
  "Alias for `:legacy-mbql/join`. Prefer that going forward."
  [:ref :legacy-mbql/join])

(mr/def :legacy-mbql/joins
...
(mr/def :legacy-mbql/indexed-idents
  ...
  [:map-of ::lib.schema.common/int-greater-than-or-equal-to-zero [:maybe :legacy-mbql/ident]])

(mr/def :legacy-mbql/expression-idents
...
(mr/def :legacy-mbql/mbql-query
  [:and
   [:map
    ...
    [:breakout-idents    {:optional true} [:ref :legacy-mbql/indexed-idents]]
    [:expressions        {:optional true} [:map-of ::lib.schema.common/non-blank-string [:ref :legacy-mbql/field-or-expression]]]
    [:expression-idents  {:optional true} [:ref :legacy-mbql/expression-idents]]
    ...
    [:page               {:optional true} [:ref :legacy-mbql/page]]
    [:joins              {:optional true} [:ref :legacy-mbql/joins]]
...
(mr/def :legacy-mbql/widget-type
...
(mr/def :legacy-mbql.clause/dimension
...
(def ^{:clause-name :dimension} ^:private dimension
  "Schema for a valid dimension clause."
  [:ref :legacy-mbql.clause/dimension])
...
(mr/def :legacy-mbql/constraints
...
(mr/def :legacy-mbql/middleware-options
...
(mr/def :legacy-mbql/database-id
...
(mr/def :legacy-mbql/query
  [:and
   [:map
    ...
    ;;
    [:settings    {:optional true} [:maybe [:ref :legacy-mbql/settings]]]
    [:constraints {:optional true} [:maybe [:ref :legacy-mbql/constraints]]]
    [:middleware  {:optional true} [:maybe [:ref :legacy-mbql/middleware-options]]]
    ;;
    ;; INFO
    ;;
    [:info
