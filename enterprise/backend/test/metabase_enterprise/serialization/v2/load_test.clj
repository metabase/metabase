...
;;; -------------------------------------------- Metric v2 dimensions --------------------------------------------

(def ^:private dim-a-id "11111111-1111-4111-8111-111111111111")
(def ^:private dim-b-id "22222222-2222-4222-8222-222222222222")

(defn- metric-v2-dimensions
  "Two curated dimensions for a metric v2 Card: `category` on the metric's own table, and `title` reached via an
   FK on a second table. Field IDs are embedded in `:sources`."
  [category-field-id title-field-id]
  [{:id            dim-a-id
    :name          "CATEGORY"
...
(defn- metric-v2-dimension-mappings
  "Mappings for [[metric-v2-dimensions]]: dim A targets `sales.category` (same table); dim B targets
   `products.title` reached through the `sales.product_id` FK (so its target carries `:source-field` and its
   `:table-id` points at a *different* table)."
...
(deftest metric-v2-dimensions-test
  ;; A metric v2 (`:model/Card` with `:type :metric`) carries curated `:dimensions` and `:dimension_mappings`. Those
  ;; embed instance-specific Field IDs (in each dimension's `:sources` and each mapping's `:target`) and Table IDs (in
  ;; each mapping's `:table-id`). This test checks they are converted to a portable form on export and resolved back to
  ;; the *dest* IDs on import, even though source and dest assign different IDs to the same tables/fields.
  (testing "curated metric v2 dimensions & mappings are portable"
    (let [serialized  (atom nil)
          coll1s      (atom nil)
          db1s        (atom nil)
          sales1s     (atom nil)
          ...
          amount1s    (atom nil)
          category1s  (atom nil)
          productfk1s (atom nil)
          title1s     (atom nil)
          user1s      (atom nil)
          card1s      (atom nil)]
      (ts/with-dbs [source-db dest-db]
        (testing "serializing a metric v2 with curated dimensions"
          (ts/with-db source-db
            (reset! coll1s      (ts/create! :model/Collection :name "Metrics"))
            (reset! db1s        (ts/create! :model/Database :name "metric-db"))
            (reset! sales1s     (ts/create! :model/Table :name "sales" :db_id (:id @db1s)))
            (reset! products1s  (ts/create! :model/Table :name "products" :db_id (:id @db1s)))
            (reset! amount1s    (ts/create! :model/Field :name "amount" :table_id (:id @sales1s)))
            (reset! category1s  (ts/create! :model/Field :name "category" :table_id (:id @sales1s)))
            (reset! productfk1s (ts/create! :model/Field :name "product_id" :table_id (:id @sales1s)))
            (reset! title1s     (ts/create! :model/Field :name "title" :table_id (:id @products1s)))
            (reset! user1s      (ts/create! :model/User :first_name "Tom" :last_name "Scholz" :email "tom@bost.on"))
            (reset! card1s
                    (ts/create! :model/Card
                                :name          "Total Amount"
                                ...
                                :collection_id (:id @coll1s)
                                :creator_id    (:id @user1s)
                                :dataset_query (mbql5-measure-definition (:id @db1s) (:id @sales1s) (:id @amount1s))
                                :dimensions    (metric-v2-dimensions (:id @category1s) (:id @title1s))
                                :dimension_mappings (metric-v2-dimension-mappings
                                                     (:id @sales1s) (:id @products1s)
                                                     (:id @category1s) (:id @title1s) (:id @productfk1s))))
            (reset! serialized (into [] (serdes.extract/extract {})))))
        (testing "exported dimensions & mappings use portable Field/Table references"
          (let [card (m/find-first #(= "Total Amount" (:name %))
            ...
            (testing "mapped tables contribute their Database as a serdes dependency"
              ;; Tables/Fields themselves are synthesized on import, so a mapping's only hard dependency is the
        ...
        (testing "deserializing resolves dimensions & mappings to the dest IDs"
          (ts/with-db dest-db
            ...
            ;; than the source has (4) so every loaded field lands above the source's field IDs — no collision
            ;; regardless of the load's field ordering.
            (let [other-db    (ts/create! :model/Database :name "other-db")
              ...
              (ts/create! :model/User :first_name "Tom" :last_name "Scholz" :email "tom@bost.on"))
            (serdes.load/load-metabase! (ingestion-in-memory @serialized))
            (let [sales1d     (t2/select-one :model/Table :name "sales")
                  products1d  (t2/select-one :model/Table :name "products")
                  category1d  (t2/select-one :model/Field :table_id (:id sales1d) :name "category")
                  productfk1d (t2/select-one :model/Field :table_id (:id sales1d) :name "product_id")
                  title1d     (t2/select-one :model/Field :table_id (:id products1d) :name "title")
                  card1d      (t2/select-one :model/Card :name "Total Amount")]
              (testing "the tables & fields have different IDs on the dest"
                (is (not= (:id @sales1s)    (:id sales1d)))
                (is (not= (:id @category1s) (:id category1d)))
                (is (not= (:id @title1s)    (:id title1d))))
              (testing "dimension :sources point at the dest Field IDs"
                (is (=? [{:id dim-a-id :sources [{:field-id (:id category1d)}]}
                         {:id dim-b-id :sources [{:field-id (:id title1d)}]}]
                        (:dimensions card1d))))
              (testing "dimension mappings point at the dest Table & Field IDs"
                (is (=? [{:dimension-id dim-a-id
                          :table-id     (:id sales1d)
                          :target       [:field {} (:id category1d)]}
                         {:dimension-id dim-b-id
                          :table-id     (:id products1d)
                          :target       [:field {:source-field (:id productfk1d)} (:id title1d)]}]
                        (:dimension_mappings card1d))))
              (testing "the metric's :default dimension flag survives the round-trip"
                (is (true? (:default (first (:dimensions card1d)))))
                (is (= #{}
                       (into #{} (keep :default)
                             (rest (:dimensions card1d)))))))))))))

(deftest metric-v2-dimensions-deletion-propagates-test
  ;; Curation must be authoritative across instances: a dimension deleted on the source instance must be deleted on the
  ;; dest by a subsequent import (not silently re-synthesized). This exercises the update path (matched by entity_id).
  (testing "deleting a curated dimension on the source deletes it on the dest via import"
    (let [db1s        (atom nil)
          sales1s     (atom nil)
          products1s  (atom nil)
          amount1s    (atom nil)
          ...
          card1s      (atom nil)]
      (ts/with-dbs [source-db dest-db]
        (let [extract-source (fn [] (ts/with-db source-db (into [] (serdes.extract/extract {}))))]
          (ts/with-db source-db
            (reset! db1s        (ts/create! :model/Database :name "metric-db"))
            (reset! sales1s     (ts/create! :model/Table :name "sales" :db_id (:id @db1s)))
            (reset! products1s  (ts/create! :model/Table :name "products" :db_id (:id @db1s)))
            ...
            (reset! card1s
                    (ts/create! :model/Card
                                ...
                                :table_id           (:id @sales1s)
                                :creator_id         (:id @user1s)
                                :dataset_query      (mbql5-measure-definition (:id @db1s) (:id @sales1s)
                                                                              (:id @amount1s))
                                :dimensions         (metric-v2-dimensions (:id @category1s) (:id @title1s))
                                :dimension_mappings (metric-v2-dimension-mappings
                                                     (:id @sales1s) (:id @products1s)
                                                     (:id @category1s) (:id @title1s) (:id @productfk1s)))))
          ;; A pre-existing db & user on the dest so the imported IDs differ from the source's.
          (ts/with-db dest-db
            (let [other-db (ts/create! :model/Database :name "other-db")]
              (ts/create! :model/Table :name "junk" :db_id (:id other-db)))
            (ts/create! :model/User :first_name "Tom" :last_name "Scholz" :email "tom@bost.on"))
          (testing "first import brings both dimensions across"
            (let [serialized (extract-source)]
              (ts/with-db dest-db
                (serdes.load/load-metabase! (ingestion-in-memory serialized))
                (let [card1d (t2/select-one :model/Card :name "Total Amount")]
                  (is (= #{dim-a-id dim-b-id} (into #{} (map :id) (:dimensions card1d))))
                  (is (= #{dim-a-id dim-b-id} (into #{} (map :dimension-id) (:dimension_mappings card1d))))))))
          (testing "removing dimension B on the source and re-importing removes it on the dest"
            (ts/with-db source-db
              (t2/update! :model/Card (:id @card1s)
                          {:dimensions         (filterv #(= dim-a-id (:id %)) (:dimensions @card1s))
                           :dimension_mappings (filterv #(= dim-a-id (:dimension-id %))
                                                        (metric-v2-dimension-mappings
                                                         (:id @sales1s) (:id @products1s)
                                                         (:id @category1s) (:id @title1s) (:id @productfk1s)))}))
            (let [serialized (extract-source)]
              (ts/with-db dest-db
                (serdes.load/load-metabase! (ingestion-in-memory serialized))
                (let [card1d (t2/select-one :model/Card :name "Total Amount")]
                  (testing "only dimension A remains, B is gone (not re-synthesized)"
                    (is (= [dim-a-id] (mapv :id (:dimensions card1d))))
                    (is (= [dim-a-id] (mapv :dimension-id (:dimension_mappings card1d))))))))))))))
