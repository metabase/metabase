...
import { skipToken, useListDatabaseSchemaTablesQuery, useListDatabaseSchemasQuery, useListDatabasesQuery } from "metabase/api";
import type { Database, SchemaName, Table } from "metabase-types/api";

import { ItemList, type OmniPickerDatabaseItem, type OmniPickerItem, type OmniPickerSchemaItem, type OmniPickerTableItem } from "../../../EntityPicker";

const createSchemaItem = (dbId: number, schema: SchemaName): OmniPickerSchemaItem => ({
  model: "schema",
  id: schema,
  name: schema,
  db_id: dbId,
...
const useGetDbItemListData = (parentItem: OmniPickerItem) => {
  ...
  const dbId = match(parentItem)
    ...
    .with({ model: "schema" }, () => (parentItem as OmniPickerSchemaItem).db_id)
    .otherwise(() => undefined);


  const { data: schemas, isLoading: isLoadingSchemas, error: schemasError } =
  ...
  const { data: tables, isLoading: isLoadingTables, error: tablesError } =
  ...
  return {
    data: tables?.map(createTableItem)
      ?? (dbId ? schemas?.map(schema => createSchemaItem(dbId, schema)): undefined)
