(ns metabase.metabot.self.bedrock-runtime
  "Legacy AWS Bedrock adapter using the ConverseStream API on the bedrock-runtime
  endpoint with the AWS binary event-stream wire format and SigV4 signing.

  Superseded by [[metabase.metabot.self.bedrock]], which targets the bedrock-mantle
  endpoint and the native Anthropic Messages / OpenAI Responses API families. Kept
  for reference; not wired into the provider dispatch in [[metabase.metabot.self]]."
  (:require
...
(comment
  ...
  (into [] (metabase.metabot.self.bedrock-runtime/bedrock
