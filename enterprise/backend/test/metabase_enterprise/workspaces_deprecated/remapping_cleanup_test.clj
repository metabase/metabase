(ns metabase-enterprise.workspaces-deprecated.remapping-cleanup-test
  "Tests for [[metabase-enterprise.workspaces-deprecated.remapping-cleanup]] — the iso-scoped
  ...
  (testing "When the driver's `destroy!` step throws (e.g. BQ dataset gone but SA delete fails),
    ...
    (binding [tu.thread-local/*thread-local* false]
      (mt/with-temp [:model/Database {db-id :id} {:engine :postgres :details {}}
                     :model/WorkspaceDeprecated {ws-id :id} {:name       "tear-down"
                                                             :creator_id (mt/user->id :crowberto)}
                     :model/WorkspaceDatabaseDeprecated {wsd-id :id} {:workspace_id     ws-id
                                                                      :database_id      db-id
        ...
        (is (= :provisioned (:status (t2/select-one :model/WorkspaceDatabaseDeprecated :id wsd-id)))
