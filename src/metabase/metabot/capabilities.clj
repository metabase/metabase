...
(defn- feature-capabilities
  ...
  []
  ;; :feature-entity-retrieval gates the curated retrieve_library_entities tool on the library index being
  ;; able to answer (pgvector configured + semantic-search licensed + the index populated). The :nlq profile
  ;; is only served with that tool when the same predicate holds (see profiles/get-profile), so the two stay
  ;; consistent; otherwise the agent is served the :nlq-fallback general search.
  (into #{}
        (remove nil?)
        [(when (or (not (premium-features/is-hosted?))
         ...
         (when (entity-retrieval/entity-retrieval-available?)
