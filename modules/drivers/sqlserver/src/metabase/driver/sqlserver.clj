...
(defn- update-aggregations [inner]
  ;; remove `percentile-aggregations` from query
  ;; in case empty seq dissoc
...
(defn- percentile-source-query [original-query]
  ...
  ;; Generate :source-query with percentile columns from use of window percentile fn to be further aggregated
  ;; by real aggregations of original-query
  ;;
  ;; Ouput of this function is to be used in [[rewrite-percentile-aggregations-impl]]
  ...
  (as-> (select-keys original-query [:source-query :source-table :joins :filter]) under-construction
    (assoc under-construction :aggregation (->> (:aggregation original-query)
                                                ...
                                                ;; following line adds fields to be used 
                                                (mapv #(assoc-in % [1 1 2 ::partition-fields] (:breakout original-query)))))
    ...
    (assoc under-construction :fields (-> (select-keys original-query [:aggregation :breakout :order-by :page :limit])
                                          (update :aggregation #(filterv (complement percentile-aggregation?) %))
                                          (mbql.u/match :field)
                                          distinct
                                          vec))))
...
(defn rewrite-percentile-aggregations-impl [original-inner]
  ...
  ;; EXPRESSIONS
  ;; Function is coded in a way that it expects nest-expressions to be called
  ;; first in preprocess, hence considering only aggregations on fields here and not the expressions

  ;; HOW IT WORKS
  ;; 1. Original query has removed source, joins and filter.
  ;;    Ie. everything which is used to create source for aggregation. Expressions are handled as fields
  ;;    because of nest-expressions application prior to percentile aggregations rewrite.
  ;; 2. __Fields generated from window percentile are added to breakout__, percentile aggregations are removed
  ;;    from modified query
  ;; 3. Subquery (see [[percentile-source-query]]) containing:
  ;;      - fields for computation of different aggregations,
...
(defmethod sql.qp/->honeysql [:sqlserver :percentile]
  ...
  (let [partition-by-fields (breakout-fields-for-transformed-aggregation sql.qp/*inner-query* field)]
