...
(expect
  [;; expanded form
   ...
   {:database {:name "test-data",
              :details {:short-lived? nil, :db "mem:test-data;USER=GUEST;PASSWORD=guest"},
              :id (id),
              :engine :h2},
   :type :query,
   :query {:source-table {:schema "PUBLIC", :name "VENUES", :id (id :venues)},
           :filter {:filter-type :>,
                    :field {:field-id (id :venues :price),
                            :field-name "PRICE",
...
(expect
  [;; expanded form
   ...
   {:database     {:name    "test-data"
                   ...
                   :engine  :h2},
    :type         :query,
    :query        {:source-table {:schema "PUBLIC", :name "CHECKINS", :id (id :checkins)},
                   :filter       {:filter-type :>,
