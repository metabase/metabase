  ...
  (let [fresh-expr-idents (into {} (map (juxt name (fn [_] (u/generate-nano-id))))
    ...
    (cond-> inner-query
      ...
      true         (vary-meta assoc :generated-idents
                              {:expression-idents  (set (keys fresh-expr-idents))
                               :aggregation-idents (set (remove (set (keys aggregation-idents)) (keys agg-idents)))
                               :breakout-idents    (set (remove (set (keys breakout-idents))    (keys brk-idents)))
                               :join-idents        (set (keep-indexed (fn [i join]
                                                                        (when-not (:ident join)
                                                                          i))
                                                                      joins))}))))
