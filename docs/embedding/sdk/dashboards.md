...
-\*\* Combining `initialParameters` and `hiddenParameters` with the purpose of decluttering the interface is fine, but if done filter data on the front-end, it's considered risky and against our terms. [More info](../authentication#security-warning-each-end-user-must-have-their-own-metabase-account).
