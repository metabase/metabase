...
(defn- try-resolve-source-card-id
  ...
  (try
    (cond
      (and (pos-int? source-card-eid) resolve/*numeric-ids-allowed?*)
      source-card-eid

      (and import-resolver (string? source-card-eid))
...
(defn- try-resolve-field-id
  ...
  JSON-unfolded parent chains via the resolver's `import-field-fk`. Returns nil on failure."
...
(defn- try-resolve-field-target-table-id
  "Resolve the target-table-id of a field target. Returns nil on failure. For callers that also
