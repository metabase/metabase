  ...
  (mt/with-premium-features #{:semantic-search}
    (with-open [index-ref (semantic.tu/open-temp-index!)]
      ...
      (testing "index table is not present before create!"
        ...
        (is (not (semantic.tu/table-has-index? (:table-name @index-ref) (:embedding-index-name @index-ref))))
        (is (not (semantic.tu/table-has-index? (:table-name @index-ref) (:fts-index-name @index-ref)))))
      (testing "index table is present after create!"
        ...
        (is (semantic.tu/table-has-index? (:table-name @index-ref) (:embedding-index-name @index-ref)))
        (is (semantic.tu/table-has-index? (:table-name @index-ref) (:fts-index-name @index-ref)))))))
...
(deftest filter-by-collection-test
  (testing "filter-by-collection function filters documents based on personal collection preferences"
    (let [user1-id (mt/user->id :rasta)
          ...
          ;; Get existing personal collections for test users
      ...
      (mt/with-temp
        ...
        (let [;; Create test documents representing different collection scenarios
              docs [{:id "doc1" :model "card" :collection_id user1-personal-coll-id}    ; User1's personal
                    ...
                    {:id "doc6" :model "card" :collection_id user2-sub-coll-id}]]       ; User2's sub-collection
