      ...
      (let [active-tbl    (search.index/active-table)
        ...
        (try
          ...
          (with-redefs [search.writer/insert-batch-size 1
            ...
            ;; *force-sync* false so background-mode commits per batch and exercises the real reindex write
            ;; path (the captured-destination path, not the dual-write path)
...
(deftest reindex-without-pending-populates-active-table-test
  ;; The initial-build path populates the freshly-activated table directly with a committing background mode
