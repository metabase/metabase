...
(defn maybe-decrypt
  ...
  as-is (there is no key to decrypt with). A caller that must tolerate a legitimately-plaintext value (e.g. reading a
  legacy row, or the encryption-check sentinel) should catch the thrown [[clojure.lang.ExceptionInfo]] and decide what
  the fallback means for that column.
...
(defn maybe-decrypt-bytes
  "Strict decrypt of a byte array `b` (the [[maybe-decrypt]] counterpart for bytes): a value that is not encrypted, or
  that cannot be decrypted with the current key, throws rather than being trusted. When no key is set, `b` is returned
  as-is."
