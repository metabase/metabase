...
const ordersTableQuestionDetails: cy.NativeQuestionDetails = {
  display: "table",
  native: {
...
const peopleTableQuestionDetails: cy.NativeQuestionDetails = {
  display: "table",
  native: {
...
const timeseriesLineQuestionDetails: cy.NativeQuestionDetails = {
  display: "line",
  native: {
...
const timeseriesWithCategoryLineQuestionDetails: cy.NativeQuestionDetails = {
  display: "line",
  native: {
    query:
...
const numericLineQuestionDetails: cy.NativeQuestionDetails = {
  display: "line",
  native: {
...
const pinMapQuestionDetails: cy.NativeQuestionDetails = {
  display: "map",
  native: {
...
const gridMapQuestionDetails: cy.NativeQuestionDetails = {
  display: "map",
  native: {
    query: "SELECT LATITUDE, LONGITUDE FROM PEOPLE ORDER BY ID LIMIT 10",
  },
...
describe("scenarios > question > native query drill", () => {
  ...
  describe("query builder metadata", () => {
    it("should allow to save an ad-hoc native query when attempting to drill", () => {
      ...
      cy.tableInteractive().findByText("October 7, 2023, 1:34 AM").click();
      cy.popover().within(() => {
  ...
  describe("query builder drills", () => {
    it("column-extract drill", () => {
      ...
      cy.createNativeQuestion(ordersTableQuestionDetails, {
        visitQuestion: true,
        wrapId: true,
      });
      ...
      cy.tableInteractive().button("Add column").click();
      cy.popover().within(() => {
    ...
    it("combine-columns drill", () => {
      cy.log("from column header");
      cy.createNativeQuestion(peopleTableQuestionDetails, {
        visitQuestion: true,
        wrapId: true,
      });
    ...
    it("distribution drill", () => {
      ...
      cy.tableHeaderClick("QUANTITY");
      cy.popover().findByText("Distribution").click();
      cy.wait("@dataset");
    ...
    it("quick-filter drill", () => {
      cy.createNativeQuestion(timeseriesLineQuestionDetails, {
        visitQuestion: true,
      });
      cy.assertQueryBuilderRowCount(10);
    ...
    it("sort drill", () => {
      ...
      cy.tableHeaderClick("QUANTITY");
      cy.popover().icon("arrow_up").click();
      cy.wait("@dataset");
      cy.assertTableData({
    ...
    it("summarize drill", () => {
      ...
      cy.tableHeaderClick("QUANTITY");
      cy.popover().findByText("Distinct values").click();
      cy.wait("@dataset");
      cy.assertTableData({
      ...
      cy.visitQuestion("@questionId");
      cy.tableHeaderClick("QUANTITY");
      cy.popover().findByText("Avg").click();
      cy.wait("@dataset");
      cy.assertTableData({
    ...
    it("summarize-column-by-time drill", () => {
      ...
      cy.tableHeaderClick("QUANTITY");
      cy.popover().findByText("Sum over time").click();
      cy.wait("@dataset");
      cy.assertTableData({
    ...
    it("unsupported drills", () => {
      ...
      cy.createNativeQuestion(timeseriesWithCategoryLineQuestionDetails, {
        visitQuestion: true,
      });
  ...
  describe("query builder brush filters", () => {
    ...
    it("numeric filter", () => {
      cy.createNativeQuestion(numericLineQuestionDetails, {
        visitQuestion: true,
      });
      cy.assertQueryBuilderRowCount(10);
      applyBrushFilter({ left: 200, right: 800 });
    ...
    it("coordinates filter", () => {
      cy.log("pin map");
      cy.createNativeQuestion(pinMapQuestionDetails, { visitQuestion: true });
      ...
      cy.log("grid map");
      cy.createNativeQuestion(gridMapQuestionDetails, { visitQuestion: true });
      cy.findByTestId("visualization-root").realHover();
      cy.findByTestId("visualization-root").within(() => {
        cy.findByText("Save as default view").should("be.visible");
  ...
  describe("dashboard drills", () => {
    it("quick-filter drill", () => {
      ...
      }).then(({ body }) => cy.visitDashboard(body.dashboard_id));
      cy.getDashboardCard().findByText("May 15, 2024, 8:04 AM").click();
      cy.popover().within(() => {
        cy.findByText("Filter by this date and time").should("be.visible");
        cy.findByText("On").click();
        cy.wait("@dataset");
...
function applyBrushFilter({ left, right }: { left: number; right: number }) {
  cy.ensureEchartsContainerHasSvg();
