      ...
      (mt/with-premium-features #{:transforms}
        (mt/with-temp [:model/TransformTag tag1 {:name "test-tag-1"}
          ...
          (testing "Creates job with valid data"
            (let [response (mt/user-http-request :lucky :post 200 "ee/transform-job"
                                                 {:name            "Test Job"
          ...
          (testing "Validates cron expression"
            (let [response (mt/user-http-request :lucky :post 400 "ee/transform-job"
                                                 {:name     "Bad Cron Job"
      ...
      (mt/with-premium-features #{:transforms}
        (mt/with-temp [:model/TransformTag tag {:name "test-tag"}
          ...
          (testing "Returns job with hydrated fields"
            (let [response (mt/user-http-request :lucky :get 200 (str "ee/transform-job/" (:id job)))]
          ...
          (testing "Returns 404 for non-existent job"
            (mt/user-http-request :lucky :get 404 "ee/transform-job/999999")))))))
        ...
        (let [lucky-id (mt/user->id :lucky)]
          (mt/with-temp [:model/Transform {transform1-id :id} {:name "tr1" :creator_id lucky-id}
            ...
            (testing "Returns the transforms of the job"
              (let [response (mt/user-http-request :lucky :get 200 (str "ee/transform-job/" job-id "/transforms"))]
            ...
            (testing "Returns 404 for non-existent job"
              (mt/user-http-request :lucky :get 404 "ee/transform-job/999999/transforms"))))))))
          ...
          (mt/with-temp [:model/TransformTag {t1-id :id} {:name "test-tag"}
            ...
            (let [our-job-ids #{j1-id j2-id j3-id}
              ...
              (testing "listing without filtering"
                (let [response (mt/user-http-request :lucky :get 200 "ee/transform-job")]
                  (is (= our-job-ids (returned-job-ids response)))))
              (testing "filtering by tag_ids"
                (let [response (mt/user-http-request :lucky :get 200 "ee/transform-job" :tag_ids [t2-id])]
                  (is (=? [{:schedule "0 0 0 * * ?"
                            :tag_ids [t2-id]
                            :name "Job 3"
                            :id j3-id}]
              ...
              (testing "filtering by last_run_start_time"
                (let [response (mt/user-http-request :lucky :get 200 "ee/transform-job" :last_run_start_time "2025-08-26~")]
                  (is (= #{j2-id} (returned-job-ids response)))))
              (testing "filtering by last_run_statuses"
                (let [response (mt/user-http-request :lucky :get 200 "ee/transform-job" :last_run_statuses ["started" "succeeded"])]
                  (is (= #{j2-id} (returned-job-ids response)))))
              (testing "filtering by last_run_end_time without scheduled job"
                (let [response (mt/user-http-request :lucky :get 200 "ee/transform-job" :next_run_start_time "2025-08-26~")]
              ...
              (testing "filtering by last_run_end_time with scheduled job"
                (mt/with-temp-scheduler!
                  ...
                  (try
                    (let [response (mt/user-http-request :lucky :get 200 "ee/transform-job" :next_run_start_time "2025-08-27~")]
                      (is (=? [{:id j1-id
    ...
    (mt/with-data-analyst-role! (mt/user->id :lucky)
      (mt/with-premium-features #{:transforms}
        (mt/with-temp [:model/TransformTag tag1 {:name "tag-1"}
          ...
          (let [job (mt/user-http-request :lucky :post 200 "ee/transform-job"
                                          {:name "Original" :schedule "0 0 0 * * ?"})]
            (testing "Updates job fields"
              (let [response (mt/user-http-request :lucky :put 200 (str "ee/transform-job/" (:id job))
                                                   {:name        "Updated"
            ...
            (testing "Validates cron expression"
              (let [response (mt/user-http-request :lucky :put 400 (str "ee/transform-job/" (:id job))
                                                   {:schedule "invalid"})]
                (is (string? response))
                (is (re-find #"Invalid cron expression" response))))))))))
    ...
    (mt/with-data-analyst-role! (mt/user->id :lucky)
      (mt/with-premium-features #{:transforms}
        (mt/with-temp [:model/TransformJob job {:name "To Delete" :schedule "0 0 0 * * ?"}]
          (testing "Deletes job"
            (mt/user-http-request :lucky :delete 204 (str "ee/transform-job/" (:id job)))
            (is (nil? (t2/select-one :model/TransformJob :id (:id job)))))

          (testing "Returns 404 for non-existent job"
            (mt/user-http-request :lucky :delete 404 "ee/transform-job/999999")))))))
      ...
      (mt/with-premium-features #{:transforms}
        (mt/with-temp [:model/TransformJob job {:name "To Execute" :schedule "0 0 0 * * ?"}]
          (testing "Returns stub run response"
            (let [response (mt/user-http-request :lucky :post 200 (str "ee/transform-job/" (:id job) "/run"))]
