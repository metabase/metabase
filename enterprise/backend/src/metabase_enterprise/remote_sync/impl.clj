...
(defn- sync-objects!
  ...
  (let [;; Standard models use entity_id UUIDs (including Segment)
        ...
        ;; Tables: batch lookup using join
        table-inserts (when (seq table-paths)
                        (->> (t2/query {:select [:t.id]
                                        ...
                                        :where (into [:or]
                                                     (for [{:keys [db_name schema table_name]} table-paths]
                                                       [:and
                                                        ...
                                                        [:= :t.name table_name]]))})
                             (map (fn [{:keys [id]}]
                                    {:model_type "Table" :model_id id
                                     :status "synced" :status_changed_at timestamp}))))
        ;; Fields: batch lookup using join through table
        field-inserts (when (seq field-paths)
                        (->> (t2/query {:select [:f.id]
