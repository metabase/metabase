...
(defn do-with-temp-env-var-value
  ...
  (metabase.test-runner/assert-test-is-not-parallel "with-temp-env-var-value")
  (let [value (str value)]
...
(defn do-with-temporary-setting-value
  ...
  (metabase.test-runner/assert-test-is-not-parallel "with-temporary-setting-values")
...
(defn do-with-temp-vals-in-db
  ...
  (metabase.test-runner/assert-test-is-not-parallel "with-temp-vals-in-db")
...
(defn do-with-model-cleanup [models f]
  ...
  (metabase.test-runner/assert-test-is-not-parallel "with-model-cleanup")
  (initialize/initialize-if-needed! :db)
...
(defn do-with-discarded-collections-perms-changes [collection-or-id f]
  ...
  (let [read-path                   (perms/collection-read-path collection-or-id)
    ...
    (metabase.test-runner/assert-test-is-not-parallel "with-discarded-collections-perms-changes")
    (try
      (f)
      (finally
...
(defn do-with-non-admin-groups-no-collection-perms [collection f]
  (metabase.test-runner/assert-test-is-not-parallel "with-non-admin-groups-no-collection-perms")
  (try
...
(defn call-with-locale
  ...
  [locale-tag f]
  (metabase.test-runner/assert-test-is-not-parallel "with-locale")
