...
- Do NOT use this tool if there is a model or metric that can answer the user's question
  ...
  (try
    ...
    (catch Exception e
      (log/error "Error collecting document schema:" (ex-message e))
...
(mu/defn ^{:tool-name "document_construct_sql_chart"
  ...
  (try
    ...
    (catch Exception e
      (log/error "Error constructing SQL chart draft:" (ex-message e))
...
(mu/defn ^{:tool-name "document_construct_model_chart"
  ...
  (try
    ...
    (catch Exception e
      (log/error "Error constructing model chart draft:" (ex-message e))
      (if (:agent-error? (ex-data e))
        {:output (ex-message e)}
