...
describe("issue 21528", () => {
  ...
  it("should show dashboard ID filter values when mapped to a native question with a foreign key field filter", () => {
    ...
    // Navigating to another page via JavaScript is faster than using `cy.visit("/bench/metadata")` to load the whole page again.
    H.appBar().findByRole("button", { name: "Settings" }).click();
    H.popover().findByText("Workbench").click();
    H.benchNav().findByText("Metadata").click();
    cy.findByRole("main")
    ...
    H.benchNavTitleMenu().click();
    H.popover().findByText("Explore").click();
