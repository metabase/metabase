 ...
 * the router context. Returns the pure v7 `Location` shape (no v3 `query`/`action`
 * fields); the legacy compat `Location` type carries those for the route-prop
 * call sites.
 *
 ...
 */
export function useMaybeLocation(): Omit<Location, "query" | "action"> | null {
