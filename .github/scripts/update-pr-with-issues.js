...
async function httpsRequestWithBackoff(
  options,
  data = null,
  backoffOptions = {},
) {
  // Set default backoff options:
  const { maxRetries = 8, totalDurationSeconds = 60 } = backoffOptions;
  ...
  while (true) {
    try {
      ...
      if (retries >= maxRetries || timeRemaining <= 0) {
        console.error(
          `Failed after ${retries} retries (${Math.round(elapsedMs / 1000)}s elapsed):`,
        );
        throw error;
      }
      ...
      console.log(
        `Request failed (attempt ${retries}/${maxRetries}). Retrying in ${Math.round(delay)}ms... (${Math.round(timeRemaining / 1000)}s remaining)`,
      );
...
async function link_issues(github) {
  try {
    ...
    const prInfo = await github.rest.issues
      .get({
        owner: repoOwner,
        repo: repoName,
    ...
    if (
      !linearData.data ||
      !linearData.data.attachmentsForURL ||
      linearData.data.attachmentsForURL.nodes.length === 0
    ) {
      console.log("No Linear tasks linked to this PR");
      return;
    } else {
      console.log(
        `Found ${linearData.data.attachmentsForURL.nodes.length} linked Linear tasks`,
      );
    }
    ...
    for (const node of linearData.data.attachmentsForURL.nodes) {
      ...
      for (const attachment of linearIssue.attachments.nodes) {
        ...
        const match = url.match(
          /github\.com\/metabase\/metabase\/issues\/(\d+)/,
        );
        if (match) {
    ...
    if (issueNumbers.length === 0) {
      console.log(
        `Found linked GitHub issues: ${JSON.stringify(issueNumbers)}`,
      );
    ...
    let issueNumbersToAdd = issueNumbers.filter(
      (num) => !linked_issues.includes(num),
    );
    ...
    if (issueNumbersToAdd.length > 0) {
      ...
      const closingRefs = issueNumbersToAdd
        .map((num) => `closes #${num}`)
        .join(" ");
      ...
      console.log(
        `Updated PR #${prNumber} with issue references:\n ${closingRefs}`,
      );
    } else {
