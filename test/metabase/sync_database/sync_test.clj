...
(expect
  ...
  (tu/with-temp* [Database  [{database-id :id}]
    ...
    (let [get-fields (fn [table-id]
                       (for [field (db/sel :many :fields [Field :name :special_type :fk_target_field_id] :table_id table-id)]
                         (tu/boolean-ids-and-timestamps field)))]
...
(expect
  ...
  (tu/with-temp* [Database [{database-id :id, :as db} {:engine :moviedb}]]
    ...
    (let [raw-table-id (db/sel :one :field [RawTable :id] :database_id database-id, :name "movies")
          table        (db/ins Table
                         :db_id        database-id
  ...
  (tu/with-temp* [Database    [{database-id :id}]
    ...
    (let [get-fields     (fn []
                           (for [field (db/sel :many Field :table_id table-id (k/order :id))]
                             (dissoc (tu/boolean-ids-and-timestamps field) :active :field_type :position :preview_display)))
...
(expect
  ...
  (tu/with-temp* [Database [{database-id :id, :as db} {:engine :moviedb}]]
    (let [driver (moviedb/->MovieDbDriver)]
      ...
      (let [raw-table-id (db/sel :one :field [RawTable :id] :database_id database-id, :name "roles")
            tbl          (db/ins Table
                           :db_id        database-id
                           :raw_table_id raw-table-id
                           :name         "roles"
...
(expect
  [(set moviedb/moviedb-raw-tables)
   (set moviedb/moviedb-tables-and-fields)
   (set moviedb/moviedb-tables-and-fields)
   (set (conj (vec (drop-last moviedb/moviedb-tables-and-fields))
              (-> (last moviedb/moviedb-tables-and-fields)
                  (assoc :active false
                         :fields []))))]
  (tu/with-temp* [Database [{database-id :id, :as db} {:engine :moviedb}]]
...
(expect
  [(set schema-per-customer/schema-per-customer-raw-tables)
   (set schema-per-customer/schema-per-customer-tables-and-fields)
   (set schema-per-customer/schema-per-customer-tables-and-fields)]
  (tu/with-temp* [Database [{database-id :id, :as db} {:engine :schema-per-customer}]]
    (let [driver     (schema-per-customer/->SchemaPerCustomerDriver)
          db-tables  (fn [db-id]
                       (->> (hydrate/hydrate (db/sel :many Table :db_id db-id (k/order :id)) :fields)
                            (map resolve-fk-targets)
                            (map tu/boolean-ids-and-timestamps)
                            set))]
      ;; do a quick introspection to add the RawTables to the db
      (introspect/introspect-database-and-update-raw-tables! driver db)
