...
(def ^:private current-user-fields
  ;; `:branch` = the git branch the user has checked out (content branching); carrying it on
  ;; `*current-user*` lets branch resolution run on every request without an extra query.
