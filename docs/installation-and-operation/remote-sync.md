...
Git-based version control for your Metabase collections. Sync dashboards, questions, and documents between Metabase and Git repositories with automatic change tracking.
...
Metabase encrypts your token before storing it.
...
3. Someone approves and merges the PR.
...
2. The "Megafaune Analytics" collection appears in production with all its dashboards and questions.
...
- If a question uses a snippet, that question can't be added to the synced collection, as snippets are not stored in collections.
...
When you try to save an item with external dependencies to a synced collection, Metabase will show an error: "Model has non-remote-synced dependencies" or "Has references outside repository."
