...
type ParsedMetabotMessage = Array<string | ParsedMetabaseEntity>;

export function parseMessage(input: string): ParsedMetabotMessage {
  const words = input.split(" ");
  return words.map((word) => {
    if (word.startsWith("https://")) {
      const parsed = parseMetabaseUrl(input);
      if (parsed) {
        return parsed;
      }
    }

    const metabaseProtocolPos = word.indexOf("metabase://");
    if (metabaseProtocolPos > -1) {
      const url = word.slice(metabaseProtocolPos, -1);
      const pathname = url.slice("metabase://".length);
      const [model, id] = pathname.split("/");
      return {
        model,
        id: parseInt(id),
        name: "", // TODO?
