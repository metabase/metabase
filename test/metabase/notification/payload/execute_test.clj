...
(deftest execute-card-result-structure-test
  (testing "execute-card returns only the expected top-level keys (no :json_query or other QP internals)"
    (mt/with-temp [:model/Card {card-id :id} {:dataset_query (mt/mbql-query venues)}]
      (let [part   (notification.payload.execute/execute-card (mt/user->id :rasta) card-id)
            result (:result part)]
        (is (= :card (:type part)))
        (is (= expected-top-level-keys (set (keys result)))
            "Result should only contain notification-safe top-level keys")
...
(deftest execute-dashboard-subscription-card-result-structure-test
  (testing "execute-dashboard-subscription-card returns only the expected top-level keys"
    (mt/with-temp [:model/Card      {card-id :id} {:dataset_query (mt/mbql-query venues)}
