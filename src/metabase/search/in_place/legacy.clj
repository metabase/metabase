...
(defmethod search.engine/model-set :search.engine/in-place
  ...
  (let [raw-queries   (vec (for [model (search.in-place.filter/search-context->applicable-models
        ...
        nested-queries (mapv (fn [query]
                               ^:allow-subquery
                               {:nest (vary-meta (sql.helpers/limit query 1) assoc :allow-subquery true)})
                             queries)
  ...
  (let [models       (:models search-ctx)
    ...
    (cond
      ...
      (let [model-queries (vec (for [model models
            ...
            subqueries    (mapv #(vary-meta % assoc :allow-subquery true) queries)]
        (cond-> {:select   [:*]
                 :from     [[^:allow-subquery {:union-all subqueries} :alias_is_required_by_sql_but_not_needed_here]]
