...
Changing Metabase to multi-tenant strategy enables special [user](#user-types) and [collection](#collection-types) types. You can create new tenants, tenant groups, and collection, and you get some additional admin settings in the People and Permissions tabs.
...
1. [Create a tenant](#create-tenant-users-in-metabase).
2. Go to **Admin settings > People** .
...
You can also [provision tenant users with JWT](#tenant-attributes).
