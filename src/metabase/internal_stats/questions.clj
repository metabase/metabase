...
(defn- contains-embedding-param
  "`param` is bound as a query parameter rather than inlined -- a jsonpath variable (`$val`) on Postgres, an ordinary
  `JSON_SEARCH` argument on MySQL -- so it is never spliced into the statement."
  [param]
  (condp = (db/db-type)
    :mysql [:!= [:json_search
                 :embedding_params
