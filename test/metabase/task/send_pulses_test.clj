...
(deftest send-pulses-test
  (mt/with-temp
    [Card                                {card-id :id}  (assoc (checkins-query-card {:breakout [[:field (mt/id :checkins :date) {:temporal-unit :day}]]})
...
(deftest clear-subscription-channels-test
  (testing "Removes empty :model/SubscriptionChannel"
    (mt/with-temp [Pulse        {pulse-id :id} {}
                   :model/SubscriptionChannel _ {:pulse_id pulse-id}]
      (#'send-pulses/clear-subscription-channels!)
      (is (= 0
             (t2/count :model/SubscriptionChannel)))
      (is (:archived (t2/select-one Pulse :id pulse-id)))))

  (testing "Has SubscriptionChannelRecipient"
    (mt/with-temp [Pulse                               {pulse-id :id} {}
                   :model/SubscriptionChannel          {sc-id :id}    {:pulse_id     pulse-id
                                                                       :channel_type :email}
                   :model/SubscriptionChannelRecipient _              {:user_id                 (mt/user->id :rasta)
