...
(#?(:clj p/defprotocol+ :cljs defprotocol) MetadataProvider
 ...
 cache).")
  ...
  (metadatas-for-names [metadata-provider metadata-type names]
...
(mu/defn- metadata-for-name :- [:maybe ::metadata]
  [metadata-provider :- ::metadata-provider
   metadata-type     :- ::metadata-type-excluding-database
   metadata-name     :- :string]
  (m/find-first (fn [object]
                  (= (:name object) metadata-name))
                (metadatas-for-names metadata-provider metadata-type [metadata-name])))
...
(mu/defn native-query-snippet-for-name :- [:maybe ::lib.schema.metadata/native-query-snippet]
  ...
  [metadata-provider :- ::metadata-provider
   snippet-name      :- :string]
  (metadata-for-name metadata-provider :metadata/native-query-snippet snippet-name))
