...
(defn- have-select-privilege-fn
  ...
  (if (and database (driver/database-supports? driver :table-privileges database))
...
(defn post-filtered-active-tables
  ...
  [driver ^Connection conn & [db-name-or-nil schema-inclusion-filters schema-exclusion-filters]]
  {:pre [(instance? Connection conn)]}
  (let [have-select-privilege-fn? (have-select-privilege-fn driver *describing-database* conn)]
    (eduction
     (comp
   ...
   (sql-jdbc.execute/do-with-connection-with-options
    ...
    (fn [^Connection conn]
      (let [schema-filter-prop      (driver.u/find-schema-filters-prop driver)
        ...
        (binding [*describing-database* database]
