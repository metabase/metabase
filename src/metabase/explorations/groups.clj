(ns metabase.explorations.groups
  "Compute auto-derived groups over a thread's `ExplorationQuery` rows.
   ...
   One grouping heuristic runs today: `[card_id dimension_id]` bundles queries that share
   a metric Card and a dimension but vary by segment. `metabase.explorations.api/generate-queries!`
   always emits an unsegmented base row (`segment_id = nil`) per (metric, dim) pair, so
   the group's display name is taken from that base row.
...
(defn auto-groups
  "Given a seq of hydrated `ExplorationQuery` rows for a single thread, return a vector
   ...
   `:display_type` tells the FE how to render the group:
     ...
     - `\"sidebar\"`   — group expands/collapses inline as a dropdown (reserved; not emitted today)

   Queries within a group keep their input order (positions). Groups are ordered by max
   `:interestingness_score` (desc), then by group `:id` for a stable tiebreak; groups
   with no scored queries sort last. `:position` is the 0-indexed slot in that ordering."
  [queries]
  (let [all-groups (->> (group-by (juxt :card_id :dimension_id) queries)
