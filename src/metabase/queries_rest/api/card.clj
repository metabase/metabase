...
;;
;; The checks a card write must pass — run permission on the query, create/write permission on the collection, the
;; cycle check, the type's shape rules — live in [[metabase.queries.card-write]], with the other surfaces that write
;; a card on a user's behalf.
...
(api.macros/defendpoint :post "/"
  ...
  [_route-params
   _query-params
   new-card :- card-write/CardCreateSchema]
  (-> (card-write/create-card! new-card)
...
(defn update-card!
  "Update a card and return it in the shape `PUT /api/card/:id` returns it."
  [id card-updates delete-old-dashcards?]
  (-> (card-write/update-card! id card-updates delete-old-dashcards?)
      (assoc :last-edit-info (revisions/edit-information-for-user @api/*current-user*))))
...
(api.macros/defendpoint :put "/:id"
  ...
  [{:keys [id]} :- [:map
   ...
   body :- card-write/CardUpdateSchema]
