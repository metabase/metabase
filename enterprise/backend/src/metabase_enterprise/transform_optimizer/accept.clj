...
(defn- proposal->create-body
  [{original-source :source :keys [target source_database_id] :as original}
   {:keys [body name id target_table] :as _proposal}
   collection-id]
  (when (and body (not (str/blank? body)))
    (let [original-query (:query original-source)
          ...
          ;; The LLM emits `target_table` for `:precompute` proposals so a
          ;; dependent rewrite can FROM the exact same table name. Fall
          ;; back to deriving it from the proposal's :name only when the
          ;; LLM forgot — older preludes didn't have this field.
          target-name    (clamp-name (or target_table name))]
