...
(deftest ^:parallel collection-in-value-clause-test
  (let [evil-values {"map inside a vector"   [{:raw "1) UNION SELECT 1 -- "}]
    ...
    ;; The MBQL 5 `[:value <opts> <value>]` argument order is normalized to the legacy order upstream on newer
    ;; releases; this line predates that normalization, so `->honeysql` only ever sees the legacy shape. The injection
    ;; guard above covers the clause form this release actually produces. (Even in MBQL 5 order the guard still fires,
    ;; because the leading opts map trips the same collection check.)
    ))
