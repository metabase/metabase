(ns ^:mb/driver-tests metabase.api.downloads-exports-test
...
(defn- normalize-num-str
  "Round any numeric-looking string in `s` to [[normalize-num-str-precision]] decimal places and strip
  *excess* trailing zeros — a driver-agnostic normalisation for unformatted numeric cells (H2 emits
  `854.19` while Postgres emits `854.1899999999999`). Preserves the integer-vs-decimal shape of the input:
  `\"1\"` stays `\"1\"`; `\"3.0\"` (or `\"3.00\"`) stays `\"3.0\"`. Non-numeric strings pass through
  untouched."
  [s]
  (or (when (string? s)
        (when-let [d (parse-double s)]
          (let [rounded  (-> ^java.math.BigDecimal (bigdec d)
                ...
                stripped (if (str/includes? rounded ".")
                           ...
                           rounded)]
            (cond
              (str/includes? s ".")          (cond-> stripped
                                               (str/ends-with? stripped ".") (str "0"))
              (str/ends-with? stripped ".") (subs stripped 0 (dec (count stripped)))
              :else                          stripped))))
      s))

(defn- normalize-nums
  "Walk `form` and normalise every numeric leaf to 2 decimal places — both numeric-looking strings (via
  [[normalize-num-str]]) and native doubles/BigDecimals (via `Math/round`). Non-numeric values pass through."
  [form]
  (walk/prewalk
   (fn [x]
     (cond
       (string? x) (normalize-num-str x)
       (double? x) (double (/ (Math/round (* 100.0 x)) 100.0))
       :else       x))
   form))
...
(deftest ^:mb/driver-tests simple-pivot-export-test
...
(deftest ^:mb/driver-tests simple-pivot-export-row-col-totals-test
...
(deftest ^:parallel ^:mb/driver-tests pivot-with-multiple-columns-no-row-totals
...
(deftest ^:parallel ^:mb/driver-tests simple-pivot-export-works-even-with-table-column-ordering-test
  (testing "Pivot table exports are not affected by table sort settings"
    (mt/test-drivers #{:h2 :postgres}
      (let [category-name (mt/format-name "category")
            created-at-name (mt/format-name "created_at")]
        (testing "Try some permutations with csv"
          (doseq [col-order   (math.combo/permutations [{:name category-name}
...
(deftest ^:parallel ^:mb/driver-tests pivot-export-test
  (mt/test-drivers #{:h2 :postgres}
    (mt/dataset test-data
      (mt/with-temp [:model/Card {pivot-data-card-id :id}
                     {:dataset_query {:database (mt/id)
...
(deftest ^:mb/driver-tests multi-measure-pivot-tables-headers-test
    ...
    (mt/test-drivers #{:h2 :postgres}
      (mt/dataset test-data
        (mt/with-temp [:model/Card {pivot-card-id :id}
                       {:display                :pivot
                        :visualization_settings {:pivot_table.column_split
                                                 {:rows    [(mt/format-name "created_at")]
                                                  :columns [(mt/format-name "category")]
                                                  :values  ["sum" "avg"]}}
                        :dataset_query          (mt/mbql-query products
                                                  {:aggregation [[:sum $price]
                                                                 [:avg $rating]]
...
(deftest ^:parallel ^:mb/driver-tests pivot-export-aggregations-test
...
(deftest ^:parallel ^:mb/driver-tests zero-column-pivot-tables-test
    ...
    (mt/test-drivers #{:h2 :postgres}
      (mt/dataset test-data
        (mt/with-temp [:model/Card {pivot-card-id :id}
                       {:display                :pivot
                        :visualization_settings {:pivot_table.column_split
                                                 {:rows    [(mt/format-name "created_at") (mt/format-name "category")]
                                                  :columns []
                                                  :values  ["sum"]}}
                        :dataset_query          (mt/mbql-query products
                                                  {:aggregation [[:sum $price]]
...
(deftest ^:parallel ^:mb/driver-tests zero-row-pivot-tables-test
        ...
        (mt/with-temp [:model/Card {pivot-card-id :id}
          ...
          (let [result (->> (mt/user-http-request :crowberto :post 200
            ...
            (is (= [["Doohickey" "Gadget" "Gizmo" "Widget" "Row totals"]
                   ...
                   (normalize-nums result)))))))))

(deftest ^:parallel ^:mb/driver-tests zero-column-multiple-measures-pivot-tables-test
    ...
    (mt/test-drivers #{:h2 :postgres}
      (mt/dataset test-data
        (mt/with-temp [:model/Card {pivot-card-id :id}
                       {:display                :pivot
                        :visualization_settings {:pivot_table.column_split
                                                 {:rows    [(mt/format-name "category") (mt/format-name "created_at")]
                                                  :columns []
                                                  :values  ["sum" "count"]}}
                        :dataset_query          (mt/mbql-query products
                                                  {:aggregation [[:sum $price]
...
(deftest ^:mb/driver-tests pivot-table-in-xlsx-test
            ...
            (mt/with-temporary-setting-values [qp.settings/enable-pivoted-exports false]
              (let [result      (mt/user-http-request :crowberto :post 200
                ...
                (is (= [["Category" "Created At: Month" "Sum of Price" "Average of Rating"]
                        ["Doohickey" "May 1, 2016, 12:00 AM" "144.12" "2.9667"]
                        ["Doohickey" "June 1, 2016, 12:00 AM" "82.92" "3.6"]]
                       (normalize-nums (take 3 sheet))))))))))))

(deftest ^:parallel ^:mb/driver-tests zero-column-native-pivot-tables-test
...
(deftest ^:parallel ^:mb/driver-tests zero-row-native-pivot-tables-test
...
(deftest ^:parallel ^:mb/driver-tests pivot-table-questions-can-export-unpivoted
...
(deftest ^:parallel ^:mb/driver-tests pivot-table-questions-can-export-unpivoted-2
  (testing "Pivot tables will export the 'classic' way by default"
    (mt/test-drivers #{:h2 :postgres}
      (testing "for xlsx"
        (mt/dataset test-data
...
(deftest ^:mb/driver-tests unpivoted-pivot-results-do-not-include-pivot-grouping
...
(deftest ^:mb/driver-tests unpivoted-pivot-results-use-correct-formatters-in-xlsx
...
(deftest ^:mb/driver-tests format-rows-value-affects-xlsx-exports
      ...
      (mt/with-temporary-setting-values [enable-pivoted-exports true
        ...
        (mt/with-temp [:model/Card card
          ...
          (is (= [["Category"
                  ...
                  ["Doohickey" "632.14" "854.19" "496.43" "203.13" "2185.89"]
                  ["Gadget" "679.83" "1059.11" "844.51" "435.75" "3019.2"]]
                 (take 3 (normalize-nums (card-download card {:export-format :xlsx :format-rows false :pivot true}))))))))))
...
(deftest ^:mb/driver-tests pivot-measures-order-test
...
(deftest ^:mb/driver-tests pivot-rows-order-test
...
(deftest ^:mb/driver-tests pivot-non-numeric-values-in-aggregations
...
(deftest ^:mb/driver-tests pivot-with-scale-test
...
(deftest ^:parallel ^:mb/driver-tests pivot-exports-handle-nil-in-breakout-column
                         ...
                         {:display                :pivot
                          ...
                          :dataset_query          (mt/mbql-query nil
                                                    {:breakout     [[:field "MEASURE" {:base-type :type/Integer}]],
                                                     ...
                                                     :source-table (format "card__%s" pivot-data-card-id)})}]
            (let [result (card-download pivot-card {:export-format :csv :pivot true})]
              ;; H2 sorts NULLs first, Postgres last — compare middle rows as a set so the test is
              ;; order-agnostic across drivers. Header + Grand totals row are still positional.
              (is (= ["MEASURE" "Count" "Sum of A"]     (first result)))
              (is (= ["Grand totals" "3" "6"]           (last result)))
              (is (= #{["" "1" "2"] ["1" "1" "1"] ["3" "1" "3"]}
                     (set (butlast (rest result))))))))))))

(deftest ^:parallel ^:mb/driver-tests pivot-exports-handle-aggregations-with-the-same-base-name
...
(deftest ^:parallel ^:mb/driver-tests pivot-exports-aggregations-work
...
(deftest ^:parallel ^:mb/driver-tests pivot-xlsx-export-respects-custom-title
...
(deftest ^:mb/driver-tests pivot-subtotal-formatting-in-xlsx-test
...
(deftest ^:parallel ^:mb/driver-tests pivot-condense-duplicate-totals-csv-test
  (testing "`pivot.condense_duplicate_totals` affects CSV output as expected"
    (mt/test-drivers #{:h2 :postgres}
      (let [viz-settings (fn [condense?]
                           {:pivot_table.column_split {:rows [(mt/format-name "category")]
                                                       :columns [(mt/format-name "created_at")]
                                                       :values ["count"]}
                            :column_settings
                            {"[\"name\",\"sum\"]" {:number_style "currency"}
                           ...
                           {:display :pivot
                            ...
                            :dataset_query (mt/mbql-query products
                                             {:aggregation [[:count]]
                                              :breakout [$category !year.created_at]
                                              :limit 10})}]
              ;; `:limit N` on a pivot has different semantics on the multi-query path (per-sub-query)
              ;; and the native `GROUPING SETS` path (whole result). Skip parity check for this test.
...
(deftest ^:parallel ^:mb/driver-tests pivot-bigint-bigdecimal-normalization-test
  (testing "Ensure key normalization for values-by-key handles BigInts and BigDecimals properly (#62724)"
    ;; `:limit N` in the pivot's underlying query diverges between multi (per-sub-query) and native (whole
    ;; GROUPING SETS output), so skip parity check for this test.
    (qp.pivot.test-util/without-pivot-parity-check
     (mt/test-drivers #{:h2 :postgres}
       (mt/dataset test-data
         (mt/with-temp [:model/Card card
                        {:display                :pivot
                         :visualization_settings {:pivot_table.column_split
                                                  {:rows    [(mt/format-name "id")]
                                                   :columns [(mt/format-name "rating")]
                                                   :values  ["avg"]}}
                         :dataset_query          (mt/mbql-query products
                        ...
                        {:dataset_query
                         {:database (mt/id)
                          ...
                          :native {:template-tags {}
                                   ;; DUAL is Oracle/H2; both H2 and Postgres allow `SELECT ...` without a FROM
                                   ;; clause, so drop it. Also quote identifiers so their case survives Postgres.
                                   :query         (str "SELECT 1 as \"ID\", 1 as \"QUARTER\", 100 as \"PRICE\" "
                                                       "UNION "
                                                       "SELECT 1 as \"ID\", 2 as \"QUARTER\", 100 as \"PRICE\" "
                                                       "UNION "
                                                       "SELECT 2, 1, 200 "
                                                       "UNION "
                                                       "SELECT 3, 1, 300")}}
...
(deftest ^:parallel ^:mb/driver-tests pivot-long-column-names-download-test
                         ...
                         {:display                :pivot
                          ...
                          :dataset_query          (mt/mbql-query orders
                                                    {:aggregation [[:sum $total]
                                                     ...
                                                     :limit       3})}]
            ;; `:limit 3` diverges between multi (per-sub-query) and native (whole GROUPING SETS). Skip
            ;; parity check for this test.
            (qp.pivot.test-util/without-pivot-parity-check
             (testing "download"
