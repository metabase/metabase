...
;; dashboard attributes are stored like this:
{:parameters       [{:name "film ID"
                     :slug "film_id"
                     :id   "570ce143"
                     :type "id"}]
 :embedding_params {:film_id "locked" ;; "id" was renamed to "film_id"
                    :id      "locked" ;; there are no longer params with the slug "id" so this is broken
                    }}

;; this is used to generate JWT token. It fails because there are no params with the slug "id":
{:resource {:dashboard 451}
 :params   {"id" "5"}
 :exp      "exp"}


;; dashboard attributes could be stored like this:
{:parameters [{:name      "film ID"
               :slug      "film_id"
               :id        "570ce143"
               :type      "id"
               :embedding "disabled | editable | locked"}]}

;; this is used to generate JWT token:
(def payload
  {:resource {:dashboard 451}
   :params   {:name "film ID"  ;; this is only used as documentation to help you remember what this refers to
              :id   "570ce143" ;; use the parameter's id, not the slug, for identification
              }
   :exp      "exp"})
