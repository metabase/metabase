...
(defop / [x y & more])
(defop ^{:style/indent [:form]} case [cases] [cases fallback])
(defop coalesce [x y & more])
(defop abs [x])
(defop log [x])
(defop exp [x])
(defop sqrt [x])
...
(defop round [x])
(defop power [n expo])
(defop interval [n unit])
(defop time [t unit])
(defop absolute-datetime [t unit])
(defop now [])
(defop convert-timezone [t source dest])
(defop get-week [t mode])
(defop get-year [t])
(defop get-month [t])
(defop get-day [t])
(defop get-hour [t])
...
(defop get-second [t])
(defop get-quarter [t])
(defop get-day-of-week [t] [t mode])
(defop datetime-add [t i unit])
(defop date [s])
(defop today [])
(defop datetime-subtract [t i unit])
(defop concat [s1 s2 & more])
(defop substring [s start end])
(defop split-part [s delimiter index])
(defop collate [s collation])
(defop replace [s search replacement])
(defop regex-match-first [s regex])
(defop length [s])
(defop trim [s])
(defop ltrim [s])
(defop rtrim [s])
