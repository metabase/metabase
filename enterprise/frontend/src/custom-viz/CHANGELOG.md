...
- Plugin settings are now stored under `custom-viz:<plugin name>:<setting id>` keys in a question's `visualization_settings`, so a plugin can never write one of Metabase's own settings. Plugins keep using their own ids: `settings`, `onChangeSettings`, `readDependencies`, `writeDependencies`, and `eraseDependencies` are translated by Metabase and refer to the plugin's own settings only. `onChangeSettings` and the dependency lists are typed accordingly, and new writes to Metabase settings such as `card.title` or `click_behavior` don't take effect. Plugin settings saved by earlier plugin versions are adopted under the new keys automatically. Metabase settings an earlier plugin version already wrote — including `click_behavior` — stay stored and keep taking effect until removed, so review click behavior on existing custom visualization cards ([#81181](https://github.com/metabase/metabase/pull/81181)).
