...
describe("a lazy route", () => {
  it("defers the location until the module resolves", async () => {
    ...
    const { store, history } = renderRoutes(makeRoutes(), {
      initialRoute: "/",
    });

    store.dispatch(push("/split"));

    expect(history?.getCurrentLocation().pathname).toBe("/");
    expect(seen).toEqual(["/"]);

    expect(await screen.findByTestId("split")).toBeInTheDocument();
    expect(history?.getCurrentLocation().pathname).toBe("/split");
  ...
  it("keeps the previous page mounted while the module loads", async () => {
    const { store } = renderRoutes(makeRoutes(), { initialRoute: "/" });
