(ns metabase-enterprise.search.semantic.embedding-circuit-breaker-test
  (:require
   ...
   [metabase-enterprise.search.semantic.embedding-health :as embedding-health]
   [metabase-enterprise.search.semantic.health]
   [metabase-enterprise.search.semantic.models.token-tracking :as token-tracking]
  ...
  (testing "the breaker transition reports the outage without an error log for every guarded request"
    (with-redefs [semantic.embedding/embedder-circuit-breakers
                  (atom {test-endpoint (dh.cb/circuit-breaker
                                        {:failure-threshold 1 :success-threshold 1 :delay-ms 60000})})]
      (is (thrown? Exception (call-through boom)))
      (mt/with-log-messages-for-level [messages [metabase-enterprise.search.semantic.embedding :error]]
        (is (=? {:cause :embedder/circuit-open :status 502}
                (try
