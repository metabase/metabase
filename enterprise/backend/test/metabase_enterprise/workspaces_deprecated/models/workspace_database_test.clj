(ns metabase-enterprise.workspaces-deprecated.models.workspace-database-test
    ...
    (try
      ...
      (finally
        ...
        (when-let [ws-ids (seq (:workspaces @ids))]
          (try (t2/delete! :model/WorkspaceDeprecated :id [:in ws-ids]) (catch Throwable _ nil)))
      ...
      (with-tracked-ids
        (fn [track-db! track-ws! track-wsd!]
          (let [db-id  (t2/insert-returning-pk! :model/Database
                ...
                _      (track-db! db-id)
                ws-id  (t2/insert-returning-pk! :model/WorkspaceDeprecated
                                                {:name "pre-delete-refuse-initialized-ws"})
                _      (track-ws! ws-id)
                wsd-id (t2/insert-returning-pk! :model/WorkspaceDatabaseDeprecated
                                                {:workspace_id ws-id
                                                 :database_id  db-id
                                                 :database_details {}
                                                 :output_namespace ""
            ...
            (let [thrown (try (t2/delete! :model/Database :id db-id) nil
              ...
              (is (true? (t2/exists? :model/WorkspaceDatabaseDeprecated :id wsd-id))
      ...
      (with-tracked-ids
        (fn [track-db! track-ws! track-wsd!]
          (let [db-id  (t2/insert-returning-pk! :model/Database
            ...
            (is (false? (t2/exists? :model/WorkspaceDatabaseDeprecated :id wsd-id))
...
;;;
;;; All WorkspaceDatabase permission predicates are superuser-only.
...
(deftest can-read?-test
  (testing "WorkspaceDatabase `mi/can-read?`"
    (mt/with-temp [:model/WorkspaceDeprecated         {ws-id :id} {}
                   :model/WorkspaceDatabaseDeprecated wsd         {:workspace_id ws-id}]
      (testing "non-superuser — false"
        (with-normal-user (is (false? (mi/can-read? wsd)))))
      (testing "Data Analyst (non-superuser) — false"
        (with-data-analyst (is (false? (mi/can-read? wsd)))))
      (testing "superuser — true"
        (with-admin (is (true? (mi/can-read? wsd))))))))

(deftest can-create?-test
  (testing "WorkspaceDatabase `mi/can-create?`"
    (testing "non-superuser — false"
      (with-normal-user (is (false? (mi/can-create? :model/WorkspaceDatabaseDeprecated {:database_id (mt/id)})))))
    (testing "Data Analyst (non-superuser) — false"
      (with-data-analyst
        (is (false? (mi/can-create? :model/WorkspaceDatabaseDeprecated {:database_id (mt/id)})))))
    (testing "superuser — true"
      (with-admin
        (is (true? (mi/can-create? :model/WorkspaceDatabaseDeprecated {:database_id (mt/id)})))))))
...
(deftest can-write?-test
  (testing "WorkspaceDatabase `mi/can-write?`"
    (mt/with-temp [:model/WorkspaceDeprecated         {ws-id :id} {}
      ...
      (testing "superuser — true"
        (with-admin (is (true? (mi/can-write? wsd))))))))
