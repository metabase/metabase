...
(defmethod sql.qp/date [:snowflake :default]         [_ _ expr _] expr)
(defmethod sql.qp/date [:snowflake :minute]          [_ _ expr _] (date-trunc :minute expr))
(defmethod sql.qp/date [:snowflake :minute-of-hour]  [_ _ expr _] (extract :minute expr))
(defmethod sql.qp/date [:snowflake :hour]            [_ _ expr _] (date-trunc :hour expr))
(defmethod sql.qp/date [:snowflake :hour-of-day]     [_ _ expr _] (extract :hour expr))
(defmethod sql.qp/date [:snowflake :day]             [_ _ expr _] (date-trunc :day expr))
(defmethod sql.qp/date [:snowflake :day-of-week]     [_ _ expr _] (extract :dayofweek expr))
(defmethod sql.qp/date [:snowflake :day-of-month]    [_ _ expr _] (extract :day expr))
(defmethod sql.qp/date [:snowflake :day-of-year]     [_ _ expr _] (extract :dayofyear expr))
(defmethod sql.qp/date [:snowflake :week]            [_ _ expr _] (date-trunc :week expr))
(defmethod sql.qp/date [:snowflake :week-of-year]    [_ _ expr _] (extract :week expr))
(defmethod sql.qp/date [:snowflake :month]           [_ _ expr _] (date-trunc :month expr))
(defmethod sql.qp/date [:snowflake :month-of-year]   [_ _ expr _] (extract :month expr))
(defmethod sql.qp/date [:snowflake :quarter]         [_ _ expr _] (date-trunc :quarter expr))
(defmethod sql.qp/date [:snowflake :quarter-of-year] [_ _ expr _] (extract :quarter expr))
(defmethod sql.qp/date [:snowflake :year] [_ _ expr padded]
  (if padded (date-trunc :year expr) (extract :year expr)))
