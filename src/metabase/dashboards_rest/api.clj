...
(defn- updated-dashboard-response
  "Hydrate a freshly saved Dashboard row into the shape the update endpoints return."
  [dashboard]
  (-> dashboard
      hydrate-dashboard-details
      collection.root/hydrate-root-collection
...
(api.macros/defendpoint :post "/:from-dashboard-id/copy"
  ...
  (dashboards.write/copy-dashboard! from-dashboard-id
                                    {:name                name
...
(api.macros/defendpoint :put "/:id"
  ...
  (updated-dashboard-response (dashboards.write/update-dashboard! id dash-updates)))
...
(api.macros/defendpoint :put "/:id/cards"
  ...
  [{:keys [id]} :- [:map
   ...
   {:keys [cards tabs]} :- [:map
                            ...
                            [:tabs  {:optional true} [:maybe (ms/maps-with-unique-key [:sequential dashboards.write/UpdatedDashboardTab] :id)]]]]
  (log/warn
   "DELETE /api/dashboard/:id/cards is deprecated. Use PUT /api/dashboard/:id instead.")
  (let [dashboard (updated-dashboard-response (dashboards.write/update-dashboard! id {:dashcards cards :tabs tabs}))]
