...
(defn write-openapi-spec-to-file!
  ...
  (try
    ...
    (catch Throwable e
      (log/error "Failed to write OpenAPI specification to file:" (ex-message e)))))
      ...
      (try
        ...
        (loop []
          (when (:needs-regen? @openapi-regen-state)
            ...
            (recur)))
        (catch Throwable e
          (log/error "Error regenerating OpenAPI specification:" (ex-message e)))
        (finally
    ...
    (let [file (io/file openapi-file-path)]
      (when (.exists ^java.io.File file)
        (with-open [f (io/reader file)]
          (json/decode+kw f))))
    (catch Throwable e
      (log/error "Failed to read OpenAPI specification from file:" (ex-message e))
      nil)))
