(ns metabase.query-processor.middleware.metrics.joined-subquery-expansion-test
  (:require
...
(defn- expand-referenced-metrics
  ...
  (-> query
      (assoc-in [:info :pivot/original-query] query)
      (#'fetch-source-query/resolve-source-cards)
      (#'metrics.joined-subquery-expansion/expand-referenced-metrics)
      (dissoc :info)))
...
#_(deftest ^:parallel adjust-basic-source-table-test
  ...
  (let [[source-metric mp] (mock-metric)
    ...
    (is (=? {:stages [{:source-table (meta/id :products)
                       :joins [{:stages [{:source-table (meta/id :products)
                                          :aggregation [[:avg {} [:field {} (meta/id :products :rating)]]]}
                                         {}]}]}
                      {:fields [[:field {} "Products__avg"]]}]}
            @(def xiz (expand-referenced-metrics query))))))

#_(deftest ^:parallel adjust-basic-source-metric-test
  (let [[source-metric mp] (mock-metric)
  ...
  (let [[source-metric mp] (mock-metric)
    ...
    (is (=? {:stages [{:source-table (meta/id :products)
                       :joins [{:stages [{:source-table (meta/id :products)
                                          :aggregation [[:avg {} [:field {} (meta/id :products :rating)]]]}
                                         {}]}]
                       :breakout [[:field map? string?]]}
                      {:fields [[:field map? string?]]}]}
            (expand-referenced-metrics query)))))

#_(deftest ^:parallel adjust-aggregation-metric-ref-test
  (let [[source-metric mp] (mock-metric)
        query (-> (lib/query mp (meta/table-metadata :products))
...
(deftest ^:parallel adjust-aggregation-metric-ref-test
  (let [[source-metric mp] (mock-metric)
        aggregation-name "Aggregation expression with metric reference"
        query (-> (lib/query mp (meta/table-metadata :products))
                  (lib/aggregate (lib.options/update-options
                                  (lib/+ (-> [:metric {} (:id source-metric)]
                                             lib.options/ensure-uuid)
                                         1)
                                  assoc :name aggregation-name)))]
    (is (=?
         {:stages [{:source-table (meta/id :products)
                    :expressions [[:+ {:lib/expression-name aggregation-name}
                                   [:field map? string?] 1]]
                    :joins [{:stages [{:source-table (meta/id :products)
                                       :aggregation [[:avg {} [:field {} (meta/id :products :rating)]]]}
                                      {}]}]
                    :breakout [[:expression map? aggregation-name]]
                    #_#_:breakout [[:field map? string?]]}
                   {:fields [[:field map? string?]]}]}
         (expand-referenced-metrics query)))))

#_(deftest ^:parallel metric-with-implicit-join-test
...
(deftest ^:parallel metric-with-implicit-join-test-xxx
  (testing "Metrics with filters on implicitly joined columns should work #43943"
    (let [[source-metric mp] (mock-metric (as-> (lib/query meta/metadata-provider (meta/table-metadata :orders)) $q
          ...
          query (-> (lib/query mp (meta/table-metadata :orders))
                    (lib/aggregate (lib.options/ensure-uuid [:metric {} (:id source-metric)])))]
      (is (=? {:stages [{:joins [{:stages [{:aggregation [[:count map?]]
                                            :filters [[:= map?
                                                       [:field
      ...
      (testing "With an explicit product join in consumer query"
        (is (=? {:stages [{:source-table (meta/id :orders)
                           :joins [{:stages [{:source-table (meta/id :products)}]}
                                   {:stages [{:source-table (meta/id :orders)
                                              :aggregation [[:count map?]]
                                              ;;
                                              :joins [;; Implicit join
                                                      {:stages [{:source-table (meta/id :products)}]}
                                                      ;; Explicit join from referencing query
                                                      {:stages [{:source-table (meta/id :products)}]}]}
                                             ;; dummy stage for source-query join
                                             {}]}]
                           :breakout [[:field map? "count"]]}
                          {:fields [vector?]}]}
                (expand-referenced-metrics
                 (as-> (lib/query mp (meta/table-metadata :orders)) $q
                   (lib/join $q (meta/table-metadata :products))
                   (lib/filter $q (lib/= (m/find-first (comp #{(meta/id :products :title)} :id) (lib/filterable-columns $q))
                                         "foobar"))
                   (lib/aggregate $q (lib.options/ensure-uuid [:metric {} (:id source-metric)])))))))
      (testing "With an implicit product join in consumer query"
        (is (=?
             {:stages [{:source-table (meta/id :orders)
                        :joins [{:stages [{:filters [[:= {} [:field {} (meta/id :products :category)] [:value {} "Gadget"]]
                                                     [:= {} [:field {} (meta/id :products :title)] "foobar"]]
                                           :aggregation [[:count {}]]}
                                          {}]}]
                        :breakout [[:field map? "count"]]}
                       {:fields [[:field map? string?]]}]}
...
#_(deftest ^:parallel multiple-source-metrics-with-implicit-join-test
...
(deftest ^:parallel multiple-source-metrics-with-implicit-join-test
  (let [[first-metric mp] (mock-metric meta/metadata-provider
                    ...
                    :joins [;; metric 1 join
                            ...
                            {:stages [{:source-table (meta/id :orders)
                                       ...
                                       :filters [[:= map?
                                                  ...
                                                  [:value map? "Title"]]]}
                                      {}]}]}
                   ;; TODO: display-name will be modified to something metric specific!!!
                   {:fields [[:field {:display-name "Mock Metric 1"} string?]
                             [:field {:display-name "Mock Metric 2"} string?]]}]}
         (expand-referenced-metrics query)))))

#_(deftest ^:parallel adjust-aggregation-metric-ordering-test
  (let [[source-metric mp] (mock-metric)
        query (-> (lib/query mp (meta/table-metadata :products))
                  (lib/aggregate (lib/+ (lib.options/ensure-uuid [:metric {} (:id source-metric)]) 1)))]
...
(deftest ^:parallel adjust-aggregation-metric-ordering-test
  (let [[source-metric mp] (mock-metric)
        query (-> (lib/query mp (meta/table-metadata :products))
                  (lib/aggregate (-> (lib/+ (lib.options/ensure-uuid [:metric {} (:id source-metric)]) 1)
                                     (lib.options/update-options assoc :name "my cool aggregation"))))]
    (doseq [agg-ref (map lib.options/uuid (lib/aggregations query))
            :let [ordered (lib/order-by query (lib.options/ensure-uuid [:aggregation {} agg-ref]))
                  adjusted (expand-referenced-metrics ordered)]]
      (is (=?
           {:stages [{:source-table (meta/id :products)
                      :joins [;; metric join
                              {:stages [{:source-table (meta/id :products)
                                        ...
                                        {}]}]
                      :expressions [[:+
                                     {:lib/expression-name "my cool aggregation"}
                                     ;; this is joined metric field
                                     [:field {} string?]
                                     1]]
                      :breakout [[:expression {} "my cool aggregation"]]}
                     {:fields [[:field {} "my cool aggregation"]]
                      :order-by [[:asc {} [:field {} "my cool aggregation"]]]}]}
           adjusted)))))

#_(deftest ^:parallel adjust-join-test
  (let [[source-metric mp] (mock-metric)
        query (-> (lib/query mp source-metric)
...
;; TODO: Resolve later!
;;       How is this meant to work? there is no products/id in that metric source
#_(deftest ^:parallel adjust-join-test
  (let [[source-metric mp] (mock-metric)
        query (-> (lib/query mp source-metric)
                  (lib/join (-> (lib/join-clause (meta/table-metadata :orders)
...
;; this should be removed in favor of my (duplicate) expression tests
(deftest ^:parallel adjust-expression-test
  (let [[source-metric mp] (mock-metric (lib/expression (basic-metric-query) "source" (lib/+ 1 1)))
    ...
    (is (=?
         {:stages [{:joins [{:stages [{:expressions [[:+ {:lib/expression-name "source"} 1 1]
                                                     [:- {:lib/expression-name "target"} 2 2]]
                                       :aggregation [[:avg {} [:field {} pos-int?]]]}
                                      {}]}]}
                   {:fields [[:field {:display-name "Mock Metric"} string?]]}]}
         (expand-referenced-metrics query)))))

#_(deftest ^:parallel adjust-expression-name-collision-test
...
(deftest ^:parallel adjust-expression-name-collision-test
  (let [[source-metric mp] (mock-metric (-> (basic-metric-query)
        ...
        query (-> (lib/query mp (meta/table-metadata :products))
                  ...
                  (lib/aggregate (lib.metadata/metric mp (:id source-metric))))]
    (is (=?
         {:stages [{:joins [{:stages [{:expressions [[:+ {:lib/expression-name "foobar"} 1 1]
                                                     [:+ {:lib/expression-name "qux"} [:expression {} "foobar"] 1]
                                                     [:- {:lib/expression-name "foobar_2"} 2 2]
                                                     [:- {:lib/expression-name "qux_2"} [:expression {} "foobar_2"] 2]]
                                       :filters [[:= {} [:expression {} "qux"] [:expression {} "foobar"]]
                                                 [:= {} [:expression {} "foobar_2"] [:expression {} "qux_2"]]]
                                       :aggregation [[:avg {} [:field {} (meta/id :products :rating)]]]}
                                      {}]}]
                    :breakout [[:field {:display-name "Mock Metric"} string?]]}
                   {:fields [[:field {:display-name "Mock Metric"} string?]]}]}
         (expand-referenced-metrics query)))))

#_(deftest ^:parallel adjust-filter-test
  ...
  (let [[source-metric mp] (mock-metric (lib/filter (basic-metric-query) (lib/> (meta/field-metadata :products :price) 1)))
    ...
    (is (=?
         {:stages [{:joins [{:stages [{:source-table (meta/id :products)
                                       :filters [[:> {} [:field {} (meta/id :products :price)] [:value {} 1]]
...
#_(deftest ^:parallel adjust-mixed-multi-source-test
...
;; TODO: Handle source metric expansion!!!
;; TODO: This is tricky!!!!!!! again adjust!!!
(deftest ^:parallel adjust-mixed-multi-source-test
  (let [[first-metric mp] (mock-metric lib.tu/metadata-provider-with-mock-cards
        ...
        query (-> (lib/query mp second-metric)
                  (lib/filter (lib/= (meta/field-metadata :products :category) "Widget")))]
    (def qq query)
    (def sm second-metric)
    (def fm first-metric)
    ;; Investigate :source-metadata key!!!
    ;; this looks very suspicious!
    (is (=? {:stages [;; source-card expansion
                      {:source-table (meta/id :products)}
                      ;; expansion of metric sourced question
                      {:joins [{:stages [;; source card expansion
                                         {:source-table (meta/id :products)
                                          :fields vector?}
                                         ;; expansion of metric aggregation
                                         {;; filters are copied (conveyed) to metric query
                                          :filters [[:< {}
                                          ...
                                          :joins [{:stages [;; source card expansion
                                                            ...
                                                            {}]}]}
                                         ;; ordering
                                         {:fields [[:field {} "Products__mock_metric"]]
                                          ;; TODO: following definitely should not be here!!!
                                          :filetrs some?}
                                         ;; dummy
                                         {}]}]
                       :filters [[:= {}
                       ...
                       :breakout [[:field {:display-name "Mock Metric"} string?]]}
                      {:fields [[:field {:display-name "Mock Metric"} string?]]}]}
            @(def eee (expand-referenced-metrics query))))))

#_(deftest ^:parallel question-based-on-metric-based-on-metric-based-on-metric-test
...
;; DONE tmp
(deftest ^:parallel question-based-on-metric-based-on-metric-based-on-metric-test-XIX
  (let [[first-metric mp] (mock-metric)
        [second-metric mp] (mock-metric mp (lib/query mp first-metric))
        [third-metric mp] (mock-metric mp (lib/query mp second-metric))
        query (lib/query mp third-metric)]
    (def que query)
    (is (=? {:stages [;; third metric expansion
                      {:source-table (meta/id :products)
                       :joins [{:stages [;; second metric expansion
                                         {:source-table (meta/id :products)
                                          :joins [{:stages [;; first metric expansion
                                                            {:source-table (meta/id :products)
                                                             :joins [{:stages [;; metric query stage of metric 1
                                                                               {:source-table (meta/id :products)
                                                                                :aggregation [[:avg {} some?]]}
                                                                               ;; complementary stage for resolved cards
                                                                               {}]}]
                                                             :breakout [[:field {} string?]]}
                                                            ;; first metric ordering stage
                                                            {:fields [[:field {} string?]]}
                                                            ;; complementary stage for resolved cards
                                                            {}]}]
                                          :breakout [[:field {} string?]]}
                                         ;; second metric ordering stage
                                         {:fields [[:field {} string?]]}
                                         ;; complementary stage for resolved cards
                                         {}]}]}
                      ;; third metric ordering stage
                      {:fields [[:field {} string?]]}]}
            @(def asdf (expand-referenced-metrics query))))))

#_(deftest ^:parallel joined-question-based-on-metric-based-on-metric-based-on-metric-test
  (let [[first-metric mp] (mock-metric)
        [second-metric mp] (mock-metric mp (lib/query mp first-metric))
...
(deftest ^:parallel joined-question-based-on-metric-based-on-metric-based-on-metric-test
  (let [[first-metric mp] (mock-metric)
    ...
    (is (=?
         #_
           {:stages [{:joins [{:stages [{:aggregation [[:avg {} [:field {} (meta/id :products :rating)]]]}
                                         ;; Empty stage added by resolved-source-cards to nest join
                                        #(= #{:lib/type :qp/stage-had-source-card :source-query/model?} (set (keys %)))]}]}]}
         ;; query
         {:stages [{:joins [;; joining the question
                            {:stages [; second metric expansion
                                      {:joins [{:stages [;; first metric expansion
                                                         {:joins [;; metric join
                                                                  {:stages [{:aggregation [[:avg map? some?]]}
                                                                            ;; expansion stage
                                                                            {}]}]}
                                                         ;; first metric ordering
                                                         {:fields vector?}
                                                         ;; first metric (root) card expansion stage
                                                         {}]}]}
                                      ;; second metric ordering
                                      {:fields vector?}
                                      ;; question source card expansion result
                                      {:lib/type :mbql.stage/mbql
                                       :qp/stage-had-source-card (:id question)
                                       :source-query/model? false}]}]}]}
         @(def xpxp (expand-referenced-metrics query))))))
...
#_(deftest ^:parallel metric-question-on-custom-column-model-test
  ...
  (let [mp meta/metadata-provider
    ...
    (is (=? {:stages
             [{:source-table (meta/id :orders)
               :expressions [[:+ {:lib/expression-name "foobar"} [:field {} (meta/id :orders :discount)] 1]]
               :fields [[:expression {} "foobar"]]}
              ;; metric expansion stage
              {:joins [;; metric query
                       {:stages [;; source model stage
                                 {:source-table (meta/id :orders)}
                                 ;; metric stage
                                 {:aggregation [[:avg {} some?]]}
                                 ;; explicit source query in join stage
                                 {}]}]
               :breakout vector?}
              ;; ordering stage
              {:fields vector?}]}
            (expand-referenced-metrics question)))))

#_(deftest ^:parallel metric-question-on-aggregate-column-model-test
  (let [mp meta/metadata-provider
        query (as-> (lib/query mp (meta/table-metadata :orders)) $q
...
(deftest ^:parallel metric-question-on-aggregate-column-model-test
  (let [mp meta/metadata-provider
    ...
    (is (=?
         {:stages
          [{:source-table (meta/id :orders)
            :aggregation [[:sum {} some?]]}
           {:joins [{:stages [{:source-table (meta/id :orders)}
                              ;; metric query
                              {:aggregation [[:avg {} some?]]}
                              ;; explicit source query stage
                              {}]}]
            :breakout [[:field {} some?]]}
           {:fields [[:field {} some?]]}]}
         (expand-referenced-metrics question)))))

#_(deftest ^:parallel metric-question-on-model-based-on-model-test
  (let [mp meta/metadata-provider
    ...
    (testing (str (dissoc question :lib/metadata))
      (is (=? 
           ...
           {:stages [;; here model
                     ...
                     ;; ordering stage
                     {:fields [[:field {} string?]]}]}
           @(def xixi (expand-referenced-metrics question)))))))

#_(deftest ^:parallel metric-question-on-multi-stage-model-test
  (let [mp meta/metadata-provider
    ...
    (is (=?
         ...
         {:stages
          [{:source-table (meta/id :orders)}
           {:filters [[:> {} [:field {} "sum"] 2]]}
           ;; metric expansion stage
           {:joins [{:stages [;; multi stage model source stages
                              {:source-table (meta/id :orders)}
                              {:filters [[:> {} some? some?]]}
                              ;; metric query stage
                              {:aggregation [[:avg {} some?]]}
...
#_(deftest ^:parallel metric-question-on-native-model-test
  (let [mp meta/metadata-provider
        sum-pred (comp #{"sum"} :name)
  ...
  (let [mp meta/metadata-provider
    ...
    (is (=?
         {:stages [{:lib/type :mbql.stage/native,
                    :native "SELECT whatever"}
                   {:joins [{:stages [;;native model metric source stage
                                      {:lib/type :mbql.stage/native,
                                       :native "SELECT whatever"}
                                      ;; metric query stage
                                      {:aggregation [[:avg {} some?]]}
...
;; TODO: Must this be handled in new implementation?
...
#_(deftest ^:parallel default-metric-names-test
  (let [[source-metric mp] (mock-metric)]
    (is (=?
...
;; TODO: expression to check that some parts of a query are same? with special symbols?
(deftest ^:parallel default-metric-names-test
  (let [[source-metric mp] (mock-metric)]
    (is (=?
         {:stages [{:joins [{:stages [{:aggregation [[:avg #(and (map? %)
                                                                 (not (contains? % :display-name))
                                                                 (= "avg" (:name %)))
                                                      [:field {} some?]]]}
                                      {}]}]
                    :breakout [[:field {} string?]]}
                   {:fields [[:field {:display-name "Average of Rating"} string?]]}]}
         @(def ex (expand-referenced-metrics (-> (lib/query mp (meta/table-metadata :products))
                                                 (lib/aggregate (lib.metadata/metric mp (:id source-metric))))))))))
...
(deftest =?-fns-as-map-values-test
  (is (=?
       {:hey complement}
       {:hey "you"})))
...
;; TODO: Make new approach conform old way of picking those metric names
#_(deftest ^:parallel include-source-names-test
  (let [[source-metric mp] (mock-metric (-> (basic-metric-query)
...
;; TODO: Examine during another pass, when I'm certain about name and display name!
(deftest ^:parallel include-source-names-test
  (let [[source-metric mp] (mock-metric (-> (basic-metric-query)
                                            (add-aggregation-options {:display-name "My cool metric" :name "Named Metric"})))]
    (is (=?
         {:stages [{:joins [{:stages [{:aggregation [[:avg {:display-name "My cool metric"
                                                            :name "Named Metric"}
                                                      [:field {} some?]]]}
                                      {}]}]
                    :breakout [[:field {} some?]]}
                   {:fields [[:field {:display-name "My cool metric"} some?]]}]}
         @(def x (expand-referenced-metrics (-> (lib/query mp (meta/table-metadata :products))
                                                (lib/aggregate (lib.metadata/metric mp (:id source-metric))))))))))

;; TODO: Will have to consider display name first!
...
#_(deftest ^:parallel override-source-names-test
  (let [[source-metric mp] (mock-metric (-> (basic-metric-query)
                                            (add-aggregation-options {:display-name "My cool metric" :name "Named Metric"})))]
    (is (=?
         {:stages [{:aggregation [[:avg {:display-name "My cooler metric" :name "Better Named Metric"} some?]]}]}
         (expand-referenced-metrics (-> (lib/query mp (meta/table-metadata :products))
                                        (lib/aggregate (lib.metadata/metric mp (:id source-metric)))
                                        (add-aggregation-options {:display-name "My cooler metric" :name "Better Named Metric"})))))))
...
;; TODO: Fix bad references log?
#_(deftest ^:parallel metric-with-nested-segments-test
...
(deftest ^:parallel metric-with-nested-segments-test
  (let [mp (lib.tu/mock-metadata-provider
    ...
    (is (=?
         {:stages vector?}
         (expand-referenced-metrics (lib/query mp source-metric))))
    ;; Segments will be expanded in this case as the metric query that is spliced in needs to be processed
    (is (=?
         {:stages vector?}
         (expand-referenced-metrics
...
;; TODO:
#_(deftest ^:parallel expand-macros-in-nested-queries-test
  ...
  (testing "expand-macros should expand things in the correct nested level (#12507)"
    (let [[source-metric mp] (mock-metric (-> (lib/query meta/metadata-provider (meta/table-metadata :venues))
          ...
          after {:source-table (meta/id :venues)
                 ...
                 :filter       [:= [:field (meta/id :venues :name) {}] [:value "abc" {}]]}
          expand-macros (fn [mbql-query]
                          (lib.convert/->legacy-MBQL (expand-referenced-metrics (lib/query mp (lib.convert/->pMBQL mbql-query)))))]
      (comment
        (testing "nested 1 level"
          (is (=? (lib.tu.macros/mbql-query nil
                    {:source-query after})
...
(comment
  ...
  (mt/test-driver
   :postgres
   (let [mp (lib.metadata.jvm/application-database-metadata-provider (mt/id))]
     @(def qi (lib/query mp (lib.metadata/card mp 765)))))

  )

;; TODO: This throws for duplicate uuid!!!
...
;; TODO: This won't work!!! until I add proper offset handling!
#_(deftest ^:parallel metric-in-offset-test
  (let [mp (lib.metadata.jvm/application-database-metadata-provider (mt/id))
        metric-query (-> (lib/query mp (lib.metadata/table mp (mt/id :orders)))
                         (lib/aggregate (lib/count)))]
    (mt/with-temp [:model/Card metric {:dataset_query (lib.convert/->legacy-MBQL metric-query)
