...
;; being able to call [[ee-perms.test-util/update-db-download-permissions!]].
      ...
      (testing "If a group has granular download perms for a DB, native download perms are removed when a new table is
        ...
        (let [table-ids (db/select-ids 'Table :db_id db-id)
          ...
          (premium-features-test/with-premium-features #{:advanced-permissions}
            (ee-perms.test-util/update-db-download-permissions! (u/the-id (perms-group/all-users)) db-id graph))
