...
(mu/defn ^:private move-join-condition-to-source-query :- ::lib.schema.join/join
...
(defn- move-top-level-params-to-inner-query
  ...
  {:pre [(= query-type :pipeline)]}
  (cond-> (set/rename-keys outer-query {:parameters :user-parameters})
    (seq parameters)
    (lib.util/update-query-stage 0 assoc :parameters parameters)))
...
(defn- hoist-database-for-snippet-tags
  ...
  (lib.util/update-stages
   ...
   (fn [_query _stage-number stage]
     (u/update-if-exists stage :template-tags (partial assoc-db-in-snippet-tag (:database query))))))
