(ns ^:mb/old-migrations-tests metabase.db.custom-migrations.old-migrations-tests
  "Tests for custom migrations from older versions of Metabase. These don't need to run on every single new PR, since
  they're super slow -- it's enough just to run them on commits to `master` and `release-` branches.

  I'm considering any migration older that the current backport target. For example at the time of this writing
  Metabase 52.6 is the currently shipping version, meaning `master` targets what will be 53.x and `release-x.52.x` is
  the backport target. Any migration tests for migrations in 51 and older can be considered 'old'.

  When we cut a new major release/new major release branches, kindly move stuff
  from [[metabase.db.custom-migrations-test]] into this namespace."
  (:require
