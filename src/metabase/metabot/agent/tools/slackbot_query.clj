(ns metabase.metabot.agent.tools.slackbot-query
  ...
  (:require
   ...
   [metabase.metabot.agent.tools.construct :as construct]
...
(defn slackbot-construct-notebook-query-tool "slackbot-construct-notebook-query-tool" []
  {:tool-name "construct_notebook_query"
   :doc       "Construct a notebook query from a metric, model, or table. The query results will be rendered as a visualization in Slack."
   :schema    [:=> [:cat slackbot-query-schema] :any]
   :fn        (fn [{:keys [_reasoning query title display]}]
                (try
