...
(mu/defn- move-join-condition-to-source-query :- :legacy-mbql/join
  ...
  (if-not filter-clause
    ...
    (if source-table
      ...
      (-> (update-in join [:source-query :filter] legacy-mbql/combine-filter-clauses filter-clause)
          (dissoc :filter)))))

(mu/defn- move-native-filter-to-parent-stage :- :legacy-mbql/mbql-query
  ...
  (if-not (and native-query filter-clause)
    ...
    (-> parent-stage
        ...
        (update :filter legacy-mbql/combine-filter-clauses filter-clause))))
...
(defn- expand-one
  ...
  (let [m        (cond-> m
                   (seq parameters) (update :parameters (partial legacy-mbql/normalize-fragment [:parameters])))
