...
Validate: `bun run lint-oxlint-pure -- <files>` and `bun run type-check-pure`.
...
After edits: `npx stylelint <css>`, `LINT_CSS_MODULES=true bun run lint-oxlint-pure -- <files>`, `bun run type-check-pure`.
