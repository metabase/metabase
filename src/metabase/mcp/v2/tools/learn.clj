...
(registry/deftool learn
  "Read this server's task docs (skills) for the write dialects the schemas can't fully describe. learn(topic) returns that skill whole; learn(topic, reference) one of its reference files. The complete topic list: query-dialect (the query language for execute_query and question_write's query; reference \"operators\" = operator catalog), native-parameters (template tags and field filters for native SQL), dashboard-filters (dashboard parameters and the wire_parameter target grammar), dashboard-layout (24-column grid, sizes, tabs), documents (document_write's Markdown grammar), visualization-settings (display choice and settings; reference \"settings\" = per-chart key catalog). There are no other topics, so never call learn() bare to discover them. Read the matching topic before your first complex write of that kind; skip when already in context."
  {:name        "learn"
