...
You can use locked parameters to display filtered data based on attributes captured by your web server, such as a username or a tenant ID. For more examples, see the [reference apps repo](https://github.com/metabase/static-embedding-reference-apps).
...
| `frame_sizing`             | `content-height`. See [Resizing embeds to fit their content](./static-embedding.md#resizing-embeds-to-fit-their-content)                           |
