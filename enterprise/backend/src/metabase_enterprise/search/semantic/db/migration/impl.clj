(ns metabase-enterprise.search.semantic.db.migration.impl
  (:require
   [clojure.string :as str]
   [honey.sql :as sql]
   [metabase-enterprise.search.semantic.db :as semantic-search.db]
...
(def schema-version
  "Version to compare the [[metabase-enterprise.search.semantic.db.migration/db-version]] with. If this is higher,
...
(def ^:private dlq-table-pattern
  ...
  (see [[metabase-enterprise.search.semantic.dlq/dlq-table-name-kw]])."
  #"\Adlq_\d+\z")

(def ^:private repair-table-pattern
  "Repair's scratch tables, `repair_<millis>_<6 chars>`
  (see [[metabase-enterprise.search.semantic.repair/repair-table-name]])."
