    ...
    (mt/test-drivers (mt/normal-drivers)
      (mt/with-temporary-setting-values [report-timezone "US/Pacific"]
        (let [metadata-provider (lib.tu/merged-mock-metadata-provider
          ...
          (qp.store/with-metadata-provider metadata-provider
            (let [preprocessed (qp/preprocess query)]
              ;; only bother testing this on databases that have an actual `:type/DateTimeWithTZ` type e.g. `timestamp
              ;; with time zone`, we're not testing anything interesting in things like SQLite that just create regular
