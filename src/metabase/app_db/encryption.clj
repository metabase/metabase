...
(defn- update-row!
  "Set `changes` on the rows of `table` matching the Honey SQL `where` clause, as a plain SQL statement rather than a
  Toucan DML statement. The cloud-migration guard on Toucan DML reads `read-only-mode` through the Setting model
  before every statement, and while the database is being re-encrypted a setting row can be plaintext under a key, or
  ciphertext under a key not yet in effect -- exactly what that model's strict read rejects."
  [table where changes]
  (t2/query {:update table, :set changes, :where where}))
...
(defn- replace-encryption-check!
  ...
  (let [sentinel (if encrypt-fn (str (random-uuid)) "unencrypted")]
    ;; plain SQL for the same reason as [[update-row!]]
  ...
  (run! (fn [{:keys [id value]}]
          (when (some? value)
            (let [decrypted (try
              ...
              (update-row! table [:= :id id] {column (encrypt-str-fn decrypted)}))))
          ...
          (when (some? value)
            (let [decrypted (try
                              ...
                              (catch Throwable e
                                (throw (ex-info (trs "Can''t decrypt app db with MB_ENCRYPTION_SECRET_KEY")
                                                {:table table, :id id, :column column} e))))]
              (update-row! table [:= :id id] {column (encrypt-bytes-fn decrypted)}))))
        (t2/reducible-select [table :id [column :value]])))
      ...
      (doseq [[table column] encrypted-string-columns]
        (run! (fn [{:keys [id value]}]
                (when (and (string? value)
                  ...
                  (update-row! table [:= :id id] {column (encryption/encrypt value)})))
      ...
      (let [encrypt-setting-fn (encrypt-setting nil)]
        (run! (fn [{:keys [key value_with_aad]}]
                (when (and (string? value_with_aad)
                  ...
                  (update-row! :setting [:= :key key] {:value_with_aad (encrypt-setting-fn value_with_aad key)})))
      ...
      (doseq [{:keys [key value value_with_aad]} (t2/select :setting)]
        (case key
          ...
          (let [aad-opts {:aad (mdb.setting/setting-aad key)}
            ...
            (when (seq changes)
              (update-row! :setting [:= :key key] changes)))))
