...
5. If the message is not sent or an error message is displayed in Metabase, try to use the same account credentials in another email program and see if they work. See the logs for a more detailed error by navigating to **Tools** and clicking **Logs** in the left sidebar.
