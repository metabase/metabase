...
if [ -z "$MB_CONTAINER_DEFAULT_MEMORY" ]; then
    export MB_CONTAINER_DEFAULT_MEMORY=false
fi
if [ "$MB_CONTAINER_DEFAULT_MEMORY" ]; then
    JAVA_OPTS="${JAVA_OPTS} -XX:+UnlockExperimentalVMOptions"
    JAVA_OPTS="${JAVA_OPTS} -XX:+UseCGroupMemoryLimitForHeap"
    JAVA_OPTS="${JAVA_OPTS} -XX:MaxRAMFraction=1"
fi
