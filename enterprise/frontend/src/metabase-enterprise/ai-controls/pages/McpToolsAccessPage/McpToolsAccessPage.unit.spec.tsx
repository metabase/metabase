...
describe("McpToolsAccessPage", () => {
  ...
  it("shows the saved state from the PUT body without refetching", async () => {
    setup();
    await findGrid();
    fetchMock.modifyRoute("update-mcp-tool-permissions", {
      response: createMockMcpToolPermissionsResponse({
        tools: mcpTools,
        permissions: createDefaultMcpPermissions().map((permission) =>
          permission.group_id === allUsersGroup.id
            ? { ...permission, tool_access: { run_sql: "no", search: "no" } }
            : permission,
        ),
      }),
    });
    ...
    expect(
      fetchMock.callHistory.calls(MCP_PERMISSIONS_URL, { method: "GET" }),
    ).toHaveLength(1);
  });
