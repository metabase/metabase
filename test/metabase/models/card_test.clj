      ...
      (letfn [(add-card-to-dash! [dash]
                (t2/insert! DashboardCard
                  {:card_id      card-id
                   ...
                   :size_y       4}))
              (get-dashboard-count []
                (card/dashboard-count (t2/select-one :m/card :id card-id)))]
        (is (= 0
               (get-dashboard-count)))
...
(deftest dropdown-widget-values-usage-count-test
  (let [hydrated-count (fn [card] (-> card
    ...
    (testing "With no associated cards"
      (tt/with-temp :m/card [card]
        (is (zero? (hydrated-count card)))))
    (testing "With one"
      (tt/with-temp* [:m/card      [{card-id :id :as card}]
...
(deftest remove-from-dashboards-when-archiving-test
  (testing "Test that when somebody archives a Card, it is removed from any Dashboards it belongs to"
    (tt/with-temp* [Dashboard     [dashboard]
      ...
      (t2/update! :m/card (u/the-id card) {:archived true})
      (is (= 0
...
(deftest public-sharing-test
  (testing "test that a Card's :public_uuid comes back if public sharing is enabled..."
    (tu/with-temporary-setting-values [enable-public-sharing true]
      (tt/with-temp :m/card [card {:public_uuid (str (java.util.UUID/randomUUID))}]
...
(deftest database-id-test
  (tt/with-temp :m/card [{:keys [id]} {:name          "some name"
                                       :dataset_query (dummy-dataset-query (mt/id))
                                       :database_id   (mt/id)}]
    (testing "before update"
      (is (= {:name "some name", :database_id (mt/id)}
             (into {} (t2/select-one [:m/card :name :database_id] :id id)))))
    (t2/update! :m/card id {:name          "another name"
                            :dataset_query (dummy-dataset-query (mt/id))})
  ...
  (mt/with-actions-enabled
          ...
          (doseq [query-change [{:limit       1}
            ...
            (testing (format "when adding %s to the query" (first (keys query-change)))
              (mt/with-actions [{model-id :id}           {:dataset true :dataset_query query}
                ...
                (is (= 1 (t2/update! :m/card :id model-id {:dataset_query (update query :query merge query-change)})))
    ...
    (testing "unhappy paths\n"
      (testing "should not attempt to delete if it's not a model"
        (t2.with-temp/with-temp [:m/card {id :id} {:dataset       false
                                                   :dataset_query (mt/mbql-query users)}]
          (with-redefs [card/disable-implicit-action-for-model! (fn [& _args]
                                                                  (throw (ex-info "Should not be called" {})))]
            (is (= 1 (t2/update! :m/card :id id {:dataset_query (mt/mbql-query users {:limit 1})}))))))
      ...
      (testing "only disable implicit actions, not http and query"
        (mt/with-actions [{model-id :id}           {:dataset true :dataset_query (mt/mbql-query users)}
          ...
          (t2/update! :m/card :id model-id {:dataset_query (mt/mbql-query users {:limit 1})})
      ...
      (testing "should not disable if change source table"
        (mt/with-actions [{model-id :id}           {:dataset true :dataset_query (mt/mbql-query users)}
          ...
          (t2/update! :m/card :id model-id {:dataset_query (mt/mbql-query categories)})
...
(deftest circular-reference-test
  (testing "Should throw an Exception if saving a Card that references itself"
    (tt/with-temp :m/card [card (card-with-source-table (mt/id :venues))]
  ...
  (testing "Do the same stuff with circular reference between two Cards... (A -> B -> A)"
    (tt/with-temp* [:m/card [card-a (card-with-source-table (mt/id :venues))]
                    :m/card [card-b (card-with-source-table (str "card__" (u/the-id card-a)))]]
      (is (thrown?
           Exception
           (t2/update! :m/card (u/the-id card-a)
             (card-with-source-table (str "card__" (u/the-id card-b))))))))

  (testing "ok now try it with A -> C -> B -> A"
    (tt/with-temp* [:m/card [card-a (card-with-source-table (mt/id :venues))]
                    :m/card [card-b (card-with-source-table (str "card__" (u/the-id card-a)))]
                    :m/card [card-c (card-with-source-table (str "card__" (u/the-id card-b)))]]
      (is (thrown?
           Exception
           (t2/update! :m/card (u/the-id card-a)
    ...
    (let [metadata (qp/query->expected-cols (mt/mbql-query venues))]
      (mt/with-temp :m/card [{card-id :id} {:dataset_query   (mt/mbql-query venues)
        ...
        (is (= (mt/derecordize metadata)
               (mt/derecordize (t2/select-one-fn :result_metadata :m/card :id card-id))))))))

(deftest populate-result-metadata-if-needed-test
  (doseq [[creating-or-updating f]
          {"creating" (fn [properties f]
                        (mt/with-temp :m/card [{card-id :id} properties]
                          (f (t2/select-one-fn :result_metadata :m/card :id card-id))))
           "updating" (fn [changes f]
                        (mt/with-temp :m/card [{card-id :id} {:dataset_query   (mt/mbql-query checkins)
                                                              :result_metadata (qp/query->expected-cols (mt/mbql-query checkins))}]
                          (t2/update! :m/card card-id changes)
                          (f (t2/select-one-fn :result_metadata :m/card :id card-id))))}]
...
(deftest normalize-visualization-settings-test
  (test-visualization-settings-normalization
   (fn [original expected]
     (mt/with-temp :m/card [card {:visualization_settings original}]
       (is (= expected
              (t2/select-one-fn :visualization_settings :m/card :id (u/the-id card))))))))
      ...
      (testing "Should not be able to update a Card to have a filter with the wrong Database ID"
        (mt/with-temp :m/card [{card-id :id} good-card-data]
          (is (thrown-with-msg?
               clojure.lang.ExceptionInfo
               #"Invalid Field Filter: Field \d+ \"VENUES\"\.\"NAME\" belongs to Database \d+ \"test-data\", but the query is against Database \d+ \"sample-dataset\""
               (t2/update! :m/card card-id bad-card-data))))))))
  ...
  (testing "Should validate Card :parameters when"
    (testing "creating"
      (is (thrown-with-msg?
           ...
           (mt/with-temp :m/card [_ {:parameters {:a :b}}])))

     (mt/with-temp :m/card [card {:parameters [{:id   "valid-id"
                                                :type "id"}]}]
       (is (some? card))))

    (testing "updating"
      (mt/with-temp :m/card [{:keys [id]} {:parameters []}]
        (is (thrown-with-msg?
             clojure.lang.ExceptionInfo
             #":parameters must be a sequence of maps with :id and :type keys"
             (t2/update! :m/card id {:parameters [{:id 100}]})))
        (is (pos? (t2/update! :m/card id {:parameters [{:id   "new-valid-id"
                                                        :type "id"}]})))))))
  ...
  (testing ":parameters should get normalized when coming out of the DB"
    (doseq [[target expected] {[:dimension [:field-id 1000]] [:dimension [:field 1000 nil]]
      ...
      (testing (format "target = %s" (pr-str target))
        (mt/with-temp :m/card [{card-id :id} {:parameter_mappings [{:parameter_id     "_CATEGORY_NAME_"
                                                                    :target target}]}]

          (is (= [{:parameter_id     "_CATEGORY_NAME_"
                   :target expected}]
                 (t2/select-one-fn :parameter_mappings :m/card :id card-id))))))))
  ...
  (testing "Should validate Card :parameter_mappings when"
    (testing "creating"
      (is (thrown-with-msg?
           ...
           (mt/with-temp :m/card [_ {:parameter_mappings {:a :b}}])))

     (mt/with-temp :m/card [card {:parameter_mappings [{:parameter_id "valid-id"
                                                        :target       [:field 1000 nil]}]}]
       (is (some? card))))

    (testing "updating"
      (mt/with-temp :m/card [{:keys [id]} {:parameter_mappings []}]
        (is (thrown-with-msg?
             clojure.lang.ExceptionInfo
             #":parameter_mappings must be a sequence of maps with :parameter_id and :type keys"
             (t2/update! :m/card id {:parameter_mappings [{:parameter_id 100}]})))

        (is (pos? (t2/update! :m/card id {:parameter_mappings [{:parameter_id "new-valid-id"
                                                                :target       [:field 1000 nil]}]})))))))

(deftest normalize-parameter-mappings-test
  (testing ":parameter_mappings should get normalized when coming out of the DB"
    (mt/with-temp :m/card [{card-id :id} {:parameter_mappings [{:parameter_id "22486e00"
                                                                :card_id      1
  ...
  (testing "Card hashes are composed of the name and the collection's hash"
    (let [now #t "2022-09-01T12:34:56"]
      (mt/with-temp* [Collection [coll  {:name "field-db" :location "/" :created_at now}]
                      :m/card       [card  {:name "the card" :collection_id (:id coll) :created_at now}]]
        (is (= "5199edf0"
...
(deftest parameter-card-test
  (let [default-params {:name       "Category Name"
    ...
    (testing "parameter with source is card create ParameterCard"
      (tt/with-temp* [:m/card  [{source-card-id-1 :id}]
                      :m/card  [{source-card-id-2 :id}]
                      :m/card  [{card-id :id}
        ...
        (testing "update values_source_config.card_id will update ParameterCard"
          (t2/update! :m/card card-id {:parameters [(merge default-params
                                                        {:values_source_type    "card"
    ...
    (testing "Delete a card will delete any ParameterCard that linked to it"
      (tt/with-temp* [:m/card  [{source-card-id :id}]
        ...
        (t2/delete! :m/card :id source-card-id)
        (is (= []
               (t2/select 'ParameterCard :card_id source-card-id)))))))
...
(deftest cleanup-parameter-on-card-changes-test
  (mt/dataset sample-dataset
    (mt/with-temp*
      [:m/card      [{source-card-id :id} (merge (mt/card-with-source-metadata-for-query
                                                  (mt/mbql-query products {:fields [(mt/$ids $products.title)
                                                                                    (mt/$ids $products.category)]
                                                                           :limit 5}))
                                              {:database_id (mt/id)
                                               :table_id    (mt/id :products)})]
       :m/card      [card                 {:parameters [{:name                  "Param 1"
                                                         :id                    "param_1"
                                                         :type                  "category"
                                                         :values_source_type    "card"
                                                         :values_source_config {:card_id source-card-id
      ...
      (testing "on update result_metadata"
        (t2/update! :m/card source-card-id
        ...
        (testing "update the dashboard parameter and remove values_config of dashboard"
          ...
          (testing "but no changes with parameter on card"
            (is (=? [{:name                 "Param 1"
                      ...
                      :values_source_config {:card_id     source-card-id
                                             :value_field (mt/$ids $products.title)}}]
                    (t2/select-one-fn :parameters :m/card :id (:id card)))))))

      (testing "on archive card"
        (t2/update! :m/card source-card-id {:archived true})
...
(deftest upgrade-to-v2-db-test
  (testing ":visualization_settings v. 1 should be upgraded to v. 2 on select"
    (mt/with-temp :m/card [{card-id :id} {:visualization_settings {:pie.show_legend true}}]
        (is (= {:version 2
                :pie.show_legend true
                :pie.percent_visibility "inside"}
               (t2/select-one-fn :visualization_settings :m/card :id card-id)))))
  (testing ":visualization_settings v. 1 should be upgraded to v. 2 and persisted on update"
    (mt/with-temp :m/card [{card-id :id} {:visualization_settings {:pie.show_legend true}}]
      (t2/update! :m/card card-id {:name "Favorite Toucan Foods"})
      (is (= {:version 2
              :pie.show_legend true
              :pie.percent_visibility "inside"}
