...
(defmethod sql-jdbc.actions/maybe-parse-sql-error [:h2 actions.error/violate-foreign-key-constraint]
  ...
  (when-let [[_match _table _constraint _ref-table _columns _value]
             (re-find #"Referential integrity constraint violation" error-message)]
    {:type    error-type
...
(comment
 (sql-jdbc.actions/maybe-parse-sql-error
  :h2 actions.error/violate-not-null-constraint nil
  "NULL not allowed for column \"RANKING\"; SQL statement:\nINSERT INTO \"PUBLIC\".\"GROUP\" (\"NAME\") VALUES (CAST(? AS VARCHAR)) [23502-214])")
 (sql-jdbc.actions/maybe-parse-sql-error
  :h2 actions.error/violate-unique-constraint (toucan2.core/select-one :model/Database :name "action-error-handling" :engine :h2)
  "Unique index or primary key violation: \"PUBLIC.CONSTRAINT_INDEX_4 ON PUBLIC.\"\"GROUP\"\"(RANKING NULLS FIRST) VALUES ( /* 2 */ 2 )\"; SQL statement:\nUPDATE \"PUBLIC\".\"GROUP\" SET \"RANKING\" = CAST(? AS INTEGER) WHERE \"PUBLIC\".\"GROUP\".\"ID\" = 1 [23505-214]")
 (sql-jdbc.actions/maybe-parse-sql-error
  :h2 actions.error/incorrect-value-type nil
 ...
 (sql-jdbc.actions/maybe-parse-sql-error
  :h2 actions.error/violate-foreign-key-constraint (toucan2.core/select-one :model/Database :name "action-error-handling" :engine :h2)
