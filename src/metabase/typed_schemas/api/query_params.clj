...
(defn query-param
  "Returns a query parameter by keyword or string key."
  [query-params k]
  (or (get query-params k)
      (get query-params (name k))))

(defn truthy-query-param?
  "Returns true when a query parameter value represents truth."
  [v]
...
(defn query-database-value
  "Returns the requested database id or name query value."
  [query-params]
...
(defn database-ids-for-value
  "Returns readable database ids matching a database id or name query value."
  [database-value]
  (when database-value
    (let [database-id (common/parse-id database-value)]
      (->> (if database-id
...
(defn database-id-filter-clause
  "Returns a HoneySQL filter clause for database ids."
  [database-ids column]
  (when database-ids
    (if (seq database-ids)
...
(defn id-filter-clause
  "Returns a HoneySQL filter clause for ids."
  [ids column]
  (when ids
    (if (seq ids)
      [:in column ids]
...
(defn query-library-value
  "Returns the requested library query value."
  [query-params]
...
(defn query-library-collection-values
  "Returns requested library collection refs from query params."
  [query-params]
...
(defn query-include-data-library?
  "Returns true when the Data library root should be included."
  [query-params]
...
(defn query-include-metric-library?
  "Returns true when the Metrics library root should be included."
  [query-params]
...
(defn query-include-models?
  "Returns true when model schemas should be included."
  [query-params]
...
(defn query-question-collection-values
  "Returns requested question collection refs from query params."
  [query-params]
...
(defn- library-collection-for-value
  [library-value]
  (when library-value
    (let [collection-id (common/parse-id library-value)]
      (->> (if collection-id
...
(defn- library-collection-for-ref
  [collection-value]
  (let [collection-id (common/parse-id collection-value)]
    (->> (if collection-id
           (t2/select :model/Collection :id collection-id)
...
(defn collection-scope
  "Returns readable collection ids for refs and their descendants."
  [collection-values]
...
(defn library-collection-scope
  "Returns the collection scope for a single library ref."
  [library-value]
  (when library-value
...
(defn library-collections-scope
  "Returns the collection scope for multiple library refs."
  [collection-values]
  (when (seq collection-values)
    (let [collections (for [collection-value collection-values]
...
(defn library-scope
  "Returns the requested typed-schema library scope."
  [query-params]
