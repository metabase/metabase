    ...
    (mt/dataset test-data
      (qp.store/with-metadata-provider (mt/id)
        (let [orders        (lib.metadata/table (qp.store/metadata-provider) (mt/id :orders))
              products-cat  (lib.metadata/field (qp.store/metadata-provider) (mt/id :products :category))
              base-query    (-> (lib/query (qp.store/metadata-provider) orders)
                                (lib/aggregate (lib/count))
                                (lib/breakout products-cat))
              query         (-> base-query
                                lib/append-stage
                                (as-> q
