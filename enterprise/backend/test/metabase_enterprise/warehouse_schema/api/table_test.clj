(ns ^:mb/driver-tests metabase-enterprise.warehouse-schema.api.table-test
  "Tests for /api/ee/table endpoints."
  (:require
...
(deftest publish-model-test
  (testing "POST /api/ee/table/publish-model"
    (testing "creates models for selected tables"
      (mt/with-model-cleanup [:model/Card]
        (mt/with-temp [:model/Collection {collection-id :id} {}]
          (let [response (mt/user-http-request :crowberto :post 200 "ee/table/publish-model"
...
(deftest published-as-model-test
  (mt/with-model-cleanup [:model/Card]
    (mt/with-temp [:model/Collection {collection-id :id} {}]
      (let [{:keys [models]} (mt/user-http-request :crowberto :post 200 "ee/table/publish-model"
...
(deftest published-models-perm-test
  ...
  (mt/with-model-cleanup [:model/Card]
    (mt/with-temp [:model/Collection {collection1 :id} {}
      ...
      (let [publish-venues        #(mt/user-http-request :crowberto :post 200 "ee/table/publish-model"
                                                         {:table_ids            [(mt/id :venues)]
...
(deftest ^:parallel bulk-edit-visibility-sync-test
  (testing "POST /api/ee/table/edit visibility field synchronization"
    (mt/with-temp [:model/Database {db-id :id} {}
      ...
      (testing "updating data_layer syncs to visibility_type for all tables"
        ...
        (mt/user-http-request :crowberto :post 200 "ee/table/edit"
      ...
      (testing "cannot update both visibility_type and data_layer at once"
        (mt/user-http-request :crowberto :post 400 "ee/table/edit"
                              {:table_ids        [table-1-id]
...
(deftest ^:parallel non-admins-cant-trigger-bulk-sync-test
  (testing "Non-admins should not be allowed to trigger sync"
    (is (= "You don't have permissions to do that."
           (mt/user-http-request :rasta :post 403 "ee/table/sync-schema" {:database_ids [(mt/id)]})))))
    ...
    (let [tables       (atom [])
      ...
      (mt/with-temp [:model/Database {d1 :id} {:engine "h2", :details (:details (mt/db))}
        ...
        (with-redefs [sync/sync-table! (fn [table]
                                         ...
                                         (.countDown latch)
                                         nil)]
          (mt/user-http-request :crowberto :post 200 "ee/table/sync-schema" {:database_ids [d1],
...
(deftest ^:parallel non-admins-cant-trigger-bulk-rescan-values-test
  (testing "Non-admins should not be allowed to trigger rescan values"
    (is (= "You don't have permissions to do that."
           (mt/user-http-request :rasta :post 403 "ee/table/rescan-values" {:database_ids [(mt/id)]})))))
    ...
    (let [tables       (atom [])
      ...
      (mt/with-temp [:model/Database {d1 :id} {:engine "h2", :details (:details (mt/db))}
        ...
        (with-redefs [sync/update-field-values-for-table! (fn [table]
                                                            ...
                                                            (.countDown latch)
                                                            nil)]
          (mt/user-http-request :crowberto :post 200 "ee/table/rescan-values" {:database_ids [d1],
                                                                               :schema_ids   [(format "%d:FOO" d2)]
                                                                               :table_ids    [t4]}))
        (testing "rescanned?"
          (is (true? (.await latch 4 TimeUnit/SECONDS)))
...
(deftest ^:parallel non-admins-cant-trigger-bulk-discard-values-test
  (testing "Non-admins should not be allowed to trigger discard values"
    (is (= "You don't have permissions to do that."
           (mt/user-http-request :rasta :post 403 "ee/table/discard-values" {:database_ids [(mt/id)]})))))

(deftest ^:parallel bulk-discard-values-test
  (testing "POST /api/ee/table/discard-values"
    (mt/with-temp
      ...
      (let [url "ee/table/discard-values"
