...
- **[SSO embeds](./introduction.md#sso-embeds)**: interactive out of the box. With a web component, make one view-only by turning off drill-through with `drills="false"` and saving with `is-save-enabled="false"`. The SDK has no `drills` prop, so use the view-only `StaticQuestion` component instead of `InteractiveQuestion`. You can also manage what people can do through [data permissions](../permissions/data.md) and [collection permissions](../permissions/collections.md).
