  ...
  CASE WHEN EXISTS (
    ...
    WHERE
      ...
      AND (p.object = concat('/db/', md.id, '/')
        OR p.object = concat('/db/', md.id, '/schema/'))) THEN
    'unrestricted'
  WHEN EXISTS (
    SELECT
      1
    ...
    WHERE
      p.group_id = pg.id
      AND p.object = concat('/block/db/', md.id, '/')) THEN
    'block'
  WHEN NOT EXISTS (
    SELECT
      1
    ...
    WHERE
      p.group_id = pg.id
      AND p.object LIKE concat('/db/', md.id, '/%')) THEN
    'no-self-service'
  END AS perm_value
FROM
...
WHERE
  ...
  WHEN NOT EXISTS (
    ...
    WHERE
      p.group_id = pg.id
      AND p.object LIKE concat('/db/', md.id, '/schema/%')) THEN
    TRUE
  ELSE
    FALSE
  END;
  ...
  CASE WHEN EXISTS (
    ...
    WHERE
      ...
      AND (p.object = concat('/db/', mt.db_id, '/schema/', mt.escaped_schema, '/')
        ...
        OR p.object = concat('/db/', mt.db_id, '/schema/', mt.escaped_schema, '/table/', mt.id, '/query/segmented/'))) THEN
    'unrestricted'
  ELSE
    'no-self-service'
  END AS perm_value
