WITH root AS (
  select 'root' AS model,
         ...
         'ttl'  AS strategy,
         json_object(
           'multiplier' VALUE coalesce((select (("VALUE"::varchar(15))::float)::int from setting where "KEY" = 'query-caching-ttl-ratio'), 10),
           'min_duration_ms' VALUE coalesce((select (("VALUE"::varchar(15))::float)::int from setting where "KEY" = 'query-caching-min-ttl'), 60000)
         ) AS config
), database AS (
