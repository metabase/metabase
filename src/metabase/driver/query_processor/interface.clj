...
(defrecord FieldPlaceholder [^Integer field-id]
  IResolve
  (resolve-field [this field-id->field]
    (or (field-id->field field-id) ; try to resolve the Field with the ones available in field-id->field
        this)))                    ; If that fails just return ourselves as-is
...
(defrecord DateTimeFieldPlaceholder [^Integer field-id
  ...
  (resolve-field [this field-id->field]
    (or (when-let [resolved-field (field-id->field field-id)]
          (assert (instance? (resolve 'DateTimeField) resolved-field))
          (assert (= (:unit resolved-field) unit)
            (format "Conflicting units for Field %d: %s and %d" field-id unit (:unit resolved-field)))
          resolved-field)
        this)))


(defn- resolve-field-or-throw [field-placeholder field-id->field & [expected-class-symbol]]
  (or (let [resolved-field (resolve-field field-placeholder field-id->field)]
        (when (not= field-placeholder resolved-field)
          (when expected-class-symbol
            (assert (instance? (resolve expected-class-symbol) resolved-field)))
          resolved-field))
  ...
  (resolve-field [_ field-id->field]
    (let [resolved-field (resolve-field-or-throw field field-id->field)]
      (map->Value {:field resolved-field
                   :value (if-not (instance? (resolve 'DateTimeField) resolved-field) value
                                  (let [parser (datetime-unit->parser (:unit resolved-field))]
                                    (parser value)))}))))
...
(defrecord DateTimeValuePlaceholder [^DateTimeFieldPlaceholder field
  ...
  IResolve
  (resolve-field [_ field-id->field]
    (map->DateTimeValue {:field           (resolve-field-or-throw field field-id->field 'DateTimeField)
                         :unit            unit
...
(defrecord DateTimeLiteralPlaceholder [^DateTimeFieldPlaceholder field
  ...
  IResolve
  (resolve-field [_ field-id->field]
    (map->DateTimeLiteral {:field (resolve-field-or-throw field field-id->field 'DateTimeField)
                           :value value})))
