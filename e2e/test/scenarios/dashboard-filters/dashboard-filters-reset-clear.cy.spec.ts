...
function checkResetAllFiltersWorksAcrossTabs({
  ...
  addDateFilter(PARAMETER_A.name, "01/01/2030");
  filter(PARAMETER_A.name).findByText("January 1, 2030").should("exist");
  if (!autoApplyFilters) {
  ...
  addDateFilter(PARAMETER_B.name, "01/01/2029");
  if (!autoApplyFilters) {
    H.applyFilterButton().click();
  }
  checkResetAllFiltersShown();
  filter(PARAMETER_B.name).findByText("January 1, 2029").should("exist");
...
function checkResetAllFiltersToDefaultWorksAcrossTabs({
  ...
  checkResetAllFiltersHidden();
  filter(PARAMETER_A.name).findByText("January 5, 2029").should("exist");
  H.getDashboardCard(0).findByText("73.99").should("be.visible");
  H.getDashboardCard(0).findByText("116.01").should("not.exist");

  updateDateFilter(PARAMETER_A.name, "01/01/2030");
  filter(PARAMETER_A.name).findByText("January 1, 2030").should("exist");
  if (!autoApplyFilters) {
    H.applyFilterButton().click();
  }
  ...
  cy.findAllByTestId("tab-button-input-wrapper").eq(1).click();
  checkResetAllFiltersShown();
  filter(PARAMETER_B.name).findByText("January 5, 2029").should("exist");
  H.getDashboardCard(0).findByText("4").should("be.visible");

  updateDateFilter(PARAMETER_B.name, "01/01/2029");
  if (!autoApplyFilters) {
    H.applyFilterButton().click();
  }
  checkResetAllFiltersShown();
