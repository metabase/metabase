...
(defn- pivot-query
  "Build a one-stage MBQL5 query with `n` breakouts and a count aggregation, then optionally attach `:pivot`."
  ([] (pivot-query 2 nil))
  ([n] (pivot-query n nil))
  ([n pivot]
...
(deftest ^:parallel query-schema-accepts-top-level-pivot-test
  (let [q       (pivot-query 2)
        [u1 u2] (breakout-uuids q)]
    (is (mr/validate :metabase.lib.schema/query
                     (assoc q :pivot {:rows [u1] :columns [u2]})))))

(deftest ^:parallel query-schema-rejects-stage-level-pivot-test
  (let [q      (pivot-query 2)
        [u1 _] (breakout-uuids q)]
    (testing ":pivot at the stage level is disallowed for v1 (top-level only)"
      (is (not (mr/validate :metabase.lib.schema/query
                            (assoc-in q [:stages 0 :pivot] {:rows [u1] :columns []})))))))
...
(deftest ^:parallel has-pivot?-test
  (let [q    (pivot-query 1)
    ...
    (is (true?  (lib.pivot/has-pivot? (assoc q :pivot {:rows [u1] :columns []}))))))

(deftest ^:parallel pivot-rows-and-columns-resolve-to-breakout-clauses-test
  (let [q             (pivot-query 2)
        [b1 b2]       (lib/breakouts q)
        [u1 u2]       (breakout-uuids q)
        piv-singles   (assoc q :pivot {:rows [u1] :columns [u2]})
        piv-reordered (assoc q :pivot {:rows [u2 u1] :columns []})]
...
(deftest ^:parallel pivot-rows-returns-nil-when-pivot-absent-test
  (is (nil? (lib.pivot/pivot-rows (pivot-query 1))))
  (is (nil? (lib.pivot/pivot-columns (pivot-query 1)))))

(deftest ^:parallel pivot-rows-throws-on-unknown-uuid-test
  (let [q   (pivot-query 1)
        bad "deadbeef-dead-dead-dead-deaddeaddead"
        piv (assoc q :pivot {:rows [bad] :columns []})]
...
(deftest ^:parallel returned-columns-splices-pivot-grouping-between-breakouts-and-aggregations-test
  (let [q       (pivot-query 2)
        [u1 u2] (breakout-uuids q)
        piv     (assoc q :pivot {:rows [u1] :columns [u2]})]
