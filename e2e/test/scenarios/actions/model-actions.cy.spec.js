...
describe(
  ...
  () => {
    beforeEach(() => {
      ...
      cy.signInAsAdmin();
      cy.setActionsEnabledForDB(WRITABLE_DB_ID);

      cy.createModelFromTableName({
        tableName: "orders",
        modelName: "Order",
        idAlias: "modelId",
      });
    ...
    it("should allow CRUD operations on model actions", () => {
      ...
      cy.fillActionQuery("DELETE FROM orders WHERE id = {{ id }}");
      ...
      cy.fillActionQuery(" AND status = 'pending'");
      cy.findByRole("radiogroup", { name: "Field type" })
      ...
      cy.popover().findByText("Archive").click();
      ...
      cy.popover().findByText("Disable basic actions").click();
      cy.modal().within(() => {
    ...
    it("should allow to create an action with the New button", () => {
      ...
      cy.popover().findByText("Action").click();

      cy.wait("@getDatabase");
      cy.fillActionQuery(QUERY);
      ...
      cy.modal().eq(1).findByText("Select a model").click();
    ...
    it("should respect permissions", () => {
      ...
      cy.signInAsAdmin();
      cy.setActionsEnabledForDB(SAMPLE_DB_ID, false);
      cy.signIn("normal");
      cy.reload();
    ...
    it("should show detailed form errors for constraint violations when executing model actions", () => {
      ...
      cy.get("@modelId").then(modelId => {
        cy.createImplicitActions({ modelId });
...
["postgres", "mysql"].forEach(dialect => {
  describe(`Write actions on model detail page (${dialect})`, () => {
    beforeEach(() => {
      ...
      cy.resetTestTable({ type: dialect, table: WRITABLE_TEST_TABLE });
    ...
    it("should allow query action execution from the model details page", () => {
      ...
      cy.fillActionQuery(" [[and status = {{ current_status}}]]");
    ...
    it("should allow public sharing of query action and execution", () => {
      ...
      cy.fillActionQuery(" [[ AND status = {{new_status}} ]]");
