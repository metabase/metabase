...
**Do NOT check backend health or Playwright tool availability here.** The agent handles all runtime checks itself (via `environment-discovery.md`) with proper retries. The orchestrator only needs to verify that config files exist.
