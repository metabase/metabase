...
export interface MetabotState {
  ...
  lastRecievedHistory: MetabotHistory | undefined;
  visible: boolean;
}
  ...
  reducers: {
    ...
    setVisible: (state, { payload: visible }: PayloadAction<boolean>) => {
      ...
      if (!visible) {
        ...
        state.lastRecievedHistory = undefined;
      }
    },
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(sendMessageRequest.fulfilled, (state, action) => {
        state.lastRecievedHistory = action.payload?.data?.history;
      });
  },
});
