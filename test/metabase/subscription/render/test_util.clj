(ns metabase.subscription.render.test-util
  ...
  for actual visual changes in the render by providing a hiccup tree instead of a png metabase.subscription.render.test-util
...
(defn render-as-hiccup
  ...
  `metabase.subscription.render.js-svg/svg-string->bytes` normally takes an svg-string from the static-viz js (via gaalvm)
  and returns PNG bytes. It is redefined to pass the svg-string without any encoding.

  `metabase.subscription.render.image-bundle/make-image-bundle` normally takes a render-type (:inline :attachment) and
