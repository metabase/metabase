...
(def ^:private TestChannelDetails
  "Details for the test-only `:channel/metabase-test` channel type, which drives the fake `can-connect?`
  implementation in tests. Declared here like the internal-query schema on `POST /api/dataset` -- not the best, but
  the alternative is a details schema that accepts anything."
  [:map
   [:return-type  [:enum "return-value" "throw"]]
   [:return-value {:optional true} :any]])

(defn- channel-body-schema
  "Schema for a channel API body: `common-entries` plus a `:details` entry whose schema depends on `:type`."
