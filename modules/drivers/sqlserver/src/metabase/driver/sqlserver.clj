...
(defmethod driver/supports? [:sqlserver :percentile-aggregations] [_ _] true)
...
(declare rewrite-percentile-aggregations)
...
(defn- percentile-aggregation? [aggregation]
  (boolean (mbql.u/match aggregation #{:percentile :median})))

(defn- percentile-query? [inner-query]
  (boolean (some #(when (percentile-aggregation? %) %) (:aggregation inner-query))))

(defn- aggregation->field [aggregation]
  ;; TODO -- Investigate further if hard coded `:base-type` wouldn't cause problems.
  [:field (annotate/aggregation-name aggregation) {:base-type :type/Float}])

(defn- remove-aggregations [pred inner-query]
  (if-let [rest-aggs (filter #(not (pred %)) (:aggregation inner-query))]
    (assoc inner-query :aggregation (vec rest-aggs))
    (dissoc inner-query :aggregation)))

(defn- remove-percentile-aggregations [inner-query]
  (remove-aggregations percentile-aggregation? inner-query))

(defn- remove-other-aggregations [inner-query]
  (remove-aggregations #(not (percentile-aggregation? %)) inner-query))

(defn- wrap-in [to-wrap wrapping]
  (assoc wrapping :source-query to-wrap))

(defn- distinct-fields-from-form [mbql-form]
  (-> mbql-form (mbql.u/match :field) distinct vec))

(defn- percentile-query-base
  "Extract basis for computing window percentile from `original-inner-query`.
  It consists of:
  1. clauses affecting query result prior to group by execution (from, joins, where in sql terms) and
  2. percentile aggregations which will be later transformed to window function calls."
  [original-inner-query]
  (-> original-inner-query
      (select-keys [:source-query :source-table :joins :filter :aggregation])
      remove-other-aggregations))

(defn- percentile-query-fields
  "Extract all fields from `original-source-query` that will be used by [[aggregating-query]]."
  [original-inner-query]
  (-> original-inner-query
      (select-keys [:aggregation :breakout :order-by :page :limit])
      remove-percentile-aggregations
      distinct-fields-from-form))

(defn- percentile-query
  "Create percentile query from `original-inner-query`.
  Resulting mbql query is meant to be used by [[aggregating-query]] as a `:source-query`. `original-inner-query` is
  [[metabase.mbql.schema/MBQLQuery]] containing `:aggregation` using percentile. `:expressions` handling is ignored,
  because [[rewrite-percentile-query]] and transitively this function is meant to be called in [[sql.qp/preprocess]]
  after [[nest-query/nest-expressions]]. Non-standard `::partition-by` key is added to query. It's content is used
  in [[sql.qp/->honeysql]] with dispatch `[:sqlserver :percentile]`."
  [original-inner-query]
  (-> (percentile-query-base original-inner-query)
      (assoc :fields (percentile-query-fields original-inner-query))
      (assoc ::partition-by (:breakout original-inner-query))))

(defn- aggregation-ref? [[_ [ref-type _] :as _order-by-element]] 
  (= ref-type :aggregation))

(defn- percentile-aggregation-ref?
  [[_ [_ index] :as order-by-element] aggregations]
  (and (aggregation-ref? order-by-element)
       (percentile-aggregation? (aggregations index))))

(defn- ref->field
  [[ordering [_ref-type index] :as _order-by-element] aggregations]
  [ordering (aggregation->field (aggregations index))])

(defn- adjust-aggregation-index
  [[ordering [ref-type index] :as _order-by-element] preceding-percentile-aggregations]
  [ordering [ref-type (- index (preceding-percentile-aggregations index))]])

(defn- adjust-aggregation-reference
  [order-by-element aggregations preceding-percentile-aggregations]
  (cond (percentile-aggregation-ref? order-by-element aggregations)
        (ref->field order-by-element aggregations)

        (aggregation-ref? order-by-element)
        (adjust-aggregation-index order-by-element preceding-percentile-aggregations)

        :else 
        order-by-element))

(defn- adjust-order-by-references
  "Adjust `:order-by` references of [[aggregating-query]] after percentile aggrs are moved to [[percentile-query]].
  References to percentile aggregations become references to fields and references to other aggregations should have
  index updated according to number of preceding percentile aggregations in original query."
  [order-by-clause {aggregations :aggregation :as _original-query}]
  (let [preceding-percentile-aggs (vec (reductions (fn [acc ag]
                                                     (if (percentile-aggregation? ag)
                                                       (inc acc)
                                                       acc))
                                                   0
                                                   aggregations))]
    (mapv #(adjust-aggregation-reference % aggregations preceding-percentile-aggs) order-by-clause)))

(defn- percentile-aggregations-into-breakout 
  [aggregating-query {aggs :aggregation :as _original-inner-query}]
  (update aggregating-query :breakout #(into (vec %)
                                             (->> aggs
                                                  (filter percentile-aggregation?)
                                                  (map aggregation->field)))))

(defn- aggregating-query-base
  "Create basis for [[aggregating-query]] from `original-inner-query`.
  Extract mbql clauses from `original-inner-query` which in sql terms affect query execution after group by
  (including). Percentile aggregations are transformed to `:breakout` fields."
  [original-inner-query]
  (-> original-inner-query
      (dissoc :source-table :source-query :joins :filter)
      remove-percentile-aggregations
      (percentile-aggregations-into-breakout original-inner-query)))

(defn- aggregating-query
  "Create aggregating query from `original-inner-query`.
  Result of `aggregating-query` is meant to nest result of [[percentile-query]] as it's `:source-query`.
  It is further responsible for real `:aggregation`s and `:breakout`. It does row filtering on the
  result of [[percentile-query]], so it aggregates also percentile values computed by window function."
  [original-inner-query]
  (-> original-inner-query
      aggregating-query-base
      (m/update-existing :order-by adjust-order-by-references original-inner-query)))

(defn- field-ordering-query
  "Create `field-ordering-query` from `original-inner-query`.
  Result of `field-ordering query` is meant to nest result of [[aggregating-query]]. In case `original-inner-query`
  was meant to compute other aggregations prior to percentile aggregations, order of results would change due to move
  of percentile aggregations into breakout fields. This level of nesting ensures the order of results is same as
  in original query."
  [original-inner-query]
  {:fields (into (vec (:breakout original-inner-query))
                 (map aggregation->field (:aggregation original-inner-query)))})

(defn- rewrite-percentile-aggregations--current-query
  [original-inner-query]
  (-> (percentile-query original-inner-query)
      (wrap-in (aggregating-query original-inner-query))
      (wrap-in (field-ordering-query original-inner-query))))

(defn- rewrite-percentile-aggregations--recursive
  [original-query]
  (cond-> original-query
    (map? (:source-query original-query)) (update :source-query rewrite-percentile-aggregations--recursive)
    (percentile-query? original-query) rewrite-percentile-aggregations--current-query))

(defn- rewrite-percentile-aggregations
  "Transform mbql query containing percentile aggregation so it is computed with use of window function.
  Meant to be called from [[sql.qp/preprocess]] `:sqlserver`'s implementation. If query contains percentile
  aggregations, new fields are created, hence alias info must be recomputed."
  [original-query]
...
;;; Following handler uses `PERCENTILE_CONT` window function. It's reference:
;;;   https://learn.microsoft.com/en-us/sql/t-sql/functions/percentile-cont-transact-sql?view=sql-server-ver16
(defmethod hformat/fn-handler "window-percentile-cont"
  [_ field percentile-val partition-fields]
  (let [percentile-val-sql (hformat/to-sql percentile-val)
        order-by-field-sql (hformat/to-sql field)
        partition-fields-sql (when (seq partition-fields)
                               (str "PARTITION BY " (->> partition-fields
                                                         (map hformat/to-sql)
                                                         hformat/comma-join)))]
    (str "PERCENTILE_CONT(" percentile-val-sql ") "
         "WITHIN GROUP (ORDER BY " order-by-field-sql ") "
         "OVER (" partition-fields-sql ")")))

(defmethod sql.qp/->honeysql [:sqlserver :percentile]
  [driver [_ field percentile-val]]
  (hx/call :window-percentile-cont
           (sql.qp/->honeysql driver field)
           (sql.qp/->honeysql driver percentile-val)
           (map #(sql.qp/->honeysql driver %)
                (::partition-by sql.qp/*inner-query*))))
