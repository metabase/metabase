...
(defn infer-model
  ...
  (let [{:keys [version prompt_template] :as template} (metabot-util/prompt-template :infer_model)
        prompt-version (format "%s:%s" prompt_template version)
        _             (log/infof "Generating SQL from prompt template: '%s'" prompt-version)
    ...
    (when-some [model (some
                       (fn [{model-id :id :as model}]
                         (when (= model-id best-model-id) model))
                       models)]
      (update model :prompt_template_versions (fnil conj []) prompt-version))))
