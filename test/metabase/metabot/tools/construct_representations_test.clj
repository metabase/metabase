...
(def ^:private join-metric-entity-id
...
(def ^:private mp-with-joined-metric
  (lib.tu/mock-metadata-provider
   mp-with-metric-base
   {:cards [{:id            700
             ...
             :database-id   1
             :table-id      10
             :entity-id     join-metric-entity-id
             :dataset-query metric-definition}]}))

(defn- lookup-joined-metric-stub [model eid]
  (when (and (or (= model 'Card) (= model :model/Card))
             (= eid join-metric-entity-id))
    {:id 700 :database_id 1 :entity_id eid}))

(defn- with-joined-metric-mp-and-stubs! [f]
  (with-redefs [lib-be/application-database-metadata-provider (fn [_] mp-with-joined-metric)
                construct/resolve-database-id-from-first-stage (fn [_] 1)
                serdes/lookup-by-id                             lookup-joined-metric-stub
                api/read-check                                  allow-read-check
                api/query-check                                 allow-read-check]
    (f)))
...
(defn- metric-query-with-joined-breakout []
  (query-data
   {"lib/type" "mbql/query"
    ...
    "stages"   [{"lib/type"     "mbql.stage/mbql"
                 "source-table" ["Sample" "PUBLIC" "ORDERS"]
                 "aggregation"  [["metric" {} join-metric-entity-id]]
