(ns metabase.metabot.metadata-perms
  ...
  (master batch-primes a table-granular perms cache via `perms/prime-table-perms-cache` here; v60 has
  no such API, so each table's permission check reads through individually.)"
  (:require
