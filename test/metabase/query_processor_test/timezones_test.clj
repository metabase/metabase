...
(deftest result-rows-test
  (mt/dataset test-data-with-timezones
    (mt/test-drivers (timezone-aware-column-drivers)
      (testing "Basic sanity check: make sure the rows come back with the values we'd expect without setting report-timezone"
    ...
    (mt/test-drivers (set-timezone-drivers)
      (doseq [[timezone expected-rows] {"UTC"        [[12 "2014-07-03T01:30:00Z"]
        ...
        (mt/with-temporary-setting-values [report-timezone timezone]
          (testing (format "There should be %d checkins on July 3rd in the %s timezone" (count expected-rows) timezone)
            (is (= expected-rows
                   (mt/formatted-rows
                    [int identity]
                    (mt/run-mbql-query users
