...
(defn- with-library-synced
  "Helper to set up Library collection as remote-synced for tests."
  [f]
  (if-let [library (collection/library-collection)]
    ...
    (mt/with-temp [:model/Collection _ {:name "Library" :type "library" :is_remote_synced true}]
      (f))))
