...
(defn- tool->claude
  ...
  Accepts a ToolEntry map with :tool-name, :doc, :schema, :fn. When :input-schema is present
  (MCP-bridged tools), it's a pre-built JSON Schema passed straight through."
  [{:keys [tool-name doc schema input-schema]}]
  (if input-schema
