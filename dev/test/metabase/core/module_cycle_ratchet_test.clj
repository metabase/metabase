...
(deftest ^:parallel an-even-merge-still-fails-test
  (is (= [":uses graph:"
          "  a-knot joins parts of x-knot, y-knot -- 4 modules in one cluster: a, b, c, d"]
         (cycle-ratchet/check-report (snapshot {'x-knot '#{a b}, 'y-knot '#{c d}})
                                     (actual [(cluster '[a b c d])])))
      (str "two clusters of the same size merging leaves neither name a clear winner, which is still a merge"
           " rather than an identity to puzzle over")))
...
(deftest ^:parallel pr-summary-test
  (let [shrunk (assoc-in observed [:uses 'galactic-center] '#{app-db queries})]
    (is (= (str "### Module cycles\n\n"
        ...
        "observing records a cluster growing as readily as one shrinking, so the summary promises neither")
    (is (str/includes? (cycle-ratchet/pr-summary (assoc observed :mode :enforce)
                                                 (assoc shrunk :mode :enforce))
                       "It only ever records a cluster shrinking, splitting or dissolving.")
        "under the ratchet the summary can say which way the record moves")
