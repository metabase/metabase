...
(api/defendpoint POST "/"
  ...
  [_]
  (api/check-superuser)
  (if (t2/select-one CloudMigration :state [:not-in cloud-migration/terminal-states])
...
(api/defendpoint PUT "/cancel"
  ...
  (t2/update! CloudMigration {:state [:not-in cloud-migration/terminal-states]} {:state :cancelled}))
