  ...
  CASE WHEN EXISTS (
    ...
    WHERE
      ...
      AND (p.object = concat('/download/db/', md.id, '/')
        OR p.object = concat('/download/db/', md.id, '/schema/'))) THEN
    'one-million-rows'
  WHEN EXISTS (
    SELECT
      1
    ...
    WHERE
      ...
      AND (p.object = concat('/download/limited/db/', md.id, '/')
        OR p.object = concat('/download/limited/db/', md.id, '/schema/'))) THEN
    'ten-thousand-rows'
  WHEN NOT EXISTS (
    SELECT
      1
    ...
    WHERE
      ...
      AND (p.object LIKE concat('/download/db/', md.id, '/schema/%')
        OR p.object LIKE concat('/download/limited/db/', md.id, '/schema/%'))) THEN
    'no'
  END AS perm_value
FROM
  ...
  CASE WHEN EXISTS (
    ...
    WHERE
      ...
      AND (p.object = concat('/download/db/', mt.db_id, '/schema/', mt.escaped_schema, '/')
        OR p.object = concat('/download/db/', mt.db_id, '/schema/', mt.escaped_schema, '/table/', mt.id, '/'))) THEN
    'one-million-rows'
  WHEN EXISTS (
    SELECT
      1
    ...
    WHERE
      ...
      AND (p.object = concat('/download/limited/db/', mt.db_id, '/schema/', mt.escaped_schema, '/')
        OR p.object = concat('/download/limited/db/', mt.db_id, '/schema/', mt.escaped_schema, '/table/', mt.id, '/'))) THEN
    'ten-thousand-rows'
  ELSE
    'no'
  END AS perm_value
