...
describe("scenarios > visualizations > table", () => {
  ...
  function selectFromDropdown(option, clickOpts) {
    cy.popover().last().findByText(option).click(clickOpts);
  }
  ...
  it("should allow you to reorder and hide columns in the table header", () => {
    cy.startNewNativeQuestion({ query: "select * from orders LIMIT 2" });
    ...
    cy.popover().icon("eye_crossed_out").click();
  ...
  it("should show field metadata in a hovercard when hovering over a table column header", () => {
    ...
    cy.openPeopleTable({ mode: "notebook", limit: 2 });
  ...
  it("should show field metadata hovercards for native query tables", () => {
    cy.startNewNativeQuestion({ query: "select * from products limit 1" });
    cy.findByTestId("native-query-editor-container").icon("play").click();
  ...
  it("popover should not be scrollable horizontally (metabase#31339)", () => {
    ...
    cy.popover().then($popover => {
      expect(cy.isScrollableHorizontally($popover[0])).to.be.false;
    });
  });
...
describe("scenarios > visualizations > table > conditional formatting", () => {
  describe("rules", () => {
    beforeEach(() => {
      ...
      cy.openVizSettingsSidebar();
      cy.sidebar().findByText("Conditional Formatting").click();
    });
    ...
    it("should be able to remove, add, and re-order rows", () => {
      ...
      cy.popover().findByText("Subtotal").click();
      cy.realPress("Escape");
      cy.findByRole("button", { name: /is equal to/i }).click();
      cy.popover().findByText("is less than").click();
  ...
  describe("operators", () => {
    beforeEach(() => {
      ...
      cy.getTable({ name: "many_data_types" }).then(
        ({ id: tableId, fields }) => {
    ...
    it("should work with boolean columns", { tags: ["@external"] }, () => {
      cy.openVizSettingsSidebar();
      cy.leftSidebar().findByText("Conditional Formatting").click();
      cy.findByRole("button", { name: /add a rule/i }).click();

      cy.popover().findByRole("option", { name: "Boolean" }).click();

      //Dismiss popover
      cy.leftSidebar().findByText("Which columns should be affected?").click();
...
describe("scenarios > visualizations > table > time formatting (#11398)", () => {
  ...
  it("should work with time columns", { tags: ["@external"] }, () => {
    ...
    cy.tableHeaderClick("CREATION_TIME");
