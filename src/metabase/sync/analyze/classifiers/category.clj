...
(mu/defn ^:private field-should-be-category? :- [:maybe :boolean]
  ...
  (let [distinct-count (get-in fingerprint [:global :distinct-count])
    ...
    (when (and (nil? (:semantic-type field))
               (or (some-> nil% (< 1))
                   (isa? (:base-type field) :type/Boolean))
...
(mu/defn infer-is-category-or-list :- [:maybe ::lib.schema.metadata/column]
  ...
  (when (and fingerprint
    ...
    (cond-> field
      (field-should-be-category? fingerprint field)  (assoc :semantic-type :type/Category)
      (field-should-be-auto-list? fingerprint field) (assoc :has-field-values :auto-list))))
