(ns metabase.mcp.v2.tools.definitions
  "The v2 MCP `segment_write` and `measure_write` tools: authoring for the two table-attached
   ...
   MBQL 5 queries, and MBQL 4 auto-convert for segments), and translating the model layer's
   raw validation exceptions into teaching errors."
  (:require
...
(defn- resolve-table
  ...
  [table-id]
  (v2.resolve/resolve-and-read :model/Table table-id))
...
(defn- clause-form->definition
  ...
  (let [clauses (if (string? (first definition)) [definition] (vec definition))]
    (v2.queries/resolve-external-query
     {:lib/type "mbql/query"
...
(defn- prepare-definition
  ...
  (cond
    ...
    (v2.queries/portable-query? definition)
    (v2.queries/resolve-external-query definition (accepted-shapes kind))
...
(def ^:private segment-write-args-schema
  (write-args-schema
   {:entity          "segment"
    ...
    :definition-desc (str "Either (a) an array of filter clauses — what get_content's "
                          ...
                          "Filters only: no aggregations, breakouts, joins, expressions, or limits. May "
...
(registry/deftool segment-write
  ...
  (let [dispatched (v2.write/dispatch-write segment-write-entry args)]
...
(def ^:private measure-write-args-schema
  (write-args-schema
   {:entity          "measure"
    ...
    :definition-desc (str "Either (a) the aggregation clause — what get_content's "
                          ...
                          "(b) a full single-stage query (\"lib/type\": \"mbql/query\") holding exactly "
                          "one aggregation. No filters, breakouts, joins, expressions, or limits. May reference other measures "
...
(registry/deftool measure-write
  ...
  or a full single-stage query. For a
  ...
  [args {:keys [token-scopes]}]
  (let [dispatched (v2.write/dispatch-write measure-write-entry args)]
    (case (first dispatched)
      :create
      (let [[_ body]   dispatched
            _          (check-method-args! :create body)
