...
(defmethod tp.backend/publish! :topic.backend/tracking
  [_ topic-name messages]
  (tp.backend/publish! :topic.backend/memory topic-name messages)
  (swap! (:published-messages *recent*) conj {:topic topic-name :messages messages}))

(defmethod tp.backend/subscribe! :topic.backend/tracking
  ...
  (let [tracking-handler (fn [{:keys [messages] :as msg}]
    ...
    (tp.backend/subscribe! :topic.backend/memory topic-name subscriber-name tracking-handler)))

(defmethod tp.backend/unsubscribe! :topic.backend/tracking
  [_ topic-name subscriber-name]
  (tp.backend/unsubscribe! :topic.backend/memory topic-name subscriber-name))

(defmethod tp.backend/cleanup! :topic.backend/tracking
  [_ topic-name max-age-ms]
  (tp.backend/cleanup! :topic.backend/memory topic-name max-age-ms))
