(ns metabase-enterprise.workspaces.isolation-manager-test
...
(comment
  ...
  (mt/test-drivers #{:postgres}
    (let [db (mt/db)
      ...
      (try
        (println "Simple API result:" result)
        (is (map? result))
        (is (contains? result :schema-name))
        (is (contains? result :populator-user))
        (is (contains? result :reader-user))
        (finally
