...
(defn- date-format [format-str expr]
  (hsql/call :unix_timestamp expr (hx/literal format-str)))

(defn- str-to-date [format-str expr]
  (hsql/call :from_unixtime expr (hx/literal format-str)))
...
(defn- week [expr]
  (hsql/call :date_add
    ...
    (hx/* (hx/- (hsql/call :weekofyear expr)
                1)
          7)))
...
(defn- date [unit expr]
  (case unit
    ...
    :day (hsql/call :trunc expr (hx/literal :MM))
    :day-of-week (hsql/call :datediff
                   expr
                   (week expr))
    ...
    :month (hsql/call :trunc expr (hx/literal :month))
...
(driver/register-driver! :spark-sql (SparkSQLDriver.))
