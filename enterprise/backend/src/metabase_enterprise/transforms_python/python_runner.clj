...
(defn- build-table-query
  ...
  (let [db-id             (t2/select-one-fn :db_id (t2/table-name :model/Table) :id table-id)
    ...
    (cond-> (-> (lib/query metadata-provider table-metadata)
      ...
      source-incremental-strategy (transforms.u/preprocess-incremental-query source-incremental-strategy (transforms.u/next-checkpoint transform) (transforms.u/get-source-range-params transform))
