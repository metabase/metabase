(ns metabase.test.data.sybase
  "Code for creating / destroying a Sybase database from a `DatabaseDefinition`."
  (:require [clojure.java.jdbc :as jdbc]
            ...
            [metabase.test.data
             ...
             [interface :as i]]
            [metabase.util :as u])
  (:import metabase.driver.sybase.SybaseDriver))
...
(defn- database->connection-details [context {:keys [database-name]}]
  {:host         (i/db-test-env-var-or-throw :sybase :host)
   :port         (Integer/parseInt (i/db-test-env-var-or-throw :sybase :port "1433"))
   :user         (i/db-test-env-var-or-throw :sybase :user)
   :password     (i/db-test-env-var-or-throw :sybase :password)
   :db           (when (= context :db)
...
(defn- cleanup-leftover-dbs!
  ...
  []
  (datasets/when-testing-engine :sybase
    (let [connection-spec (sql/connection-details->spec (SybaseDriver.) (database->connection-details :server nil))
      ...
      (with-redefs [+suffix identity]
        (doseq [db leftover-dbs]
          (u/ignore-exceptions
            (printf "Deleting leftover Sybase DB '%s'...\n" db)
