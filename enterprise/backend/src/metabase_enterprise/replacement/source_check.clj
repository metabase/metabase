...
(mu/defn- has-gtap-policies? :- :boolean
  ...
  (or (replacement.db/sandbox-exists-for-table? table-id)
      (let [sandbox-card-ids (replacement.db/sandbox-card-ids)]
        (boolean
...
(mu/defn- fetch-source-database-id :- ::lib.schema.id/database
  ...
  (case entity-type
    :card  (replacement.db/card-database-id entity-id)
    :table (replacement.db/table-database-id entity-id)))
