...
describe("issue 48824", { tags: "@skip" }, () => {
  ...
  it("should correctly translate relative filters in dashboards (metabase#48824)", () => {
    ...
    // The frontend joins the prefix and suffix with `, ` in TypeScript, but each
    // part is generated by a separate ttag call, so the rendered text contains
    // two `[zz] ` prefixes — one from `describeTemporalInterval` and one from
    // `describeRelativeDatetime`. See `frontend/src/metabase/querying/common/utils/dates.ts`.
    H.filterWidget()
