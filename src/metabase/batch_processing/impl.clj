...
(mr/def ::grouper-wrapper
  [:map {:closed true, :probe/id "src/metabase/batch_processing/impl.clj:34"}
   [:f       ::fn-or-var]
...
(def ^:private sdk-tracked-row
  [:map {:closed true, :probe/id "src/metabase/batch_processing/impl.clj:47"}
   ...
   [:metadata                    {:optional true} [:maybe [:map {:closed true, :probe/id "src/metabase/batch_processing/impl.clj:52"}]]]
   ...
   [:lens_params                 {:optional true} [:maybe [:map {:closed true, :probe/id "src/metabase/batch_processing/impl.clj:75"} [:join_step {:optional true} [:maybe :int]]]]]
...
(def ^:private id+timestamp
  [:map {:closed true, :probe/id "src/metabase/batch_processing/impl.clj:93"}
...
(def ^:private submit-object
  [:or
   ...
   [:map {:closed true, :probe/id "src/metabase/batch_processing/impl.clj:104"}
    [:model [:enum :model/Card :model/Dashboard :model/Table :model/Document]]
    [:id :int]]
   sdk-tracked-row
   id+timestamp
   [:map {:closed true, :probe/id "src/metabase/batch_processing/impl.clj:109"}
    [:user-id      ms/PositiveInt]
    [:dashboard-id ms/PositiveInt]
    [:parameters   [:sequential :metabase.parameters.schema/parameter-with-value]]]
   [:map {:closed true, :probe/id "src/metabase/batch_processing/impl.clj:113"}
    [:user-id   ms/PositiveInt]
