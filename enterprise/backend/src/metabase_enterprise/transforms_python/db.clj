...
(mu/defn database-engine :- [:maybe :any]
...
(mu/defn python-library :- [:maybe (ms/InstanceOf :model/PythonLibrary)]
  "The PythonLibrary with `library-id`, or nil."
  [library-id :- ms/PositiveInt]
  (t2/select-one :model/PythonLibrary library-id))

(mu/defn python-library-by-path :- [:maybe (ms/InstanceOf :model/PythonLibrary)]
...
(mu/defn top-level-fields-metadata :- [:sequential (ms/InstanceOf :model/Field)]
