(ns metabase.metabot.agent.tools.document
  "Document-generation specific tool wrappers."
  (:require
   [metabase.metabot.agent.tools.construct :as construct-tools]
...
(defn document-schema-collect-tool "document_schema_collect" []
  {:tool-name "document_schema_collect"
   :doc       "Collects the schema of a database in order to construct a SQL query.
...
(defn document-construct-sql-chart-tool "document-construct-sql-chart-tool" []
  {:tool-name "document_construct_sql_chart"
   :doc       "Construct SQL-backed chart draft payload for document insertion."
   :schema    [:=> [:cat sql-chart-schema] :any]
   :fn        (fn [{:keys [database_id name description analysis approach sql viz_settings]}]
                (try
...
(defn document-construct-model-chart-tool "document-construct-model-chart-tool" []
  {:tool-name "document_construct_model_chart"
   :doc       "Construct notebook/model-backed chart draft payload for document insertion."
   :schema    [:=> [:cat model-chart-schema] :any]
   :fn        (fn [{:keys [name description query viz_settings]}]
                (try
                  (let [chart-type (get viz_settings :chart_type)
                        tool-def   (construct-tools/construct-notebook-query-tool)
                        result     ((:fn tool-def)
                                    {:query query
