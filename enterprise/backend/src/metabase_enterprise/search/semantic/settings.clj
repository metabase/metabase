(ns metabase-enterprise.search.semantic.settings
  (:require
...
;; Topic for the just-in-time HNSW build, handled in metabase-enterprise.search.semantic.events. Declared
...
(defsetting ee-search-gate-write-timeout
  (str "Timeout of gate write statements in seconds. Used to determine lag tolerance of the indexer (see the "
       "[[metabase-enterprise.search.semantic.gate/poll]]) in conjunction "
...
(defsetting ee-search-indexer-lag-tolerance-multiplier
  (str "Multiplier for computation of [[metabase-enterprise.search.semantic.indexer/lag-tolerance]]. The formula "
