  ...
  (mt/with-premium-features #{:custom-viz}
    (cvp.tu/with-dev-mode true
      (mt/with-model-cleanup [:model/CustomVizPlugin]
        ...
        (testing "a structurally invalid manifest is refused, the same as on the upload path"
          (let [resp (mt/user-http-request :crowberto :post 400 "ee/custom-viz-plugin/dev"
                                           {:dev_bundle_url "http://localhost:5174"
                                            :manifest       {:name 123}})]
            (is (str/includes? resp "is invalid"))))
        (testing "SECURITY: a non-loopback dev URL is refused before a row is written"
          (let [resp (mt/user-http-request :crowberto :post 400 "ee/custom-viz-plugin/dev"
                                           {:dev_bundle_url "http://host.docker.internal:5174"
                                            :manifest       {:name "docker-viz"}})]
            (is (str/includes? resp "must point at localhost"))
            (is (false? (t2/exists? :model/CustomVizPlugin :identifier "docker-viz"))
                "the URL is validated before the insert, not after it")))))))
  ...
  (mt/with-premium-features #{:custom-viz}
    (mt/with-temp [:model/CustomVizPlugin {id :id} {:identifier   "bundle-test"
      ...
      (testing "returns bundle with correct content-type and ETag"
        (mt/with-dynamic-fn-redefs [cache/get-bundle (constantly {:content "console.log('hello')" :hash "deadbeef"})]
      ...
      (testing "returns 503 when bundle is not available"
        (mt/with-dynamic-fn-redefs [cache/get-bundle (constantly nil)]
...
(deftest bundle-and-list-auth-test
  (mt/with-premium-features #{:custom-viz}
    (mt/with-temp [:model/CustomVizPlugin {id :id} {:identifier   "auth-bundle"
                                                    ...
                                                    :enabled      true
                                                    :bundle_hash  "abc123"}]
      (mt/with-dynamic-fn-redefs [cache/get-bundle (constantly {:content "console.log('hi')" :hash "h1"})]
    ...
    (cvp.tu/with-dev-mode true
      (mt/with-model-cleanup [:model/CustomVizPlugin]
        (testing "registering a dev plugin records a custom-viz-plugin-create audit event"
          (let [resp  (mt/user-http-request :crowberto :post 200 "ee/custom-viz-plugin/dev"
                                            {:dev_bundle_url "http://localhost:5174"
                                             :manifest       {:name "audit-dev-chart" :icon "icon.svg"}})
                entry (mt/latest-audit-log-entry "custom-viz-plugin-create" (:id resp))]
            (is (partial=
                 {:topic    :custom-viz-plugin-create
                  :user_id  (mt/user->id :crowberto)
