...
function syncHarmony() {
    ...
    for (let roundIndex = 1; roundIndex < 5; roundIndex++) {
        for (
            let colorIndex = 0; colorIndex < initialColorHarmonies.length; colorIndex++
        ) {
...
export const getColorScale = (
    ...
    if (quantile) {
        ...
        return d3.scale
            .linear()
            .domain(
                colors.length === 3 ? [start, start + (end - start) / 2, end] : [start, end],
            )
            .range(colors);
    }
};
...
export function getColorsForValues(
    ...
    return deterministicAssign(
        ...
        primaryTier,
        existingAssignments,
        getPreferredColor, [secondaryTier],
    );
}
