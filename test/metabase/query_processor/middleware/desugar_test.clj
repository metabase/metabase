...
(defn- drop-legacy-idents [query]
  (cond-> query
    (:query query)        (update :query drop-legacy-idents)
    (:source-query query) (update :source-query drop-legacy-idents)
    true                  (dissoc :expression-idents :aggregation-idents)))

;; actual desugaring logic and tests are in [[metabase.legacy-mbql.util-test]]
(deftest ^:parallel e2e-test
  (is (= (drop-legacy-idents
