...
(deftest ^:parallel source-filter-value-test
  (let [flt (fn [value] {:field-id 1, :op :>, :value value})]
    (testing "a source-filter :value holds a parameter value"
      (are [value] (mr/validate ::lib.schema.template-tag/source-filter (flt value))
        "abc" 500 true nil ["a" "b"]))
