...
(defn delete-non-active-index!
  ...
  Returns the number of rows deleted, using `conn` when provided."
  ([engine version index-name]
   (delete-non-active-index! nil engine version index-name))
  ([conn engine version index-name]
   (search.db/delete-non-active-index-metadata! conn engine version (i18n/site-locale-string) (name index-name))))
...
(defn active-pending!
  ...
  (t2/with-transaction [conn]
    (let [lang-code (i18n/site-locale-string)]
      ...
      (when (search.db/lock-pending-index-metadata! conn engine version lang-code)
...
(defn activate-named-pending!
  ...
  When its pending row is gone, the existing active name comes back unchanged.
  Compare the result with the name you expected to promote."
  [conn engine version index-name]
  ;; Naming the index is what stops an old worker promoting a replacement owner's pending table.
  (let [lang-code  (i18n/site-locale-string)
        index-name (name index-name)]
    (when (search.db/lock-named-pending-index-metadata! conn engine version lang-code index-name)
      (retire-active! conn engine version lang-code)
