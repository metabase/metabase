...
(defn run!
  "Main entry point for cibot workmux mode."
  [{:keys [arguments options]}]
  (let [branch-name (first arguments)]
    (when (str/blank? branch-name)
      (println (c/red "Usage: ./bin/mage cibot-go <branch-name> --prompt-file <path>"))
      (u/exit 1))
    (let [branch-name  (str/trim branch-name)
          app-db       (or (:app-db options) "postgres")
          prompt-file  (:prompt-file options)
          session-name (bot/branch-to-session-name "cibot" branch-name)
          config       (launch/generate-workmux-config "cibot" app-db)]
      (when (str/includes? branch-name "/")
        (println (c/red "Pass a local branch name, not a remote ref."))
        (println "Example: ./bin/mage cibot-go master  (not origin/master)")
        (u/exit 1))
      ...
      (when (bot/tmux-session-running? session-name)
        ...
        (println)
        (println (str "Stop it first with: ./bin/mage -cibot-stop " branch-name))
        (u/exit 1))
