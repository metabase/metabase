...
const createMetadataProvider = () => ML.metadataProvider(SampleDatabase.SAMPLE_DATABASE.id, SampleDatabase.metadata);

const createQuery = () => {
  const query = ML.query(SampleDatabase.SAMPLE_DATABASE.id, createMetadataProvider(), {
...
describe("orderableColumns", () => {
  ...
  describe("returns metadata for columns in the source table", () => {
    it("contains ORDERS.ID", () => {
      const ordersID = _.find(orderableColumns, ({id}) => id === SampleDatabase.ORDERS.ID.id);

      expect(ordersID).toEqual(expect.objectContaining({
  ...
  describe("returns metadata for columns in implicitly joinable tables", () => {
    it("contains PRODUCTS.TITLE", () => {
      const productsTitle = _.find(orderableColumns, ({id}) => id === SampleDatabase.PRODUCTS.TITLE.id);

      expect(productsTitle).toEqual(expect.objectContaining({
