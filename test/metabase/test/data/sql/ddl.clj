(ns metabase.test.data.sql.ddl
  "Methods for creating DDL/DML statements for things like creating/dropping databases and loading data."
  (:require
...
(defmulti insert-rows-honeysql-form
  "Return an appropriate Honey SQL form for inserting `rows` (as maps) into a Table named by `table-identifier`."
  {:arglists '([driver table-identifier rows])}
  tx/dispatch-on-driver-with-test-extensions
  :hierarchy #'driver/hierarchy)
                        ...
                        (try
                          (mapv (fn [column]
                                  (let [value (get row column)]
                                    ...
                                    (if (and (vector? value)
                                             (#{:raw :metabase.driver.sql.query-processor/compiled} (first value)))
                                      value
...
(defmulti insert-rows-dml-statements
  "Return appropriate SQL DDL statemtents for inserting `rows` (each row should be a map) into a Table named by
...
(defmethod insert-rows-dml-statements :sql/test-extensions
  [driver table-identifier rows]
  [(sql.qp/format-honeysql driver (insert-rows-honeysql-form driver table-identifier rows))])
