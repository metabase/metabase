(ns metabase.search.visualizer.core
  (:require
   ...
   [metabase.search.visualizer.filter :as search.visualizer.filter]
   [metabase.search.visualizer.scoring :as search.scoring]
   [metabase.util :as u]
...
(derive :search.engine/visualizer :search.engine/in-place)
...
(mu/defn- base-query-for-model :- [:map {:closed true}
  ...
  (-> {:select (select-clause-for-model model)
      ...
      (search.visualizer.filter/build-filters model context)))
...
(defmethod search.engine/model-set :search.engine/in-place
  [search-ctx]
  (let [model-queries (for [model (search.visualizer.filter/search-context->applicable-models
...
;; for the visualizer search, need to create some legacy replacements:
;; metabase.search.in-place.legacy/all-search-columns need to add :result_metadata and :visualization_settings

;; then, for card/metric/dataset:
;; add the keys to columns-for-model method
