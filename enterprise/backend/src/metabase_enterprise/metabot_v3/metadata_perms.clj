(ns metabase-enterprise.metabot-v3.metadata-perms
  ...
  table-details request asks all three once for the table's own columns, once to find its FK
  ...
  same scope. When no cache is bound every call reads through to the app DB, so callers outside a tool
  call behave exactly as they did before."
  (:require
...
(defn- sandbox-restricted-fields*
  ...
  fields from source card result_metadata."
  [table-ids]
