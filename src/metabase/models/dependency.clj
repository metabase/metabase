...
(defn retrieve-dependencies
  ...
  {:pre [(i/metabase-entity? entity)
         (integer? id)]}
  (db/sel Dependency :model (:name entity) :model_id id))
...
(defn update-dependencies
  ...
  (let [entity-name      (:name entity)
        ...
        dependencies-old (set (db/sel [Dependency :dependent_on_model :dependent_on_id] :model entity-name :model_id id))
