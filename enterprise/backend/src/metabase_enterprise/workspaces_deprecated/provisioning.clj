(ns metabase-enterprise.workspaces-deprecated.provisioning
  (:require
   [metabase-enterprise.workspaces-deprecated.models.workspace-database]
   [metabase-enterprise.workspaces-deprecated.remapping-cleanup :as ws.remapping-cleanup]
...
(defn- wsd-lock-key
  ...
  (keyword "metabase-enterprise.workspaces-deprecated.provisioning"
...
(defn provision-workspace-database!
  ...
  (try
    (with-workspace-database-lock workspace-database-id
      (let [wsd (t2/select-one :model/WorkspaceDatabaseDeprecated :id workspace-database-id)]
        (when-not wsd
              ...
              (try
                ...
                (catch Throwable destroy-t
                  (log/warnf destroy-t "Failed to clean up after provisioning failure for WorkspaceDatabase %s"
                             workspace-database-id)))
              (throw t)))
          (t2/update! :model/WorkspaceDatabaseDeprecated
    ...
    (catch Throwable t
      ...
      (throw t)))
  (t2/select-one :model/WorkspaceDatabaseDeprecated :id workspace-database-id))
...
(defn provision-single!
  ...
  ([wsd-id provisioner]
   (t2/update! :model/WorkspaceDatabaseDeprecated {:id wsd-id :status :unprovisioned}
               {:status :provisioning})
...
(defn deprovision-single!
  ...
  ([wsd-id provisioner]
   (t2/update! :model/WorkspaceDatabaseDeprecated {:id wsd-id :status :provisioned}
...
(defn provision-workspace!
  ...
  ([workspace-id provisioner]
   (let [triggered (t2/update! :model/WorkspaceDatabaseDeprecated
                               {:workspace_id workspace-id :status :unprovisioned}
                               {:status :provisioning})]
     (when (pos? triggered)
       (doseq [{:keys [id]} (t2/select [:model/WorkspaceDatabaseDeprecated :id]
...
(defn teardown-workspace-database!
  ...
  (with-workspace-database-lock (:id wsd)
    (when-let [wsd (t2/select-one :model/WorkspaceDatabaseDeprecated :id (:id wsd))]
      (let [db         (t2/select-one :model/Database :id (:database_id wsd))
            driver     (driver.u/database->driver db)
        ...
        (try
          ...
          (catch Throwable t
            ...
            (binding [t2.connection/*current-connectable* nil]
              (t2/update! :model/WorkspaceDatabaseDeprecated {:id (:id wsd)}
        ...
        (t2/delete! :model/WorkspaceDatabaseDeprecated :id (:id wsd))
        nil))))
