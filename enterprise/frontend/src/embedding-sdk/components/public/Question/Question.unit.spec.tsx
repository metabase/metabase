...
import { useQuestionContext } from "../../private/Question/context";
...
function QuestionCustomLayout({ title }: { title?: SdkQuestionTitleProps }) {
  const { resetQuestion } = useQuestionContext();

  return (
    <div>
      <button onClick={resetQuestion}>Run Query</button>
      <QuestionDefaultView title={title} />
    </div>
  );
}
...
const setup = ({
  ...
  Pick<BaseQuestionProps, "initialSqlParameters"> &
    Pick<QuestionDefaultViewProps, "withChartTypeSelector" | "title"> & {
      isValidCard?: boolean;
...
> = {}) => {
  ...
  return renderWithSDKProviders(
    ...
    >
      {withCustomLayout ? <QuestionCustomLayout /> : undefined}
    </Question>,
    {
      sdkProviderProps: {
