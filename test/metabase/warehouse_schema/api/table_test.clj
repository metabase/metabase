...
(defn- table-defaults
  ...
  ([driver]
   (merge
    (update (mt/object-defaults :model/Table) :data_source name)
    {:db          (db-details)
     :db_id       (mt/id)
...
(deftest ^:parallel update-data-source-test
  (testing "PUT /api/table/:id"
    (testing "data_source field behavior"
      (mt/with-temp [:model/Table table {}]
        (testing "Initially data_source should be unconfigured"
          (is (= :unconfigured (t2/select-one-fn :data_source :model/Table :id (u/the-id table)))))

        (testing "Can save an unrelated change with this field redundantly included"
          (mt/user-http-request :crowberto :put 200 (format "table/%d" (u/the-id table))
                                {:active false, :data_source "unconfigured"})
          (is (= :unconfigured (t2/select-one-fn :data_source :model/Table :id (u/the-id table)))))

        (testing "Can set data_source to source-data"
          (mt/user-http-request :crowberto :put 200 (format "table/%d" (u/the-id table))
                                {:data_source "source-data"})
          (is (= :source-data (t2/select-one-fn :data_source :model/Table :id (u/the-id table)))))

        (testing "Can set data_source between different values"
          (mt/user-http-request :crowberto :put 200 (format "table/%d" (u/the-id table))
                                {:data_source "transforms"})
          (is (= :transforms (t2/select-one-fn :data_source :model/Table :id (u/the-id table)))))

        (testing "Can set data_source to ingested"
          (mt/user-http-request :crowberto :put 200 (format "table/%d" (u/the-id table))
                                {:data_source "ingested"})
          (is (= :ingested (t2/select-one-fn :data_source :model/Table :id (u/the-id table)))))

        (testing "Cannot un-configure again"
          (is (= "Cannot set data_source back to unconfigured once it has been configured"
                 (mt/user-http-request :crowberto :put 400 (format "table/%d" (u/the-id table))
                                       {:data_source "unconfigured"}))))

        (testing "Cannot set data_source to unknown via API"
          (is (= [:data_source]
                 (keys (:errors (mt/user-http-request :crowberto :put 400 (format "table/%d" (u/the-id table))
                                                      {:data_source "unknown"}))))))))))

(deftest ^:parallel unknown-data-source-value-test
  (testing "Tables with unknown data_source values from database are read as :unknown"
    (mt/with-temp [:model/Table table {}]
      ;; Directly insert an unknown value into the database, bypassing Toucan transforms
      (t2/query-one {:update :metabase_table
                     :set    {:data_source "federated"}
                     :where  [:= :id (:id table)]})

      (testing "Unexpected values are converted to :unknown"
        (is (= :unknown (t2/select-one-fn :data_source [:model/Table :data_source] :id (:id table)))))

      (testing "API GET endpoint returns :unknown for tables with unknown data_source"
        (let [api-response (mt/user-http-request :crowberto :get 200 (format "table/%d" (:id table)))]
          (is (= "unknown" (:data_source api-response))))))))
