...
(defn create-autogenerated-dashboard
  ...
  - :data-parts - An `automagic_dashboard` data part the frontend renders inline
  ...
  (let [memory                (when memory-atom @memory-atom)
    ...
    ;; The actual automagic analysis happens when the /auto/dashboard/... URL is
    ;; fetched. We emit it as a data part so the frontend renders the X-ray
    ;; dashboard inline in the chat instead of navigating the user away.

    (log/info "Autogenerated dashboard created" {:path path})
    {:structured-output {:message response-message
                         :path    path}
     :data-parts        [(streaming/automagic-dashboard-part
                          (cond-> {:url path}
                            title (assoc :title title)))]
     :instructions      "The generated dashboard is now shown to the user inline in the chat. Briefly acknowledge this (1 sentence) and stop. Do not describe the dashboard contents."}))
