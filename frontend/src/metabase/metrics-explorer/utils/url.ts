...
type AllProjectionKeys = keyof TemporalProjectionConfig | keyof NumericProjectionConfig;
...
const TAB_SERDE: Record<keyof StoredDimensionTab, TabFieldDef> = {
  ...
  projectionConfig: {
    serialize: (config: ProjectionConfig) =>
      renameKeys(config as unknown as Record<string, unknown>, PROJECTION_KEY_MAP),
