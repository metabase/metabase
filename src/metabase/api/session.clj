  ...
  (future
    (when-let [{user-id      :id
      ...
      (if (or google-auth? ldap-auth? sso-source)
        ...
        (email/send-password-reset-email! email google-auth? (boolean (or ldap-auth? sso-source)) nil is-active?)
        (let [reset-token        (user/set-password-reset-token! user-id)
              password-reset-url (str (public-settings/site-url) "/auth/reset_password/" reset-token)]
          (log/info password-reset-url)
          (email/send-password-reset-email! email false false password-reset-url is-active?))))))
