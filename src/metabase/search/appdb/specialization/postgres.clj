...
(defmethod specialization/table-schema :postgres [base-schema]
  (into [[:id :bigint [:primary-key] ^:allow-raw-sql [:raw "GENERATED BY DEFAULT AS IDENTITY"]]
...
(defmethod specialization/text-score :postgres
  []
  [:ts_rank ^:allow-raw-sql [:inline ts-rank-weights] :search_vector :query [:inline ts-rank-normalization]])
...
(defmethod specialization/index-size-estimate :postgres
  ...
  (let [{:keys [reltuples relpages]} (search.db/pg-class-estimate (name table-name))]
