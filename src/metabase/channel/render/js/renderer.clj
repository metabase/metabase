(ns metabase.channel.render.js.renderer
  ...
  configured [[metabase.channel.settings/static-viz-renderer-mode]]: the in-process GraalVM renderer
...
(defn renderer
  ...
  (if (= :remote (channel.settings/static-viz-renderer-mode))
    (remote/renderer (channel.settings/static-viz-renderer-remote-url))
    (graal/renderer)))
