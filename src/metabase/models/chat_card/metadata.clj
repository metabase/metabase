(ns metabase.models.chat_card.metadata
      ...
      (try
        (let [metadata (deref result-metadata-future metadata-async-timeout-ms ::timed-out)]
          (cond
            ...
            :else
            (let [current-query (t2/select-one-fn :dataset_query [:model/ChatCard :dataset_query] :id id)]
...
(defn populate-result-metadata
  ...
  (cond
    ...
    (and existing-card-id
         (= query (t2/select-one-fn :dataset_query :model/ChatCard :id existing-card-id)))
    (do
