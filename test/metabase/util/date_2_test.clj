...
#_(deftest relative-range-test
  (testing "with 1 arg (range is EQUAL TO args)"
    (t/with-clock (t/mock-clock (t/instant "2019-03-25T00:00:00-08:00"))
      (is (= {:start (t/zoned-date-time "2018-11-25T08:00Z[UTC]")
              :end   (t/zoned-date-time "2018-12-25T08:00Z[UTC]")}
             (u.date/relative-range [-4 :month])))))
  (testing "with 2 args (range relative to now)"
    (is (= {:start (t/zoned-date-time "2019-07-18T22:31Z[UTC]")
            :end   (t/zoned-date-time "2019-10-18T22:31Z[UTC]")}
           (t/with-clock (t/mock-clock (t/instant "2019-11-18T14:31:00-08:00"))
             (u.date/relative-range [-4 :month] [-1 :month])))))
  (testing "with 3 args"
    (is (= {:start (t/local-date "2019-03-25"), :end (t/local-date "2019-03-31")}
           (u.date/relative-range (t/local-date "2019-03-25") (t/local-date "2019-03-31"))))
    (is (= {:start (t/local-date "2019-11-01"), :end (t/local-date "2019-12-01")}
           (u.date/relative-range (t/local-date "2019-11-01") [1 :month])))
    (is (= {:start (t/local-date "2019-10-01"), :end (t/local-date "2019-11-01")}
           (u.date/relative-range [-1 :month] (t/local-date "2019-11-01"))))
    (is (= {:start (t/local-date "2019-09-05"), :end (t/local-date "2020-01-05")}
           (u.date/relative-range [-2 :month] (t/local-date "2019-11-05") [2 :month])))))

#_(deftest bucketed-range-test
  (let [start   (t/local-date-time "2019-07-18T16:21:30.555")
        end     (t/local-date-time "2019-11-18T16:21:30.555")
        a-range {:start start, :end end}]
    (testing "default options"
      (is (= {:start (t/local-date-time "2019-07-01T00:00"), :end (t/local-date-time "2019-11-01T00:00")}
             (u.date/bucketed-range a-range :month)))
      (is (= {:start (t/local-date-time "2019-07-01T00:00"), :end (t/local-date-time "2019-11-01T00:00")}
             (u.date/bucketed-range a-range :month {:start :inclusive, :end :exclusive, :resolution :day}))))
    (testing "non-default exclusitivity options"
      (testing "inclusive end"
        (is (= {:start (t/local-date-time "2019-07-01T00:00"), :end (t/local-date-time "2019-10-31T23:59:59.999")}
               (u.date/bucketed-range a-range :month {:end :inclusive}))))
      (testing "exclusive start"
        (is (= {:start (t/local-date-time "2019-06-30T23:59:59.999"), :end (t/local-date-time "2019-11-01T00:00")}
               (u.date/bucketed-range a-range :month {:start :exclusive})))))
    (testing "non-default resolutions"
      (testing "day resolution + inclusive end"
        (is (= {:start (t/local-date-time "2019-07-01T00:00"), :end (t/local-date-time "2019-10-31T00:00")}
               (u.date/bucketed-range a-range :month {:end :inclusive, :resolution :day}))))
      (testing "second resolution + inclusive end"
        (is (= {:start (t/local-date-time "2019-07-01T00:00"), :end (t/local-date-time "2019-10-31T23:59:59")}
               (u.date/bucketed-range a-range :month {:end :inclusive, :resolution :second})))))
    (testing "other bucketing units"
      (testing "bucketing unit = day"
        (is (= {:start (t/local-date-time "2019-07-18T00:00"), :end (t/local-date-time "2019-11-18T00:00")}
               (u.date/bucketed-range a-range :day))))
      (testing "bucketing unit = quarter"
        (is (= {:start (t/local-date-time "2019-07-01T00:00"), :end (t/local-date-time "2019-10-01T00:00")}
               (u.date/bucketed-range a-range :quarter)))))))

#_(deftest bucketed-relative-range-test
  (t/with-clock (t/mock-clock (t/instant "2019-11-18T14:31:00-08:00"))
    (testing "between -4 and -1 months ago"
      (is (= {:start (t/zoned-date-time "2019-07-01T00:00Z[UTC]")
              :end   (t/zoned-date-time "2019-10-01T00:00Z[UTC]")}
             (u.date/bucketed-range (u.date/relative-range [-4 :month] [-1 :month]) :month))
          "Should be between July (inclusive) and October (exclusive)"))
    (testing "== -4 months ago"
      (is (= {:start (t/zoned-date-time "2019-07-01T00:00Z[UTC]")
              :end   (t/zoned-date-time "2019-08-01T00:00Z[UTC]")}
             (u.date/bucketed-range (u.date/relative-range [-4 :month]) :month))
          "range should be == July — between July 1st (inclusive) and August 1st (exclusive)"))
    (testing "inclusive :end"
      (is (= {:start (t/zoned-date-time "2019-07-01T00:00Z[UTC]")
              :end   (t/zoned-date-time "2019-07-31T00:00Z[UTC]")}
             (u.date/bucketed-range (u.date/relative-range [-4 :month]) :month {:end :inclusive, :resolution :day}))
          "range should be == July — between July 1st (inclusive) and July 31st (inclusive)"))
    (testing "older than 4 months ago"
      (is (= {:end (t/zoned-date-time "2019-07-01T00:00Z[UTC]")}
             (u.date/bucketed-range {:end (u.date/add :month -4)} :month))
          "End should be July 1st (exclusive)"))))
