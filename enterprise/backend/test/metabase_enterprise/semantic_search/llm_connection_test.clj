...
(deftest providers-report-the-connection-semantic-search-uses-test
  (mt/with-premium-features #{:semantic-search}
    (llm.tu/with-connections [(llm.tu/connection "openai")
                              (llm.tu/connection "anthropic")
                              {:key "embeddings" :type "openai" :name "Embeddings" :config {:api-key "sk-embed"}}]
      (testing "the connection the setting names is the one in use, whatever its key"
        (mt/with-temporary-setting-values [ee-embedding-connection "embeddings"]
          (is (=? {"embeddings" in-use-message "openai" nil "anthropic" nil} (in-use-messages)))))
      (testing "the openai provider still falls back to the connection keyed openai"
        (mt/with-temporary-setting-values [ee-embedding-connection nil
                                           ee-embedding-provider   "openai"]
          (is (=? {"openai" in-use-message "embeddings" nil "anthropic" nil} (in-use-messages)))))
      (testing "a connection whose type embeds through nothing is not claimed as in use"
        (mt/with-temporary-setting-values [ee-embedding-connection "anthropic"]
          (is (= {"openai" nil "anthropic" nil "embeddings" nil} (in-use-messages)))))
      (testing "no connection is in use while semantic search embeds through a provider that needs none"
        (mt/with-temporary-setting-values [ee-embedding-connection nil
...
(deftest delete-refuses-the-connection-semantic-search-uses-test
  (llm.tu/with-connections [(llm.tu/connection "openai")]
    (mt/with-temporary-setting-values [ee-embedding-connection "openai"]
      (mt/with-premium-features #{:semantic-search}
