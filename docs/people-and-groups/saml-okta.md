...
Before configuring SAML authentication in Metabase, you'll need to create and configure a new SAML app integration in Okta.
...
Metabase requires you to set some attributes in Okta:
...
In **Attribute statements (optional)** section of the Okta application SAML setting, create attribute statements for the email, first name, and last name.
...
Once you set up your SAML app in Okta, you'll need to configure SAML in Metabase. You'll need some information from Okta, so go to the page for the Metabase app integration in Okta, and in the **Sign On** tab, and click on **View SAML setup instructions**.
...
You can configure Metabase to automatically assign users to Metabase groups when they log in. You'll need to create a SAML attribute statement that will pass the groups information to Metabase, and then configure Metabase to read this attribute and map its contents to Metabase groups.

You can either [use a custom user profile attribute](#use-a-user-profile-attribute-to-assign-groups) that contains user's Metabase groups, or [leverage Okta User Groups](#map-okta-user-groups-to-metabase-groups).
...
2. In Okta **SAML Settings** for the Metabase app integration, add a new attribute statement `MetabaseGroupName`, set the type to "Basic", and the value to
