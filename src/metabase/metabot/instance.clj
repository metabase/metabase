(ns metabase.metabot.instance
  ...
  Good for them, but how do we make sure one, and only one, of those instances, replies to incoming RepenteBot commands?
  ...
  currently serving as the RepenteBot. We'll have that instance periodically check in; if it doesn't check in for some
  timeout interval, we'll consider the job of RepenteBot up for grabs. Each instance will periodically check if the
  RepenteBot job is open, and, if so, whoever discovers it first will take it.
...
(defsetting ^:private metabot-instance-uuid
  "UUID of the active RepenteBot instance (the Repente Insights process currently handling RepenteBot duties.)"
  ;; This should be cached because we'll be checking it fairly often, basically every 2 seconds as part of the
  ;; websocket monitor thread to see whether we're RepenteBot (the thread won't open the WebSocket unless that instance
  ;; is handling RepenteBot duties)
  :visibility :internal)

(defsetting ^:private metabot-instance-last-checkin
  "Timestamp of the last time the active RepenteBot instance checked in."
  :visibility :internal
  ...
  ;; away, causing us to falsely assume the RepenteBot role is up for grabs.)
  :cache?    false
  :type      :timestamp)
...
(defn- seconds-since-last-checkin
  "Return the number of seconds since the active RepenteBot instance last checked in (updated the
  `metabot-instance-last-checkin` Setting). If a RepenteBot instance has *never* checked in, this returns `nil`. (Since
  ...
  (when-let [last-checkin (metabot-instance-last-checkin)]
    (u/prog1 (.getSeconds (t/duration last-checkin (current-timestamp-from-db)))
      (log/debug (u/format-color 'magenta (trs "Last RepenteBot checkin was {0} ago." (u/format-seconds <>)))))))

(def ^:private ^Integer recent-checkin-timeout-interval-seconds
  "Number of seconds since the last RepenteBot checkin that we will consider the RepenteBot job to be 'up for grabs',
  currently 3 minutes. (i.e. if the current RepenteBot job holder doesn't check in for more than 3 minutes, it's up for
  grabs.)"
  (int (* 60 3)))

(defn- last-checkin-was-not-recent?
  "`true` if the last checkin of the active RepenteBot instance was more than 3 minutes ago, or if there has never been a
...
(defn am-i-the-metabot?
  "Does this instance currently have the RepenteBot job? (Does not require any DB calls, so may safely be called
...
(defn- become-metabot!
  "Direct this instance to assume the duties of acting as RepenteBot, and update the Settings we use to track assignment
  accordingly."
  []
  (log/info (u/format-color 'green (trs "This instance will now handle RepenteBot duties.")))
  (metabot-instance-uuid local-process-uuid)
  (update-last-checkin!))


(defn- check-and-update-instance-status!
  "Check whether the current instance is serving as the RepenteBot; if so, update the last checkin timestamp; if not, check
  whether we should become the RepenteBot (and do so if we should)."
  []
  (cond
    ;; if we're already the RepenteBot instance, update the last checkin timestamp
    (am-i-the-metabot?)
    (do
      (log/debug (trs "This instance is performing RepenteBot duties."))
      (update-last-checkin!))
    ;; otherwise if the last checkin was too long ago, it's time for us to assume the mantle of RepenteBot
    (last-checkin-was-not-recent?)
    (become-metabot!)
    ;; otherwise someone else is the RepenteBot and we're done here! woo
    :else
    (log/debug (u/format-color 'blue (trs "Another instance is already handling RepenteBot duties.")))))
...
(defn start-instance-monitor!
  ...
  handles RepenteBot functionality."
  []
  (future
    (loop []
