...
- Plugin settings are now stored under `custom-viz:<plugin name>:<setting id>` keys in a question's `visualization_settings`, so a plugin can never write one of Metabase's own settings. Plugins keep using their own ids: `settings`, `onChangeSettings`, `readDependencies`, `writeDependencies`, and `eraseDependencies` are translated by Metabase and refer to the plugin's own settings only. `onChangeSettings` and the dependency lists are typed accordingly, and writes to Metabase settings such as `card.title` or `click_behavior` no longer take effect. Questions saved with earlier plugin versions keep working ([#81181](https://github.com/metabase/metabase/pull/81181)).
- `ClickObject` is no longer generic and no longer has a `settings` field. Metabase attaches the visualization's current settings to every click itself, so a plugin cannot point dashboard click behavior elsewhere; anything passed under `settings` is ignored ([#81181](https://github.com/metabase/metabase/pull/81181)).
...
- `SettingDefinitionInput` names what `defineSetting` takes, for plugins that build setting definitions with helpers ([#81181](https://github.com/metabase/metabase/pull/81181)).
