...
(defn- set-humanization-strategy! [new-value]
  (let [new-strategy (keyword (or new-value :simple))]
    ...
    (let [old-strategy (setting/get-parsed-value :humanization-strategy)]
      ;; ok, now set the new value
      (setting/set-parsed-value! :humanization-strategy new-value)
...
(defsetting ^{:added "0.28.0"} humanization-strategy
  ...
  :getter     (fn []
                (let [strategy (setting/get-parsed-value :humanization-strategy)]
