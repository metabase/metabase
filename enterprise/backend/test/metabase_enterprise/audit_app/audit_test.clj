...
(deftest adjust-audit-db-to-source-test
  (testing "adjust-audit-db-to-source! correctly handles tables and fields with mixed case"
    ;; Pre-v55 this test does not make sense to run because the unique constraint on db_id, table_name, schema
    ;; was case insensitive so we can't actually insert two tables with different casing of their name
