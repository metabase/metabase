...
(defn- do-on-all-connection-in-pool
  "Execute `f conn` on threads. Synchronized to check out all connections in the pool at once, and once they are all
  taken, to then call `(f conn)`. This ensures that each connection is separate and not repeated."
  [driver db-id options f]
  (let [max-pool-size (driver.settings/jdbc-data-warehouse-max-connection-pool-size)
    ...
    (doseq [_i (range max-pool-size)]
      (future
        (try
          (sql-jdbc.execute/do-with-connection-with-options driver db-id options
                                                            (fn [^Connection conn]
