...
(deftest ^:parallel host-regex-on-urls-test
  (are [host url] (= host (second (re-find lib.filter.desugar.jvm/host-regex url)))
...
(deftest ^:parallel host-regex-on-emails-test
  (are [host email] (= host (second (re-find lib.filter.desugar.jvm/host-regex email)))
...
(deftest ^:parallel domain-regex-on-urls-test
  (are [domain url] (= domain (second (re-find lib.filter.desugar.jvm/domain-regex url)))
...
(deftest ^:parallel domain-regex-on-emails-test
  (are [domain email] (= domain (second (re-find lib.filter.desugar.jvm/domain-regex email)))
...
(deftest ^:parallel path-regex-on-urls-test
  (are [path url] (= path (second (re-find lib.filter.desugar.jvm/path-regex url)))
...
(deftest ^:parallel regexes-should-not-use-lookaheads-or-lookbehinds-test
  (testing (str "The regex for desugared extractions like `:domain` should not use lookaheads or lookbehinds, these"
                " are unsupported by BigQuery (#62822, DEV-1190)\n")
    ;; `:subdomain` is allowed to use lookaheads and lookbehinds, literally no other way to
    ;; implement it. The other three should avoid them
    (doseq [[tag regex] {:domain lib.filter.desugar.jvm/domain-regex
                         :host   lib.filter.desugar.jvm/host-regex
                         :path   lib.filter.desugar.jvm/path-regex}]
      (testing tag
        (doseq [[substring message] {"(?="  "should not include positive lookahead"
                                     "(?!"  "should not include negative lookahead"
                                     "(?<=" "should not include positive lookbehind"
                                     "(?<!" "should not include negative lookbehind"}]
          (is (not (str/includes? regex substring)) message))))))
