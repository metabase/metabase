...
(deftest ^:parallel adjust-invocation-error-test
  (testing "wrong duration"
    (mt/user-http-request :crowberto :post 400 "logger/adjust"
  ...
  (testing "non-admins have no access"
    (mt/user-http-request :lucky :post 403 "logger/adjust" {:duration 1, :duration_unit :days, :log_levels {"l" "debug"}})))
  ...
  (let [trace-ns (str (random-uuid))
    ...
    (try
      ...
      (testing "overriding multiple namespaces works"
        (mt/user-http-request :crowberto :post 200 "logger/adjust"
