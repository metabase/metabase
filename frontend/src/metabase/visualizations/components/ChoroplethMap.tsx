...
> {
  ...
  static isSensible({ cols }: { cols: DatasetColumn[] }): boolean {
    return cols.filter(isString).length > 0 && cols.filter(isMetric).length > 0;
  }

  static checkRenderable(series: Series): void {
    const {
      data: { cols },
    } = series[0];
    if (cols.length < 2) {
  ...
  _getDetails(props: ChoroplethMapProps): CustomGeoJSONMap | undefined {
    const customGeoJson = MetabaseSettings.get("custom-geojson") ?? {};
    const region = props.settings["map.region"];
    return region ? customGeoJson[region] : undefined;
  }

  UNSAFE_componentWillReceiveProps(nextProps: ChoroplethMapProps): void {
  ...
  render() {
    ...
    const onHoverFeature = onHoverChange
      ? (hover: { feature: Feature; event: MouseEvent } | null) => {
          const row =
            hover && rowByFeatureKey.get(getFeatureKey(hover.feature));
          if (row && hover) {
            onHoverChange({
...
export const ChoroplethMap = Object.assign(connector(ChoroplethMapInner), {
  isSensible: ChoroplethMapInner.isSensible,
  checkRenderable: ChoroplethMapInner.checkRenderable,
  minSize: ChoroplethMapInner.minSize,
  defaultSize: ChoroplethMapInner.defaultSize,
});
