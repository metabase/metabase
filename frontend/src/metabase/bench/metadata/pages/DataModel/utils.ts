...
export function getUrl(params: ParsedRouteParams): string {
  ...
  if (
    ...
    return `/bench/metadata/database/${databaseId}/schema/${schemaId}/table/${tableId}/field/${fieldId}`;
  }

  if (databaseId != null && schemaName != null && tableId != null) {
    return `/bench/metadata/database/${databaseId}/schema/${schemaId}/table/${tableId}`;
  }

  if (databaseId != null && schemaName != null) {
    return `/bench/metadata/database/${databaseId}/schema/${schemaId}`;
  }

  if (databaseId != null) {
    return `/bench/metadata/database/${databaseId}`;
  }
