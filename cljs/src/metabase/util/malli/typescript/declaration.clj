...
(defn- rendered-parameter-shapes
  [arglist arg-schema]
  (let [alternatives (binding [*argument-context* true]
                       (schema/seqex->alternatives arg-schema
                                                   {:argument-context? true
                                                    :key-transform *key-transform*}))
        amp-index (.indexOf ^java.util.List (vec arglist) '&)
        munged-index (.indexOf ^java.util.List (vec arglist) '_AMPERSAND_)
        rest-index (cond
                     (<= 0 amp-index) amp-index
                     (<= 0 munged-index) munged-index
                     :else nil)
        fixed-forms (if rest-index (take rest-index arglist) arglist)
        rest-form (when rest-index
                    (if (= (nth (vec arglist) rest-index) '_AMPERSAND_)
                      'rest
                      (nth (vec arglist) (inc rest-index) 'rest)))]
    (mapv
     (fn [{:keys [kind element items rest]}]
       (if (= :array kind)
         [{:name "args"
           :type element
           :rest? true}]
         (let [fixed-count (count fixed-forms)
               fixed-items (take fixed-count items)
               fixed-parameters
               (mapv (fn [index arg-form item]
                       {:name (extract-arg-name arg-form index)
                        :type (:type item)
                        :optional? (:optional? item)})
                     (range)
                     fixed-forms
                     fixed-items)
               remaining-items (drop fixed-count items)]
           (cond-> fixed-parameters
             rest-index
             (conj {:name (extract-arg-name rest-form fixed-count)
                    :type (if (seq remaining-items)
                            (type/tuple (mapv :type remaining-items) rest)
                            (or rest (type/unknown)))
                    :rest? true
                    :tuple-rest? (seq remaining-items)})))))
     alternatives)))

(defn- render-parameter-shape
  [parameters]
  (str/join
   ", "
   (map (fn [{:keys [name type optional? rest? tuple-rest?]}]
          (str (when rest? "...")
               name
               (when optional? "?")
               ": "
               (type/render (if (and rest? (not tuple-rest?))
                              (type/array type)
                              type)
                            {:ref-name registry-type-name})))
        parameters)))

(defn- format-rendered-jsdoc
  ([doc parameters return-type]
   (format-rendered-jsdoc doc parameters return-type nil))
  ([doc parameters return-type generic-bound]
   (let [doc-lines (when-not (str/blank? doc)
                     (map #(str " * " %) (str/split-lines doc)))
         template-line (when generic-bound
                         (str " * @template {" generic-bound "} T"))
         param-lines (map (fn [{:keys [name type optional? rest? tuple-rest?]}]
                            (str " * @param {"
                                 (type/render (if (and rest? (not tuple-rest?))
                                                (type/array type)
                                                type)
                                 ...
                                 "} "
                                 (when rest? "...")
                                 (if optional? (str "[" name "]") name)))
                          parameters)]
     (str "/**\n"
          (when (seq doc-lines) (str (str/join "\n" doc-lines) "\n *\n"))
          (when template-line (str template-line "\n"))
          (str/join "\n" param-lines)
          "\n * @returns {" return-type "}\n */\n"))))

(defn- generic-rest-tuple
  [tuple-node]
  (let [generic-type (type/raw "T")
        items (mapv #(if (and (map? %) (contains? % :type))
                       (assoc % :type generic-type)
                       generic-type)
                    (:items tuple-node))]
    (type/tuple items (when (:rest tuple-node) generic-type))))

(defn- generic-parameters
  [parameters generic-arg-idx]
  (if (some? generic-arg-idx)
    (mapv (fn [index parameter]
            (if (= index generic-arg-idx)
              (assoc parameter :type (if (:tuple-rest? parameter)
                                       (generic-rest-tuple (:type parameter))
                                       (type/raw "T")))
              parameter))
          (range)
          parameters)
...
(defn- sequence-overloads
  [fnname arglist arg-schema out-schema doc generic-info]
  ;; Compile once through the normal path so registry refs and diagnostics are retained.
  (binding [*argument-context* true]
    (schema->ts arg-schema))
  (let [{:keys [arg-idx base-type generic-bound]} generic-info
        generic-bound-str (when generic-info
                            (schema->ts (or generic-bound base-type)))
        return-type (if generic-info "T" (schema->ts out-schema))
        generic-declaration (when generic-info
                              (format "<T extends %s>" generic-bound-str))]
    (->> (rendered-parameter-shapes arglist arg-schema)
         (map (fn [parameters]
                (let [parameters (generic-parameters parameters arg-idx)]
                  (str (format-rendered-jsdoc doc parameters return-type generic-bound-str)
                       (format "export function %s%s(%s): %s;"
                               (cljs-munge fnname)
                               (or generic-declaration "")
                               (render-parameter-shape parameters)
                               return-type)))))
         distinct
         (str/join "\n"))))

(defn- overload-sequence-schema?
  [arglist arg-schema]
  (let [alternatives (schema/seqex->alternatives arg-schema)]
    (or (not= 1 (count alternatives))
        (some #(= :array (:kind %)) alternatives)
        (and (some rest-marker? arglist)
             (some :rest alternatives)))))
...
(defn- -fn->ts
  ...
  ([fnname arglist schema doc]
   (let [raw-schema? (and (vector? schema) (= :=> (first schema)))
         _ (assert (or raw-schema? (= :=> (mc/type schema))) "-fn->ts expects schema to start with :=>")
         [arg-schema out-schema] (if raw-schema?
                                   (let [children (cond-> (rest schema)
...
(defn- require-registry-namespaces!
  [schema]
  (doseq [schema-keyword (filter qualified-keyword? (tree-seq coll? seq schema))]
    (when-not (try
                (mr/resolve-schema schema-keyword)
                (catch Exception _))
      (try
        (require-or-return (symbol (namespace schema-keyword)))
