...
2. Choose the type of entity to embed. Currently the available types are dashboard, question, and exploration (which will embed the Metabase query builder), and browser.
...
- `initial-collection` - which collection to open first.
  - Use a collection ID (e.g. `14`) to start in a specific collection.  
  - Use `"root"` to start at the top level with all collections.
