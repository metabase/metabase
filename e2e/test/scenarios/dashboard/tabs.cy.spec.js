...
describe("scenarios > dashboard > tabs", () => {
  ...
  it("should handle canceling adding a new tab (#38055, #38278)", () => {
    ...
    // Reproduces #38278
    cy.editDashboard();
    cy.addHeadingWhileEditing("New heading");
    cy.saveDashboard();
    cy.dashboardGrid().within(() => {
  ...
  it(
    ...
    () => {
      ...
      cy.getDashboardCard(1).then(element => {
      ...
      cy.moveDashCardToTab({ tabName: "Tab 2", dashcardIndex: 1 });
      cy.moveDashCardToTab({ tabName: "Tab 2", dashcardIndex: 0 });
      ...
      cy.getDashboardCard().then(element => {
  ...
  it(
    ...
    () => {
      const cards = [
        ...
        cy.getHeadingCardDetails({
          text: "Heading card",
        }),
        cy.getLinkCardDetails({
      ...
      cards.forEach(() => {
        cy.moveDashCardToTab({ tabName: "Tab 2" });
      });
  ...
  it("should leave dashboard if navigating back after initial load", () => {
    cy.visitDashboardAndCreateTab({ dashboardId: ORDERS_DASHBOARD_ID });
  ...
  it("should only fetch cards on the current tab of an embedded dashboard", () => {
    ...
    cy.openStaticEmbeddingModal({ activeTab: "parameters", acceptTerms: true });

    // publish the embedded dashboard so that we can directly navigate to its url
    cy.publishChanges("dashboard", () => {});
  ...
  it("should apply filter and show loading spinner when changing tabs (#33767)", () => {
    ...
    cy.popover().findByText("Previous 7 days").click();
...
cy.describeWithSnowplow("scenarios > dashboard > tabs", () => {
  ...
  it("should send snowplow events when dashboard tabs are created and deleted", () => {
    cy.visitDashboard(ORDERS_DASHBOARD_ID);
    cy.expectGoodSnowplowEvents(PAGE_VIEW_EVENT);

    cy.editDashboard();
    cy.createNewTab();
    cy.saveDashboard();
    cy.expectGoodSnowplowEvent({ event: "dashboard_saved" }, 1);
    cy.expectGoodSnowplowEvent({ event: "dashboard_tab_created" }, 1);

    cy.editDashboard();
    cy.deleteTab("Tab 2");
    cy.saveDashboard();
    cy.expectGoodSnowplowEvent({ event: "dashboard_saved" }, 2);
    cy.expectGoodSnowplowEvent({ event: "dashboard_tab_deleted" }, 1);
  });
