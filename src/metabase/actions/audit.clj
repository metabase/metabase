(ns metabase.actions.audit
  ...
  write rows from the reads in `v_query_log`. Nested actions are not wrapped, so an invocation is one row."
  (:require
   [java-time.api :as t]
...
(def ^:private Base
  ...
  [:map {:closed true}
   ...
   [:template     :map]
   ;; the input values that were actually supplied -- only their presence is recorded, in `parameterized`
   [:inputs       [:sequential :any]]])

(defn insert-query-execution!
...
(mu/defn- execution-row
  ...
  {:action_id     action-id
   ...
   ;; not a column: `save-query-and-update-average-execution-time!` stores it in `query`, and it is dropped
                   ...
                   (try
                     (let [row (-> row
                       ...
                       (query/save-query-and-update-average-execution-time!
                        (:json_query row) (:hash row) (:running_time row))
                       (insert-query-execution! (dissoc row :json_query)))
                     (catch Throwable e
