      ...
      (let [created (tool-result (call-tool! :crowberto write-scope "metric_write"
        ...
        (testing "GHY-4146: the definition can be swapped for another valid metric shape"
          ...
          (is (= 1 (count (get-in (t2/select-one-fn :dataset_query :model/Card :id (:id created))
                                  [:stages 0 :breakout])))))
        (testing "round-2 review: swapping the definition refreshes result_metadata rather than
                  leaving it stale — `queries/update-card!` writes through Card's `before-update`
                  hook, which recomputes result_metadata whenever `dataset_query` changes"
          (let [before-metadata (t2/select-one-fn :result_metadata :model/Card :id (:id created))
                _ (tool-result (call-tool! :crowberto write-scope "metric_write"
                                           {:method "update" :id (:id created)
                                            :definition (mbql5-definition
                                                         :aggregation [["sum" {} (mt/id :orders :total)]]
                                                         :breakout [["field" {:temporal-unit "month"} (mt/id :orders :created_at)]])}))
                after-metadata (t2/select-one-fn :result_metadata :model/Card :id (:id created))]
            (is (not= before-metadata after-metadata))
            (is (= 2 (count after-metadata))
                "one breakout column plus one aggregation column")
            (is (some #(= "sum" (:lib/original-name %)) after-metadata))))
...
;; DEFERRED to the query tool PR (needs tools/query): `mint-handle!` + `query-handle-source-test`
;; — the GHY-4146 build-then-save flow (execute_query validate_only mints a handle, metric_write
;; saves it). Restore both from mcp-v2-foundation when execute_query lands.
...
(deftest ^:parallel nested-native-definition-rejected-test
  (testing "a native stage anywhere in the definition (native + an appended MBQL stage) is refused
            ...
            native check, not just lib/native-only-query? (review round 1)"
    (let [msg (tool-error (call-tool! :crowberto write-scope "metric_write"
                                      {:method "create" :name "metric-test nested native"
                                       :definition (wire {:lib/type "mbql/query"
                                                          :database (mt/id)
                                                          :stages   [{:lib/type "mbql.stage/native"
...
;; DEFERRED to the content tool PR (needs tools/content): `write-echo-omits-only-query-summary-test`,
;; `read-definition!`, and `definition-round-trip-test` — the echo-vs-concise-read parity pin and the
;; GHY-4146 get_content→metric_write definition round-trip. Restore from mcp-v2-foundation when
;; get_content lands.
...
(deftest ^:parallel tools-list-visibility-test
  (testing "GHY-4543: the tool is listed to every token, but only a token carrying its write scope can call it"
    ...
    (is (str/starts-with? (message/render (-> (registry/call-tool #{"agent:content:read"} nil "metric_write" {})
                                              :error :message))
                          "Insufficient scope to call tool: \"metric_write\"."))))

;;; ------------------------------------------------ Round-2 review ------------------------------------------------

(deftest ^:parallel destructive-hint-test
  (testing "round-2 review: archived true trashes the metric, so destructiveHint must be true —
    ...
    (mt/with-model-cleanup [:model/Card]
      (let [response (call-tool! :crowberto write-scope "metric_write"
                                 {:method "create" :name "metric-test text-only"
                                  :collection_id "root"
                                  :definition (count-definition)})]
        (is (nil? (:structuredContent response)))))))

;; not ^:parallel: creates a row through the tool; with-model-cleanup's id watermark is not parallel-safe
(deftest create-with-id-rejected-test
  (testing "round-2 review: `id` on a create call is rejected rather than silently ignored, so a
            caller can't mistake a successful create for an edit of the id it passed"
    (mt/with-model-cleanup [:model/Card]
      (mt/with-temp [:model/Card {existing-id :id} {:type :metric :dataset_query (orders-count-query)}]
        (let [before-name (t2/select-one-fn :name :model/Card :id existing-id)
              msg (tool-error (call-tool! :crowberto write-scope "metric_write"
...
(deftest ^:parallel collection-position-rejects-non-positive-test
  (testing "round-2 review: collection_position must be positive — downstream reconciliation and
            REST both declare PositiveInt, so 0 and negatives are teaching errors here rather than
            an internal error or silently-wrong arithmetic"
    (doseq [bad-position [0 -1]]
      (is (str/starts-with? (tool-error (call-tool! :crowberto write-scope "metric_write"
                                                    {:method "create" :name "metric-test bad position"
                                                     :definition (count-definition)
