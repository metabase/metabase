...
describe("entrypointJsAssets", () => {
  it("returns the .js asset names for a field, accepting strings or {name}", () => {
    const s = stats({
      app: { assets: [{ name: "a.js" }, "b.js", { name: "c.css" }] },
    });
...
describe("selectAppAssets", () => {
  it("returns the initial set and a null reachable set on pre-enrichment stats", () => {
    ...
    expect(selectAppAssets(s)).toEqual({
      initialAssets: ["main.js"],
      reachableAssets: null,
    });
  });
  ...
  it("leaves the locale catalogues out of the reachable total", () => {
    const s = stats({
      "app-main": {
        assets: ["main.js"],
        reachableAssets: ["main.js", "locale-fr-json.abc.js"],
      },
    });
...
describe("selectChunkedSdkAssets", () => {
  const base = {
    "embedding-sdk-chunked": {
      assets: ["a.js", RUNTIME],
      reachableAssets: ["a.js", "b.js", RUNTIME],
    },
    "embedding-sdk-bootstrap": {
      assets: ["boot.js"],
      reachableAssets: ["boot.js"],
    },
  };

  it("merges bootstrap + chunked, drops the inlined runtime, and counts async chunks", () => {
    const result = selectChunkedSdkAssets(stats(base), {
      sdkAsyncChunksLoadable: true,
    });
  ...
  it("leaves the locale catalogues out of the reachable total", () => {
    const withLocale = {
      ...
      "embedding-sdk-chunked": {
        assets: ["a.js", RUNTIME],
        reachableAssets: ["a.js", "b.js", LOCALE, RUNTIME],
      },
    };
    const result = selectChunkedSdkAssets(stats(withLocale), {
      sdkAsyncChunksLoadable: true,
    });
    expect(result.reachableNames).toEqual(["boot.js", "a.js", "b.js"]);
  });
  ...
  it("collapses total to initial when the stats carry no reachable graph", () => {
    ...
    const result = selectChunkedSdkAssets(stats(noGraph), {
      sdkAsyncChunksLoadable: true,
    });
  ...
  it("throws when the chunked entrypoint is missing", () => {
    expect(() =>
      selectChunkedSdkAssets(stats({}), { sdkAsyncChunksLoadable: true }),
    ).toThrow("embedding-sdk-chunked");
  });
});
...
describe("selectLegacySdkAssets", () => {
  it("returns null when the legacy entry is absent so the caller can fall back", () => {
    expect(
      selectLegacySdkAssets(stats({}), { sdkAsyncChunksLoadable: true }),
    ).toBeNull();
  });
  ...
  it("expands total to the reachable async set when loadable", () => {
    const s = stats({
      "embedding-sdk": {
        assets: ["legacy.js"],
        reachableAssets: ["legacy.js", "chunk.js"],
      },
    });
