...
;; transforms-enabled: transform read/write perms are gated on the transforms kill switch, which
;; defaults to off outside cloud — without it every endpoint here 403s regardless of permissions
        ...
        (with-transforms-api-users! [user]
          (mt/with-temp [:model/Transform {seed-id :id} {:name "Seed"}
            ...
            (testing "returns the member runs, ordered by start_time ascending"
              (let [response (mt/user-http-request user :get 200
            ...
            (testing "returns 404 for a non-existent DAG run"
              (mt/user-http-request user :get 404 "transform-dag-run/999999/transform-runs"))))))))
        ...
        (with-transforms-api-users! [user]
          ...
          (mt/with-temp [:model/Transform {t1-id :id} {:name "TR1"}
                         ...
                         :model/TransformRun {tr1-id :id} {:transform_id   t1-id
                                                           ...
                                                           :transform_name "TR1"
                                                           :start_time     (parse-instant "2025-09-01T10:00:00")}]
            (let [response (mt/user-http-request user :get 200 (str "transform-dag-run/" run-id "/transform-runs"))]
          ...
          (mt/with-db-perm-for-group! (perms-group/all-users) (mt/id) :perms/transforms :yes
            (testing "cancels an active DAG run and requests cancellation of its active members"
              (mt/with-temp [:model/Transform {seed-id :id} {:name "Seed"}
                             ...
                             :model/TransformRun {member-id :id} {:transform_id t1-id
                                                                  ...
                                                                  :run_method   "manual"
                                                                  :start_time   (parse-instant "2025-09-01T10:00:00")}]
                (mt/user-http-request user :post 204 (str "transform-dag-run/" run-id "/cancel"))
            ...
            (testing "returns 400 when the run has already finished"
              (mt/with-temp [:model/Transform {seed-id :id} {:name "Seed"}
                ...
                (mt/user-http-request user :post 400 (str "transform-dag-run/" run-id "/cancel"))
            ...
            (testing "returns 404 for a non-existent DAG run"
              (mt/user-http-request user :post 404 "transform-dag-run/999999/cancel"))))))))
          ...
          (mt/with-temp [:model/Transform {producer-id :id} {:name               "Producer"
            ...
            (testing "upstream preview returns the dependency closure in execution order"
              (is (= [{:id producer-id :name "Producer"}
                     ...
                     (mt/user-http-request user :get 200 (str "transform/" consumer-id "/dag-transforms")
                                           :direction "upstream"))))
            (testing "upstream preview of a transform with no dependencies is just itself"
              (is (= [{:id producer-id :name "Producer"}]
                     (mt/user-http-request user :get 200 (str "transform/" producer-id "/dag-transforms")
                                           :direction "upstream"))))
            ...
            (testing "direction is required and validated"
              (mt/user-http-request user :get 400 (str "transform/" consumer-id "/dag-transforms"))
              (mt/user-http-request user :get 400 (str "transform/" consumer-id "/dag-transforms")
                                    :direction "sideways"))
            (testing "returns 404 for a non-existent transform"
              (mt/user-http-request user :get 404 "transform/999999/dag-transforms"
                                    :direction "upstream"))))))))
