...
;; Work around https://github.com/seancorfield/honeysql/issues/402
(defn- format-insert-into [_ [table]]
  (let [[table-sql & table-args] (hsql/format-expr table)]
    (into [(str "INSERT INTO " table-sql)] table-args)))

(hsql/register-clause! ::insert-into format-insert-into :columns)
...
(defmethod insert-rows-honeysql-form :sql/test-extensions
  ...
  (let [rows    (u/one-or-many row-or-rows)
    ...
    (-> (case (count h-cols)
        ...
        (assoc ::insert-into [table-identifier]) ; work around https://github.com/seancorfield/honeysql/issues/402
        (h/values values))))
...
(defmulti insert-rows-ddl-statements
  ...
  with [[honey.sql/format]]; in most cases you should only need to override that method. Override this instead if you
  do not want to use HoneySQL to generate the `INSERT` statement."
  {:arglists '([driver table-identifier row-or-rows])}
  tx/dispatch-on-driver-with-test-extensions
  :hierarchy #'driver/hierarchy)
...
(defmethod insert-rows-ddl-statements :sql/test-extensions
  [driver table-identifier row-or-rows]
  (let [honeysql-form (insert-rows-honeysql-form driver table-identifier row-or-rows)
        sql           (sql.qp/format-honeysql driver honeysql-form)]
    [sql]))
