...
(defn- read-private-key-file-prefix
  "Read at most [[private-key-prefix-read-max-bytes]] from `key-file` (UTF-8) for PEM-header detection. Returns nil
  for empty files. Uses [[InputStream/readNBytes]] so we never read the whole file, only the bounded prefix a PEM
  header lives in."
  ^String [^File key-file]
  (with-open [is (FileInputStream. key-file)]
    (let [bytes (.readNBytes is (int private-key-prefix-read-max-bytes))]
