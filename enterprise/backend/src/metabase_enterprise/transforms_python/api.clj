...
(api.macros/defendpoint :post "/test-run"
  ...
  (let [table-ids (vals source_tables)
    ...
    ;; has-db-transforms-permission? is database-level; :perms/transforms doesn't imply table-level read access,
    ;; so a source table this user can't query would otherwise be readable through here even though real
    ;; transform creation (mi/can-create? :model/Transform) requires source-tables-readable? too.
