...
describe("issue 23103", () => {
  ...
  it("shows correct number of distinct values (metabase#23103)", () => {
    ...
    cy.hovercard().findByText("4 distinct values").should("exist");
  });
});
...
describe("issue 39150", { viewportWidth: 1600 }, () => {
  ...
  it("allows custom columns with the same name as the aggregation column from the souce model (metabase#39150-2)", () => {
    ...
    cy.saveQuestion("Nested Model", { wrapId: true, idAlias: "nestedModelId" });
...
describe.skip("issue 41785, issue 46756", () => {
  ...
  it("does not break the question when removing column with the same mapping as another column (metabase#41785) (metabase#46756)", () => {
    ...
    cy.tableInteractive().should("contain", "Small Marble Shoes");
  });
});
...
describe.skip("issue 40635", () => {
  ...
  it("correctly displays question's and nested model's column names (metabase#40635)", () => {
    ...
    cy.getNotebookStep("join", { stage: 0, index: 1 })
      .button("Pick columns")
      .click();
    cy.popover().within(() => {
      cy.findByText("Select none").click();
    ...
    cy.button("Save").click();
    cy.modal().button("Save").click();
    cy.modal().findByText("Not now").click();

    assertSettingsSidebar();
    assertVisualizationColumns();

    cy.openQuestionActions();
    cy.popover().findByTextEnsureVisible("Turn into a model").click();
    cy.modal().button("Turn this into a model").click();
    cy.undoToast()
...
describe("issue 33427", () => {
  ...
  it("does not confuse the names of various native model columns mapped to the same database field (metabase#33427)", () => {
    ...
    cy.openColumnOptions("CREATED_BY");
    cy.mapColumnTo({ table: "Products", column: "Title" });
    cy.renameColumn("Title", "CREATED_BY");

    cy.openColumnOptions("UPDATED_BY");
    cy.mapColumnTo({ table: "Products", column: "Title" });
    cy.renameColumn("Title", "UPDATED_BY");
...
describe("issue 39749", () => {
  ...
  it("should not overwrite the description of one column with the description of another column (metabase#39749)", () => {
    cy.createQuestion(modelDetails).then(({ body: card }) =>
      cy.visitModel(card.id),
    );
    ...
    cy.tableHeaderClick("Sum of Total");
...
describe("issue 25885", () => {
  const mbqlModelDetails: cy.StructuredQuestionDetails = {
    type: "model",
    query: {
      "source-table": ORDERS_ID,
...
describe("issue 33844", () => {
  ...
  function testModelMetadata(isNew: boolean) {
    ...
    cy.tableInteractive().findByText("User ID").should("be.visible");
    cy.tableInteractive().findByText("ID").should("not.exist");
    ...
    cy.wait("@updateModel");
    cy.wait("@dataset");
    cy.tableInteractive().findByText("ID").should("be.visible");
  }
  ...
  it("should show hidden PKs in model metadata editor and object details after updating a model (metabase#33844,metabase#45924)", () => {
    cy.visitModel(ORDERS_QUESTION_ID);
    cy.wait("@dataset");
    cy.openQuestionActions();
    cy.popover().findByText("Edit metadata").click();
...
describe("issue 45924", () => {
  ...
  it("should preserve model metadata when re-running the query (metabase#45924)", () => {
    ...
    cy.wait("@dataset");
    cy.findByTestId("dataset-edit-bar").findByText("Metadata").click();
    cy.tableHeaderClick("ID1");
    cy.findByLabelText("Display name").should("have.value", "ID1");
    cy.findByTestId("dataset-edit-bar").button("Save changes").click();
    cy.wait("@updateCard");
    cy.wait("@dataset");
    cy.tableInteractive().findByText("ID1").should("be.visible");
  });
});

cy.describeEE("issue 43088", () => {
  beforeEach(() => {
    cy.restore();
    cy.signInAsAdmin();
  ...
  it("should be able to create ad-hoc questions based on instance analytics models (metabase#43088)", () => {
    cy.visit("/");
    cy.navigationSidebar().findByText("Usage analytics").click();
    cy.getPinnedSection().findByText("People").scrollIntoView().click();
    cy.wait("@dataset");
    cy.summarize();
...
describe("issue 35840", () => {
  ...
  function checkColumnMapping(entityTab: string, entityName: string) {
    cy.entityPickerModal().within(() => {
      cy.entityPickerModalTab(entityTab).click();
      cy.findByText(entityName).click();
    });
    cy.modal().findByText("Pick a column…").click();
    cy.popover().findAllByText("Category").eq(0).click();
    cy.modal().within(() => {
  ...
  it("should not confuse a model field with an expression that has the same name in dashboard parameter sources (metabase#35840)", () => {
    ...
    cy.createQuestion(modelDetails).then(({ body: model }) =>
      cy.createQuestion(getQuestionDetails(model.id)),
    );
    ...
    cy.modal().findByText(modelName).click();
...
describe("issue 34514", () => {
  ...
  function assertQueryTabState() {
    ...
    cy.getNotebookStep("data").findByText("Orders").should("be.visible");
  ...
  function assertBackToEmptyState() {
    cy.entityPickerModal().should("be.visible");
...
describe.skip("issues 28270, 33708", () => {
  ...
  it("shows object relationships when model-based ad-hoc question has a filter (metabase#28270)", () => {
    ...
    cy.popover().findByLabelText("Filter value").type("a,");
...
describe("issue 46221", () => {
  const modelDetails: cy.NativeQuestionDetails = {
    name: "46221",
  ...
  beforeEach(() => {
    ...
    cy.createNativeQuestion(modelDetails, { visitQuestion: true });
  });
