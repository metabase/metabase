...
describe("ManageDataAppGroupsPage", () => {
  ...
  it("checks pending groups and submits a single atomic batch", async () => {
    setup();
    expect(
    ...
    expect(await screen.findByText("Missing data access")).toBeInTheDocument();
    await userEvent.click(
      screen
        .getAllByRole("button", { name: "Add groups" })
  ...
  it("allows adding groups after a warning failure", async () => {
    setup({ warningsFail: true });
    await openPicker();
    await userEvent.click(screen.getByRole("option", { name: "Finches" }));

    expect(
      await screen.findByText(/We couldn't check data access/),
    ).toBeInTheDocument();
    const card = screen.getByTestId("data-app-groups-card");
    expect(
      within(card).getByRole("button", { name: "Add groups" }),
    ).toBeEnabled();
    await userEvent.click(
      within(card).getByRole("button", { name: "Add groups" }),
    );
  ...
  it("links warning details to the selected group's database and schema", async () => {
    setup({ groups: [{ id: 3, name: "Finches", member_count: 0 }] });
    await userEvent.hover(
      await screen.findByRole("button", { name: "Missing data access" }),
    );

    const details = await screen.findByTestId("data-access-warning-popover");
    expect(
      within(details).getByRole("link", { name: "Birds" }),
    ).toHaveAttribute("href", "/admin/permissions/data/group/3/database/2");
    expect(
      within(details).getByRole("link", { name: "Orders" }),
    ).toHaveAttribute(
      "href",
      "/admin/permissions/data/group/3/database/2/schema/PUBLIC",
    );
  });
});
