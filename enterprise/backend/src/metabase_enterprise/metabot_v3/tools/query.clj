...
(defn- find-query
  ;; something similar will be provided by #51425
  [_full-history _queryid])

(defn- base-query
  [data-source full-history]
  (let [{:keys [table_id query_id report_id]} data-source]
    (cond
      ...
      (some? query_id)
      (if-let [query (find-query full-history query_id)]
...
(defn- filter-data
  [{:keys [data-source filters] :as _arguments} full-history]
  (let [[filter-field-id-prefix base] (base-query data-source full-history)
...
(comment
  (binding [api/*current-user-permissions-set* (delay #{"/"})
    ...
    (filter-data {:data-source {:table_id 27}
...
(mu/defmethod metabot-v3.tools.interface/*invoke-tool* :metabot.tool/filter-data
  [_tool-name arguments context full-history]
  {:output (json/generate-string (filter-data arguments full-history))
   :context context})
