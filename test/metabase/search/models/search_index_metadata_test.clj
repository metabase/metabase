...
(deftest lifecycle-test
  (t2/with-transaction [conn t2.connection/*current-connectable* {:rollback-only true}]
    (let [engine  :something-futureproof
      ...
      (is (= {:pending index-1}
             (search-index-metadata/indexes-on-current-connection conn engine version)))
      ...
      (testing "You can activate an index"
        ...
        (testing "Deleting an active index is a no-op"
          (is (zero? (search-index-metadata/delete-index! engine version index-1)))
          (is (= {:active index-1} (indexes)))))
      ...
      (testing "You can retire an index"
        ...
        (testing "Deleting a pending index removes it"
          (is (= 1 (search-index-metadata/delete-index! engine version index-2)))
          (is (= {:active index-1} (indexes)))
          (is (search-index-metadata/create-pending! engine version index-2)))
