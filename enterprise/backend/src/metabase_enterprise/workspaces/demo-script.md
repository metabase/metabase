...
1. Clone stats-remote-sync locally
...
1. Start "Child" instance locally with config.yml
   1. config.yml is loaded to workspace config state
1. Run a transform on the child instance
   1. output goes to temp schema
1. Run a query on the child instance
