...
(let [file-schema (mjs/transform ms/File)]
  (defn- fix-json-schema
    ...
    (cond
      ...
      (-> (dissoc schema :enum) (assoc :type "boolean"))

      (= (:enum schema) (mc/children ms/MaybeBooleanValue))
      (-> (dissoc schema :enum) (assoc :type "boolean") (assoc :optional true))
...
(defn- schema->params
  ...
  (let [{:keys [properties required]} (mjs-collect-defintions schema)
...
(defn- defendpoint->path-item
  ...
  (let [{:keys [method] :as data} (meta handler-var)
        ...
        body-schema               (when (seq body-params)
                                    (fix-json-schema
                                     (mjs-collect-defintions (into [:map] body-params))))
