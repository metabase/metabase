...
(deftest reindex-mode-write-routing-test
  (let [write-plan #'search.writer/write-plan
        ;; Mirror how index-docs! captures the destination table ONCE at the start of a committing reindex.
        captured   (fn [mode] (when (#'search.index/commits-per-batch? mode)
                                (or (search.index/pending-table) (search.index/active-table))))
        ;; write-plan owns the whole per-batch decision: the [label table-fn] targets AND the commit flag.
        plan       (fn [mode] (write-plan mode (captured mode)))
        labels     (fn [mode] (map first (:targets (plan mode))))]
    (testing "a committing background reindex pins ALL writes to the single table captured at the start"
      ...
      (binding [search.index/*state-store*    (index-state/mock-store {:active :active-t :pending :pending-t})
        ...
        (is (= [:pending] (labels (search.index/background-mode))))
        (is (true? (:commit? (plan (search.index/background-mode)))))))
    ...
    (testing "*force-sync* disables per-batch commits and keeps active current via dual-write"
      ...
      (binding [search.index/*state-store*    (index-state/mock-store {:active :active-t :pending :pending-t})
        ...
        (is (false? (:commit? (plan (search.index/background-mode)))))
        (is (= [:active :pending] (labels (search.index/background-mode))))))
    (testing "in-place and incremental modes always dual-write without committing"
      ...
      (binding [search.index/*state-store*    (index-state/mock-store {:active :active-t :pending :pending-t})
        ...
        (doseq [mode [(search.index/in-place-mode) (search.index/incremental-mode)]]
          (is (false? (:commit? (plan mode))))
          (is (= [:active :pending] (labels mode))))))))
