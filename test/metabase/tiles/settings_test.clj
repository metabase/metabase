...
(deftest map-tile-server-url-rejects-unreachable-hosts-test
  (testing "under the default strategy the setter refuses loopback, link-local, and unresolvable hosts"
...
(deftest map-tile-server-url-allows-private-tile-servers-test
  (testing "a tile server on the deployment's own network is a long-standing, legitimate configuration:
           this setting is consumed by the browser, so an on-prem instance with no internet egress can
           only show maps at all by pointing at an internal host"
    (mt/with-temporary-setting-values [map-tile-server-url osm-template]
