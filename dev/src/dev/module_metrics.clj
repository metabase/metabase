...
(defn- canonical-api-namespaces
  "The conventional API namespaces for `module`, under its effective `:ns-prefix`."
  [config module]
  (let [prefix (hooks.modules/module-ns-prefix config module)]
    (into (sorted-set)
          (map (fn [suffix]
...
(defn- cycle-section
  ...
  (let [{:keys [component-count module-count namespace-count edge-count components]}
        (module-scc/cycle-stats graph module->namespace-count)]
    (ordered-map/ordered-map
...
(defn- build-graph-context
  ...
  (let [modules'                 (modules deps config)
        ...
        prefix->module           (hooks.modules/build-prefix->module config)
        ...
        module->relevant-test-files (into (sorted-map)
                                          (map (fn [module]
                                                 [module (deps-graph/source-filenames->relevant-test-filenames
                                                          deps config prefix->module
                                                          (get module->sources module))]))
                                          modules')
        ...
        combined-deps-graph      (merge-with into
                                             direct-deps-graph
                                             (module-scc/model-import-dependencies config))
