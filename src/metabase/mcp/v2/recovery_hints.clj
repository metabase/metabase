...
(defn- uri-hint
  ...
  (let [id (or (parse-long (str entity-id)) entity-id)]
    (case entity-type
      "metric"
      (message/msg ["Metrics are aggregations, not sources. To use metric %s, put its base table's numeric id into `source-table:` (find it via `search` or `browse_data`) and reference the metric by its numeric id: `aggregation: [[metric, {}, %s]]`."]
                   id id)

      ("question" "model" "card")
      (message/msg ["To reference a saved question or model as a query source, put its bare numeric id into `source-card:` — not a URI: `\"source-card\": %s`."]
                   id)
...
(defn recovery-hint
  "The v2 recovery sentence for an agent error's `ex-data`, as text for the shared pipeline, or nil when it has none."
  ...
  (some->
   (case error
     ...
     (message/msg ["Call `browse_data` with action \"list_tables\" to list available tables with their numeric ids, then use one as `source-table`."])

     :ambiguous-table
     (message/msg ["Call `browse_data` with action \"list_tables\" for the database, then retry with the numeric table id — it is never ambiguous."])

     (:unknown-field :unknown-field-id)
     (message/msg ["Call `browse_data` with action \"get_fields\" for the table to list its columns with their numeric ids."])

     :ambiguous-fk
     (message/msg ["Call `browse_data` with action \"get_fields\" for the source table to list the available foreign-key columns."])

     :no-fk-path
     (message/msg ["If a metric relates to that table, call `browse_data` with action \"get_fields\" for the table that owns the metric to read its dimensions, which give the exact `joins:` clause."])
     ...
     (message/msg ["Call `browse_data` with action \"get_fields\" for the table that owns the measure and use the numeric id from its measures list."])

     (:unknown-segment :unknown-segment-id)
     (message/msg ["Call `browse_data` with action \"get_fields\" for the table that owns the segment and use the numeric id from its segments list."])

     :unknown-database
     (message/msg ["Use a numeric table id in `source-table:` (from `browse_data` action \"list_tables\"), which needs no database name at all."])
