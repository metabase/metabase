(ns metabase-enterprise.metabot-v3.handle-envelope
  ...
  (:require
   ...
   [metabase-enterprise.metabot-v3.tools :as tools]
   [metabase.util :as u]
   [metabase.util.o11y :as o11y]))

(defn- invoke-all-tool-calls! [e]
  (reduce (fn [e {tool-name :name, tool-call-id :id, :as tool-call}]
            (let [result (promise)
                  output
                  (o11y/with-span :info {:name tool-name}
                    (u/prog1 (tools/invoke-tool! tool-call)
                      (deliver result <>)))]
              (envelope/add-tool-response e tool-call-id output)))
          e
...
(defn handle-envelope
  ...
  [e]
  (tools/with-invocation-tracking
    (handle-envelope* e)))
