...
describe("bulk-selection.utils", () => {
  ...
  describe("isItemSelected", () => {
    ...
    it("should return 'no' when selection is null/undefined", () => {
      ...
      expect(isItemSelected(table, null as unknown as NodeSelection)).toBe(
        "no",
      );
    });
  });
  ...
  describe("getSchemaId", () => {
    it("should return schema ID for table node", () => {
      const table = createMockTableNode(1, "public", 101);

      expect(getSchemaId(table)).toBe("1:public");
    });
    ...
    it("should return undefined for database node", () => {
      const database = createMockDatabaseNode(1, []);

      expect(getSchemaId(database)).toBeUndefined();
    });
