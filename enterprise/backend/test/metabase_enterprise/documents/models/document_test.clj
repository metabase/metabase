...
(deftest document-position-reconciliation-on-insert-test
  (testing "Position reconciliation works for new documents"
    (mt/with-temp [:model/Collection {collection-id :id} {:name "Test Collection"}
                   ;; Create existing document with position 3
                   :model/Document {existing-doc-id :id} {:name "Existing Document"}
                   :collection_id collection-id
                   :collection_position 3]

      (testing "inserting document at existing position shifts others"
        ;; Insert new document at position 3 - should shift existing document
        (let [new-document (t2/insert-returning-instance! :model/Document
                                                          {:name "New Document"
                                                           :collection_id collection-id
                                                           :collection_position 3})]
          ;; New document should have position 3
          (is (= 3 (:collection_position new-document)))
      ...
      (testing "inserting document without position works"
        (let [no-position-doc (t2/insert-returning-instance! :model/Document
                                                             {:name "No Position Document"
                                                              :collection_id collection-id})]
          (is (nil? (:collection_position no-position-doc))))))))

(deftest document-position-reconciliation-on-update-test
  (testing "Position reconciliation works for document updates"
    (mt/with-temp [:model/Collection {collection-id :id} {:name "Test Collection"}
                   :model/Document {doc1-id :id} {:name "Document 1"
                                                  :collection_id collection-id
      ...
      (testing "moving document to different position reconciles others"
        ;; Move document 3 to position 1 - should shift others
        (t2/update! :model/Document doc3-id {:collection_position 1})

        (let [doc1 (t2/select-one :model/Document :id doc1-id)
              doc2 (t2/select-one :model/Document :id doc2-id)
              doc3 (t2/select-one :model/Document :id doc3-id)]
          ;; Document 3 should now be at position 1
          (is (= 1 (:collection_position doc3)))
      ...
      (testing "moving document to different collection reconciles both"
        (mt/with-temp [:model/Collection {other-collection-id :id} {:name "Other Collection"}]
          ;; Move document 1 to other collection at position 1
          (t2/update! :model/Document doc1-id {:collection_id other-collection-id
                                               :collection_position 1})

          (let [moved-doc (t2/select-one :model/Document :id doc1-id)
                doc2 (t2/select-one :model/Document :id doc2-id)]
...
(deftest sync-document-cards-collection-with-position-test
  (testing "sync-document-cards-collection! handles collection_position updates"
    (mt/with-temp [:model/Collection {collection-id :id} {:name "Target Collection"}
                   :model/Document {document-id :id} {:name "Test Document"
                                                      :collection_position 5}
                   :model/Card {card1-id :id} {:name "Document Card 1"
      ...
      (testing "syncing with collection_position updates associated cards"
        (let [updated-count (document/sync-document-cards-collection!)
              document-id collection-id
              :collection-position 5]
          (is (= 2 updated-count))

          ;; Verify cards were updated with correct collection and position
          (let [card1 (t2/select-one :model/Card :id card1-id)
                card2 (t2/select-one :model/Card :id card2-id)]
            (is (= collection-id (:collection_id card1)))
            (is (= collection-id (:collection_id card2)))
            (is (= 5 (:collection_position card1)))
            (is (= 5 (:collection_position card2))))))

      (testing "syncing without collection_position doesn't update card positions"
        ;; Update cards to different position first
        (t2/update! :model/Card :id [:in [card1-id card2-id]] {:collection_position 3})

        (let [updated-count (document/sync-document-cards-collection!)
              document-id collection-id]
          (is (= 2 updated-count))

          ;; Verify positions were not changed (still 3)
          (let [card1 (t2/select-one :model/Card :id card1-id)
                card2 (t2/select-one :model/Card :id card2-id)]
            (is (= collection-id (:collection_id card1)))
            (is (= collection-id (:collection_id card2)))
            (is (= 3 (:collection_position card1)))
            (is (= 3 (:collection_position card2))))))

      (testing "syncing with nil collection_position sets cards to nil"
        (let [updated-count (document/sync-document-cards-collection!)
              document-id collection-id
              :collection-position nil]
          (is (= 2 updated-count))

          ;; Verify card positions were set to nil
          (let [card1 (t2/select-one :model/Card :id card1-id)
                card2 (t2/select-one :model/Card :id card2-id)]
            (is (= collection-id (:collection_id card1)))
            (is (= collection-id (:collection_id card2)))
            (is (nil? (:collection_position card1)))
            (is (nil? (:collection_position card2)))))))))

(deftest document-after-update-hook-position-sync-test
  (testing "Document after-update hook syncs collection_position to cards"
    (mt/with-temp [:model/Collection {collection-id :id} {:name "Test Collection"}
                   :model/Document {document-id :id} {:name "Test Document"
                                                      :collection_id collection-id
                                                      :collection_position 2}
                   :model/Card {card-id :id} {:name "Document Card"
                                              :document_id document-id
                                              :collection_id collection-id
                                              :collection_position 2
                                              :dataset_query (mt/mbql-query venues)}]

      (testing "updating document position syncs to associated cards"
        ;; Update document position
        (t2/update! :model/Document document-id {:collection_position 7})

        ;; Verify associated card was updated
        (let [updated-card (t2/select-one :model/Card :id card-id)]
          (is (= 7 (:collection_position updated-card)))))

      (testing "updating document collection and position syncs both"
        (mt/with-temp [:model/Collection {new-collection-id :id} {:name "New Collection"}]
          ;; Update both collection and position
          (t2/update! :model/Document document-id {:collection_id new-collection-id
                                                   :collection_position 1})

          ;; Verify associated card got both updates
          (let [updated-card (t2/select-one :model/Card :id card-id)]
            (is (= new-collection-id (:collection_id updated-card)))
            (is (= 1 (:collection_position updated-card)))))))))
