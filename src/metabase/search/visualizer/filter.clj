(ns metabase.search.visualizer.filter
...
(mu/defn- search-string-clause-for-model
  ...
  (when-let [query (:search-string search-context)]
    (into
     [:or]
     (for [column           (->> (let [search-columns-fn (requiring-resolve 'metabase.search.visualizer.core/searchable-columns)]
...
(defn- search-model->revision-model [model]
  ((requiring-resolve 'metabase.search.visualizer.core/search-model->revision-model) model))
...
(defn- feature->supported-models
  ...
  (merge
   ...
   ;; argument is true
   {:search-native-query (->> (dissoc (methods @(requiring-resolve 'metabase.search.visualizer.core/searchable-columns)) :default)
                              (filter (fn [[model f]]
