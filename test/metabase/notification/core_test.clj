...
(deftest send-notification!*-test
  (testing "sending a ntoification will call render on all of its handlers"
    (mt/with-model-cleanup [:model/Notification]
      (mt/with-temp [:model/Channel         chn-1 notification.test/default-can-connect-channel
                     :model/Channel         chn-2 (assoc notification.test/default-can-connect-channel :name "Channel 2")
                     :model/ChannelTemplate tmpl  {:channel_type notification.test/test-channel-type}]
          ...
          (mt/with-dynamic-redefs [channel/render-notification (fn [channel-type notification template recipients]
            ...
            (testing "render-notification is called on all handlers with the correct channel and template"
              (is (=? [{:channel-type (keyword notification.test/test-channel-type)
                       ...
                       {:channel-type (keyword notification.test/test-channel-type)
                        :notification notification-info
                        :template     nil
