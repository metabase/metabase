...
(def ^:dynamic *lease-context*
  "The claim and local lost state for the reindex running on this thread.
  Nil outside a leased reindex."
  nil)
  ...
  (loop [error error]
    (cond
      ...
      (let [^SQLException sql-error error]
        (or (= (sql-errors/sql-states :unique-violation) (.getSQLState sql-error))
            (and (= (sql-errors/sql-states :integrity-constraint-violation) (.getSQLState sql-error))
                 (= (sql-errors/error-codes :mysql/duplicate-entry) (.getErrorCode sql-error)))
...
(defn- try-insert-claim!
  [conn claim]
  (try
    (search.db/insert-lease! conn (select-keys claim [:engine :version :lang_code :owner
...
(defn try-acquire!
  ...
  The expired-owner update and absent-row insert are individually atomic autocommit statements.
  A conditional update elects one expired-row taker; the coordinate primary key elects one concurrent
  inserter."
  [coordinate]
  (do-with-lifecycle-connection
   (fn [conn]
     (let [attempt-start-ns (System/nanoTime)
           ...
           {:keys [now expires_at]} (search.db/lease-times conn (lease-duration-millis))
...
(defn do-in-fenced-transaction!
  "Run `thunk` in a short transaction on the explicit mutation `conn`, fencing lease ownership first.

  The transaction is isolated from Toucan's ambient thread-local transaction state: a streaming source query
  or caller can appear transaction-bound even though `conn` is the independent batch-write connection."
...
(defn do-with-ddl-connection
  ...
  metadata after a test rollback).
  Index structure contends with nothing the ambient transaction holds, so the deadlock that makes
  [[do-with-mutation-connection]] join the caller does not apply.
  Index content is the opposite: it follows [[do-with-mutation-connection]] so test writes roll back with
  their fixtures."
  [thunk]
...
(defn assert-current!
  "Prove current ownership with a short lifecycle operation.
  Used only where no protected DB mutation follows."
  []
  (if *lease-context*
