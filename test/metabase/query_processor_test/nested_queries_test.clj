...
(defn- select-*-from-venues-sql-query
  "Generate the equivalent of `SELECT * FROM venues` for the current driver. (This varies slightly between drivers
  because some drivers need to fully-qualify `venues`, or quote it differently, which is why we piggyback off of the
  MBQL QP to generate it.)"
  ^String []
  (-> (:query (qp/query->native (data/mbql-query venues)))
      ;; replace the stuff generated by the implicit Fields clause with a plain `*`
      (str/replace #"SELECT.*FROM" "SELECT * FROM")))
  ...
  (rows+cols
    (format-rows-by [int int]
      (qp/process-query
        {:database (data/id)
         :type     :query
         :query    {:source-query {:native (select-*-from-venues-sql-query)}
                    :aggregation  [:count]
                    :breakout     [[:field-literal (keyword (data/format-name :price)) :type/Integer]]}}))))
