...
(s/defn check-columns-match-table
  ...
  ([{card-id :card_id, table-id :table_id}]
   ...
   (when card-id
     ...
     (when-let [result-metadata (t2/select-one-fn :result_metadata :m/card :id card-id)]
...
(defn update-card-check-gtaps
  ...
  (when new-result-metadata
    (when-let [gtaps-using-this-card (not-empty (t2/select [GroupTableAccessPolicy :id :table_id] :card_id card-id))]
      (let [original-result-metadata (t2/select-one-fn :result_metadata :m/card :id card-id)]
