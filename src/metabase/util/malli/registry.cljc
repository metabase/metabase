...
(defn- stable-sym-names
  "Returns a lazy seq of symbols that can be used as stable argument names."
  []
  (letfn [(int-to-stable-arg [n]
            (loop [num n
                   result ""]
              (if (zero? num)
                (symbol result)
                (let [remainder (mod (dec num) 26)
                      char (char (+ (int \a) remainder))]
                  (recur (quot (dec num) 26)
                         (str char result))))))]
    (map int-to-stable-arg (iterate inc 1))))
...
(defn- stabilize-reader-fn-args
  ...
  "
  [form]
  (let [arguments (remove
                   ;; do not replace the `&` argument, since it is used to
                   ;; indicate that the function takes a varargs
                   #{'&} (second form))
        argument-replacement (zipmap arguments (stable-sym-names))]
    (walk/postwalk
...
(defn- enforce-with-key-idempotency
  ...
  [form]
  (let [unwrap-forms #{'with-key 'mr/with-key 'metabase.util.malli.registry/with-key}]
    (loop [frm form]
      (if (and (coll? frm)
               (unwrap-forms (first frm)))
        (recur (second frm))
        frm))))
...
(defmacro with-key
  ...
  `(try (with-meta ~body
                   (assoc (meta ~body) ::key ~(-> body
                                                  enforce-with-key-idempotency
                                                  stabilize-schema-fn-args
                                                  pr-str)))
        (catch Exception e#
          (when (or config/is-dev? config/is-test?)
            (throw (ex-info (str "Error in with-key:" (pr-str ~body))
                            {:body ~body
                             :error e#})))
          ~body)))
