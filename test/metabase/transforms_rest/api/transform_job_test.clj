...
(deftest create-job-test
  (testing "POST /api/transform-job"
    (mt/with-model-cleanup [:model/TransformJob]
        ...
        (with-transforms-api-users! [user]
          (mt/with-temp [:model/TransformTag tag1 {:name "test-tag-1"}
            ...
            (testing "Creates job with valid data"
              (let [response (mt/user-http-request user :post 200 "transform-job"
                                                   {:name            "Test Job"
            ...
            (testing "Validates cron expression"
              (let [response (mt/user-http-request user :post 400 "transform-job"
                                                   {:name     "Bad Cron Job"
      ...
      (with-transforms-api-users! [user]
        (mt/with-temp [:model/TransformTag tag {:name "test-tag"}
          ...
          (testing "Returns job with hydrated fields"
            (let [response (mt/user-http-request user :get 200 (str "transform-job/" (:id job)))]
          ...
          (testing "Returns 404 for non-existent job"
            (mt/user-http-request user :get 404 "transform-job/999999")))))))
...
(deftest get-job-transforms-test
  (testing "GET /api/transform-job/:id/transforms"
    (mt/with-premium-features #{:advanced-permissions :transforms-basic :hosting}
      (with-transforms-api-users! [user]
        (let [user-id (mt/user->id user)]
          (mt/with-temp [:model/Transform {transform1-id :id} {:name "tr1" :creator_id user-id}
                         :model/Transform {transform2-id :id} {:name "tr2" :creator_id user-id}
            ...
            (testing "Returns the transforms of the job"
              (let [response (mt/user-http-request user :get 200 (str "transform-job/" job-id "/transforms"))]
                (is (=? [{:id transform1-id, :name "tr1"} {:id transform2-id, :name "tr2"}]
                        (sort-by :id response)))
                (testing "Response hydrates creator"
                  (is (every? #(map? (:creator %)) response))
                  (is (every? #(= user-id (get-in % [:creator :id])) response)))))
            (testing "Returns 404 for non-existent job"
              (mt/user-http-request user :get 404 "transform-job/999999/transforms"))))))))
          ...
          (mt/with-temp [:model/TransformTag {t1-id :id} {:name "test-tag"}
            ...
            (let [our-job-ids #{j1-id j2-id j3-id}
              ...
              (testing "listing without filtering"
                (let [response (mt/user-http-request user :get 200 "transform-job")]
                  (is (= our-job-ids (returned-job-ids response)))))
              (testing "filtering by tag_ids"
                (let [response (mt/user-http-request user :get 200 "transform-job" :tag-ids [t2-id])]
                  (is (=? [{:schedule "0 0 0 * * ?"
                            :tag_ids [t2-id]
                            :name "Job 3"
                            :id j3-id}]
              ...
              (testing "filtering by last_run_start_time"
                (let [response (mt/user-http-request user :get 200 "transform-job" :last-run-start-time "2025-08-26~")]
                  (is (= #{j2-id} (returned-job-ids response)))))
              (testing "filtering by last_run_statuses"
                (let [response (mt/user-http-request user :get 200 "transform-job" :last-run-statuses ["started" "succeeded"])]
                  (is (= #{j2-id} (returned-job-ids response)))))
              (testing "filtering by last_run_end_time without scheduled job"
                (let [response (mt/user-http-request user :get 200 "transform-job" :next-run-start-time "2025-08-26~")]
              ...
              (testing "filtering by last_run_end_time with scheduled job"
                (mt/with-temp-scheduler!
                  ...
                  (try
                    (let [response (mt/user-http-request user :get 200 "transform-job" :next-run-start-time "2025-08-27~")]
                      (is (=? [{:id j1-id
      ...
      (mt/with-premium-features #{:advanced-permissions :transforms-basic}
        (with-transforms-api-users! [user]
          (mt/with-temp [:model/TransformTag tag1 {:name "tag-1"}
            ...
            (let [job (mt/user-http-request user :post 200 "transform-job"
                                            {:name "Original" :schedule "0 0 0 * * ?"})]
              (testing "Updates job fields"
                (let [response (mt/user-http-request user :put 200 (str "transform-job/" (:id job))
                                                     {:name        "Updated"
                  ...
                  (is (= (set [(:id tag1) (:id tag2)]) (set (:tag_ids response))))))
              (testing "Validates cron expression"
                (let [response (mt/user-http-request user :put 400 (str "transform-job/" (:id job))
                                                     {:schedule "invalid"})]
                  (is (string? response))
                  (is (re-find #"Invalid cron expression" response)))))))))))
      ...
      (with-transforms-api-users! [user]
        (mt/with-temp [:model/TransformJob job {:name "Toggle Me" :schedule "0 0 0 * * ?"}]
          (testing "Defaults to true"
            (is (true? (:active (mt/user-http-request user :get 200 (str "transform-job/" (:id job)))))))
          (testing "Can be set to false"
            (let [response (mt/user-http-request user :put 200 (str "transform-job/" (:id job))
                                                 {:active false})]
              (is (false? (:active response))))
            (is (false? (:active (mt/user-http-request user :get 200 (str "transform-job/" (:id job))))))
          ...
          (mt/with-temp [:model/TransformJob job {:name "Triggered" :schedule "0 0 0 * * ?"}]
            ...
            (try
              (testing "Deactivating removes the trigger"
                (mt/user-http-request user :put 200 (str "transform-job/" (:id job))
                                      {:active false})
    ...
    (mt/with-premium-features #{:advanced-permissions :transforms-basic}
      (with-transforms-api-users! [user]
        (mt/with-temp [:model/TransformJob job {:name "To Delete" :schedule "0 0 0 * * ?"}]
          (testing "Deletes job"
            (mt/user-http-request user :delete 204 (str "transform-job/" (:id job)))
            (is (nil? (t2/select-one :model/TransformJob :id (:id job)))))
          (testing "Returns 404 for non-existent job"
            (mt/user-http-request user :delete 404 "transform-job/999999")))))))
            ...
            (mt/with-temp [:model/TransformTag tag {:name "execute-test-tag"}
              ...
              (let [called (promise)]
                (mt/with-dynamic-fn-redefs [jobs/run-transforms! (fn [run-id & _]
                  ...
                  (let [response (mt/user-http-request user :post 202 (str "transform-job/" (:id job) "/run"))]
      ...
      (with-transforms-api-users! [user]
        (mt/with-temp [:model/TransformJob {job-id :id} {:name "Run History Job" :schedule "0 0 0 * * ?"}
          ...
          (testing "returns paginated runs for a job"
            (let [response (mt/user-http-request user :get 200 (str "transform-job/" job-id "/runs"))
          ...
          (testing "returns 404 for non-existent job"
            (mt/user-http-request user :get 404 "transform-job/999999/runs"))
        ...
        (let [user-id (mt/user->id user)]
          (mt/with-temp [:model/TransformJob {job-id :id} {:name "Drill-down Job" :schedule "0 0 0 * * ?"}
                         :model/TransformJobRun {run-id :id} {:job_id     job-id
                                                              ...
                                                              :run_method "cron"
                                                              :start_time (parse-instant "2025-09-01T10:00:00")}
                         :model/Transform {t1-id :id} {:name "TR1" :creator_id user-id}
                         :model/Transform {t2-id :id} {:name "TR2" :creator_id user-id}
            ...
            (testing "returns 404 for non-existent job"
              (mt/user-http-request user :get 404
        ...
        (testing "cancels an active job run and requests cancellation of its active members"
          (mt/with-temp [:model/TransformJob {job-id :id} {:name "Cancel Job" :schedule "0 0 0 * * ?"}
                         ...
                         :model/TransformRun {member-id :id} {:transform_id t1-id
                                                              ...
                                                              :start_time   (parse-instant "2025-09-01T10:00:00")
                                                              :end_time     nil}]
            (mt/user-http-request user :post 204 (str "transform-job/" job-id "/runs/" run-id "/cancel"))
        ...
        (testing "returns 400 when the run has already finished"
          (mt/with-temp [:model/TransformJob {job-id :id} {:name "Cancel Job" :schedule "0 0 0 * * ?"}
                         :model/TransformJobRun {run-id :id} {:job_id     job-id
                                                              ...
                                                              :start_time (parse-instant "2025-09-01T10:00:00")
                                                              :end_time   (parse-instant "2025-09-01T10:05:00")}]
            (mt/user-http-request user :post 400 (str "transform-job/" job-id "/runs/" run-id "/cancel"))
        ...
        (testing "returns 404 for a non-existent job or run, or a run of another job"
          (mt/with-temp [:model/TransformJob {job-id :id} {:name "Cancel Job" :schedule "0 0 0 * * ?"}
                         ...
                         :model/TransformJobRun {run-id :id} {:job_id     job-id
                                                              ...
                                                              :run_method "manual"
                                                              :start_time (parse-instant "2025-09-01T10:00:00")}]
            (mt/user-http-request user :post 404 (str "transform-job/999999/runs/" run-id "/cancel"))
            (mt/user-http-request user :post 404 (str "transform-job/" job-id "/runs/999999/cancel"))
            (mt/user-http-request user :post 404 (str "transform-job/" other-job-id "/runs/" run-id "/cancel"))))))))
