  ...
  () => {
    ...
    describe("modifying only dashboard", () => {
      it("should handle toggling auto applying filters on and off", () => {
        ...
        cy.filterWidget().findByText(FILTER.name).click();
        cy.popover().within(() => {
          cy.findByText("Gadget").click();
          cy.button("Add filter").click();
          cy.wait("@cardQuery");
        });
        cy.getDashboardCard().findByText("Rows 1-4 of 53").should("be.visible");

        cy.log(
          "parameter values should be preserved when disabling auto applying filters",
        );
        cy.openDashboardSettingsSidebar();
        cy.sidesheet().within(() => {
          cy.findByText(filterToggleLabel).click();
          cy.wait("@updateDashboard");
          cy.findByLabelText(filterToggleLabel).should("not.be.checked");
        });
        cy.closeDashboardSettingsSidebar();
        cy.filterWidget().findByText("Gadget").should("be.visible");
        cy.getDashboardCard().findByText("Rows 1-4 of 53").should("be.visible");

        cy.log("draft parameter values should be applied manually");
        cy.filterWidget().findByText("Gadget").click();
        cy.popover().within(() => {
        ...
        cy.filterWidget().findByText("2 selections").click();
        cy.popover().within(() => {
          cy.findByText("Gadget").click();
          cy.button("Update filter").click();
        });
        cy.filterWidget().findByText("Widget").should("be.visible");
        cy.dashboardParametersContainer().button("Apply").should("be.visible");
        ...
        cy.filterWidget().findByText("Widget").should("be.visible");
        cy.getDashboardCard().findByText("Rows 1-4 of 54").should("be.visible");
        ...
        cy.filterWidget().findByText("Widget").click();
        cy.popover().within(() => {
          cy.findByText("Gadget").click();
          cy.button("Update filter").click();
        });
    ...
    it("should not save filter state for dashboard parameter w/o auto-apply enabled", () => {
      ...
      cy.filterWidget().should("not.contain", "Gadget");
    });
    ...
    describe("modifying dashboard and dashboard cards", () => {
      it("should not preserve draft parameter values when editing the dashboard", () => {
        ...
        cy.sidebar().findByDisplayValue("Text").clear().type("Vendor");
        cy.getDashboardCard().findByText("Select…").click();
        cy.popover().findByText("Vendor").click();
        cy.saveDashboard();
    ...
    describe("parameter with default values", () => {
      ...
      it("should handle toggling auto applying filters on and off", () => {
        ...
        cy.dashboardParametersContainer()
          .button("Apply")
          .should("be.visible")
          .click();
      ...
      it.skip("should display a toast when a dashboard takes longer than 15s to load even without parameter values (but has parameters with default values)", () => {
        ...
        cy.filterWidget().findByText(filterDefaultValue).should("be.visible");
        cy.getDashboardCard().findByText("Rows 1-4 of 53").should("be.visible");
      });
    ...
    describe("auto-apply filter toast", () => {
      ...
      it.skip("should display the toast indefinitely unless dismissing manually", () => {
        ...
        cy.tick(TOAST_TIMEOUT);
        cy.wait("@cardQuery");
        cy.undoToast().findByText(TOAST_MESSAGE).should("be.visible");
    ...
    describe("no collection curate permission", () => {
      ...
      it("should not be able to toggle auto-apply filters toggle", () => {
        ...
        cy.dashboardHeader().icon("ellipsis").click();
        cy.popover().findByText("Edit settings").should("not.exist");
      });
    ...
    describe("embeddings", () => {
      ...
      describe("public embeds", () => {
        it("should apply filters after clicking the apply button when auto-apply filters is turned off", () => {
          ...
          cy.dashboardParametersContainer().button("Apply").should("not.exist");
          cy.filterWidget().findByText("Category").click();
          cy.popover().within(() => {
            cy.findByText("Widget").click();
            cy.button("Add filter").click();
          });
      ...
      describe("signed embeds", () => {
        it("should apply filters after clicking the apply button when auto-apply filters is turned off", () => {
          ...
          cy.get("@dashboardId").then(dashboardId => {
            ...
            cy.visitEmbeddedPage(embeddingPayload);
          });
...
cy.describeWithSnowplow("scenarios > dashboards > filters > auto apply", () => {
...
const openDashboard = (params = {}) => {
  ...
  cy.visitDashboard("@dashboardId", { params });
};
...
const openSlowDashboard = (params = {}) => {
  ...
  cy.getDashboardCard().should("be.visible");
};
...
const openSlowPublicDashboard = (params = {}) => {
  ...
  cy.get("@dashboardId").then(dashboardId => {
    cy.visitPublicDashboard(dashboardId, { params });
  });
