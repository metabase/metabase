...
#
# Matches the `version "x.y.z"` line immediately following a `cypress@<range>:` entry header.
CYPRESS_VERSIONS="$(sed -n '/^"\{0,1\}cypress@[^"]*"\{0,1\}:$/{n;s/^  version "\([^"]*\)"$/\1/p;}' yarn.lock)"
CYPRESS_MATCHES="$(printf '%s\n' "$CYPRESS_VERSIONS" | grep -c . || true)"
if [ "$CYPRESS_MATCHES" != "1" ]; then
  echo "::error::cache-keys.sh: expected exactly one resolved cypress version in yarn.lock, found $CYPRESS_MATCHES" >&2
  exit 1
fi
...
spec() {
  ...
  # Pinned rather than discovered via `yarn cache dir` so that the path is known before yarn has run;
  # prepare-frontend exports YARN_CACHE_FOLDER to match and fails if yarn disagrees. Cache the folder
  # itself, not the `vN` directory yarn creates inside it, so a yarn cache-format bump does not leave a
  # stale sibling behind in the restored tree.
