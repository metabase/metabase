...
You can customize which filter values to apply to each [dashboard subscription](../dashboards/subscriptions.md). That way you can send different groups of people an email (or Slack message) the contents of the dashboard with different filters applied. You only need to maintain one dashboard, which you can use to send results relevant to each subscriber.
