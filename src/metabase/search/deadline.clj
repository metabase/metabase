...
(defn- run-with-deadline [identity thunk]
  (let [timeout-ms (* 60000 (long (search.settings/search-reindex-timeout-minutes)))
        ...
        state      (:state context)
        ^ScheduledFuture timeout (when (pos? timeout-ms)
                                   (schedule! timeout-ms (bound-fn [] (expire! context))))
        ^ScheduledFuture observation (when (pos? timeout-ms)
                                       (schedule! (+ timeout-ms *observation-grace-ms*)
                                                  (bound-fn []
                                                    (when-not (:exited? @state)
    ...
    (binding [*run-context* context
              ...
              jdbc.options/*options* (assoc jdbc.options/*options* :timeout
                                            (min 60 (or (:timeout jdbc.options/*options*) 60)))]
      (try
        (let [result (thunk)]
          (check!)
          result)
        ...
        (finally
          (locking state
            (swap! state assoc :exited? true :status (if (= :running (:status @state)) :finished :timed-out))
            (when (= :timed-out (:status @state))
              (Thread/interrupted)))
          (when timeout (.cancel timeout false))
          (when observation (.cancel observation false)))))))
