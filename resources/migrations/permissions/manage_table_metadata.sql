  ...
  CASE WHEN EXISTS (
    ...
    WHERE
      ...
      AND (p.object = concat('/data-model/db/', md.id, '/')
        OR p.object = concat('/data-model/db/', md.id, '/schema/'))) THEN
    'yes'
  ELSE
    'no'
  END AS perm_value
...
WHERE
  ...
  WHEN NOT EXISTS (
    ...
    WHERE
      p.group_id = pg.id
      AND p.object LIKE concat('/data-model/db/', md.id, '/schema/%')) THEN
    TRUE
  ELSE
    FALSE
  END;
...
WITH escaped_schema_table AS (
  SELECT
    ...
    mt.db_id,
    mt.schema,
    replace(replace(mt.schema, '\', ' \\'), ' / ', ' / ') AS escaped_schema FROM metabase_table mt)
      INSERT INTO data_permissions (group_id, perm_type, db_id, schema_name, table_id, perm_value)
      SELECT
        pg.id AS group_id, 'perms/manage-table-metadata' AS perm_type, mt.db_id, mt.schema AS schema_name, mt.id AS table_id, CASE WHEN EXISTS (
          SELECT
            1
          FROM permissions p
          WHERE
            ...
            AND (p.object = concat('/data-model/db/', mt.db_id, '/schema/', mt.escaped_schema, '/')
              OR p.object = concat('/data-model/db/', mt.db_id, '/schema/', mt.escaped_schema, '/table/', mt.id, '/'))) THEN
          'yes'
        ELSE
          'no'
        END AS perm_value
