...
(def ^:private enforced-tool-types
  "Tool names that must (a) declare a valid `:tool-type` in metadata and
  (b) have their results validated against
  ...
  four under that name. Likewise both `create_sql_query` variants
  (`create-sql-query-tool` and `create-sql-query-code-edit-tool`) share that
  single tool-name, and both `create_dashboard_subscription` variants
  (web `create-dashboard-subscription-tool` and Slack
  `slackbot-create-dashboard-subscription-tool`) register under this name."
  #{"search"
    ;; Phase 3b — SQL authoring tools
    "create_sql_query"
    "edit_sql_query"
    "replace_sql_query"
    ;; Phase 3b — Notebook authoring tools. Both the in-app
    ;; (`construct-notebook-query-tool`) and Slack
    ;; (`slackbot-construct-notebook-query-tool`) variants share this name.
    "construct_notebook_query"
    ;; Phase 3b — Transform write tools (Python is EE-overridden but the OSS
  ...
  (reduce-kv
   (fn [acc tool-name tool-var]
     (let [m         (meta tool-var)
           ...
           enforced? (contains? enforced-tool-types tool-name)]
       (when enforced?
       ...
       (let [base-fn    (if (contains? state-dependent-tools tool-name)
             ...
             validated  (if enforced?
                          (wrap-with-entity-usage-validation base-fn tool-name tool-type)
                          base-fn)
             tool-scope (:scope m)
