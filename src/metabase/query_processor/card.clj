...
(defn- target-template-tag-name
  ...
  [tag-id->name target]
  (lib.util.match/match-lite target
    [:template-tag (tag-name :guard (string? tag-name))]
    tag-name
    [:template-tag (tag-ref :guard (map? tag-ref))]
...
(mu/defn- validate-card-parameters
  ...
  (when-not *allow-arbitrary-mbql-parameters*
    (let [dataset-query (api/check-404 (t2/select-one-fn :dataset_query :model/Card :id card-id))
          template-tags (card-template-tag-parameters card-id)
