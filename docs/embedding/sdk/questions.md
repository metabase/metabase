...
> A navigation button that returns to the previous view. Only renders when `onNavigateBack` is available in context.
...
| Prop | Type | Description |
|------|------|-------------|
| withColumnItemIcon* | boolean | Whether to show column icons in the dropdown interface |
...
> A set of badges for managing data groupings (breakouts). Similar to the Filter component but for grouping operations.
...
No props. Uses summarize data from context.
...
| Prop | Type | Description |
|------|------|-------------|
| onApply | () => void | Callback when changes are applied, defaults to empty function |
...
| Prop | Type | Description |
|------|------|-------------|
| height | CSSProperties["height"] | Height for the dropdown menu |
| className | string | |
| style | React.CSSProperties | |

#### `InteractiveQuestion.ChartTypeSelector`
> Detailed chart type selection interface with sensible and non-sensible visualization options.

Uses [Mantine Stack props](https://v6.mantine.dev/core/stack/?t=props) under the hood, as well as:

| Prop | Type | Description |
|------|------|-------------|
| onChange | (display: CardDisplayType) => void | Callback when visualization type changes |
| className | string | |
| style | React.CSSProperties | |

#### `InteractiveQuestion.ChartTypeDropdown`
> Dropdown for selecting the visualization type (bar chart, line chart, table, etc.). Automatically filters to show only compatible visualization types for the current data.
...
> - For new questions: Calls `onCreate` from context
> - For existing questions: Calls `onSave` from context
