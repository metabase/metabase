...
- `enable-entity-navigation` (default is false) - preserves dashboard custom [click behaviors](../dashboards/interactive.md#customizing-click-behavior) that navigate to other dashboards and questions within the embed. When disabled, only external URL links are kept. See also [`handleLink`](./sdk/plugins.md#handlelink) for customizing what happens when people click URL links.
