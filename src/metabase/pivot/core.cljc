          ...
          (cond
            ...
            (loop [node tree, [c & r] collapsed-subtotal]
              (let [children (:children node)
                    idx (perf/map-get (:child-lookup node) c)
...
(defrecord TreeNode [value children child-lookup isCollapsed])
...
(defn- add-path-to-tree
  ...
  (if (< i (count indexes-path))
    ...
    (let [index (nth indexes-path i)
          ...
          child-lookup (:child-lookup tree)
          children (:children tree)
          node (or (some->> (perf/map-get child-lookup v) (perf/list-nth children))
                   (let [node (->TreeNode v (perf/make-list) (perf/make-map) false)]
                     (perf/map-put! child-lookup v (count children))
...
(defn- sort-tree
  ...
  (let [children (:children tree)
    ...
    (when-let [curr-compare-fn (compare-fn (first sort-orders))]
      ...
      ;; Don't forget to recreate child-lookup because we messed child order.
      #?(:clj (.clear ^java.util.HashMap child-lookup)
         :cljs (.clear child-lookup))
      (doseq [i (range (count children))]
        (perf/map-put! child-lookup (:value (perf/list-nth children i)) i)))
    tree))
                ...
                (loop [i 0
                  ...
                  (if (>= i (count nodes))
                    ...
                    (let [{:keys [children rawValue isCollapsed isGrandTotal isSubtotal isValueColumn] :as node} (nth nodes i)
                          ...
                          _               (vswap! result conj! nil) ;; Placeholder for parent item to place into once
                                                                    ;; children are added and processes.
