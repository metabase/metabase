...
(deftest publish-table-test
  (mt/with-premium-features #{:data-studio}
    (testing "POST /api/ee/data-studio/table/publish-table"
      (testing "creates table symlinks for selected tables"
        (mt/with-model-cleanup [:model/TableSymlink]
            ...
            (let [response (mt/user-http-request :crowberto :post 200 "ee/data-studio/table/publish-table"
              ...
              (testing "symlinks are hydrated on tables"
                (doseq [table (:tables response)]
                  (is (= 1 (count (:published_symlinks table))))
                  (is (= collection-id (:collection_id (first (:published_symlinks table)))))))
              (testing "symlinks are persisted in database"
                (is (= 2 (t2/count :model/TableSymlink :collection_id collection-id)))))))))))

(deftest published-as-symlink-test
  (mt/with-premium-features #{:data-studio}
    (mt/with-model-cleanup [:model/TableSymlink]
      (mt/with-temp [:model/Collection {collection-id :id} {}]
        (let [_                          (mt/user-http-request :crowberto :post 200 "ee/data-studio/table/publish-table"
          ...
          (testing "list tables shows published_as_symlink=true for symlinked table"
            (let [list-response        (list-tables-via-table-api)
                  published-as-symlink (u/index-by :id :published_as_symlink list-response)]
              (is (true? (published-as-symlink (mt/id :venues))))
              (is (false? (published-as-symlink (mt/id :users))))))
          (testing "list tables via schema shows published_as_symlink"
            (let [list-response        (list-tables-via-schema-api)
                  published-as-symlink (u/index-by :id :published_as_symlink list-response)]
              (is (true? (published-as-symlink (mt/id :venues))))
              (is (false? (published-as-symlink (mt/id :users)))))))))))

(deftest published-symlinks-collection-perms-test
  (mt/with-premium-features #{:data-studio}
    (mt/with-model-cleanup [:model/TableSymlink]
      (mt/with-temp [:model/Collection {collection1 :id} {}
                     :model/Collection {collection2 :id} {}]
        (let [publish-venues          #(mt/user-http-request :crowberto :post 200 "ee/data-studio/table/publish-table"
                                                             {:table_ids            [(mt/id :venues)]
                                                              :target_collection_id %})
              _                       (publish-venues collection1)
              _                       (publish-venues collection2)
              symlink-ids             (t2/select-pks-vec :model/TableSymlink :table_id (mt/id :venues))
              query-meta-url          #(format "table/%d/query_metadata" %)
              list-published-symlinks #(:published_symlinks (mt/user-http-request %1 :get 200 (query-meta-url %2)))]
          (testing "admin sees both symlinks"
            (is (= (set symlink-ids) (set (map :id (list-published-symlinks :crowberto (mt/id :venues)))))))
          (testing "no published symlinks for table without any"
            (is (nil? (list-published-symlinks :crowberto (mt/id :users)))))
          (testing "non admin does not see published symlinks (superuser-only for now)"
            (is (nil? (list-published-symlinks :rasta (mt/id :venues))))
            (is (nil? (list-published-symlinks :rasta (mt/id :users))))))))))
