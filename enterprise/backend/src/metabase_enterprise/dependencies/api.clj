...
(api.macros/defendpoint :get "/erd" :- ::erd/erd-response
  "Return an Entity Relationship Diagram (ERD) for the given focal tables and their
  immediate FK connections. Each focal table's 1-hop FK neighbors are included.
  To walk the graph, add more table-ids to subsequent requests."
  [_route-params
   {:keys [database-id table-ids]} :- ::erd/erd-request]
