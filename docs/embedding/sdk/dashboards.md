...
-\*\* Combining `initialParameters` and `hiddenParameters` to filter data on the frontend is a [security risk and is against our terms](../authentication#security-warning-each-end-user-must-have-their-own-metabase-account). Combining `initaliParameters` and `hiddenParaters` to declutter the user interface is fine. 
