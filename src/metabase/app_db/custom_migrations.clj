...
(define-migration BackfillMetabotConversationEmbeddingHostname
  ...
  ;;
  ;; Collect IDs into a vector first, then iterate UPDATEs to avoid
  ;; cursor-during-UPDATE fragility on MySQL/H2.
  (let [rows (vec (t2/query {:select [:id :embed_url]
                             :from   [:metabot_conversation]
                             :where  [:and
                                      [:not= :embed_url nil]
                                      [:= :embedding_hostname nil]]}))]
    (run! (fn [{:keys [id embed_url]}]
            (let [parsed (try (java.net.URI. ^String embed_url) (catch Exception _ nil))
