...
import type { CurrentTaskResponse, SyncTaskType } from "metabase-types/api";
...
export const remoteSyncSlice = createSlice({
  ...
  initialState,
  reducers: {
    taskStarted: (state, action: { payload: { taskType: SyncTaskType } }) => {
      state.currentTask = {
        id: 0,
    ...
    taskUpdated: (state, action: { payload: CurrentTaskResponse }) => {
