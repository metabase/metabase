...
(deftest url-proxy-rejects-any-local-ula-cgnat-test
  (testing "hosts the old valid-host? let through -- any-local (0.0.0.0 / [::]), IPv6 ULA, IPv4 CGNAT --
    ...
    (doseq [host ["0.0.0.0" "[::]" "100.64.0.1" "[fc00::1]"]]
      (testing host
        (is (= (str "Invalid GeoJSON file location: must either start with http:// or https:// or be a "
                    "relative path to a file on the classpath. URLs referring to hosts that supply "
                    "internal hosting metadata are prohibited.")
