(ns metabase.channel.render.js.svg
  "Functions to render charts as svg strings by using graal's js engine. A bundle is built by `pnpm build-static-viz`
...
;; the bundle path goes through webpack. Changes require a `pnpm build-static-viz`
