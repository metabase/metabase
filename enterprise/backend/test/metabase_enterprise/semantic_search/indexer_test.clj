...
(deftest candidate-filter-test
  (let [filter (semantic.indexer/candidate-filter 3)]
    (testing "remove-redundant-candidates filters out seen candidates"
      (let [candidates       [{:id "card_123" :gated_at (Instant/parse "2025-01-01T12:00:00Z")}
                              {:id "card_456" :gated_at (Instant/parse "2025-01-01T12:01:00Z")}]
    ...
    (testing "update-candidate-filter maintains capacity limits"
      (let [candidates     [{:id "card1" :gated_at (Instant/parse "2025-01-01T12:00:00Z")}
                            {:id "card2" :gated_at (Instant/parse "2025-01-01T12:01:00Z")}
                            {:id "card3" :gated_at (Instant/parse "2025-01-01T12:02:00Z")}
                            {:id "card4" :gated_at (Instant/parse "2025-01-01T12:03:00Z")}
                            {:id "card5" :gated_at (Instant/parse "2025-01-01T12:04:00Z")}]
            ...
            (testing "watermark has been updated"
              (let [[{poll-result :ret}] @poll-calls]
                ...
                (testing "watermark has been flushed to the metadata table"
                  (is (= (:watermark @indexing-state) (semantic.gate/resume-watermark (get-metadata-row pgvector index-metadata index)))))))))
...
(deftest init-indexing-state-test
  (testing "initializes indexing state from metadata row"
    (let [metadata-row {:indexer_last_poll (Instant/parse "2025-01-01T12:00:00Z")
      ...
      (is (= (Instant/parse "2025-01-01T11:30:00Z") (get-in state-value [:watermark :last-seen])))
