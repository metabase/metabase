...
(defn duration-queries-to-rerun-honeysql
  "Honey SQL selecting the query definitions to rerun for the duration cache `scopes`, each
  `{:model :model-id :rerun-cutoff}`, counting only executions started after `started-after`. `parameterized?`
  selects parameterized or plain queries."
...
(defn duration-queries-to-rerun
  "The query definitions [[duration-queries-to-rerun-honeysql]] selects."
  [scopes started-after parameterized?]
  (t2/select :model/Query (duration-queries-to-rerun-honeysql scopes started-after parameterized?)))

(defn scheduled-base-query-to-rerun-honeysql
  "Honey SQL selecting the unparameterized query definition of the Card with `card-id` executed after
  `started-after`."
...
(defn scheduled-base-query-to-rerun
  "The query definition [[scheduled-base-query-to-rerun-honeysql]] selects, or nil."
  [card-id started-after]
  (t2/select-one :model/Query (scheduled-base-query-to-rerun-honeysql card-id started-after)))

(defn scheduled-parameterized-queries-to-rerun-honeysql
  "Honey SQL selecting the `limit` most common parameterized query definitions of the Card with `card-id` executed
  after `rerun-cutoff`."
...
(defn scheduled-parameterized-queries-to-rerun
  "The query definitions [[scheduled-parameterized-queries-to-rerun-honeysql]] selects."
  [card-id rerun-cutoff limit]
  (t2/select :model/Query (scheduled-parameterized-queries-to-rerun-honeysql card-id rerun-cutoff limit)))
