  ...
  (mt/with-temp [:model/User {user-id :id} {}]
    (mt/with-model-cleanup [:model/SupportAccessGrantLog]
      (let [ticket-number "SUPPORT-12345"
            duration 240
            grant (grants/create-grant! user-id ticket-number duration)]
        (is (some? grant))
  ...
  (mt/with-temp [:model/User {user-id :id} {}]
    (mt/with-model-cleanup [:model/SupportAccessGrantLog]
      (let [ticket-number "SUPPORT-12347"]
        (grants/create-grant! user-id ticket-number 240)
        (is (thrown-with-msg?
             clojure.lang.ExceptionInfo
             #"Cannot create grant: an active grant already exists"
             (grants/create-grant! user-id "SUPPORT-12348" 120)))))))
  ...
  (mt/with-temp [:model/User {user-id :id} {}]
    (mt/with-model-cleanup [:model/SupportAccessGrantLog]
      (let [ticket-number1 "SUPPORT-12351"
            ...
            grant1 (grants/create-grant! user-id ticket-number1 240)]
        (grants/revoke-grant! user-id (:id grant1))
        (let [grant2 (grants/create-grant! user-id ticket-number2 240)]
          (is (some? grant2))
  ...
  (mt/with-temp [:model/User {user-id :id} {}
    ...
    (mt/with-model-cleanup [:model/SupportAccessGrantLog]
      (let [ticket-number "SUPPORT-22345"
            grant (grants/create-grant! user-id ticket-number 240)
...
(deftest get-current-grant-returns-most-recent-test
  (mt/with-temp [:model/User {user-id :id} {}]
    (mt/with-model-cleanup [:model/SupportAccessGrantLog]
      (grants/create-grant! user-id "SUPPORT-77348" 240)
      (let [now (t/instant)
