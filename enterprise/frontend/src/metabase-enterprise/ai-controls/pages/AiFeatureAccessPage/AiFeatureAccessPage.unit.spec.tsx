...
import { AiFeatureAccessPage } from "./AiFeatureAccessPage";
import { AiFeatureAccessUpsellPage } from "./AiFeatureAccessUpsellPage";

const FEATURE_ACCESS_PATH = "/admin/metabot/usage-controls/ai-feature-access";
const METABOT_ADVANCED_URL = "path:/api/ee/ai-controls/permissions/advanced";
...
function setup({
  ...
  setupUpdateMetabotGroupPermissionsEndpoint();
  fetchMock.post(METABOT_ADVANCED_URL, 200);
  fetchMock.delete(METABOT_ADVANCED_URL, 200);

  renderWithProviders(
    <Route path={FEATURE_ACCESS_PATH} element={<AiFeatureAccessPage />} />,
    {
      withRouter: true,
      withUndos: true,
      initialRoute: FEATURE_ACCESS_PATH,
      storeInitialState: {
...
function getSwitchModeButton() {
  return screen.getByRole("button", {
    name: "Switch to group-level permissions",
  });
}

describe("AiFeatureAccessPage", () => {
  ...
  it("renders the Metabot table only, with no tab strip and no MCP request", async () => {
    setup();
    await screen.findByTestId("ai-feature-access-table");

    expect(
      screen.getByRole("heading", { name: "AI feature access" }),
    ).toBeInTheDocument();
    expect(screen.queryByRole("tab")).not.toBeInTheDocument();
    expect(screen.queryByRole("tablist")).not.toBeInTheDocument();
    expect(fetchMock.callHistory.called(MCP_PERMISSIONS_URL)).toBe(false);
  });

  it("the switch button posts to the Metabot advanced endpoint", async () => {
    setup();
    await screen.findByTestId("ai-feature-access-table");

    await userEvent.click(getSwitchModeButton());
    const modal = await screen.findByRole("dialog", {
      name: "Switch to group-level permissions?",
    });
    await userEvent.click(
      within(modal).getByRole("button", { name: "Switch" }),
    );

    await waitFor(() =>
      expect(
        fetchMock.callHistory.called(METABOT_ADVANCED_URL, {
          method: "POST",
        }),
      ).toBe(true),
    );
...
describe("AiFeatureAccessUpsellPage", () => {
  it("renders the upsell instead of the table", () => {
    renderWithProviders(<AiFeatureAccessUpsellPage />);

    expect(
      screen.getByText("Set AI usage limits and controls"),
    ).toBeInTheDocument();
    expect(
      screen.queryByTestId("ai-feature-access-table"),
    ).not.toBeInTheDocument();
  });
});
