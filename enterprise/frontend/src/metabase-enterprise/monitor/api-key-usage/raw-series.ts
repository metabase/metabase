 ...
 * nothing to collapse (in particular, when `max` is unset — used for the day breakout, which
 * should never be collapsed).
 */
...
/**
 * Build a single-breakout rawSeries (one dimension + the count metric) for `bar`/`row`/`pie`/`line`
 * displays. Long tails are collapsed into a single "Other" category when `maxCategories` is set.
 */
