...
If a group has "Blocked" or "Row and column security" View data permissions for _any_ of the tables in the database, then this group will have native queries disabled for the _all_ tables in that database. That's because Metabase parse SQL queries, so it can't know for sure whether the SQL queries are using the tables with restricted access.
