...
(defn- prevent-sync-protected-writes
  ...
  [instance]
  (when (and (try ((requiring-resolve 'metabase-enterprise.library.settings/git-sync-read-only))
...
(defmethod can-write? :hook/git-sync-protected
  ([instance]
   (try (false? ((requiring-resolve 'metabase-enterprise.library.settings/git-sync-read-only)))
