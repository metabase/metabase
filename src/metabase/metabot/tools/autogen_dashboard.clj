...
(defn- autogen-entity-usage
  "Build `:entity-usage` for an autogen dashboard call. `query_id` is a
  memory-only ref and is intentionally excluded from `:input`."
  [{:keys [table_id model_id metric_id report_id]}]
  (let [seed (cond
               table_id  {:type "table"    :id table_id}
               model_id  {:type "model"    :id model_id}
               metric_id {:type "metric"   :id metric_id}
               report_id {:type "question" :id report_id})]
    {:input  (if seed [seed] [])
     :output []}))
...
(mu/defn ^{:tool-name    "create_autogenerated_dashboard"
  ...
  (let [entity-usage (autogen-entity-usage source)
