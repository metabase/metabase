...
(api.macros/defendpoint :get "/:id/config"
  ...
  of the workspace's databases is not `:provisioned`. Treated as a write because the config grants
  enough access to a developer instance that partial per-database coverage isn't sufficient."
  [{:keys [id]} :- [:map [:id ms/PositiveInt]]]
  (api/write-check :model/Workspace id)
