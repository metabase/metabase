...
import S from "./JsBundleDropzone.module.css";
...
const ACCEPT_TYPES = {
  "application/javascript": ALLOWED_EXTENSIONS,
  "text/javascript": ALLOWED_EXTENSIONS,
};

export function JsBundleDropzone({ name = "file" }: { name?: string }) {
  ...
  const rejectionMessage = fileRejections[0]?.errors[0]?.code
    ? match(fileRejections[0].errors[0].code)
        .with("file-invalid-type", () => t`Bundle must be a .js file.`)
        .with("file-too-large", () => t`Bundle must be smaller than 5 MB.`)
        .otherwise(() => t`This file isn't a valid data-app bundle.`)
    : null;
...
function DropHint({ isDragActive }: { isDragActive: boolean }) {
  return (
    <>
      ...
      <Text fw={500}>
        {isDragActive
          ? t`Drop the index.js here`
          : t`Drag an index.js here, or click to browse`}
      </Text>
