...
The group warning endpoint checks each group plus All Users against synchronized table dependencies. It accepts
unrestricted, sandboxed, and impersonated access at the Metabase permission layer. Warnings do not depend on
current members and do not block assignments.

Only administrators can manage assignments or inspect warnings. Group listing and removal remain available after
the feature token expires; additions still require the Data Apps feature.
...
`group_access.clj` manages assignments and advisory warnings. `models/data_app_group.clj` defines the local association model.
