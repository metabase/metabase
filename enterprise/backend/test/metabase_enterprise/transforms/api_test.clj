...
;;; Transform Tag API Tests

(deftest transform-tag-endpoints-accessible-test
  (mt/with-premium-features #{:transforms}
    (testing "GET /ee/transform-tag"
      (mt/user-http-request :crowberto :get 200 "ee/transform-tag"))
    (testing "POST /ee/transform-tag"
      (mt/user-http-request :crowberto :post 200 "ee/transform-tag"
                            {:name "Test Tag"}))
    (testing "PUT /ee/transform-tag/:tag-id"
      (mt/user-http-request :crowberto :put 200 "ee/transform-tag/1"
                            {:name "Updated Tag"}))
    (testing "DELETE /ee/transform-tag/:tag-id"
      (mt/user-http-request :crowberto :delete 204 "ee/transform-tag/1"))))

;;; Transform Job API Tests

(deftest transform-job-endpoints-accessible-test
  (mt/with-premium-features #{:transforms}
    (testing "GET /ee/transform-job"
      (mt/user-http-request :crowberto :get 200 "ee/transform-job"))
    (testing "POST /ee/transform-job"
      (mt/user-http-request :crowberto :post 200 "ee/transform-job"
                            {:name "Test Job"
                             :schedule "0 0 * * *"
                             :description "Test job description"
                             :tag_ids [1 2]}))
    (testing "GET /ee/transform-job/:job-id"
      (mt/user-http-request :crowberto :get 200 "ee/transform-job/1"))
    (testing "PUT /ee/transform-job/:job-id"
      (mt/user-http-request :crowberto :put 200 "ee/transform-job/1"
                            {:name "Updated Job"
                             :schedule "0 */4 * * *"}))
    (testing "POST /ee/transform-job/:job-id/execute"
      (mt/user-http-request :crowberto :post 200 "ee/transform-job/1/execute"))
    (testing "DELETE /ee/transform-job/:job-id"
      (mt/user-http-request :crowberto :delete 204 "ee/transform-job/1"))))
