...
(deftest mbql-query-reads-the-workspace-table-test
  (testing "An MBQL query on a remapped table compiles to SQL naming the workspace table"
...
(deftest the-remappings-are-the-instance-default-test
  (testing "the workspace remappings are what the query processor reads with when none are bound"
    (mt/with-premium-features #{:workspaces}
      (mt/with-temporary-setting-values [workspaces-enabled true]
        (let [{:keys [schema name]} (lib.metadata/table (mt/metadata-provider) (mt/id :orders))]
          (mt/with-temp [:model/WorkspaceTableRemapping _ {:db_id       (mt/id)
                                                           ...
                                                           :to_table    "ws_remap_target"}]
            (clear-remappings-cache!)
            (is (= [{:from-schema schema, :from-table name, :to-schema schema, :to-table "ws_remap_target"}]
                   (table-remapping/remappings-for (mt/id))))))))))
