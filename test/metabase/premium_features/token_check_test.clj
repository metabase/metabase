  ...
  (testing "true when enable embedding sdk is true and dashboard count is greater than 0"
    (mt/with-temporary-setting-values [enable-embedding-sdk true]
      (mt/with-temp [:model/Dashboard _ {:enable_embedding true :archived false}]
        (is (:enabled-embedding-static (#'token-check/stats-for-token-request))))))
  (testing "true when enable embedding sdk is true and question count is greater than 0"
    (mt/with-temporary-setting-values [enable-embedding-sdk true]
      (mt/with-temp [:model/Card _ {:enable_embedding true :archived false :type :question}]
        (is (:enabled-embedding-static (#'token-check/stats-for-token-request))))))
  (testing "false when enable embedding sdk is true and no cards are dashboards are enabled"
    (mt/with-temporary-setting-values [enable-embedding-sdk true]
      (is (not (:enabled-embedding-static (#'token-check/stats-for-token-request))))))
    ...
    (mt/with-temporary-setting-values [saml-enabled                       true
      ...
      (testing "with enabled interactive embedding and a configured app origin"
        (mt/with-temporary-setting-values [enable-embedding-interactive      true
          ...
          (is (:enabled-embedding-interactive (#'token-check/stats-for-token-request)))))
      (testing "with disabled interactive embedding and a configured app origin"
        (mt/with-temporary-setting-values [enable-embedding-interactive      false
                                           embedding-app-origins-interactive "example.com"]
          (is (not (:enabled-embedding-interactive (#'token-check/stats-for-token-request))))))
...
(deftest enabled-embedding-sdk
  (testing "with sdk enabled and jwt enabled"
    (mt/with-temporary-setting-values [enable-embedding-sdk true
                                       jwt-enabled          true
                                       jwt-shared-secret    "asdfasdf"]
      (is (:enabled-embedding-sdk (#'token-check/stats-for-token-request)))))

  (testing "with sdk disabled and jwt enabled"
    (mt/with-temporary-setting-values [enable-embedding-sdk false
                                       jwt-enabled          true]
      (is (not (:enabled-embedding-sdk (#'token-check/stats-for-token-request))))))
