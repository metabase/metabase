...
(defn- save-execution-metadata!*
  ...
  (if-not context
    ...
    (do
      ...
      (t2/insert-returning-pk! :model/QueryExecution (dissoc query-execution :json_query :data-scanned-bytes :estimated-bytes :bytes-processed :bytes-billed :amount :unit :cents-per-unit :total-cents)))))
