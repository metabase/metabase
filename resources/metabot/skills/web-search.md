...
Do **not** search the web to find this instance's tables, models, questions or dashboards — use `search` and `read_resource` for those. Do not search the web for Metabase product documentation unless the user explicitly asks you to.
...
1. `web_search` with a query phrased like a Google search. For anything recent or time-sensitive, set `date` (`d`, `w`, `m`, …) so old pages don't crowd out current ones. Today's date is in your system prompt.
