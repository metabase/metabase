...
(def ^:private ConversationSummary
  ...
  `:user` is the conversation originator. Participant-based filtering is
  controlled by the query params."
  [:map
...
(def ^:private ListConversationsQueryParams
  ...
  `participant-user-id` / `participant-user-ids` are the preferred filters for
  matching any involved user. `user-id` / `user-ids` are compatibility aliases
  with the same semantics."
  [:map
   [:sort-by              {:optional true} [:maybe SortColumn]]
   [:sort-dir             {:optional true} [:maybe SortDirection]]
   [:participant-user-id  {:optional true} [:maybe ms/PositiveInt]]
   [:participant-user-ids {:optional true} [:maybe (ms/QueryVectorOf ms/PositiveInt)]]
   [:user-id              {:optional true} [:maybe ms/PositiveInt]]
   [:user-ids             {:optional true} [:maybe (ms/QueryVectorOf ms/PositiveInt)]]])
...
(def ^:private ConversationDetail
  ...
  `:user` is the conversation originator."
  [:map
   [:conversation_id ms/UUIDString]
   [:created_at      ms/TemporalInstant]
   [:summary         [:maybe :string]]
...
(defn- normalize-participant-user-ids
  "Normalize preferred participant filters plus the legacy `user-*` aliases into
  one deduplicated set of participant user IDs."
  [{:keys [participant-user-id participant-user-ids user-id user-ids]}]
  (not-empty
   (cond-> (set (concat participant-user-ids user-ids))
     participant-user-id (conj participant-user-id)
...
(api.macros/defendpoint :get "/conversations" :- ListConversationsResponse
  ...
  Response payloads return the conversation originator as `:user`. Filtering can
  target any participant via `participant-user-id` / `participant-user-ids`."
  [_route-params
   {:keys [sort-by sort-dir]
    :as   query-params} :- ListConversationsQueryParams]
  (api/check-superuser)
  (analytics.conversations/list-conversations
   {:limit    (request/limit)
    ...
    :sort-by  sort-by
    :sort-dir sort-dir
    :participant-user-ids (normalize-participant-user-ids query-params)}))
