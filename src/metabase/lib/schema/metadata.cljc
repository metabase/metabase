...
(mr/def ::column
  ...
  [:and
   [:map
    ...
    ;;
    ;; TODO (Cam 6/16/25) -- this should be REQUIRED if `:lib/source` = `:source/expressions`
    ...
    [:lib/source-uuid {:optional true} [:maybe ::lib.schema.common/uuid]]]
   ;; TODO (Cam 6/13/25) -- go add this to some of the other metadata schemas as well.
   ::kebab-cased-map
   ;; if this column metadata was derived from something like `:fields` then make sure we include `:lib/source-uuid`
   ;; for equality purposes.
   [:fn
    {:error/message "column should include :lib/source-uuid for columns that come from the query"}
    (fn [col]
      (if (#{:source/fields
             :source/aggregations
             :source/breakouts
           ...
           (:lib/source col))
        (:lib/source-uuid col)
        ;; things from table defaults and what not are not necessarily referenced in the query.
        true))]])
...
(mr/def ::stage
  ...
  [:map
   ...
   ;; TODO (Cam 6/16/25) -- if `:lib/desired-column-alias` is specified we should enforce uniqueness
   ;; like [[metabase.lib.metadata.calculation/ColumnsWithUniqueAliases]] (maybe move that schema into this
   ;; namespace?)
