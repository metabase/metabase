(ns metabase.models.field-values-test
  ...
  (:require
   ...
   [metabase.models.field-values :as field-values :refer [AdvancedFieldValues FullFieldValues MixedFieldValues]]
...
(deftest clear-field-values-for-field!-test
  (mt/with-temp [Database            {database-id :id} {}
                 ...
                 FullFieldValues     _              {:field_id field-id :values "[1, 2, 3]"}
                 AdvancedFieldValues _              {:field_id field-id :values "[4, 5, 6]"
                                                     :type     :sandbox :hash_key "1"}
                 ]
    (is (= [1 2 3]
           (t2/select-one-fn :values FullFieldValues, :field_id field-id)))
    (is (= [4 5 6]
           (t2/select-one-fn :values AdvancedFieldValues, :field_id field-id :type :sandbox :hash_key "1")))
...
(defn- find-values [field-values-id]
  (-> (t2/select-one FullFieldValues :id field-values-id)
...
(deftest implicit-deduplication-test
  (let [before (t/zoned-date-time)
    ...
    (mt/with-temp [:model/Database        {database-id :id} {}
                   ...
                   :model/FullFieldValues _                 {:field_id field-id :type :full :values ["a" "b"] :human_readable_values ["A" "B"] :created_at before :updated_at before}
                   :model/FullFieldValues _                 {:field_id field-id :type :full :values ["c" "d"] :human_readable_values ["C" "D"] :created_at before :updated_at later}
                   :model/FullFieldValues _                 {:field_id field-id :type :full :values ["e" "f"] :human_readable_values ["E" "F"] :created_at after :updated_at after}]

      (testing "When we have multiple FieldValues rows in the database, "
        (is (= 3 (count (t2/select FullFieldValues :field_id field-id :type :full :hash_key nil))))
        (testing "we always return the most recently updated row"
          ...
          (testing "... and older rows are implicitly deleted"
            (is (= 1 (count (t2/select FullFieldValues :field_id field-id :type :full))))
...
(deftest get-or-create-full-field-values!-test
  (mt/dataset test-data
    (testing "create a full Fieldvalues if it does not exist"
      (t2/delete! FullFieldValues :field_id (mt/id :categories :name) :type :full)
      (is (= :full (-> (t2/select-one Field :id (mt/id :categories :name))
                       field-values/get-or-create-full-field-values!
                       :type))
          (is (= 1 (t2/count FullFieldValues :field_id (mt/id :categories :name) :type :full))))

      (testing "if an Advanced FieldValues Exists, make sure we still returns the full FieldValues"
        (t2.with-temp/with-temp [AdvancedFieldValues _ {:field_id (mt/id :categories :name)
                                                        :type          :sandbox
      ...
      (testing "if an old FieldValues Exists, make sure we still return the full FieldValues and update last_used_at"
        ...
        (is (= (t/offset-date-time 2001 12)
               (:last_used_at (t2/select-one FullFieldValues :field_id (mt/id :categories :name) :type :full))))
...
(deftest normalize-human-readable-values-test
  (testing "If FieldValues were saved as a map, normalize them to a sequence on the way out"
    (t2.with-temp/with-temp [FullFieldValues fv {:field_id (mt/id :venues :id)
      ...
      (is (t2/query-one {:update :metabase_fieldvalues
                         ...
                         :where  [:= :id (:id fv)]}))
      (is (= ["a" "b" "c"]
             (:human_readable_values (t2/select-one FullFieldValues :id (:id fv))))))))
     ...
     (fn [^java.sql.Connection conn]
       ...
       (t2.with-temp/with-temp [Database db {:engine       :h2
         ...
         (let [table-id        (t2/select-one-fn :id Table :db_id (u/the-id db) :name "FOO")
               ...
               field-values-id (t2/select-one-fn :id FullFieldValues :field_id field-id)]
           ;; Add in human readable values for remapping
           (is (t2/update! FullFieldValues field-values-id {:human_readable_values ["a" "b" "c"]}))
  ...
  (testing "Should validate FieldValues :human_readable_values when"
    (testing "creating"
      (is (thrown-with-msg?
           ...
           (t2.with-temp/with-temp [FullFieldValues _ {:field_id (mt/id :venues :id), :human_readable_values {"1" "A", "2", "B"}}]))))
    (testing "updating"
      (t2.with-temp/with-temp [FullFieldValues {:keys [id]} {:field_id (mt/id :venues :id), :human_readable_values []}]
        (is (thrown-with-msg?
             clojure.lang.ExceptionInfo
             #"Invalid human-readable-values"
             (t2/update! FullFieldValues id {:human_readable_values {"1" "A", "2", "B"}})))))))
  ...
  (testing "Make sure FieldValues are calculated and saved correctly when remapping is in place (#13235)"
    (mt/dataset test-data
      (mt/with-temp-copy-of-db
        (letfn [(field-values []
                  (t2/select-one FullFieldValues :field_id (mt/id :orders :product_id)))]
    ...
    (let [field-id (mt/id :venues :id)]
      (try
        (is (thrown-with-msg? ExceptionInfo
                              #"FullFieldValues shouldnt have hash_key"
                              (t2/insert! FullFieldValues :field_id field-id :hash_key "12345")))
        (is (thrown-with-msg? ExceptionInfo
                              #"FullFieldValues shouldnt have hash_key"
                              (t2/insert! FullFieldValues :field_id field-id :type :full :hash_key "12345")))
        (is (thrown-with-msg? ExceptionInfo
                              #"AdvancedFieldValues require a hash_key"
                              (t2/insert! AdvancedFieldValues :field_id field-id :type :sandbox)))
        (is (thrown-with-msg? ExceptionInfo
                              #"AdvancedFieldValues require a hash_key"
                              (t2/insert! AdvancedFieldValues :field_id field-id :type :sandbox :hash_key " ")))
        (finally
          ;; Clean up in case there were any failed assertions, and we ended up inserting values
          (t2/delete! MixedFieldValues :field_id field-id))))))

(deftest update-field-values-hook-test
  (t2.with-temp/with-temp [FullFieldValues     {full-id :id}    {:field_id (mt/id :venues :id)
                                                                 :type      :full}
                           AdvancedFieldValues {sandbox-id :id} {:field_id (mt/id :venues :id)
                                                                 :type      :sandbox
                                                                 :hash_key  "random-hash"}]
    (testing "The model hooks prevent us changing the intrinsic identity of a field values"
      (doseq [[id update-map] [[sandbox-id {:field_id 1}]
        ...
        (is (thrown-with-msg? ExceptionInfo
                              ...
                              ;; TODO specialize
                              (t2/update! MixedFieldValues id update-map)))))
    ...
    (testing "The model hooks permits mention of the existing values"
      (doseq [[id update-map] [[full-id {:field_id (mt/id :venues :id)}]
                               ...
                               ;; TODO specialize
        (is (some? (t2/update! MixedFieldValues id update-map)))))))

(deftest insert-full-field-values-should-remove-all-cached-field-values
  (doseq [explicitly-full? [false true]]
    (mt/with-temp [AdvancedFieldValues sandbox-fv {:field_id (mt/id :venues :id)
                                           :type          :sandbox
                                           :hash_key      "random-hash"}]
      (t2/insert! FullFieldValues (cond-> {:field_id (mt/id :venues :id)} explicitly-full? (assoc :type :full)))
      (is (not (t2/exists? FullFieldValues :id (:id sandbox-fv)))))))

(deftest update-full-field-values-should-remove-all-cached-field-values
  (mt/with-temp [FullFieldValues fv      {:field_id (mt/id :venues :id)
                                         :type          :full}
                 AdvancedFieldValues sandbox-fv {:field_id (mt/id :venues :id)
                                         :type          :sandbox
                                         :hash_key      "random-hash"}]
    (t2/update! FullFieldValues (:id fv) {:values [1 2 3]})
    ;; TODO this still passes even if you specify FullFieldValues.. will the hook adding a :type have perf edges?
    (is (not (t2/exists? AdvancedFieldValues :id (:id sandbox-fv))))))
...
(deftest update-full-field-without-values-should-remove-not-all-cached-field-values
  (mt/with-temp [FullFieldValues fv         {:field_id (mt/id :venues :id)
                 ...
                 AdvancedFieldValues sandbox-fv {:field_id (mt/id :venues :id)
                                         :type          :sandbox
                                         :hash_key      "random-hash"}]
    (t2/update! FullFieldValues (:id fv) {:updated_at (t/zoned-date-time)})
    (is (t2/exists? AdvancedFieldValues :id (:id sandbox-fv)))))
...
(deftest identity-hash-test
  (testing "Field hashes are composed of the name and the table's identity-hash"
    (mt/with-temp [Database        db    {:name "field-db" :engine :h2}
                   ...
                   FullFieldValues fv    {:field_id (:id field)}]
      (is (= "6f5bb4ba"
...
(deftest select-coherence-test
  (testing "We cannot perform queries with invalid mixes of type and hash_key, which would return nothing"
    (t2/select FullFieldValues :field_id 1)
    (t2/select FullFieldValues :field_id 1 :type :full)
    (is (thrown-with-msg? ExceptionInfo
                          #"Assert failed" ;; #"Invalid query - FullFieldValues cannot have a hash_key"
                          (t2/select FullFieldValues :field_id 1 :type :full :hash_key "12345")))

    ;(t2/select AdvancedFieldValues :field_id 1 :type :sandbox)
    (t2/select AdvancedFieldValues :field_id 1 :type :sandbox :hash_key "12345")
    (is (thrown-with-msg? ExceptionInfo
                          #"Invalid query - AdvancedFieldValues can only specify a non-empty hash_key"
