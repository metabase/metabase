function encodeExplorerState(state) {
  ...
  if (state.expanded.size)
    params.set("exp", [...state.expanded].sort().join(","));
...
function decodeExplorerState(hash, validIds, validTeams) {
  ...
  return {
    ...
    teams: new Set(
      params.getAll("team").filter((team) => validTeams.has(team)),
    ),
  };
}
