...
(defn tile-parameters-for-card
  ...
  [card-or-id token-params parameter-values]
  (let [parameters (resolve-card-parameters card-or-id)]
    (apply-slug->value parameters
                       (validate-and-merge-params (t2/select-one-fn :embedding_params :model/Card :id (u/the-id card-or-id))
                                                  token-params
...
(defn tile-parameters-for-dashboard
  ...
  [dashboard-id token-params parameter-values]
  (let [{:keys [parameters embedding_params]} (t2/select-one [:model/Dashboard :parameters :embedding_params]
                                                             :id dashboard-id)]
    (resolve-dashboard-parameters dashboard-id
                                  (validate-and-merge-params embedding_params
                                                             token-params
                                                             (tile-slug->value parameters parameter-values)))))
