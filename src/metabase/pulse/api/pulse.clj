...
(def ^:private PulseChannelType
  "Matches pulse-channel.clj's `channel-types` map: :email, :slack, :http."
  [:enum "email" "slack" "http"])

(def ^:private PulseScheduleType
  "Matches pulse-channel.clj's `schedule-types` set."
  [:enum "hourly" "daily" "weekly" "monthly"])

(def ^:private PulseScheduleDay
  "Matches pulse-channel.clj's `days-of-week` :id values (== frontend ScheduleDayType,
...
(def ^:private PulseChannelRecipient
  "A recipient is either a Metabase user (by id) or a plain email address -- see
  update-pulse-channel!/create-pulse-channel!'s `#(or (:id %) (:email %))`."
  [:map
   [:id    {:optional true} [:maybe ms/PositiveInt]]
   [:email {:optional true} [:maybe ms/Email]]])

(def ^:private PulseChannelDetails
  "The detail keys the send pipeline reads off a pulse channel: `attachment_only`/`include_pdf` for email
  channels, `channel`/`channel_id` for Slack channels, plus `channels` -- the legacy spelling of `channel` the FE
  still sends and reads back for Slack subscriptions. `emails` is populated server-side from `recipients` but
  legacy callers may send it directly."
  [:map
...
(api.macros/defendpoint :post "/test"
  ...
  [_route-params
   ...
   {:keys [cards channels] :as body} :- [:map
                                         ...
                                         [:collection_position {:optional true} [:maybe ms/PositiveInt]]
                                         [:dashboard_id        {:optional true} [:maybe ms/PositiveInt]]
                                         [:parameters          {:optional true} [:maybe ::parameters.schema/parameter-values]]
