...
(defmethod sso.i/sso-get :slack-connect
  ...
  (cond
    ...
    (not code)
    (let [;; For link-only mode, require authenticated session
          _ (when (and (= "link-only" (sso-settings/slack-connect-authentication-mode))
                       (not (:metabase-user-id session)))
          ...
          ;; Call provider to get authorization URL
      ...
      (cond
        ;; Need redirect to Slack
        (= :redirect (:success? auth-result))
        (do
          ;; Store state and nonce in session for validation on callback
          (log/infof "Redirecting to Slack for authentication")
          (-> (response/redirect (:redirect-url auth-result))
              (assoc :session (merge session
                                     {:slack-oauth-state (:state auth-result)
                                      :slack-oauth-nonce (:nonce auth-result)
                                      :slack-redirect redirect-url}))))
    ...
    (let [;; Validate state matches what we stored
          stored-state (get session :slack-oauth-state)
          stored-nonce (get session :slack-oauth-nonce)
          stored-redirect (get session :slack-redirect "/")
          ...
          ;; Call provider to complete login
      ...
      (cond
        ...
        (let [;; Get redirect URL (prefer from result, fallback to stored)
              final-redirect (or (:redirect-url login-result) stored-redirect)
              ;; Clear OAuth-related session data
              clean-session (dissoc session :slack-oauth-state :slack-oauth-nonce :slack-redirect)]
          ...
          (request/set-session-cookies request
                                       ...
                                       (merge (:session login-result) clean-session)
