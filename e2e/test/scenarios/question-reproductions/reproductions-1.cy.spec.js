...
describe("issue 9027", () => {
  beforeEach(() => {
    ...
    cy.focusNativeEditor().type("select 0");
    cy.findByTestId("native-query-editor-container").icon("play").click();

    cy.saveQuestion(QUESTION_NAME, undefined, {
      tab: "Browse",
      path: ["Our analytics"],
    });
  });
...
describe("issue 13097", { tags: "@mongo" }, () => {
  beforeEach(() => {
    ...
    cy.withDatabase(MONGO_DB_ID, ({ PEOPLE_ID }) => {
      ...
      const hash = cy.adhocQuestionHash(questionDetails);
...
describe("issue 14957", { tags: "@external" }, () => {
  ...
  it("should save a question before query has been executed (metabase#14957)", () => {
    cy.openNativeEditor({ databaseName: PG_DB_NAME }).type(
      "select pg_sleep(60)",
    );
    cy.saveQuestion("14957", undefined, {
      tab: "Browse",
      path: ["Our analytics"],
    });
...
describe("postgres > question > custom columns", { tags: "@external" }, () => {
  ...
  it("`Percentile` custom expression function should accept two parameters (metabase#15714)", () => {
    ...
    // eslint-disable-next-line no-unscoped-text-selectors -- deprecated usage
    cy.findByText("Custom Expression").click();
    cy.enterCustomColumnDetails({ formula: "Percentile([Subtotal], 0.1)" });
...
describe("issue 17963", { tags: "@mongo" }, () => {
  ...
  it("should be able to compare two fields using filter expression (metabase#17963)", () => {
    ...
    cy.getNotebookStep("filter").findByText(
      "Discount is greater than Quantity",
    );
...
function typeAndSelect(arr) {
  arr.forEach(({ string, field }) => {
    ...
    cy.popover().contains(field).click();
  });
}
...
describe("issues 11914, 18978, 18977, 23857", () => {
  ...
  it("should not display query editing controls and 'Browse databases' link", () => {
    ...
    cy.popover().findByText("Duplicate").should("not.exist");
    ...
    cy.commandPaletteSearch("Repro", false);
    cy.commandPalette().findByText("Repro").click();
    cy.findByLabelText("Move, trash, and more…").click();
    cy.popover().findByText("Duplicate").should("not.exist");
...
describe("issue 19341", () => {
  ...
  it("should correctly disable nested queries (metabase#19341)", () => {
    ...
    cy.entityPickerModal().findAllByRole("tab").should("not.exist");
...
function selectFromDropdown(optionName) {
  cy.popover().findByText(optionName).click();
}
...
describe("issue 20809", () => {
  ...
  beforeEach(() => {
    ...
    cy.createQuestion(questionDetails).then(({ body: { id } }) => {
      ...
      cy.visitQuestionAdhoc(nestedQuestion, { mode: "notebook" });
    });
  });
