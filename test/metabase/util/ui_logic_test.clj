...
(deftest find-goal-value-test
  (testing "Progress chart goal value extraction"
    (testing "with numeric goal"
    ...
    (testing "with column reference that exists and is numeric"
      (let [result {:card {:display :progress
    ...
    (testing "with column reference that exists but is not numeric"
      (let [result {:card {:display :progress
    ...
    (testing "with column reference that doesn't exist"
      (let [result {:card {:display :progress
...
(deftest extract-goal-value-from-column-test
  (testing "Extract goal value from column"
    (testing "with valid column and data"
      (let [columns [{:name "value"} {:name "target"}]
            rows [[100 200]]]
        (is (= 200 (#'ui-logic/extract-goal-value-from-column "target" columns rows)))))

    (testing "with infinity value"
      (let [columns [{:name "value"} {:name "target"}]
            rows [[100 "Infinity"]]]
