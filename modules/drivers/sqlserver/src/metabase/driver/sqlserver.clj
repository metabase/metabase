...
(defn- bit->boolean-expr
  ...
  ;;
  ;; SQLServer uses 0 and 1 for boolean constants, but these actually have type BIT, and BITs are not allowed in the
  ;; top level of a WHERE clause or AND or OR expressions. These expect a true Boolean, but seemingly the only way to
  ;; get one is as the result of a comparison operator. For example, none of the following queries are valid:
  ;;
  ;; - SELECT 1 WHERE 1
...
(defn- compile-and-or [driver [tag & _ :as clause]]
