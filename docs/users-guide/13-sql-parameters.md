...
Some databases requires the schema in the `FROM` clause. Example for Oracle would be `FROM "schema"."table"` and for BigQuery backticks are needed like ``FROM `dataset_name.table` ``
