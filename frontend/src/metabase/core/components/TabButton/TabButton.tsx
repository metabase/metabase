...
const getBorderStyle = (theme: Theme) => css`
  ...
  box-shadow: 0 0 0 1px ${lighten(theme.fn.themeColor("brand"), 0.28)};
`;
export const RenameableTabButtonStyled = styled(_TabButton)<{
  ...
  ${TabButtonInputWrapper} {
    ${({ isRenaming, theme }) => isRenaming && getBorderStyle(theme)}
    :hover {
      ${({ canRename, isSelected, theme }) =>
        canRename && isSelected && getBorderStyle(theme)}
    }
  }
`;
