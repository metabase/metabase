...
;; -----------------------------------------------------------------------------
;; GHY-3709: foreign-grantor default-priv handling (Redshift only).
;;
;; Repro shape (provision-side):
;;   1. As ROLE A (=stats_admin in prod): `ALTER DEFAULT PRIVILEGES IN SCHEMA <s>
;;      GRANT SELECT ON TABLES TO <some-role>`. Catalog row owned by A.
;;   2. Metabase later runs `grant-workspace-read-access!` on schema <s> while
;;      connected as ROLE B (the warehouse's Metabase user). New default-priv
;;      row owned by B targets iso-user.
;;   3. At destroy time, REVOKE without `FOR USER A` leaves A's row in place.
;;      `DROP USER iso-user` fails on "privileges for default privileges on new
;;      relations belonging to user A in schema <s>".
;;
;; Coverage goals:
;;   - Prevention: grant-time pre-flight raises if A is not member of B.
;;   - Cure:       destroy-time REVOKE iterates pg_default_acl and emits
;;                 `FOR USER <grantor>` REVOKEs so DROP USER succeeds.
;;
;; TODO(bryan): write the body. ~5-15 lines. Suggested shape:
;;
;;   (mt/test-driver :redshift
;;     (let [admin-spec (sql-jdbc.conn/connection-details->spec :redshift (:details (mt/db)))
;;           run-id     (random-suffix)
;;           in-schema  (str "mb_iso_in_" run-id)
;;           grantor    (str "mb_test_grantor_" run-id)
;;           workspace  {:id (Long/parseLong run-id 16) :name (str "wsd-ghy3709-" run-id)}]
;;       (try
;;         ;; 1. Create input schema + a side-role to act as foreign grantor.
;;         ;; 2. As that side-role, ALTER DEFAULT PRIVILEGES IN SCHEMA <s>
;;         ;;    GRANT SELECT ON TABLES TO PUBLIC (or some dummy role) -- this
;;         ;;    seeds a pg_default_acl row owned by the foreign grantor.
;;         ;; 3. Decide: do you assert prevention (grant-workspace-read-access!
;;         ;;    throws 412) OR cure (init + grant + destroy round-trips clean)?
;;         ;;    Cleanest is BOTH:
;;         ;;     - assert grant throws because current_user isn't member of grantor
;;         ;;     - then GRANT current_user TO grantor-or-vice-versa, re-run,
;;         ;;       provision+destroy, and verify no leftover pg_default_acl rows
;;         ;;       reference the iso-user.
;;         (finally
;;           ;; drop the side-role + input schema as admin
;;           ))))
;;
;; Key design choices for you:
;;   A. Does the side-role get superuser? (simpler; closer to stats_admin)
;;   B. Do you want one deftest with both prevention+cure subtests, or two
;;      separate deftests? I'd vote one deftest with two `testing` blocks --
;;      tighter cleanup story.
;;
(deftest ^:synchronized ^:mb/driver-tests ghy-3709-foreign-grantor-default-priv-test
  (mt/test-driver :redshift
    (testing "TODO(bryan): write the body — skeleton + decisions above this deftest"
      (is true))))
