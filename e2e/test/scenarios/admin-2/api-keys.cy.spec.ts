...
describe("scenarios > admin > settings > API keys", () => {
  ...
  it("should show number of API keys on auth card", () => {
    ...
    cy.createApiKey("Test API Key One", ALL_USERS_GROUP_ID);
    ...
    cy.createApiKey("Test API Key Two", ALL_USERS_GROUP_ID);
    cy.createApiKey("Test API Key Three", ALL_USERS_GROUP_ID);
  ...
  it("should list existing API keys", () => {
    cy.createApiKey("Test API Key One", ALL_USERS_GROUP_ID);
    cy.createApiKey("Test API Key Two", NOSQL_GROUP_ID);
    cy.createApiKey("Test API Key Three", READONLY_GROUP_ID);

    cy.visitApiKeySettings();
  ...
  it("should allow creating an API key", () => {
    ...
    cy.visitApiKeySettings();
    cy.tryToCreateApiKeyViaModal({ name, group });
    cy.wait("@getKeys");
  ...
  it("should show an error when a previously used key name is submitted", () => {
    ...
    cy.tryToCreateApiKeyViaModal({ name, group }).then(({ response }) => {
  ...
  it("should allow editing an API key", () => {
    cy.createApiKey("Development API Key", ALL_USERS_GROUP_ID);
    cy.visitApiKeySettings();
  ...
  it("should allow regenerating an API key", () => {
    cy.createApiKey("Personal API Key", ALL_USERS_GROUP_ID);

    cy.visitApiKeySettings().then(({ response }) => {
  ...
  describe("api key actions", () => {
    it("should allow creating questions and dashboards with an API key", () => {
      cy.createApiKey("Test API Key One", ADMINISTRATORS_GROUP_ID).then(
        ({ body }) => {
