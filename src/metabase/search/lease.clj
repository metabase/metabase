  ...
  (loop [error error]
    (cond
      ...
      (let [^SQLException sql-error error]
        (or (= (search.db/sql-states :unique-violation) (.getSQLState sql-error))
            (and (= (search.db/sql-states :integrity-constraint-violation) (.getSQLState sql-error))
                 (= (search.db/error-codes :mysql/duplicate-entry) (.getErrorCode sql-error)))
