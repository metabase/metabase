...
(mr/def ::row
  "A row being written by an action: column name -> value. Each value is held to
  `:metabase.lib.schema.parameter/parameter.value` -- a scalar, or a sequence of scalars -- the same schema the
  request layer enforces on action parameter and cell values."
