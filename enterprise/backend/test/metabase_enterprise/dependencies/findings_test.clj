...
(deftest ^{:parallel false} can-analyze-entity-batches-test
...
(deftest ^{:parallel false} does-not-repeatedly-analyze-entities-test
  (backfill-all-entity-analyses!)
  (let [mp (mt/metadata-provider)
        products-id (mt/id :products)
        orders-id (mt/id :orders)
...
(deftest ^{:parallel false} re-analyze-entities-when-analysis-version-bumped-test
  (backfill-all-entity-analyses!)
  (let [mp (mt/metadata-provider)
...
(deftest ^{:parallel false} does-analyze-native-entities-test
...
(deftest ^{:parallel false} does-report-errors-for-missing-refs-in-fields-test
...
(deftest ^{:parallel false} analyze-batch-picks-up-missing-analyses-test
...
(deftest ^{:parallel false} mark-subtree-stale-then-drain-test
...
(deftest ^{:parallel false} transitive-marking-through-non-analyzable-entity-test
...
(deftest ^{:parallel false} cyclic-dependency-drain-terminates-test
...
(deftest ^{:parallel false} unanalyzable-entity-records-terminal-error-test
...
(deftest ^{:parallel false} unanalyzable-entity-rechecks-when-database-resolvable-again-test
...
(deftest ^{:parallel false} analyze-batch-shares-one-metadata-provider-test
