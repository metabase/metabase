...
(defn- resolve-definition
  ...
  (some-> (cond
            definition   (if (v2.queries/portable-query? definition)
                           ...
                           definition)
            query_handle (:query (v2.queries/resolve-query-handle-for-save!
...
(defn- check-metric-shape!
  ...
  [dataset-query]
  ;; Whole-tree, not lib/native-only-query? (which sees only a single pure-native stage): a
  ;; definition with a native stage anywhere — e.g. native + an appended MBQL stage — is still SQL
  ...
  (when (query-guards/native-query? dataset-query)
    (common/throw-teaching-error
     (message/msg [(str "A metric can't be built from a native (SQL) query — metrics are "
                        "MBQL so other queries can reuse them. Save it with question_write "
                        "instead, or rebuild the aggregation with execute_query.")])))
  (when-not (lib/can-save? dataset-query :metric)
    (common/throw-teaching-error
     (message/msg [(str "This query can't be saved as a metric. %s Build it with execute_query first "
...
(defn- update!
  ...
  (let [card-before  (v2.resolve/resolve-and-read-with
        ...
        new-coll-id  (when (contains? args :collection_id)
...
(def ^:private metric-write-args-schema
  [:map {:closed true}
   ...
   [:query_handle {:optional true}
    [:maybe [:string {:min 1 :description (str "A query_handle from execute_query — saves exactly the query "
   ...
   [:collection_id {:optional true}
    [:maybe [:or
             ...
             [:string {:description
                       "21-character entity_id of the collection, or \"root\" for the root collection."}]]]]
   [:collection_position {:optional true}
    [:maybe [:int {:min 1 :description "Pins the metric at this position in its collection."}]]]
   [:clear {:optional true}
    [:maybe [:sequential [:enum {:description
                                 (str "Update only: property names to unset (description, collection_position). "
                                      "Needed because a null cannot say \"clear this\" — strict clients fill "
                                      "every unset property with null, so nulls are stripped at the boundary.")}
