...
(doseq [driver [:redshift :databricks :athena]]
...
(comment
  ...
  (mt/set-test-drivers! #{:athena})

  (routed-dataset-name :athena)

  (mt/test-driver :athena
    (binding [metabase.test.data.athena/*allow-database-creation* true]
      (mt/dataset (mt/dataset-definition "temp_data_1"
                                         [["t"
                                           [{:field-name "f", :base-type :type/Text}]
                                           [["temp-foo-1"]
                                            ["temp-bar-1"]]]])
  ...
  (mt/test-drivers (mt/normal-driver-select {:+features [:database-routing]})
    (mt/with-premium-features #{:database-routing}
      (binding [tx/*use-routing-dataset* true
                tx/*use-routing-details* true]
        (mt/dataset (mt/dataset-definition "db_routing_test"
                                           [["t"
                                             [{:field-name "f", :base-type :type/Text}]
                                             [["east_2_foo"]
                                              ["east_2_bar"]]]])
                ...
                (mt/dataset (mt/dataset-definition "db_routing_test"
                  ...
                  (let [router (mt/db)]
                    ...
                    (mt/with-temp [:model/DatabaseRouter _ {:database_id    (u/the-id router)
                                                            :user_attribute "db_name"}]
                      (met/with-user-attributes! :rasta {"db_name" (:name routed)}
                        (tap> {:crowberto (mt/with-current-user (mt/user->id :crowberto)
                                            (mt/rows (mt/process-query (mt/query t))))})
                        (tap> {:rasta (mt/with-current-user (mt/user->id :rasta)
                                        (mt/rows (mt/process-query (mt/query t))))}))))))))))))
