...
(def ^:private construct-query-tool-description
  ...
  (str
   ...
   "2. MUST read the MCP widget context before calling `construct_query` if the user refers to a visible Metabase view using words like `this`, `that`, `based on this`, "
   "`current chart`, `current table`, `drilled result`, or asks to continue from a chart/table already shown, "
   "then use its active view / drill nextCard as the source context. "
   "If multiple widget views could match, ask the user which one they mean instead of guessing.\n"
   "3. Call `construct_query` with the program. You get back `{\"query_handle\": \"<uuid>\"}`.\n"
   "4. Pass that handle to `execute_query` or `visualize_query`.\n"
