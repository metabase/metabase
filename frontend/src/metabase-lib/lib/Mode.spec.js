    ...
    describe("actions()", () => {
        describe("for a new question with Orders table and Raw data aggregation", () => {
            ...
            it("returns 'View this as a table' as mode action 1", () => {
                expect(rawDataQuestionMode.actions()[0].name).toBe("underlying-data");
                expect(rawDataQuestionMode.actions()[0].icon).toBe("table");
                expect(rawDataQuestionMode.actions()[0].title).toBe("View this as a table");
            });
            it("returns a defined metric as mode action 2", () => {
                expect(rawDataQuestionMode.actions()[1].name).toBe("common-metric");
                // TODO: Sameer 6/16/17
            ...
            it("returns a count timeseries as mode action 3", () => {
                expect(rawDataQuestionMode.actions()[2].name).toBe("count-by-time");
            ...
            it("returns summarize as mode action 4", () => {
                ...
                expect(rawDataQuestionMode.actions()[3].title).toBe("Summarize this segment");
            });
        })
        ...
        describe("for a question with an aggregation and a time breakout", () => {
            it("has pivot as mode actions 1 and 2", () => {
                ...
                expect(timeBreakoutQuestionMode.actions()[0].name).toBe("pivot-by-category");
                expect(timeBreakoutQuestionMode.actions()[1].name).toBe("pivot-by-location");
            })
        })
    })
