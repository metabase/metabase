...
(def ^:private ^{:arglists '([db-id])} cached-remappings-for-db
  "The remappings of the Database with `db-id` in the worktree being worked in, cached for a few seconds so the query
  processor does not hit the app DB on every query. Writes on this instance clear it; other instances see them once
  the entry expires.

  The worktree is part of the cache key, and has to be. `:hook/worktree-id` restricts the *query*, but a cache hit
  returns a previous result without running one: keyed on the database alone, a read in one worktree would be served
  another worktree's remappings from a warm entry, naming tables it does not own. Plausible rows that are wrong,
  which is worse than an error."
  (memoize/ttl
   ^{::memoize/args-fn (fn [[db-id]] [(mdb/unique-identifier) (mdb.worktree/worktree-id) db-id])}
...
(mu/defn- workspace-schema-for-database :- ::lib.schema.common/non-blank-string
  ...
  Takes the whole Database rather than its id: `setting/with-database` and `driver.u/supports?` both read it, and
  its database-local settings are where the schema lives."
  [{:keys [engine] :as database} :- (ms/InstanceOf :model/Database)]
