(ns metabase.driver.mongo.execute.common
  (:require
   [clojure.set :as set]
  ...
  (:import
   (com.mongodb BasicDBObject Cursor DBObject)
...
(def ^:const batch-size
  "Common option for aggregations. Used eg. in [[mongo.execute.session/init-aggregate!]]"
  100)

(def ^:const allow-disk-use
  "Common option for aggregations. Used eg. in [[mongo.execute.session/init-aggregate!]]"
  true)
...
;; using Iterator so we are able to call also for MongoCursor and ... CONTINUE HERE
