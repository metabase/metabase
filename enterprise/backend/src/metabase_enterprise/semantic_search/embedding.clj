...
(def ^:private ^Encoding openai-encoding
  "Cached OpenAI tokenizer encoding for cl100k_base (used by text-embedding models)."
...
(defn- create-batches
  ...
  (let [step (fn [{:keys [current-batch current-tokens batches] :as acc} text]
               (let [tokens (count-tokens text)]
                 (cond
                   ;; Single text exceeds the limit
                   (> tokens *max-tokens-per-batch*)
                   (do
                     ;; TODO: handle this case more gracefully
                     (log/warn "Text exceeds maximum tokens per batch:"
                               {:tokens tokens :max-tokens *max-tokens-per-batch*})
                     (assoc acc
                            :current-batch []
                            :current-tokens 0
                            :batches (conj batches [text])))
