...
(mu/defmethod channel/render-notification [:channel/http :notification/card]
  [_channel-type {:keys [payload creator data-provider]} _template _recipients]
  (let [{:keys [card alert card_part]} payload
        card_part                         (update card_part :result channel.shared/realize-qp-data-rows data-provider)
