...
(mr/def ::data-app.allowed-hosts
  "The `:allowed_hosts` column of a DataApp, decoded."
  [:sequential :string])
...
(mr/def ::data-app
  ...
  [:map {:closed true}
   ...
   [:name            :string]
   [:display_name    :string]
   [:allowed_hosts   [:maybe ::data-app.allowed-hosts]]
   [:bundle_path     :string]
...
(mr/def ::data-app.update
  ...
  [:map {:closed true}
   ...
   [:allowed_hosts   {:optional true} [:maybe ::data-app.allowed-hosts]]
