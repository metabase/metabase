...
(defn ^:hydrate fields
  ...
  [{:keys [id]}]
  (db/sel Field, :table_id id, :visibility_type [:not= "retired"], {:order-by [[:postion :asc]
                                                                               [:name    :asc]]}))
...
(defn field-values
  ...
  [{:keys [id]}]
  (let [field-ids (db/sel-ids Field, :table_id id, :visibility_type "normal", {:order-by [[:position :asc]
                                                                                          [:name     :asc]]})]
    (db/sel-field->field :field_id :values FieldValues, :field_id [:in field-ids])))
...
(defn table-id->database-id
  ...
  (db/sel-1-field :db_id Table, :id table-id))
