...
(s/defn bookmarks-for-user :- [BookmarkResult]
  ...
  (->> (mdb.query/query
        {:select    [[:bookmark.created_at        :created_at]
                     ...
                     [:card.name                  (db/qualify :m/card :name)]
                     [:card.dataset               (db/qualify :m/card :dataset)]
                     [:card.display               (db/qualify :m/card :display)]
                     [:card.description           (db/qualify :m/card :description)]
                     [:card.archived              (db/qualify :m/card :archived)]
