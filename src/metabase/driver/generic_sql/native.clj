...
(defn- value->base-type
  ...
  (if-not v :UnknownField
          (or (common/class->base-type (type v))
              (throw (ApiException. (int 500) (format "Missing base type mapping for %s in common/metabase.driver.generic-sql.native/class->base-type. Please add an entry."
                                                      (str (type v))))))))
