...
(defn access-control-headers
  ...
  (let [localhost-origin? (approved-origin? origin "localhost:*")]
    ;; If the origin is localhost, we allow it to be used in the SDK, even if `enabled?` is `false`.
    (when (or enabled? localhost-origin?)
      (merge
