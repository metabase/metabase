...
(deftest table-details-own-columns-take-data-access-test
  (testing "a bare manage-table-metadata grant admits the caller to the table but not to its columns"
    (mt/test-driver :h2
      (mt/with-no-data-perms-for-all-users!
        (perms/set-table-permission! (perms-group/all-users) (mt/id :people) :perms/view-data :blocked)
        ...
        (mt/with-current-user (mt/user->id :rasta)
          (let [output (:structured-output
                        (entity-details/get-table-details {:table-id           (mt/id :people)
                                                           :with-field-values? false}))]
            (testing "the read check passes, so the table itself renders"
              (is (= (mt/id :people) (:id output))))
            (testing "but every column is filtered out"
              (is (empty? (:fields output))))))))))
