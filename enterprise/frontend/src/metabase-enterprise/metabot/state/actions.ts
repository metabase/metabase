...
export const selectUserConfirmationOption = createAsyncThunk(
  ...
  async (message: string, { dispatch, getState }) => {
    ...
    if (!confirmationOption) {
      // TODO: work with design to handle error case
      const optionsValues = Object.keys(userConfirmationOptions);
      const optionString = t`Valid options: ${optionsValues.join(", ")}.`;
      dispatch(addUserMessage(optionString));
    } else {
      dispatch(clearUserMessages());
...
export const sendWritebackMessageRequest = createAsyncThunk(
  ...
  async (message: string, { dispatch, getState }) => {
    ...
    const lastHistory = getLastRecievedHistory(getState() as any);
