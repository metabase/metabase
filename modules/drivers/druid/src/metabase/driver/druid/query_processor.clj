...
(def ^:private ^{:arglists '([clause])} field?
  (partial legacy-mbql/is-clause? :field))
...
(defn- filter:nil?
  [clause-or-field]
  (if (legacy-mbql/is-clause? #{:+ :- :/ :*} clause-or-field)
...
(mu/defn- add-datetime-units* :- :legacy-mbql/date-time-value
  "Return a `relative-datetime` clause with `n` units added to it."
  [absolute-or-relative-datetime :- :legacy-mbql/date-time-value
   n                             :- number?]
  (if (legacy-mbql/is-clause? :relative-datetime absolute-or-relative-datetime)
...
(defmulti ^:private filter-clause->intervals
  ...
  (fn [filter-clause]
    (when (lib.util.match/match-one filter-clause [:field _ (_ :guard :temporal-unit)])
      (legacy-mbql/dispatch-by-clause-name-or-class filter-clause))))
...
(defmethod filter-clause->intervals :not
  ...
  (when (seq (filter-clause->intervals subclause))
    (filter-clause->intervals (legacy-mbql/negate-filter-clause subclause))))
...
(defn- expression->field-names
  ...
  (flatten (for [arg   args
                 :when (or (field? arg)
                           (legacy-mbql/is-clause? #{:+ :- :/ :*} arg))]
             (cond
               (legacy-mbql/is-clause? #{:+ :- :/ :*} arg) (expression->field-names arg)
...
(defn- ag:doubleSum
  ...
  (if (legacy-mbql/is-clause? #{:+ :- :/ :*} field-clause)
...
(defn- ag:doubleMax
  [field output-name]
  (if (legacy-mbql/is-clause? #{:+ :- :/ :*} field)
...
(defn- ag:count
  ...
  ([field output-name]
   (if (and (legacy-mbql/is-clause? #{:field-id} field)
            (hyper-unique? field))
     {:type      :hyperUnique
      :name      output-name
      :fieldName (->rvalue field)}
...
(mu/defn- handle-aggregation
  [query-type
   ag-clause :- :legacy-mbql.clause/aggregation
   druid-query]
...
(defn- unwrap-name
  [x]
  (if (legacy-mbql/is-clause? :aggregation-options x)
    (second x)
    x))
