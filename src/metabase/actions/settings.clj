...
(setting/defsetting database-enable-actions
  (i18n-be/deferred-tru "Whether to enable Actions for a specific Database.")
  :default          false
...
(def ^:private db-routing-reason
  {:key     :setting/database-routing
   :type    :error
   :message (i18n-be/deferred-tru "Table editing is not supported with database routing.")})

(def ^:private no-writable-tables-reason
  {:key     :permissions/no-writable-table
   :type    :error
   :message (i18n-be/deferred-tru "Table editing requires at least one table with INSERT, UPDATE, and DELETE support.")})

(def ^:private busy-sync-reason
  {:key     :database-metadata/sync-in-progress
   :type    :warning
   :message (i18n-be/deferred-tru "Unable to determine whether the database connection is readonly, as it is still syncing.")})

(def ^:private missing-permissions-reason
  {:key     :database-metadata/not-populated
   :type    :warning
   :message (i18n-be/deferred-tru "Unable to determine whether the database connection is readonly, as we are missing metadata from sync.")})
...
(setting/defsetting database-enable-table-editing
  (i18n-be/deferred-tru "Whether the Database has table data editing enabled.")
  :default          false
