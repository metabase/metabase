(ns ^:synchronous metabase-enterprise.advanced-config.file.api-keys-test
  (:require
...
(deftest env-var-api-key-test
  (mt/with-temp-env-var-value! ["MB_API_KEY_FROM_ENV" "mb_envvariablekey123"]
    (binding [config.file/*env* (assoc @#'config.file/*env* :api-key-from-env "mb_envvariablekey123")]
      (is (= :ok (config.file/initialize!
                  {:version 1
...
(deftest validate-env-var-api-key-format-test
  (binding [config.file/*env* (assoc @#'config.file/*env* :invalid-api-key "invalid_key_format")]
    (is (thrown-with-msg?
         clojure.lang.ExceptionInfo
