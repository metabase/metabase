...
;; v60 renders a permitted transform query as pprint'd pMBQL (there is no representations export on
;; this branch), so these tests key on the query body (`:source-table` etc.) rendering at all rather
;; than on exported table names.

(deftest transform-source-query-omits-blocked-tables-test
  (testing "a client-supplied transform source query is not rendered when it names a blocked table"
    (do-with-orders-readable-people-blocked!
     (fn []
       (let [source (fn [field-clause table-id field-id]
         ...
         (testing "a field clause whose head is the string \"field\", as JSON decoding delivers it,
                   still resolves to its table and blocks the render"
           ...
           (testing "a card the caller cannot read"
             (mt/with-non-admin-groups-no-collection-perms coll
               (let [out (render-viewing (item (:id card)))]
                 (is (str/includes? out "viewing a Transform"))
                 (is (not (str/includes? out ":source-card"))))))
           (testing "a card the caller can read (control)"
             (let [out (render-viewing (item (:id card)))]
               (is (str/includes? out ":source-card"))))))))))
