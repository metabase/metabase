...
(mu/defn bookmark-rows-for-user :- [:sequential ::bookmark-row]
  ...
  (let [select-fields    [[:bookmark.created_at :created_at]
        ...
        ;; the joins are restricted to the current world, so an item of another world joins as all NULLs -- which a
        ;; `worktree_id` filter would take for the main app's; requiring the joined id drops it
