(ns metabase.mcp.v2.tools.learn
  "The v2 MCP `learn` tool: on-demand delivery of the skill packs in
   ...
   (template-tag shapes, the portable MBQL dialect, parameter wiring, layout conventions).
...
(registry/deftool learn
  "Read this server's task-shaped docs (skills) for the write dialects the tool schemas can't fully describe. learn() lists the topics; learn(topic) returns that skill whole; learn(topic, reference) returns one of its named reference files. Topics: query-dialect (the portable MBQL 5 dialect for execute_query and question_write's query; reference \"operators\" is the full operator catalog), native-parameters (template tags and field filters for question_write's native SQL), dashboard-filters (dashboard parameters and the wire_parameter target grammar), dashboard-layout (the 24-column grid, sizes, tabs), documents (document_write's Markdown grammar), visualization-settings (display choice and visualization_settings; reference \"settings\" is the per-chart key catalog). Consult the matching topic before your first complex write of that kind; skip it when the knowledge is already in context."
  {:name        "learn"
