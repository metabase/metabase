...
(defn instance-limit
  ...
  [tenant-id]
  (metabot.db/select-one-metabot-instance-limit {:tenant_id tenant-id}))
...
(defn set-instance-limit!
  ...
  [tenant-id max-usage]
  (if (nil? max-usage)
    (metabot.db/delete-metabot-instance-limits! {:tenant_id tenant-id})
    (if-let [existing (instance-limit tenant-id)]
      (metabot.db/update-metabot-instance-limits! {:id (:id existing)} {:max_usage max-usage})
      (metabot.db/insert-metabot-instance-limit! {:tenant_id tenant-id :max_usage max-usage})))
...
(defn all-tenant-limits
  ...
  []
  (metabot.db/select-metabot-instance-limits {:tenant_id_set true :order-by [:tenant_id]}))
