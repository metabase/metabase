...
;;   resources/migrations/instance_analytics_views/ai_usage_log/v5/{h2,mysql,postgres}-ai_usage_log.sql
;;   resources/migrations/instance_analytics_views/metabot_conversations/v7/{h2,mysql,postgres}-metabot_conversations.sql
;; When adding a value here, add the matching CASE branch to those SQL files (the v7 metabot_conversations
;; changeset is runOnChange, so editing it in place reinstalls the view on upgrade); a genuinely new value on a
;; checksum-pinned version needs a new view version (vN+1) plus a reinstall migration instead. Parity is enforced
;; by `metabase-enterprise.metabot.usage-test/known-sources-and-profiles-have-view-case-branches-test`.
