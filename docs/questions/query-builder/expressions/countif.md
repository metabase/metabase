...
> [Aggregations](../expressions-list.md#aggregations) like `CountIf` should be added to the query builder's [**Summarize** menu](../../query-builder/summarizing-and-grouping) > **Custom Expression** (scroll down in the menu if needed).
...
2. Add a [**Group by**](../../query-builder/summarizing-and-grouping) column in the query builder.
...
The `GROUP BY` part of the SQL query matches a Metabase [**Group by**](../../query-builder/summarizing-and-grouping) set to the "Plan" column.
...
The Python code above will produce the same result as the Metabase `CountIf` expression (with the [**Group by**](../../query-builder/summarizing-and-grouping) column set to "Plan").
