...
(deftest normalize-attribute-remappings-test
  (testing "make sure attribute-remappings come back from the DB normalized the way we'd expect"
    (t2.with-temp/with-temp [:model/GroupTableAccessPolicy gtap {:table_id             (mt/id :venues)
      ...
      (is (= {"venue_id" {:type   :category
                          :target [:variable [:field (mt/id :venues :id) nil]]
                          :value  5}}
             (t2/select-one-fn :attribute_remappings :model/GroupTableAccessPolicy :id (u/the-id gtap)))))
  ...
  (testing "You can't change the table_id of a sandbox after it has been created."
    (t2.with-temp/with-temp [:model/GroupTableAccessPolicy gtap {:table_id (mt/id :venues)
      ...
      (is (thrown-with-msg?
           ...
           (t2/update! :model/GroupTableAccessPolicy (:id gtap) {:table_id (mt/id :checkins)}))))))
  ...
  (testing "Don't allow saving a Sandboxing query that contains columns not in the Table it replaces (#13715)"
    (doseq [[msg f] {"Create a new GTAP"
                     ...
                     (fn [query]
                       (mt/with-temp [:model/Card                   card {:dataset_query   query
                                                                          :result_metadata (qp.preprocess/query->expected-cols query)}
                                      :model/GroupTableAccessPolicy gtap {:table_id (mt/id :venues)
                                                                          :group_id (u/the-id (perms-group/all-users))}]
                         (t2/update! :model/GroupTableAccessPolicy (:id gtap) {:card_id (:id card)})
                         :ok))
  ...
  (mt/with-premium-features #{:sandboxes}
    (mt/with-full-data-perms-for-all-users!
      (testing "Sandbox definitions in the DB are automatically added to the permissions graph"
        (mt/with-temp [:model/GroupTableAccessPolicy _gtap {:table_id (mt/id :venues)
                                                            :group_id (u/the-id (perms-group/all-users))}]
          (is (partial=
