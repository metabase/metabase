...
(defn op-error!
  "Throw a teaching error attributing message `detail` to the op at `idx` (0-based, as sent), rendered as
   `op <idx> (<op-name>): <detail>`. `op-name` is server-controlled text (an op name the op schema or dispatch
   has matched, or a key name the server declares) and is interpolated unquoted."
  [idx op-name detail]
  (common/throw-teaching-error (message/msg ["op %d (%s): %s"] idx (message/raw op-name) detail)))
...
(defmethod apply-op :default
  [_state idx op]
  ;; The op name is the caller's own unmatched string, so it is cleaned here rather than passed to `op-error!`.
  (common/throw-teaching-error
   (message/msg ["op %d (%s): unknown op — see the tool description for the supported list."] idx (:op op))))
...
(defmethod apply-op "add_link"
  ...
  (when (= (some? url) (some? entity))
    (op-error! idx "add_link" (message/msg ["pass exactly one of `url` or `entity`."])))
...
(defmethod apply-op "patch_dashcard"
  ...
  (doseq [k (keys patch)]
    (cond
      ...
      (op-error! idx "patch_dashcard" (message/msg ["`%s` is not patchable — use the `%s` op."]
...
(defmethod apply-op "move_tab"
  ...
  (or (move-row state :tabs tab_id index)
      (op-error! idx "move_tab" (message/msg ["index %d is out of range — this dashboard has %d tabs."]
...
(defn- resolve-parameter!
  ...
  (or (find-row state :parameters id)
      (op-error! idx "parameter_id" (message/msg ["no parameter with id %s on this dashboard."] id))))
...
(defmethod apply-op "add_parameter"
  ...
  (when (find-row state :parameters parameter_id)
    (op-error! idx "add_parameter" (message/msg ["parameter %s already exists — use `update_parameter`."]
                                                parameter_id)))
...
(defmethod apply-op "update_parameter"
  ...
  (let [cleared (map keyword clear)]
    (doseq [field cleared
      ...
      (op-error! idx "update_parameter" (message/msg ["%s is both set and cleared — pass one or the other."]
                                                     (name field))))
...
(defmethod apply-op "move_parameter"
  ...
  (when (and (contains? op :index) (contains? op :dashcard_id))
    (op-error! idx "move_parameter" (message/msg ["pass exactly one of `index` or `dashcard_id`."])))
  (cond
    ...
    (or (move-row state :parameters parameter_id index)
        (op-error! idx "move_parameter" (message/msg ["index %d is out of range — this dashboard has %d parameters."]
...
(defmethod apply-op "wire_parameter"
  ...
  (let [parameter (resolve-parameter! state idx parameter_id)
    ...
    (cond
      ...
      :else
      (op-error! idx "wire_parameter" (message/msg ["pass one of `target_field`, `target_tag`, or `target`. %s"]
                                                   (message/raw skills/wire-target-grammar))))))
