(ns dev.module-scc
  ...
  The fns here measure the component itself — size, membership, condensation — and score candidate cuts by
  how much they fragment it, giving carving experiments a continuous metric and a ranked target list.
...
(defn largest-scc
  ...
  ([graph] (largest-scc graph (deps-graph/strongly-connected-components graph)))
...
(defn cycle-stats
  ...
  (let [components (mapv (fn [component]
                           {:module-count    (count component)
                            ...
                            :members         component})
                         (deps-graph/cyclic-components graph))]
...
(defn condensation
  ...
  (let [node->scc (into {}
                        (for [[i component] (map-indexed vector sccs)
                              node          component]
                          [node i]))
        ;; seed every SCC id with an empty edge set so sink and isolated components appear in the DAG,
        ;; not just the ones with outgoing cross-component edges.
        cgraph    (reduce-kv (fn [acc v ws]
                               (let [sv (node->scc v)]
                                 (reduce (fn [acc w]
                                           (let [sw (node->scc w)]
                                             (cond-> acc
                                               (not= sv sw) (update sv conj sw))))
                                         acc
                                         ws)))
                             (into {} (map-indexed (fn [i _] [i #{}]) sccs))
                             graph)]
    {:node->scc node->scc
     :graph     cgraph}))
...
(defn scc-summary
  ...
  ([graph module->namespace-count]
   (let [sccs       (deps-graph/strongly-connected-components graph)
         ...
         cycles     (cycle-stats graph module->namespace-count)]
     {:num-sccs               (count sccs)
...
(comment
  ...
  (def combined*  (merge-with into graph* (model-import-dependencies (deps-graph/kondo-config))))
  (def ns-counts* (frequencies (keep :module deps*)))
  (def cycles*    (cycle-stats combined* ns-counts*))
