          ...
          (let [{:keys [additions deletions]} (find-changes {:current-index current-index-values
            ...
            (when (seq deletions)
              (doseq [deletions-part (partition-all 10000 deletions)
                ...
                (search/async-delete! :model/ModelIndexValue search-model-ids)))
            (when (seq additions)
          ...
          (t2/update! :model/ModelIndex (:id model-index)
                      {:indexed_at :%now
                       ...
                       :state      (if (> (count values-to-index) max-indexed-values)
                                     "overflow"
                                     "indexed")}))
        (run! search/async-update! (t2/reducible-select :model/ModelIndexValue :model_index_id (:id model-index)))
        (catch Exception e
