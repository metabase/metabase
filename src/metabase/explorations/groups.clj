...
(defn- leaf-id
  "Stable, opaque leaf id derived from `[group-id card-id dim-id]`, plus an optional
   `fp-key` (a query's `filter-path-key`) so drilled survivors at the same `(card, dim)`
   get distinct leaves, and an optional `query-type` so a `(card, dim)` pair split across
   query types gets distinct leaves. The group-id component keeps it unique when the same
   (card, dim) appears in two groups. Opaque to the FE; the public deep-link surface is
   [[chart-page-url]]."
  ([group-id card-id dim-id] (leaf-id group-id card-id dim-id nil nil))
  ([group-id card-id dim-id fp-key] (leaf-id group-id card-id dim-id fp-key nil))
  ([group-id card-id dim-id fp-key query-type]
...
(defn chart-page-url
  "Relative URL of a chart's leaf page in the exploration detail view, scoped to the group
   ...
   chart's `query-type` and, for a drilled survivor, its `filter-path-key` (see
   [[filter-path-key]]) so the link targets its specific leaf."
  ...
  ([exploration-id group-id card-id dimension-id query-type fp-key]
   (str "/question/research/" exploration-id
        "/group/" (codec/url-encode (leaf-id group-id card-id dimension-id fp-key query-type)))))
                       ...
                       (fn [group]
                         (let [labels (dimension-id->display-label (:dimensions group))
                               leaves (->> (get rows-by-group (:id group) [])
                                           (group-by (juxt :card_id :dimension_id filter-path-key :query_type))
