...
- **No access**: can't view, write or edit SQL/native queries using the SQL editor. Users with this level of access will still be able to view the results of questions created from SQL/native queries, but not the SQL code itself. Users with this level of permissions won't see the button "View the SQL" when doing "Custom Questions".
