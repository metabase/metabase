...
(deftest ^:synchronized we-can-update-existing-router-databases-to-point-to-a-new-user-attribute
  (mt/with-temp [:model/Database {db-id :id} {}
...
(deftest ^:synchronized deleting-database-routers-works
  (mt/with-temp [:model/Database {db-id :id} {}
                 :model/DatabaseRouter {router-id :id} {:database_id db-id :user_attribute "foo"}
...
(deftest ^:synchronized router-databases-have-a-router-user-attribute-on-the-get-api
  (mt/with-temp [:model/Database {db-id :id} {}
...
(deftest ^:synchronized cannot-create-duplicate-names
  (mt/with-temp [:model/Database {db-id :id} {}
                 :model/DatabaseRouter _ {:database_id db-id :user_attribute "foobar"}
...
(deftest ^:synchronized cannot-enable-db-routing-when-other-features-enabled
...
(deftest ^:synchronized can-delete-router-databases
  (mt/with-temp [:model/Database {db-id :id} {}
...
(deftest ^:synchronized destination-databases-excluded-from-permissions-graph
  (mt/with-temp [:model/Database {db-id :id} {}
                 :model/DatabaseRouter _ {:database_id db-id :user_attribute "foo"}
                 :model/Database {destination-db-id :id} {:router_database_id db-id}]
...
(deftest ^:synchronized new-group-does-not-get-destination-database-permissions
  (mt/with-temp [:model/Database {db-id :id} {}
                 :model/DatabaseRouter _ {:database_id db-id :user_attribute "foo"}
                 :model/Database {destination-db-id :id} {:router_database_id db-id}]
