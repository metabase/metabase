...
(mr/def ::persisted-dimension
  ...
  [:map {:closed true, :probe/id "src/metabase/lib_metric/schema.cljc:337"}
   [:id               ::dimension-id]
   [:name             {:optional true} [:maybe :string]]
   [:display-name     {:optional true} [:maybe ::lib.schema.common/non-blank-string]]
   [:description      {:optional true} [:maybe :string]]
...
(mr/def ::metric-definition
  ...
  [:map {:closed true, :probe/id "src/metabase/lib_metric/schema.cljc:368"}
...
(mr/def ::metadata-dimension
  ...
  [:map {:closed true, :probe/id "src/metabase/lib_metric/schema.cljc:387"}
...
(mr/def ::computed-dimension
  ...
  [:map {:closed true, :probe/id "src/metabase/lib_metric/schema.cljc:426"}
...
(mr/def ::computed-pair
  ...
  [:map {:closed true, :probe/id "src/metabase/lib_metric/schema.cljc:439"}
   [:dimension ::computed-dimension]
   [:mapping [:map {:closed true, :probe/id "src/metabase/lib_metric/schema.cljc:441"}
              [:type ::dimension-mapping.type]
              [:table-id {:optional true} [:maybe ::lib.schema.id/table]]
              [:target ::dimension-mapping.target]]]])
