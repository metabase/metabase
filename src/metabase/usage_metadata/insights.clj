...
(defn- predicate-field-ids
  ...
  (when predicate
    (try
      ...
      (catch Throwable e
        (log/debug "usage-metadata: predicate-field-ids failed:" (ex-message e))
        []))))
...
(defn- wrap-query
  ...
  (when (and (pos-int? database-id) (seq query-map))
    (try
      ...
      (catch Throwable e
        (log/debug "Failed to wrap query for usage-metadata insights:" (ex-message e))
        nil))))
...
(defn- extract-facts
  ...
  (when-let [q (wrap-query database-id query-map)]
    (try
      ...
      (catch Throwable e
        (log/debug "Failed to extract usage facts for usage-metadata insights:" (ex-message e))
        nil))))
