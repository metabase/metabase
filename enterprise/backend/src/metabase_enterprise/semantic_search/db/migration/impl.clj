...
(defn- add-worktree-id-column!
  "Migration 6: add `worktree_id` to index tables. Every existing row is main-app content — worktrees were
  excluded from the index until now — so the NULL default is already correct and no backfill is needed."
  [tx index-metadata]
