(ns metabase-enterprise.content-diagnostics.checkers.imbalanced
  ...
  - **Card** (`empty` only): the latest unparameterized, non-cache-hit execution returned 0 rows.
    Never run unparameterized -> skipped (unknown, not empty); a newer parameterized or cache-hit run
    is outside the evidence set - it neither flags nor clears. `as_of` = the deciding run's start.
...
(defn- empty-card-id->as-of
  "`{card-id -> started_at of the deciding run}` for every **non-archived** card whose latest
  unparameterized, non-cache-hit execution returned 0 rows. One windowed query (executions are not
  serialized, so a grouped MAX can't pick the latest row). `parameterized = false` strictly: a NULL
  (pre-column legacy row) is unknown, and unknown runs are outside the evidence set."
  []
  (into {}
