(ns metabase.query-processor.middleware.annotate.legacy-impl
...
(defn update-metadata
  ...
  [{query-type :type, :as query {:keys [card-entity-id metadata/model-metadata]} :info}
   metadata]
  ...
  (if (and (= query-type :query)
    ...
    (cond-> (update metadata :cols annotate-native-cols card-entity-id)
