(ns metabase.mcp.v2.projections
  "Per-type concise/detailed response projections for v2 MCP read tools, plus the generated
   ...
   A projection takes an already-fetched, already-redacted row and returns its shape: pure, with
   no app-DB access and no read of the current user. Hydration and the permission-dependent
   redaction happen before the row gets here — see [[metabase.mcp.v2.redaction]].
...
(defn all-catalogs
  "Map of registered type -> dot-path catalog, published as the `catalog://metabase/fields`
...
(defn- dashcard-card-ref
  "The `{id, name}` card reference for a card-backed dashcard. A dashcard carrying no `:card` —
   never hydrated, or redacted away as unreadable — collapses to `{id}` only, as the REST
   dashboard response does."
...
(defn- dashcard-summary
  ...
  (m/remove-vals
   ...
   {:id                (:id dc)
    ...
    :size_y            (:size_y dc)
    :series            (not-empty
                        (mapv #(m/remove-vals nil? (select-keys % [:id :name])) (:series dc)))
...
(defn- dashboard-parameters-summary
  "One row per dashboard parameter: id, name, type, and the ids of the dashcards it is wired
   to through their `:parameter_mappings`, including the negative ids of a dry run's unsaved dashcards."
  [dash]
  (let [param-id->dashcard-ids (reduce (fn [acc {dashcard-id :id :keys [parameter_mappings]}]
                                         (reduce #(update %1 (u/qualified-name (:parameter_id %2)) (fnil conj #{}) dashcard-id)
                                                 acc
                                                 parameter_mappings))
                                       {}
                                       (:dashcards dash))]
            ...
            (m/remove-vals
             ...
             {:id           param-id
              ...
              :dashcard_ids (not-empty
                             (vec (sort (remove nil? (get param-id->dashcard-ids (u/qualified-name param-id))))))}))
          (:parameters dash))))

(defn dashboard-row
  "The projection row for `dash`, a dashboard hydrated with `[:dashcards :series :card] :tabs` and
  already redacted. Both the `:concise` and `:detailed` projections select from this shape."
  [dash]
...
(defn- notification-handler-rows
  "Projected handler rows. Recipients are projected as given: redaction happens before the
   projection, and a handler whose `:recipients` were removed projects without the key."
  [handlers]
  (mapv (fn [handler]
          (m/remove-vals
           nil?
           {:id           (:id handler)
            ...
            :recipients   (some->> (:recipients handler) (mapv notification-recipient-row))}))
        handlers))

(defn notification-row
  "The projection row for `notification`, already hydrated and recipient-redacted. Both the
  ...
  (m/remove-vals
   ...
   {:id            (:id notification)
    ...
    :handlers      (notification-handler-rows (:handlers notification))}))
...
(defn subscription-row
  "The projection row for `pulse-row`, a Pulse already hydrated and redacted. Both `get_content`
   and `subscription_write` return this shape."
  [pulse-row]
  (compact
