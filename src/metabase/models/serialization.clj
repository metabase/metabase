...
(def ^:private cached-vars
  (filter (comp ::cache meta) (vals (ns-interns *ns*))))

(defn init-cache
  "(Re-)initializes serdes import/export lookup caching."
  []
  (doseq [v cached-vars]
    (if (memoize/memoized? @v)
      (memoize/memo-clear! @v)
      (alter-var-root v memoize/memo))))

(defmacro with-cache
  "Calls body with certain serdes functions memoized for performance."
  [& body]
  `(do
     (init-cache)
     ~@body))
