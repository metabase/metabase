...
(define-migration DeleteInternalUserMostRecentCardRevisions
  ;; Delete the most recent revision for Cards when it was created by the internal Metabase user.
  ;; These were mistakenly created during dependency backfill in v57.
  ;; See: https://github.com/metabase/metabase/issues/66365
  (let [bad-revisions (t2/query {:select [:id :model_id]
                                 :from   [:revision]
                                 :where  [:and
                                          [:= :model "Card"]
                                          [:= :user_id config/internal-mb-user-id]
                                          [:= :most_recent true]]})]
    (when (seq bad-revisions)
      (log/infof "Deleting %d most-recent Card revisions made by internal user" (count bad-revisions))
      (doseq [{:keys [id model_id]} bad-revisions]
        (let [next-revision-id (-> (t2/query {:select   [:id]
                                              :from     [:revision]
                                              :where    [:and
                                                         [:= :model "Card"]
                                                         [:= :model_id model_id]
                                   ...
                                   first
                                   :id)]
          (t2/query {:delete-from :revision
                     :where       [:= :id id]})
          (when next-revision-id
            (t2/query {:update :revision
                       :set    {:most_recent true}
                       :where  [:= :id next-revision-id]})))))))

(define-migration DeleteInternalUserCardUpdateAuditLogEntries
  ;; Delete the most recent audit log entry for card-update events when it was created by the internal Metabase user.
  ;; These were mistakenly created during dependency backfill in v57.
  ;; See: https://github.com/metabase/metabase/issues/66365
  (let [bad-entry-ids (map :id (t2/query {:select   [[:%max.id :id]]
                                          :from     [:audit_log]
                                          :where    [:and
                                                     [:= :topic "card-update"]
                                                     [:= :model "Card"]
                                                     [:= :user_id config/internal-mb-user-id]]
                                          :group-by [:model_id]}))]
    (when (seq bad-entry-ids)
      (log/infof "Deleting %d most-recent card-update audit log entries made by internal user" (count bad-entry-ids))
      (t2/query {:delete-from :audit_log
                 :where       [:in :id bad-entry-ids]}))))
