...
(mr/def ::metadata-import-payload
  "Body shape accepted by POST /api/database/metadata. Mirrors the per-workspace
   `table_metadata.json` produced by EE workspace endpoints (and the
   `@metabase/database-metadata` extractor), but the import side only cares about
