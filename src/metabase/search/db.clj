...
(mu/defn drop-search-index-table-if-exists!
  ...
  [conn       :- [:maybe (ms/InstanceOfClass java.sql.Connection)]
   table-name :- [:or :keyword :string]]
  (t2/query conn (sql.helpers/drop-table :if-exists table-name)))
...
(mu/defn drop-search-index-table!
  ...
  (t2/query conn (sql.helpers/drop-table table-name)))
...
(mu/defn lock-pending-index-metadata!
  ...
  [conn      :- [:maybe (ms/InstanceOfClass java.sql.Connection)]
   ...
   version   :- :string
   lang-code :- :string]
  (t2/select-one :conn conn [:model/SearchIndexMetadata :id]
                 :engine engine
                 :version version
                 :lang_code lang-code
                 :status :pending
...
(mu/defn lock-named-pending-index-metadata!
  "Lock and return the pending SearchIndexMetadata row of `engine`, `version`, `lang-code`, and
  `index-name`, if one exists. Must be called inside the transaction that will promote the row."
  [conn       :- [:maybe (ms/InstanceOfClass java.sql.Connection)]
   engine     :- :keyword
   version    :- :string
   lang-code  :- :string
...
(def ^:private LeaseCoordinate
  [:map {:closed true}
   [:engine :string]
   [:version :string]
   [:lang_code :string]])

(def ^:private LeaseClaimUpdate
  [:map {:closed true}
   [:owner :string]
   [:acquired_at ms/TemporalInstant]
   [:last_renewed_at ms/TemporalInstant]
   [:expires_at ms/TemporalInstant]])

(def ^:private LeaseRow
  [:map {:closed true}
   [:engine :string]
   [:version :string]
   [:lang_code :string]
