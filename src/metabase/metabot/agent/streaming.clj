...
(defn automagic-dashboard-part
  "Create an AUTOMAGIC_DASHBOARD data part for streaming.
  Value should be a map with :url (an `/auto/dashboard/...` path that the client
  renders inline as an X-ray dashboard embed) and optionally :title.

  Used by dashboard-generating tools (`create_autogenerated_dashboard`,
  `generate_insights`) to surface the result inline in the chat instead of
  navigating the user to a separate page."
  [value]
  {:type :data
   :data-type automagic-dashboard-type
   :version 1
   :data value})
