...
(defn query-executions-all-time-and-last-24h
  ...
  []
  (let [qe          (internal-stats.queries/query-execution-statistics-all-time)
        one-day-ago (t/minus (t/offset-date-time) (t/days 1))
        qe-24h      (internal-stats.queries/query-execution-statistics-since one-day-ago)]
    {:query-executions     qe
...
(defn query-execution-last-utc-day
  ...
  (let [yesterday-utc (t/minus (t/offset-date-time (t/zone-offset "+00")) (t/days 1))]
    (-> (internal-stats.queries/query-execution-statistics-on yesterday-utc)
        (dissoc :row_count)
