...
SEARCH_URL_LOCAL = "http://localhost:3000/api/search"
SEARCH_URL_STATS = "http://stats.metabase.com/api/search"
SEARCH_URL_FBLUE = "http://fiery-blue.hosted.staging.metabase.com/api/search"
...
SESSION_ID_FBLUE = '<SESSION_ID>'
SESSION_ID_STATS = '<SESSION_ID>'
...
async def async_run_benchmark(test_queries, url, session_id, k_values=[5, 10], max_concurrent=5, cohort="Nil"):
    ...
    query_results = []
    for (i, query_data), (status_code, search_results) in zip(query_data_list, request_results):
        query_text = query_data["search_text"]
...
async def run_all_benchmarks_async(test_queries, max_concurrent=5):
    ...
    for cohort in sorted(cohorts):
        ...
        tasks = [
            # stats hardcoded to no concurrency since it seems to reject us when we try? might be different with API key
            async_run_benchmark(cohort_tests, SEARCH_URL_STATS, SESSION_ID_STATS, max_concurrent=1, cohort=cohort),
            async_run_benchmark(cohort_tests, SEARCH_URL_LOCAL, SESSION_ID_LOCAL, max_concurrent=max_concurrent, cohort=cohort),
            async_run_benchmark(cohort_tests, SEARCH_URL_FBLUE, SESSION_ID_FBLUE, max_concurrent=max_concurrent, cohort=cohort)
        ]
        
        stats_result, local_result, fblue_result = await asyncio.gather(*tasks)
        results[cohort] = {
            "stats": stats_result,
            "local": local_result,
            "fblue": fblue_result
