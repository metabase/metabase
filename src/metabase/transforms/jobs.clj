...
(defn- get-plan [transform-ids]
  (tracing/with-span :tasks "task.transform.plan" {:transform/count (count transform-ids)}
    (let [all-transforms (dependencies/hydrate-stored-deps
                          (t2/select [:model/Transform :id :target :target_table_id :created_at]))
          ...
          ;; only on closure transforms whose stored deps couldn't be hydrated — never on
          ;; unrelated transforms elsewhere in the system. This is what prevents a single broken
          ;; transform (e.g. one on a routing-enabled database) from poisoning the scheduler
          ;; when no job has asked for it.
