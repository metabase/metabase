      ...
      (let [db-details (tx/dbdef->connection-details :redshift nil nil)]
        (mt/with-temp [:model/Database database {:engine :redshift, :details db-details}]
          (let [schema   (redshift.tx/unique-session-schema)
                ...
                il-table (tx/db-qualified-table-name (:name database) "fetch_il")
            ...
            (try
              ...
              (driver/create-table! :redshift (u/the-id database) (keyword schema il-table)
                                    inline-columns {:indexes [{:kind :sortkey :style :interleaved
                                                               :columns [{:name "a"} {:name "b"}]}]})
              (driver/create-table! :redshift (u/the-id database) (keyword schema no-sk) inline-columns {})
              (testing "a compound sortkey returns one unnamed :sortkey entry"
              ...
              (testing "an interleaved sortkey reports its style in :definition"
                (is (= "INTERLEAVED SORTKEY (a, b)"
                       (:definition (first (driver/fetch-table-indexes :redshift database schema il-table))))))
              ...
              (testing "a table that does not exist returns [] rather than throwing"
                (is (= [] (driver/fetch-table-indexes :redshift database schema "fetch_does_not_exist"))))
              (finally
                (drop! sk-table)
                (drop! il-table)
                (drop! no-sk)))))))))
