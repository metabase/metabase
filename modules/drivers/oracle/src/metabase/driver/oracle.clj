...
(defn- date-interval*
  [datetime unit amount]
  (hx/+ datetime (case unit
...
(defmethod driver/date-interval :oracle
  ([_ unit amount]
   (date-interval* now unit amount))
  ([_ field unit amount]
   (date-interval* (hx/->timestamp field) unit amount)))
