...
**For `MB_EDITION=ee`:**
These tokens must be exported in the shell before starting Codex:
...
If tokens are missing, tell the user: "EE tokens are not set. Either export them and restart Codex, or add `MB_EDITION=oss` to run OSS-only tests."
