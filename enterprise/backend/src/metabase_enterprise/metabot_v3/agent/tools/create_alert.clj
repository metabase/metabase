(ns metabase-enterprise.metabot-v3.agent.tools.create-alert
  ...
  (:require
   ...
   [metabase-enterprise.metabot-v3.tools.create-alert :as tools.create-alert]
...
(mu/defn ^{:tool-name          "create_alert"
           :capabilities       #{"backend:/api/ee/metabot-tools/create-alert"}
  ...
  [{:keys [card_id send_condition schedule send_once]} :- [:map {:closed true}
                                                           [:card_id :int]
