...
export const HEATMAP_DISPLAY = `custom:${PLUGIN_IDENTIFIER}` as VisualizationDisplay;
...
function buildYearData(year: number): RowValues[] {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const value = Math.round(
      60 + 40 * Math.sin(i / 9) + 20 * Math.cos(i / 37),
    );
    rows.push([iso, value]);
  }
  return rows;
...
export const HEATMAP_ROWS = buildYearData(2024);
