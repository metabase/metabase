(ns metabase-enterprise.workspaces.db
  ...
  None of the remapping queries name a worktree. `:model/WorkspaceTableRemapping` derives `:hook/worktree-id`, so
  Toucan restricts each of them to the worktree being worked in and fills `worktree_id` in on insert. A lookup here
  answers for the current worktree, and for the main app when there is none."
  (:require
