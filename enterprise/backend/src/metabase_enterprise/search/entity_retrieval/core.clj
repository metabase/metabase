(ns metabase-enterprise.search.entity-retrieval.core
  ...
  OSS shims live in [[metabase.search.entity-retrieval.mirror]]; the background sync they nudge is the
  [[metabase-enterprise.search.entity-retrieval.task.sync]] Quartz job."
  (:require
...
(defn pgvector-schedulable?
  ...
  [[metabase-enterprise.search.semantic.db.datasource/pgvector-mode]] would probe the app db of instances
...
(defn- probe-index
  ...
  (try
    (let [ds              (semantic.db.datasource/ensure-initialized-data-source!)
          metadata-status (index-table/index-status ds (resolved-configured-model))
...
(defenterprise search-unfiltered
  ...
  Hits are not permission-filtered — see [[metabase.search.entity-retrieval.mirror/search-unfiltered]] for what a
