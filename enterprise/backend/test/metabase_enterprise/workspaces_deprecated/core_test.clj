(ns metabase-enterprise.workspaces-deprecated.core-test
  ...
  (:require
   ...
   [metabase-enterprise.workspaces-deprecated.provisioning :as provisioning]
      ...
      (mt/with-model-cleanup [:model/WorkspaceDeprecated]
            ...
            (with-redefs [provisioning/dispatching-provisioner failing-provisioner]
              (is (thrown-with-msg? ExceptionInfo #"boom"
                                    (ws/create-workspace! {:name         "Boom"
                                                           :creator_id   (mt/user->id :crowberto)
                                                           :database_ids [db-id]}))))
            (is (not (t2/exists? :model/WorkspaceDeprecated :name "Boom"))
                "the failed create must not leave a Workspace row behind")
            (is (not (t2/exists? :model/WorkspaceDatabaseDeprecated :database_id db-id))
        ...
        (testing "provisioning failure whose cleanup also fails keeps the workspace and returns it for retry"
          (let [wedged (reify provisioning/Provisioner
            ...
            (let [ws (t2/select-one :model/WorkspaceDeprecated :name "Wedged")]
              (is (some? ws)
                  "the workspace must be kept while any database row remains")
              (is (=? [{:status :unprovisioned}]
                      (t2/select :model/WorkspaceDatabaseDeprecated :workspace_id (:id ws)))
...
(defn- add-database!
  ...
  (t2/with-transaction [_conn]
    (let [wsd-id (t2/insert-returning-pk! :model/WorkspaceDatabaseDeprecated
        ...
        (mt/with-model-cleanup [:model/WorkspaceDeprecated]
          (let [ws     (ws/create-workspace! {:name       (str "Pending " (name pending-status))
                ...
                _      (with-redefs [provisioning/dispatching-provisioner (stub-provisioner)]
                         (add-database! (:id ws) (mt/id) ["PUBLIC"]))
                wsd-id (t2/select-one-pk :model/WorkspaceDatabaseDeprecated :workspace_id (:id ws))]
            ;; force the row into the pending state. :provisioning rows that crashed
            ;; never stored their iso details, so clear them too.
            (t2/update! :model/WorkspaceDatabaseDeprecated {:id wsd-id}
            ...
            (let [destroyed? (atom false)
              ...
              (is (nil? (ws/get-workspace (:id ws))))
              (is (not (t2/exists? :model/WorkspaceDatabaseDeprecated :id wsd-id))))))))))
  ...
  (testing "when teardown fails, nothing is deleted: the row and the workspace are kept for retry"
    (mt/with-model-cleanup [:model/WorkspaceDeprecated]
      (let [ws     (ws/create-workspace! {:name "Unreachable WS" :creator_id (mt/user->id :crowberto)})
        ...
        (is (=? {:status :unprovisioned}
                (t2/select-one :model/WorkspaceDatabaseDeprecated :id wsd-id))
    ...
    (mt/with-temp [:model/Database {db2-id :id} {:engine   :postgres
      ...
      (mt/with-model-cleanup [:model/WorkspaceDeprecated]
        (let [ws   (ws/create-workspace! {:name "Multi fail" :creator_id (mt/user->id :crowberto)})
          ...
          (is (= 2 (t2/count :model/WorkspaceDatabaseDeprecated :workspace_id (:id ws)))
