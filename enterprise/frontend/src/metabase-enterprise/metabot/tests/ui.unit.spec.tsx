...
describe("metabot > ui", () => {
  ...
  it("should prevent submission when ui-bug is selected with empty feedback", async () => {
    ...
    await userEvent.click(
      await within(lastMessage!).findByTestId(
        "metabot-chat-message-thumbs-down",
      ),
    );

    const modal = await screen.findByTestId("metabot-feedback-modal");

    // Select ui-bug issue type
    await userEvent.click(
      await within(modal).findByPlaceholderText("Select issue type"),
    );
    await userEvent.click(await screen.findByText("UI bug"));
    ...
    // Should NOT submit
    expect(fetchMock.callHistory.calls(feedbackPath)).toHaveLength(0);
  });
  ...
  it("should allow submission when non-required issue types have empty feedback", async () => {
    ...
    // Select a non-required issue type (e.g., "Not factually correct")
    await userEvent.click(
      await within(modal).findByPlaceholderText("Select issue type"),
    );
    await userEvent.click(await screen.findByText("Not factually correct"));

    // Submit without feedback should succeed
    await userEvent.click(
      await within(modal).findByRole("button", { name: /Submit/ }),
    );
