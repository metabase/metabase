...
import { shouldShowSaveButton } from "../Question/components";
import { useQuestionContext } from "../Question/context";

import { DefaultViewTitle } from "./DefaultViewTitle";
import QuestionS from "./QuestionDefaultView.module.css";

export interface QuestionDefaultViewProps extends FlexibleSizeProps {
  /**
...
export const QuestionDefaultView = ({
  height,
  width,
  className,
  style,
...
}: QuestionDefaultViewProps): ReactElement => {
  ...
  return (
    <FlexibleSizeComponent
      height={height}
      width={width}
      className={cx(QuestionS.Container, className)}
      style={style}
    >
      {queryResults && (
        <Stack className={QuestionS.TopBar} gap="sm" p="md">
          <Group justify="space-between" align="flex-end">
            ...
            {showSaveButton && <Question.SaveButton onClick={openSaveModal} />}
          </Group>
          <Group
          ...
          >
                  ...
                  {withChartTypeSelector && (
                    <>
                      <Button.Group>
                        ...
                        <Question.QuestionSettingsDropdown />
                      </Button.Group>
                      <Divider
                        mx="xs"
            ...
            <Group gap="sm">
              {withDownloads && <Question.DownloadWidgetDropdown />}
              <Question.EditorButton
                isOpen={isEditorOpen}
                onClick={toggleEditor}
              />
            </Group>
      ...
      <Box className={QuestionS.Main} p="sm" w="100%" h="100%">
        <Box className={QuestionS.Content}>
          {isEditorOpen ? (
            <Question.Editor onApply={closeEditor} />
          ) : (
            <Question.QuestionVisualization height="100%" />
          )}
        </Box>
      </Box>
