...
(deftest authoring-share-absent-from-governance-counts-non-canonical-test
  (testing "an authored card the appdb can't resolve counts as non-canonical"
    ...
    (let [g (gov {["card" 1] canonical-facts})]
      (is (= 0.5 (:canonical-authoring-share
                  (metrics/compute (normalized :Q [(atom-rec "card" 1) (atom-rec "card" 2)]) g)))))))

(deftest authoring-share-na-on-empty-authored-set-test
  (testing "nothing authored → share is N/A"
    (is (= :na (:canonical-authoring-share (metrics/compute (normalized) {}))))))

(deftest authoring-share-na-when-authored-set-has-no-data-sources-test
  (testing "an authored set with only non-data-source entities (here a lone
            ...
            divide-by-zero"
    (is (= :na (:canonical-authoring-share
                (metrics/compute (normalized :Q [(atom-rec "field" 1)]) {}))))))

(deftest authoring-share-excludes-fields-from-denominator-test
...
(deftest bypass-rate-zero-when-canonical-shown-but-ignored-test
  (testing "a canonical surface shown but never inspected or used → fully
            bypassed → health 0.0"
    (let [g (gov {["card" 1] canonical-facts})]
      (is (= 0.0 (:canonical-bypass-rate
                  (metrics/compute (normalized :D [(atom-rec "card" 1)]) g)))))))

(deftest bypass-rate-one-when-shown-canonical-then-inspected-test
  (testing "a canonical surface later inspected is not a bypass → health 1.0"
    (let [g (gov {["card" 1] canonical-facts})]
      (is (= 1.0 (:canonical-bypass-rate
                  (metrics/compute (normalized :D [(atom-rec "card" 1)]
                                               :I [(atom-rec "card" 1)]) g)))))))

(deftest bypass-rate-one-when-shown-canonical-then-used-test
  (testing "a canonical surface later authored against is not a bypass → 1.0
            — surfaced-then-used must never read as a bypass"
    (let [g (gov {["card" 1] canonical-facts})]
      (is (= 1.0 (:canonical-bypass-rate
                  (metrics/compute (normalized :D [(atom-rec "card" 1)]
                                               :Q [(atom-rec "card" 1)]) g)))))))

(deftest bypass-rate-na-when-no-canonical-shown-test
  (testing "no canonical surface in the discovered set → N/A"
    (let [g (gov {["card" 1] uncanonical-facts})]
      (is (= :na (:canonical-bypass-rate
                  (metrics/compute (normalized :D [(atom-rec "card" 1)]) g))))
      (testing "and N/A when nothing was discovered at all"
        (is (= :na (:canonical-bypass-rate (metrics/compute (normalized) g))))))))

(deftest bypass-rate-ignores-non-canonical-fields-in-discovery-test
  (testing "field-type discoveries never enter the shown-canonical set, so a
            discovered field can't make bypass applicable"
    (is (= :na (:canonical-bypass-rate
                (metrics/compute (normalized :D [(atom-rec "field" 1)]) {}))))))

;;; ---------------------------------------------------------------------------
;;; Unproductive Search Rate
;;; ---------------------------------------------------------------------------

(deftest unproductive-search-flags-repeated-results-test
  (testing "a second search that rediscovers the first's results → 1 of 2
            unproductive → health 0.5"
    (is (= 0.5 (:unproductive-search-rate
                (metrics/compute
                 (normalized :tool-events [(search-event ["card" 1] ["card" 2])
                                           (search-event ["card" 1] ["card" 2])])
                 {}))))))

(deftest unproductive-search-one-when-results-disjoint-test
  (testing "searches with disjoint result sets are all productive → health 1.0"
    (is (= 1.0 (:unproductive-search-rate
                (metrics/compute
                 (normalized :tool-events [(search-event ["card" 1] ["card" 2])
                                           (search-event ["card" 3] ["card" 4])])
                 {}))))))

(deftest unproductive-search-na-on-single-call-test
  (testing "a single search can't rediscover anything → N/A"
    (is (= :na (:unproductive-search-rate
                (metrics/compute (normalized :tool-events [(search-event ["card" 1])]) {}))))))

(deftest unproductive-search-na-on-no-calls-test
  (testing "no search calls → N/A"
    (is (= :na (:unproductive-search-rate (metrics/compute (normalized) {}))))))

(deftest unproductive-search-catches-non-adjacent-overlap-test
  (testing "rediscovery is compared against all prior calls, not just the
    ...
    (let [health (:unproductive-search-rate
                  (metrics/compute
                   (normalized :tool-events [(search-event ["card" 1] ["card" 2])
...
(deftest unproductive-search-ignores-non-search-tool-events-test
