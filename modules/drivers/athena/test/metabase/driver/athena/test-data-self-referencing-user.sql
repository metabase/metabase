...
DROP TABLE IF EXISTS ` test_data_self_referencing_user `.` users `

CREATE EXTERNAL TABLE ` test_data_self_referencing_user `.` users ` (
  ` id ` INT,
  ` name ` STRING,
  ` last_login ` TIMESTAMP,
  ` password ` STRING,
  ` created_by ` INT
) LOCATION 's3://metabase-ci-athena-results/test_data_self_referencing_user/users/';

-- 15 rows
INSERT INTO "test_data_self_referencing_user"."users" ("name", "last_login", "password", "created_by")
VALUES
