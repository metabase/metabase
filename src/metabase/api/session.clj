...
(defendpoint POST "/"
  ...
  (let [user (db/sel-1 [User :id :password_salt :password :last_login], :email email, :is_active true)]
...
(defn- valid-reset-token->user
  ...
  (when-let [[_ user-id] (re-matches #"(^\d+)_.+$" token)]
    (let [user-id (Integer/parseInt user-id)]
      (when-let [{:keys [reset_token reset_triggered], :as user} (db/sel-1 [User :id :last_login :reset_triggered :reset_token] :id user-id)]
