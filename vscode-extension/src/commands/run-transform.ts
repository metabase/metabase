...
import { loadTableSchema } from './view-table-data'
      ...
      async (progress) => {
        try {
          ...
          if (result.result.status === 'succeeded') {
            ...
            // Fetch and cache the schema for the newly created output table
            try {
              const outputTables = ctx.workspaceDataProvider.getOutputTables()
              const newTable = outputTables.find(
                t => t.schema === result.result.table.schema && t.tableName === result.result.table.name,
              )
              if (newTable) {
                ctx.outputChannel.appendLine(
                  `Refreshing schema cache for newly created table ${newTable.schema}.${newTable.tableName} (id=${newTable.tableId})`,
                )
                const outputTableNode: OutputTableNode = { kind: 'output-table', table: newTable }
                await loadTableSchema(ctx, outputTableNode, true)
              }
              else {
                ctx.outputChannel.appendLine(
                  `Could not find newly created table ${result.result.table.schema}.${result.result.table.name} in output tables`,
                )
              }
            }
            catch (cacheError) {
              const message = cacheError instanceof Error ? cacheError.message : String(cacheError)
              ctx.outputChannel.appendLine(`Error caching table schema: ${message}`)
            }

            // Execute the dependency graph check command
            await commands.executeCommand('metastudio.checkDependencyGraph')
          }
          else {
