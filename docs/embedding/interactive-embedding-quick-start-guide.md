...
Visit that dashboard in the "Automatically generated dashboards" collection.. Click on the **info** button. On the **Overview**, look for the dashboard's **Entity ID**. Copy that Entity ID. You'll that Entity ID to point the iframe you'll embed in your app (e.g., `src=/dashboard/entity/[Entity ID]`).
...
`METABASE_DASHBOARD_PATH` should be pointing to the relative, entity path of the dashboard you created at the beginning of this guide (`/dashboard/entity/[Entity ID]`).

```javascript
app.get("/analytics", restrict, function (req, res) {
  const METABASE_DASHBOARD_PATH = "/dashboard/entity/[Entity ID]"; // e.g., `/dashboard/entity/nXg0q7VOZJp5a3_hceMRk`
