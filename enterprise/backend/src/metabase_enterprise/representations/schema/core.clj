(ns metabase-enterprise.representations.schema.core
  "Core schema registry for representations validation.
   ...
   Type format: 'v0/entity-type' where:
   - v0 is the schema version
   - entity-type is question, model, metric, collection, database, etc."
  (:require
   [clojure.string :as str]
   [metabase-enterprise.representations.schema.card.v0 :as v0-card]
   [metabase-enterprise.representations.schema.collection.v0 :as v0-coll]
   [metabase-enterprise.representations.schema.database.v0 :as v0-db]
   [metabase-enterprise.representations.schema.document.v0 :as v0-doc]
   [metabase-enterprise.representations.schema.snippet.v0 :as v0-snippet]
   [metabase-enterprise.representations.schema.transform.v0 :as v0-transform]
...
(defn schema-for-type
  "Returns the Malli schema for a given type string.
   
   Examples:
   - (schema-for-type \"v0/question\") => ::v0-card/question
   - (schema-for-type \"v0/collection\") => ::v0-coll/collection
   
   Returns nil if type is not recognized."
  [type-str]
  (if (str/includes? type-str "/")
