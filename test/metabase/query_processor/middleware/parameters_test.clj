...
(deftest nested-template-tags-with-params-test
  ...
  ;; TODO Finish or remove this test, based on which direction we go with nested template tag parsing
  #_(testing "parameter for nested sub-query from template tag parameter"
    (tt/with-temp Card [inner-card {:dataset_query
                                    (merge
                                     (data/native-query
                                      {:query "SELECT * FROM venues WHERE price < {{price_limit}}"
      ...
      (is (= (data/native-query
             ...
             (substitute-params
              (data/native-query
               {:query (format "SELECT * FROM {{#%s|price_limit=2}} AS x" (:id inner-card))
                :template-tags (card-template-tags [(:id inner-card)])})))))))
