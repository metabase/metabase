...
(defn- unknown-table-ex-info
  ...
  The LLM can still self-correct in one turn by listing the database's tables. The message names
  that action rather than the tool that performs it: two catalogs share this pipeline (Metabot's
  `entity_details` and the agent API's `browse_data`), and an error that names the wrong one sends
  the model after a tool it does not have."
  [_metadata-provider [_path-db-name _path-schema _path-table-name :as path]]
  (ex-info (tru "No table found matching portable FK {0}. List the database''s tables and schemas, then retry with an exact portable FK from the response."
                (pr-str path))
           {:status-code  400
            :error        :unknown-table
            :agent-error? true
...
(defn- table-candidates
  ...
  raised for a table the caller can plainly see in a metadata listing."
  ...
  (let [{current-db :name, current-db-id :id} (lib.metadata/database metadata-provider)]
    ...
    (let [candidates (table-candidates metadata-provider current-db-id path-schema path-table-name)]
      (case (count candidates)
        ...
        (throw (ex-info (tru "Ambiguous portable table FK {0}: {1} candidates. List the database''s tables and retry with a more specific portable FK."
...
(defn- find-field
  ...
  (let [table-meta (find-table metadata-provider [db schema table-name])
        ...
        ;; by listing the parent table's columns.
        unknown-field-ex
        (fn [segment]
          (ex-info (tru "No column {0} on table {1}.{2}.{3}. List the table''s columns and retry with one of them."
...
(defn- import-card-by-entity-id
  ...
  (let [current-db-id (:id (lib.metadata/database metadata-provider))
    ...
    (cond
      (nil? card)
      (throw (ex-info (tru "No saved question or model found with entity_id {0}. Do not invent or guess entity_ids: read the question or model first and copy its exact entity_id into `source-card:`."
                           (pr-str entity-id))
                      {:agent-error? true
                       :status-code  400
                       :error        :unknown-card
...
(defn- import-measure-by-entity-id
  ...
  (let [measure         (measure-by-entity-id content-store entity-id)
    ...
    (cond
      (nil? measure)
      (throw (ex-info (tru "No measure found with entity_id {0}. Do not invent or guess entity_ids: read the table that owns the measure and copy the measure''s exact entity_id."
                           (pr-str entity-id))
                      {:agent-error? true
                       :status-code  400
...
(defn- import-segment-by-entity-id
  ...
  (let [segment          (segment-by-entity-id content-store entity-id)
    ...
    (cond
      (nil? segment)
      (throw (ex-info (tru "No segment found with entity_id {0}. Do not invent or guess entity_ids: read the table that owns the segment and copy the segment''s exact entity_id."
                           (pr-str entity-id))
                      {:agent-error? true
                       :status-code  400
