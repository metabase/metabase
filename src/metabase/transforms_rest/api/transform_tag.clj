(ns metabase.transforms-rest.api.transform-tag
  (:require
   ...
   [metabase.models.transforms.transform-tag :as transform-tag.model]
...
(api.macros/defendpoint :post "/" :- TransformTagResponse
  ...
  (api/check-400 (not (transform-tag.model/tag-name-exists? name))
                 (deferred-tru "A tag with that name already exists."))
...
(api.macros/defendpoint :put "/:tag-id" :- TransformTagResponse
  ...
  (api/check-400 (not (transform-tag.model/tag-name-exists-excluding? name tag-id))
                 (deferred-tru "A tag with that name already exists."))
