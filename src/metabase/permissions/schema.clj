...
(def data-permissions
  ...
  ;; `:sandboxed` means the user has access through a sandbox (GTAP). Without the `:sandboxes` feature, it is
  ;; equivalent to `:blocked`.
  ;; `:impersonated` means the user has access through connection impersonation. Without the `:advanced-permissions`
  ;; feature, it is equivalent to `:blocked`.
  {:perms/view-data             {:model :model/Table,    :values [:unrestricted :sandboxed :impersonated :legacy-no-self-service :blocked]}
