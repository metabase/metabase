...
redirect_from:
  ...
  - /docs/latest/data-modeling/metrics
...
Metrics live in collections. You can save metrics into the [Library](./library.md) collection to surface your org's official curated metrics and encourage people to use them for their own questions.
...
To break metrics out by different dimensions, compare metrics and [measures](./measures.md), and do math with metrics (like add or divide metrics by each other), check out the [Metrics explorer](../../questions/metrics-explorer.md).
...
We don't recommend using joins in metric definitions. If you need to build a metric that uses information from multiple tables, consider making a [transform](../transforms/transforms-overview.md) to join those tables, _then_ create a metric on the table the transform creates.
...
By adding metrics to your Library, the query builder will display the metrics more prominently in menus. See [Library metrics](./library.md#metrics).
...
Prefer using the [Library](./library.md#metrics) to let people know which metrics they should use.
