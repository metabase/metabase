...
DROP TABLE IF EXISTS ` test_data_with_time `.` users `

CREATE EXTERNAL TABLE ` test_data_with_time `.` users ` (
  ` id ` INT,
  ` name ` STRING,
  ` last_login_date ` TIMESTAMP,
  ` last_login_time ` TIMESTAMP,
  ` password ` STRING
) LOCATION 's3://metabase-ci-athena-results/test_data_with_time/users/';

-- 15 rows
INSERT INTO "test_data_with_time"."users" ("name", "last_login_date", "last_login_time", "password")
VALUES
