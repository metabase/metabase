...
(comment
  (create-index-metadata-table! db)
...
(defn create-index-table-metadata!
  ...
  (let [row {:provider          (:provider embedding-model)
             ...
             :table_name        (index-table-name embedding-model)}]
    (jdbc/execute! db (-> (sql.helpers/insert-into :index_metadata (sql.helpers/values [row]))
                          (sql.helpers/returning :id)
                          (sql/format :quoted true)))))

(comment
  (create-index-table-metadata! db embedding-model)
  (jdbc/execute! db ["select * from index_metadata"]))
