(ns metabase-enterprise.remote-sync.content-read-isolation-test
  ...
  (`api/*current-worktree-id*` `nil`) must never see worktree-tagged content, and a worktree-scoped caller must
  never see main or other worktrees' content."
  (:require
...
(deftest segment-can-read-cross-worktree-test
    ...
    (mt/with-temp [:model/RemoteSyncWorktree wt {:branch (str (random-uuid))}]
      (binding [api/*current-worktree-id* (:id wt)]
        (mt/with-temp [:model/Segment wt-segment {:table_id (mt/id :venues) :name "wt segment"}]
          (testing "the worktree scope can read its own segment"
            (is (true? (mi/can-read? wt-segment))))
          (testing "the main scope cannot read a worktree segment"
      ...
      (mt/with-temp [:model/Segment main-segment {:table_id (mt/id :venues) :name "main segment"}]
        ...
        (testing "a worktree scope cannot read a main segment"
          (binding [api/*current-worktree-id* (:id wt)]
            (is (false? (mi/can-read? main-segment)))))))))

(deftest measure-can-read-cross-worktree-test
  (mt/with-test-user :crowberto
    (mt/with-temp [:model/RemoteSyncWorktree wt {:branch (str (random-uuid))}]
      (binding [api/*current-worktree-id* (:id wt)]
        (mt/with-temp [:model/Measure wt-measure {:table_id (mt/id :venues) :name "wt measure"}]
          (testing "the worktree scope can read its own measure"
            (is (true? (mi/can-read? wt-measure))))
          (testing "the main scope cannot read a worktree measure"
            (binding [api/*current-worktree-id* nil]
      ...
      (mt/with-temp [:model/Measure main-measure {:table_id (mt/id :venues) :name "main measure"}]
        ...
        (testing "a worktree scope cannot read a main measure"
          (binding [api/*current-worktree-id* (:id wt)]
            (is (false? (mi/can-read? main-measure)))))))))

(deftest transform-tag-can-read-write-cross-worktree-test
  (mt/with-test-user :crowberto
    (mt/with-temp [:model/RemoteSyncWorktree wt {:branch (str (random-uuid))}]
      (binding [api/*current-worktree-id* (:id wt)]
        (mt/with-temp [:model/TransformTag wt-tag {:name "wt tag"}]
          (testing "the worktree scope can read/write its own tag"
            (is (true? (mi/can-read? wt-tag)))
            (is (true? (mi/can-write? wt-tag))))
          (testing "the main scope cannot read/write a worktree tag"
            (binding [api/*current-worktree-id* nil]
      ...
      (mt/with-temp [:model/TransformTag main-tag {:name "main tag"}]
        ...
        (testing "a worktree scope cannot read/write a main tag"
          (binding [api/*current-worktree-id* (:id wt)]
...
(defn- action-instance
  ...
  and `:worktree_id` on the instance."
  [card-id worktree-id]
  (t2/instance :model/Action {:model_id card-id :worktree_id worktree-id}))

(deftest action-can-read-write-cross-worktree-test
  ;; Two distinct superusers, each with a *fixed* worktree scope for the whole test (one per `:worktree_id` on the
      ...
      (mt/with-temp [:model/Card wt-card {:type :model :collection_id (:id wt-coll) :dataset_query (lib/query (mt/metadata-provider) (lib.metadata/table (mt/metadata-provider) (mt/id :venues)))}]
        (let [wt-action (action-instance (:id wt-card) (:id wt))]
          ...
          (mt/with-current-user (:id main-superuser)
            (testing "the main scope cannot read/write a worktree action"
    ...
    (mt/with-current-user (:id main-superuser)
      (mt/with-temp [:model/Card main-card {:type :model :collection_id (:id main-coll) :dataset_query (lib/query (mt/metadata-provider) (lib.metadata/table (mt/metadata-provider) (mt/id :venues)))}]
        (let [main-action (action-instance (:id main-card) nil)]
          ...
          (mt/with-current-user (:id wt-superuser)
            (testing "a worktree scope cannot read/write a main action"
              (is (false? (mi/can-read? main-action)))
              (is (false? (mi/can-write? main-action))))))))))

;;; ------------------------------------------------- List endpoints ---------------------------------------------

(deftest segment-list-worktree-isolation-test
  (mt/with-full-data-perms-for-all-users!
    (mt/with-temp [:model/RemoteSyncWorktree wt {:branch (str (random-uuid))}
                   :model/User wt-user {:worktree_id (:id wt)}]
      (binding [api/*current-worktree-id* (:id wt)]
        (mt/with-temp [:model/Segment wt-segment {:table_id (mt/id :venues) :name "wt segment"}]
          (testing "GET /api/segment/ excludes worktree content from the main user"
            (is (not (contains? (into #{} (map :id) (mt/user-http-request :rasta :get 200 "segment/"))
                                (:id wt-segment)))))
          (testing "GET /api/segment/ includes worktree content for the worktree user"
      ...
      (mt/with-temp [:model/Segment main-segment {:table_id (mt/id :venues) :name "main segment"}]
        (testing "GET /api/segment/ includes main content for the main user"
          (is (contains? (into #{} (map :id) (mt/user-http-request :rasta :get 200 "segment/"))
                         (:id main-segment))))
        (testing "GET /api/segment/ excludes main content from the worktree user"
          (is (not (contains? (into #{} (map :id) (mt/user-http-request wt-user :get 200 "segment/"))
                              (:id main-segment)))))))))

(deftest measure-list-worktree-isolation-test
  (mt/with-full-data-perms-for-all-users!
    (mt/with-temp [:model/RemoteSyncWorktree wt {:branch (str (random-uuid))}
                   :model/User wt-user {:worktree_id (:id wt)}]
      (binding [api/*current-worktree-id* (:id wt)]
        (mt/with-temp [:model/Measure wt-measure {:table_id (mt/id :venues) :name "wt measure"}]
          (testing "GET /api/measure/ excludes worktree content from the main user"
            (is (not (contains? (into #{} (map :id) (mt/user-http-request :rasta :get 200 "measure/"))
                                (:id wt-measure)))))
          (testing "GET /api/measure/ includes worktree content for the worktree user"
      ...
      (mt/with-temp [:model/Measure main-measure {:table_id (mt/id :venues) :name "main measure"}]
        (testing "GET /api/measure/ includes main content for the main user"
          (is (contains? (into #{} (map :id) (mt/user-http-request :rasta :get 200 "measure/"))
                         (:id main-measure))))
        (testing "GET /api/measure/ excludes main content from the worktree user"
          (is (not (contains? (into #{} (map :id) (mt/user-http-request wt-user :get 200 "measure/"))
                              (:id main-measure)))))))))

(deftest card-list-worktree-isolation-test
  (mt/with-full-data-perms-for-all-users!
    (mt/with-temp [:model/RemoteSyncWorktree wt {:branch (str (random-uuid))}
                   :model/User wt-user {:worktree_id (:id wt)}]
      (binding [api/*current-worktree-id* (:id wt)]
        (mt/with-temp [:model/Card wt-card {:name "wt card"}]
          (testing "GET /api/card excludes worktree content from the main user"
            (is (not (contains? (into #{} (map :id) (mt/user-http-request :rasta :get 200 "card"))
                                (:id wt-card)))))
          (testing "GET /api/card includes worktree content for the worktree user"
      ...
      (mt/with-temp [:model/Card main-card {:name "main card"}]
        (testing "GET /api/card includes main content for the main user"
          (is (contains? (into #{} (map :id) (mt/user-http-request :rasta :get 200 "card"))
                         (:id main-card))))
        (testing "GET /api/card excludes main content from the worktree user"
          (is (not (contains? (into #{} (map :id) (mt/user-http-request wt-user :get 200 "card"))
                              (:id main-card)))))))))

(deftest dashboard-list-worktree-isolation-test
  (mt/with-full-data-perms-for-all-users!
    (mt/with-temp [:model/RemoteSyncWorktree wt {:branch (str (random-uuid))}
                   :model/User wt-user {:worktree_id (:id wt)}]
      (binding [api/*current-worktree-id* (:id wt)]
        (mt/with-temp [:model/Dashboard wt-dash {:name "wt dashboard"}]
          (testing "GET /api/dashboard excludes worktree content from the main user"
            (is (not (contains? (into #{} (map :id) (mt/user-http-request :rasta :get 200 "dashboard"))
                                (:id wt-dash)))))
          (testing "GET /api/dashboard includes worktree content for the worktree user"
      ...
      (mt/with-temp [:model/Dashboard main-dash {:name "main dashboard"}]
        (testing "GET /api/dashboard includes main content for the main user"
          (is (contains? (into #{} (map :id) (mt/user-http-request :rasta :get 200 "dashboard"))
                         (:id main-dash))))
        (testing "GET /api/dashboard excludes main content from the worktree user"
