      ...
      (mt/with-temporary-setting-values [remote-sync-transforms true
        ...
        (let [task-id (t2/insert-returning-pk! :model/RemoteSyncTask {:sync_task_type "import" :initiated_by (mt/user->id :rasta)})]
          (mt/with-temp [:model/Collection {_coll-id :id} {:name "Test Collection" :is_remote_synced true :entity_id "test-collection-1xxxx" :location "/"}]
            (let [transform-entity-id "test-transform-xxxxx"
                  test-files {"main" {"collections/test-collection-1xxxx_test_collection/test-collection-1xxxx.yaml"
                                      (test-helpers/generate-collection-yaml "test-collection-1xxxx" "Test Collection")
                                      (str "transforms/" transform-entity-id "_test_transform.yaml")
...
(deftest import-excludes-transforms-when-setting-disabled-test
  (testing "Import excludes transforms when setting is disabled"
    (mt/with-premium-features #{:transforms}
      (mt/with-temporary-setting-values [remote-sync-transforms false
                                         remote-sync-enabled true]
        (let [task-id (t2/insert-returning-pk! :model/RemoteSyncTask {:sync_task_type "import" :initiated_by (mt/user->id :rasta)})]
          (mt/with-temp [:model/Collection {_coll-id :id} {:name "Test Collection" :is_remote_synced true :entity_id "test-collection-1xxxx" :location "/"}]
            (let [transform-entity-id "excluded-transform-x"
                  test-files {"main" {"collections/test-collection-1xxxx_test_collection/test-collection-1xxxx.yaml"
                                      (test-helpers/generate-collection-yaml "test-collection-1xxxx" "Test Collection")
                                      (str "transforms/" transform-entity-id "_excluded_transform.yaml")
                                      (generate-transform-yaml transform-entity-id "Excluded Transform")}}
                  mock-source (test-helpers/create-mock-source :initial-files test-files)
                  result (impl/import! (source.p/snapshot mock-source) task-id)]
              (is (= :success (:status result)))
