...
(defn- parse-pattern
  ...
  (cond
    ...
    (let [preds (into {} (partitionv 2 (rest pattern)))]
      (cond-> {:type :guard
    ...
    (or (number? pattern) (keyword? pattern))
    {:type :equality
     :value pattern}
     ...
     (case (:type parsed)
       ...
       :vector (let [s (if (symbol? value) value (gensym "vec"))
                 ...
                 (when-not dont-ensure-vectors?
                   (vswap! bindings conj [s `(vector! ~value)]))
                 (dorun (map-indexed #(process-pattern %2 (list `nth s %1) bindings conditions false) parts))
                 (if rest-part
                   (process-pattern rest-part (list `drop cnt value) bindings conditions false)
                   (vswap! conditions conj (list `count= s cnt))))
       :guard (let [s (:symbol parsed)
                ...
                (when (:predicate parsed)
                  (vswap! conditions conj (with-meta (if (and (seq? predicate) (not= (first predicate) 'fn*))
                                                     ...
                                                     {:depends-on s})))
                (when (:length parsed)
                  (vswap! conditions conj (with-meta (list `count= (or bind value) (:length parsed))
                                                     {:depends-on s}))))
       :equality (vswap! conditions conj (list `= value (:value parsed)))
       :set (vswap! conditions conj (list (:value parsed) value))))))

(defn- seq-contains? [seq item]
  (some #(= % item) seq))

(defn- collect-common [processed-patterns value]
  (let [all-bindings (mapv :bindings processed-patterns)
        common-bindings (filter (fn [binding] (every? #(seq-contains? % binding) all-bindings))
                                (first all-bindings))
    ...
    {:common-bindings common-bindings
     ...
     :all-bindings (mapv (fn [bindings]
                         ...
                         all-bindings)
     :all-conditions (mapv (fn [conditions]
                             (remove #(seq-contains? common-conditions %) conditions))
                           all-conditions)}))
...
(defmacro match-lite
  ...
  (let [value-sym (if (symbol? value) value (gensym "value"))
        ...
        (collect-common processed-patterns value-sym)
        same-result? (apply = (map second pairs))]
    `(let [~@(when-not (= value-sym value)
               [value-sym value])
           ~@(when all-vectors?
               [value-sym `(vector! ~value-sym)])
       ...
       ~(expand-conditions
         ...
         (if same-result?
           `(when (or ~@(mapv (fn [bindings conditions]
              ...
              ~(second (first pairs)))
           `(or ~@(mapv (fn [bindings conditions [_ return-expr]]
