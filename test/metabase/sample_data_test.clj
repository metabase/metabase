...
(defmacro ^:private with-temp-sample-database-db
  ...
  `(mt/with-temp Database [db# (sample-database-db)]
     (#'sample-data/sync-sample-database! db#)
     (let [~db-binding db#]
       ~@body)))
...
(deftest sync-sample-database-test
  (testing (str "Make sure the Sample Database is getting synced correctly. In prod, only metadata sync is performed")
    ...
    ;; `fingerprint` and `semantic_type` are nil in prod since DB analysis is done async
    (with-redefs [config/is-prod? true
      ...
      (with-temp-sample-database-db [db]
        (is (= {:description      "The name of the user who owns an account"
                ...
                :base_type        :type/Text}
               (-> (field db "PEOPLE" "NAME")
                   (select-keys [:name :description :database_type :semantic_type :active :visibility_type
                                 :preview_display :display_name :fingerprint :base_type]))))))))
