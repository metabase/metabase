...
cy.describeWithSnowplow(
  ...
  () => {
    ...
    it("Can upload a CSV file to an empty postgres schema", () => {
      const testFile = cy.VALID_CSV_FILES[0];
      ...
      cy.findByLabelText("Upload Settings Form")
        ...
        .click();
      cy.popover().findByText("Writable Postgres12").click();
      cy.findByLabelText("Upload Settings Form")
        .findByText("Select a schema")
        .click();

      cy.popover().findByText(EMPTY_SCHEMA_NAME).click();
      ...
      cy.queryWritableDB(tableQuery, "postgres").then(result => {
    ...
    ["postgres", "mysql"].forEach(dialect => {
      describe(`CSV Uploading (${dialect})`, () => {
        ...
        cy.VALID_CSV_FILES.forEach(testFile => {
          it(`Can upload ${testFile.fileName} to a collection`, () => {
            ...
            cy.queryWritableDB(tableQuery, dialect).then(result => {
              expect(result.rows.length).to.equal(1);
              const tableName =
        ...
        cy.INVALID_CSV_FILES.forEach(testFile => {
        ...
        describe("CSV appends", () => {
          it("Can append a CSV file to an existing table", () => {
            uploadFileToCollection(cy.VALID_CSV_FILES[0]);
            cy.findByTestId("view-footer").findByText(
              `Showing ${cy.VALID_CSV_FILES[0].rowCount} rows`,
            );

            uploadToExisting({
              testFile: cy.VALID_CSV_FILES[0],
              uploadMode: "append",
            });
            cy.findByTestId("view-footer").findByText(
              `Showing ${cy.VALID_CSV_FILES[0].rowCount * 2} rows`,
            );
          });
...
describe("Upload Table Cleanup/Management", () => {
  ...
  it("should allow a user to delete an upload table", () => {
    cy.headlessUpload(FIRST_COLLECTION_ID, cy.VALID_CSV_FILES[0]);
    cy.headlessUpload(FIRST_COLLECTION_ID, cy.VALID_CSV_FILES[0]);
    cy.headlessUpload(FIRST_COLLECTION_ID, cy.VALID_CSV_FILES[0]);

    cy.headlessUpload(FIRST_COLLECTION_ID, cy.VALID_CSV_FILES[1]);
    cy.headlessUpload(FIRST_COLLECTION_ID, cy.VALID_CSV_FILES[1]);
...
function uploadFileToCollection(testFile) {
  ...
  cy.uploadFile("#upload-input", "Uploads Collection", testFile);
...
function uploadToExisting({
  ...
  cy.popover().findByText(uploadOptions[uploadMode]).click();

  cy.fixture(`${cy.FIXTURE_PATH}/${testFile.fileName}`).then(file => {
  ...
  if (identicalSchema) {
    ...
    cy.modal().findByText("Upload error details");
  }
}
