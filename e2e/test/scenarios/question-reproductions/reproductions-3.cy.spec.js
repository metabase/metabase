...
describe("issue 44532", () => {
  ...
  it("should update chart metrics and dimensions with each added breakout (metabase #44532)", () => {
    ...
    H.echartsContainer().within(() => {
      ...
      cy.findByText("January 2030").should("exist");
      cy.findByText("January 2031").should("exist");
...
describe("issue 31960", () => {
  ...
  // the dot that corresponds to July 10–16, 2028
  const dotIndex = 10;
  const rowCount = 11;
  ...
  it("should apply a date range filter for a query broken out by week (metabase#31960)", () => {
    ...
    cy.findByTestId("qb-filters-panel")
      .findByText("Created At: Week is Jul 10–16, 2028")
      .should("be.visible");
...
describe("issue 43057", () => {
  ...
  it("should differentiate between date and datetime filters with 00:00 time (metabase#43057)", () => {
    ...
    H.popover().within(() => {
      ...
      cy.findByLabelText("Date").clear().type("November 18, 2030");
      cy.button("Add filter").click();
    });
    cy.wait("@dataset");
    H.assertQueryBuilderRowCount(16);
    cy.findByTestId("qb-filters-panel")
      .findByText("Created At is on Nov 18, 2030")
      .should("be.visible");
    ...
    H.assertQueryBuilderRowCount(1);
    cy.findByTestId("qb-filters-panel")
      .findByText("Created At is Nov 18, 2030, 12:00 AM")
      .should("be.visible");
