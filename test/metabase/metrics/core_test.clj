...
(deftest metric-sync-dimensions-leaves-schema-upgraded-dimensions-unpersisted-test
  (testing (str "A metric that predates curated dimensions is served by the `card_schema` 23→24 "
                "upgrade, and sync-dimensions! deliberately does not persist that. Like every other "
                "card-schema upgrade, the modernized shape is stored only when something writes the "
                "card. What must hold instead is that reading it twice yields the same dimensions: "
                "the upgrade mints ids derived from each mapping target rather than random ones, so "
                "the metric's serialized content is stable and it does not look perpetually dirty to "
                "remote sync (UXW-4943).")
    (mt/with-temp [:model/Card metric {:name          "Legacy Metric"
      ...
      (let [stored (t2/query-one {:select [:dimensions :dimension_mappings]
                                  :from   [:report_card]
                                  :where  [:= :id (:id metric)]})
            read-1 (:dimensions (t2/select-one :model/Card :id (:id metric)))
            read-2 (:dimensions (t2/select-one :model/Card :id (:id metric)))]
        (is (nil? (:dimensions stored))
            "the upgraded dimensions are not written back")
        (is (nil? (:dimension_mappings stored))
            "nor are the upgraded mappings")
        (is (seq read-1)
            "but the metric still reads as having its full dimension set")
        (is (= read-1 read-2)
            "and two reads produce identical dimensions, ids included")))))
