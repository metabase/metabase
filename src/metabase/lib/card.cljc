...
(mu/defn- remove-bad-field-ids-xform [metadata-providerable :- ::lib.schema.metadata/metadata-providerable]
        ...
        (not-empty
         (into []
               (comp
                ...
                ;; sometimes old metadata was saved with incorrect column IDs (usually when a join had a column with
                ;; the same name), we should strip these out if they're obviously wrong (`:table_id` differs from the
                ;; Table for column with `:id`)
