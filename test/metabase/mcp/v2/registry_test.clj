...
(defn- described
  "The published description of tool `tool-name`, as `tools/list` would send it."
  [tool-name]
  (->> (registry/list-tools)
       (filter (comp #{tool-name} :name))
       first
       :description))

;; not ^:parallel: registers a tool in the shared registry
(deftest description-suffix-test
  (testing "GHY-4522: a :description-suffix is appended to the published description, and is not itself published"
    (do-with-temp-tool!
     {:name               "suffix_probe"
      :scope              "agent:content:read"
      :description        "Static part."
      :description-suffix (constantly " Dynamic part.")
      :args               [:map]
      :handler            (fn [_ _] nil)}
     (fn []
       (is (str/ends-with? (described "suffix_probe") "Static part. Dynamic part."))
       (is (not (contains? (->> (registry/list-tools)
                                (filter (comp #{"suffix_probe"} :name))
                                first)
                           :description-suffix))))))
  (testing "GHY-4522: the suffix is recomputed per list, so the manifest cache can't freeze a stale one in"
    (let [calls (atom 0)]
      (do-with-temp-tool!
       {:name               "suffix_counter"
        :scope              "agent:content:read"
        :description        "Static part."
        :description-suffix (fn [] (str " call " (swap! calls inc)))
        :args               [:map]
        :handler            (fn [_ _] nil)}
       (fn []
         (is (str/ends-with? (described "suffix_counter") "call 1"))
         (is (str/ends-with? (described "suffix_counter") "call 2"))))))
  (testing "GHY-4522: a tool without one is published unchanged"
    (do-with-temp-tool!
     {:name        "no_suffix_probe"
      :scope       "agent:content:read"
      :description "Static part."
      :args        [:map]
      :handler     (fn [_ _] nil)}
...
(deftest tools-hash-does-not-compute-description-suffixes-test
  (testing "GHY-4522: the keepalive polls tools-hash every 30s on every open stream, and a suffix may query the app
            DB — so the hash must not pay for one it then discards"
    (let [calls (atom 0)]
      (do-with-temp-tool!
       {:name               "hash_suffix_probe"
        :scope              "agent:content:read"
        :description        "Static part."
        :description-suffix (fn [] (swap! calls inc) " Dynamic part.")
        :args               [:map]
        :handler            (fn [_ _] nil)}
       (fn []
         (registry/tools-hash)
         (is (zero? @calls))
         (registry/list-tools)
         (is (= 1 @calls) "a list still computes it"))))))
