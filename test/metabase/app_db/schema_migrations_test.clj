...
(deftest clear-tenant-metabot-perms-in-advanced-mode-test
  (let [magic-group-id  (fn [magic-type] (t2/select-one-fn :id :permissions_group :magic_group_type magic-type))
        ...
        migration-range ["v61.2026-08-21T10:00:00" "v61.2026-08-21T10:00:00"]]
    (testing "v61.2026-08-21: in group-level mode the seeded All tenant users rows are cleared, All Users rows stay"
      (impl/test-migrations migration-range [migrate!]
        (let [tenant-id   (magic-group-id "all-external-users")
          ...
          (t2/insert! :setting {:key "metabot-advanced-permissions" :value "true"})
          (is (pos? (perm-count tenant-id)))
          (migrate!)
          (is (zero? (perm-count tenant-id)))
          (is (pos? (perm-count internal-id))))))
    (testing "v61.2026-08-21: in simple mode the All tenant users rows are untouched"
      (impl/test-migrations migration-range [migrate!]
        (let [tenant-id (magic-group-id "all-external-users")
              before    (perm-count tenant-id)]
          (is (pos? before))
          (migrate!)
          (is (= before (perm-count tenant-id))))))))
