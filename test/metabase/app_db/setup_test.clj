...
(deftest supported-app-db-version?-test
  (testing "Should be able to check if an app DB is a supported version"
    (letfn [(test-supported-versions [db expected-min-version]
      ...
      (test-supported-versions :postgres {:major 12 :minor 0 :patch 0})
      (test-supported-versions :mysql {:major 8 :minor 0 :patch 17})
      (test-supported-versions :mariadb {:major 10 :minor 2 :patch 2}))))
