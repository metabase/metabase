...
(deftest minimum-connection-test
  (let [min-conn-params {:athena {:region (tx/db-test-env-var :athena :region)
                                  :s3_staging_dir (tx/db-test-env-var :athena :s3-staging-dir)
                                  :access_key (tx/db-test-env-var :athena :access-key)
                                  :secret_key (tx/db-test-env-var :athena :secret-key)}
                         :redshift {:host (tx/db-test-env-var :redshift :host)
                                    :db (tx/db-test-env-var :redshift :db)
                                    :user (tx/db-test-env-var :redshift :user)
                                    :password (tx/db-test-env-var :redshift :password)}
                         :bigquery-cloud-sdk {:service-account-json (tx/db-test-env-var :bigquery-cloud-sdk :service-account-json)}
                         :databricks {:host (tx/db-test-env-var :databricks :host)
    ...
    (mt/test-drivers (set/intersection (mt/normal-drivers) (set (keys min-conn-params)))
      ...
      (let [params (driver/*driver* min-conn-params)
            bad-params (map #(assoc params % nil) (keys params))]
        (tap> params)
        (is (= true (driver/can-connect? driver/*driver* params)))
        (doseq [params bad-params]
          (let [connected (try
                            (driver/can-connect? driver/*driver* params)
                            (catch Throwable e e))]
            (is (or (instance? Throwable connected) (= false connected)))))))))
