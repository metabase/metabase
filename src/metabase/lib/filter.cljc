...
(defmethod lib.metadata.calculation/display-name-method ::varargs
  ...
  (let [->display-name #(lib.metadata.calculation/display-name query stage-number % style)
    ...
    (lib.util.match/match-lite expr
      ...
      (i18n/tru "{0} {2} is at {1}" (->unbucketed-display-name a) (u.time/format-unit b :hour-of-day)
                (if (#{:= :in} op) "is at" "excludes the hour"))

      [(op :guard #{:= :in :!= :not-in}) _ [:get-day-of-week _ (a :guard temporal?) :iso] & (args :guard (every? int? args))]
      (let [cnt (count args)]
        (if (clojure.core/= cnt 1)
          (i18n/tru "{0} {2} {1}" (->display-name a) (u.time/format-unit (first args) :day-of-week-iso)
                    (if (#{:= :in} op) "is on" "excludes"))
          (i18n/tru "{0} {3} {1} {2} selections" (->display-name a) cnt
      ...
      (i18n/tru "{0} is {2}equal to {1} selections" (->display-name a) (count args) (if (#{:= :in} op) "" "not "))

      [(op :guard #{:= :in :!= :not-in}) _ a b]
      (i18n/tru "{0} is {2}{1}" (->display-name a) (if (string? b) b (->display-name b)) (if (#{:= :in} op) "" "not "))

      [(op :guard #{:= :in :!= :not-in}) _ a & args]
      (i18n/tru "{0} is {2}{1} selections" (->display-name a) (count args) (if (#{:= :in} op) "" "not "))
