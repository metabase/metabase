(ns metabase.subscription.core-test
  (:require
   ...
   [metabase.subscription.test-util :as subscription.test-util]
...
(defn- rasta-pulse-email [& [email]]
  (mt/email-to :rasta (merge {:subject "Pulse: Pulse Name"
                              :body    [{"Pulse Name" true}
                                        subscription.test-util/png-attachment
                                        subscription.test-util/png-attachment]
                              :bcc?    true}
                             email)))
...
(defn do-with-pulse-for-card
  ...
  (mt/with-temp [Pulse        {pulse-id :id, :as pulse}
    ...
    (if (= channel :email)
      (t2.with-temp/with-temp [:model/SubscriptionChannelRecipient _ {:user_id                 (subscription.test-util/rasta-id)
...
(defn- do-test
  ...
  (doseq [channel-type [:email :slack]
    ...
    (testing (format "sent to %s channel" channel-type)
      (mt/with-temp [Card          {card-id :id} (merge {:name    subscription.test-util/card-name
          ...
          (letfn [(thunk* []
                  ...
                  (thunk []
                    (if fixture
                      ...
                      (thunk*)))]
            (case channel-type
              :email (subscription.test-util/email-test-setup (thunk))
              :slack (subscription.test-util/slack-test-setup (thunk)))))))))
...
(def ^:private test-card-result {subscription.test-util/card-name true})
(def ^:private test-card-regex  (re-pattern subscription.test-util/card-name))
...
(deftest basic-timeseries-test
  (do-test
   {:card    (subscription.test-util/checkins-query-card {:breakout [!day.date]})
     ...
     (fn [{:keys [card-id]} [pulse-results]]
       (is (= {:channel-id "#general"
               ...
               [{:blocks [{:type "header", :text {:type "plain_text", :text "Pulse: Pulse Name", :emoji true}}
                ...
                {:title           subscription.test-util/card-name
                 :rendered-info   {:attachments false
                                   :content     true}
                 :title_link      (str "https://metabase.com/testmb/question/" card-id)
                 :attachment-name "image.png"
                 :channel-id      "FOO"
                 :fallback        subscription.test-util/card-name}]}
              (subscription.test-util/thunk->boolean pulse-results))))}}))

(deftest basic-table-test
  (tests {:pulse {:skip_if_empty false} :display :table}
    "9 results, so no attachment"
    {:card    (subscription.test-util/checkins-query-card {:aggregation nil, :limit 9})

     :fixture
     (fn [_ thunk]
       (with-redefs [body/attached-results-text (subscription.test-util/wrap-function @#'body/attached-results-text)]
         (thunk)))
      ...
      (fn [{:keys [card-id]} [pulse-results]]
        (testing "\"more results in attachment\" text should not be present for Slack Pulses"
          ...
          (testing "attached-results-text should be invoked exactly once"
            (is (= 1
                   (count (subscription.test-util/input @#'body/attached-results-text)))))
          (testing "attached-results-text should return nil since it's a slack message"
            (is (= [nil]
                   (subscription.test-util/output @#'body/attached-results-text))))))}}

    "11 results results in a CSV being attached and a table being sent"
    {:card (subscription.test-util/checkins-query-card {:aggregation nil, :limit 11})
     ...
     {:email
      (fn [_ _]
        (is (= (rasta-pulse-email {:body [{"Pulse Name"                      true
                                           ...
                                           "ID</th>"                         true}
                                          subscription.test-util/png-attachment
                                          subscription.test-util/csv-attachment]})
               (mt/summarize-multipart-email
                #"Pulse Name"
     ...
     {:email
      (fn [_ _]
        (is (= (-> (rasta-pulse-email)
                   ...
                   (assoc-in ["rasta@metabase.com" 0 :body] [{"Pulse Name" true} subscription.test-util/png-attachment])
                   (add-rasta-attachment subscription.test-util/csv-attachment))
               (mt/summarize-multipart-email #"Pulse Name"))))}}))
      ...
      {:email
       (fn [_ _]
         (is (= (-> (rasta-pulse-email)
                   ;; There's no PNG with a table visualization, so only assert on one png (the dashboard icon)
                    (assoc-in ["rasta@metabase.com" 0 :body] [{"Pulse Name" true} subscription.test-util/png-attachment])
                    (add-rasta-attachment subscription.test-util/xls-attachment))
                (mt/summarize-multipart-email #"Pulse Name"))))}})))
  ...
  (testing "Basic test, 1 card, 1 recipient, with XLS attachment"
    (do-test
     {:card       (subscription.test-util/checkins-query-card {:breakout [!day.date]})
      ...
      {:email
       (fn [_ _]
         (is (= (add-rasta-attachment (rasta-pulse-email) subscription.test-util/xls-attachment)
                (mt/summarize-multipart-email #"Pulse Name"))))}})))

(deftest csv-xls-no-data-test
  (testing "card with CSV and XLS attachments, but no data. Should not include an attachment"
    (do-test
     {:card       (subscription.test-util/checkins-query-card {:filter   [:> $date "2017-10-24"]
                                                               :breakout [!day.date]})
      :pulse      {:skip_if_empty false}
      :pulse-card {:include_csv true
                   :include_xls true}
...
(deftest ensure-constraints-test
  (testing "Validate pulse queries are limited by `default-query-constraints`"
    (do-test
     {:card
      (subscription.test-util/checkins-query-card {:aggregation nil})
      :display :table
...
(deftest multiple-recipients-test
  (testing "Pulse should be sent to two recipients"
    (do-test
     {:card
      (subscription.test-util/checkins-query-card {:breakout [!day.date]})
...
(deftest two-cards-in-one-pulse-test
  (testing "1 pulse that has 2 cards, should contain two query image attachments (as well as an icon attachment)"
    (do-test
     {:card
      (assoc (subscription.test-util/checkins-query-card {:breakout [!day.date]}) :name "card 1")

      :fixture
      (fn [{:keys [pulse-id]} thunk]
        (mt/with-temp [Card {card-id-2 :id} (assoc (subscription.test-util/checkins-query-card {:breakout [!month.date]})
                                                   :name "card 2"
                                                   :display :line)
...
(deftest empty-results-test
  (testing "Pulse where the card has no results"
    (tests {:card (subscription.test-util/checkins-query-card {:filter   [:> $date "2017-10-24"]
                                                               :breakout [!day.date]})}
      "skip if empty = false"
      {:pulse    {:skip_if_empty false}
       :assert {:email (fn [_ _]
...
(deftest rows-alert-test
  (testing "Rows alert"
    (tests {:pulse {:alert_condition "rows", :alert_first_only false}}
        ...
        (fn [_ _]
          (is (= (rasta-alert-email
                  ...
                  [(assoc test-card-result "More results have been included" false)
                   subscription.test-util/png-attachment subscription.test-util/png-attachment])
          ...
          (is (= {:channel-id  "#general",
                  :attachments [{:blocks [{:type "header", :text {:type "plain_text", :text "🔔 Test card", :emoji true}}]}
                                {:title                  subscription.test-util/card-name
                                 ...
                                 :channel-id             "FOO"
                                 :fallback               subscription.test-util/card-name}]}
                 (subscription.test-util/thunk->boolean result)))
          (is (every? produces-bytes? (rest (:attachments result)))))}}

      "with no data"
      {:card
       (subscription.test-util/checkins-query-card {:filter   [:> $date "2017-10-24"]
                                                    :breakout [!day.date]})
       :assert
       {:email
        (fn [_ _]
      ...
      "too much data"
      {:card
       (subscription.test-util/checkins-query-card {:limit 21, :aggregation nil})
       :display :table
        ...
        (fn [_ _]
          (is (= (rasta-alert-email "Alert: Test card has results"
                                    [test-card-result
                                     ...
                                     subscription.test-util/png-attachment
                                     subscription.test-util/csv-attachment
                                     subscription.test-util/xls-attachment])
                 (mt/summarize-multipart-email test-card-regex))))}})))
...
(deftest above-goal-alert-test
  (testing "above goal alert"
    (tests {:pulse {:alert_condition  "goal"
      ...
      "with data"
      {:card
       (merge (subscription.test-util/checkins-query-card {:filter   [:between $date "2014-04-01" "2014-06-01"]
                                                           :breakout [!day.date]})
              {:display                :line
       ...
       {:email
        (fn [_ _]
          (is (= (rasta-alert-email "Alert: Test card has reached its goal"
                                    [test-card-result subscription.test-util/png-attachment subscription.test-util/png-attachment])
                 (mt/summarize-multipart-email test-card-regex))))}}

      "no data"
      {:card
       (merge (subscription.test-util/checkins-query-card {:filter   [:between $date "2014-02-01" "2014-04-01"]
                                                           :breakout [!day.date]})
              {:display                :area
               :visualization_settings {:graph.show_goal true :graph.goal_value 5.9}})
      ...
      "with progress bar"
      {:card
       (merge (subscription.test-util/venues-query-card "max")
              {:display                :progress
...
(deftest below-goal-alert-test
  (testing "Below goal alert"
    (tests {:card  {:visualization_settings {:graph.show_goal true :graph.goal_value 1.1}}
      ...
      "with data"
      {:card
       (subscription.test-util/checkins-query-card {:filter   [:between $date "2014-02-12" "2014-02-17"]
                                                    :breakout [!day.date]})
       :display :line
      ...
      {:card
       (subscription.test-util/checkins-query-card {:filter   [:between $date "2014-02-10" "2014-02-12"]
                                                    :breakout [!day.date]})
       :display :bar
      ...
      "with progress bar"
      {:card
       (merge (subscription.test-util/venues-query-card "min")
              {:display                :progress
...
(deftest goal-met-test
  (let [alert-above-pulse {:alert_above_goal true}
        ...
        goal-met? (fn [pulse [first-result]] (#'subscription/goal-met? pulse [first-result]))]
  ...
  (testing "Native query with user-specified x and y axis"
    (t2.with-temp/with-temp [Card {card-id :id} {:name                   "Test card"
      ...
      (with-pulse-for-card [{pulse-id :id} {:card card-id, :pulse {:alert_condition  "goal"
                                                                   :alert_first_only false
                                                                   :alert_above_goal true}}]
        (subscription.test-util/email-test-setup
         (subscription/send-pulse! (pulse/retrieve-notification pulse-id))
         (is (= (rasta-alert-email "Alert: Test card has reached its goal"
                                   [test-card-result subscription.test-util/png-attachment subscription.test-util/png-attachment])
                (mt/summarize-multipart-email test-card-regex))))))))
...
(deftest dashboard-description-markdown-test
  (testing "Dashboard description renders markdown"
    (mt/with-temp [Card                  {card-id :id} {:name "Test card"}
                   ...
                   :model/SubscriptionChannelRecipient _ {:user_id                 (subscription.test-util/rasta-id)
                                                          :subscription_channel_id sc-id}]
        (subscription.test-util/email-test-setup
         (subscription/send-pulse! (pulse/retrieve-notification pulse-id))
...
(deftest basic-slack-test-2
  (testing "Basic slack test, 2 cards, 1 recipient channel"
    (mt/with-temp [Card         {card-id-1 :id} (subscription.test-util/checkins-query-card {:breakout [!day.date]})
                   Card         {card-id-2 :id} (-> {:breakout [:field (mt/id :checkins :date) {:temporal-unit :month}]}
                                                    subscription.test-util/checkins-query-card
                   ...
                   :model/SubscriptionChannel _               {:pulse_id     pulse-id
                                                               :channel_type "slack"
                                                               :details      {:channel "#general"}}]
      (subscription.test-util/slack-test-setup
       (let [[slack-data] (subscription/send-pulse! (pulse/retrieve-pulse pulse-id))]
         (is (= {:channel-id "#general",
                 :attachments
                 [{:blocks
                   [{:type "header", :text {:type "plain_text", :text "Pulse: Pulse Name", :emoji true}}
                ...
                (subscription.test-util/thunk->boolean slack-data)))
         (testing "attachments"
      ...
      (let [titles         (atom [])
            attachments    [{:title           "a"
                            ...
                            {:title           "b"
                             ...
                             :channel-id      "FOO"}]
            processed      (subscription/create-and-upload-slack-attachments! attachments (slack-uploader titles))]
...
(deftest multi-channel-test
  (testing "Test with a slack channel and an email"
    (t2.with-temp/with-temp [Card {card-id :id} (subscription.test-util/checkins-query-card {:breakout [!day.date]})]
      ...
      (with-pulse-for-card [{pulse-id :id} {:card card-id, :pulse {:skip_if_empty false}}]
        ...
        (t2.with-temp/with-temp [:model/SubscriptionChannel _ {:pulse_id     pulse-id
                                                               ...
                                                               :details      {:channel "#general"}}]
          (subscription.test-util/slack-test-setup
           (let [pulse-data (subscription/send-pulse! (pulse/retrieve-pulse pulse-id))
  ...
  (testing "Pulses should be sent with the Permissions of the user that created them."
    (letfn [(send-pulse-created-by-user!* [user-kw]
              (mt/with-temp [Collection coll {}
                ...
                (subscription.test-util/send-pulse-created-by-user! user-kw card)))]
...
(defn- get-retry-metrics []
  (let [^io.github.resilience4j.retry.Retry retry (:retry @@#'subscription/retry-state)]
...
(defn- reset-retry []
  (let [old (get-retry-metrics)]
    (#'subscription/reconfigure-retrying nil nil)
    old))
...
(deftest email-notification-retry-test
  (testing "send email succeeds w/o retry"
    (with-redefs [email/send-email! mt/fake-inbox-email-fn]
      (mt/with-temporary-setting-values [email-smtp-host "fake_smtp_host"
        ...
        (mt/reset-inbox!)
        (reset-retry)
        (#'subscription/send-notifications! [fake-email-notification])
  ...
  (testing "send email succeeds w/ retry"
    (let [retry-config (#'subscription/retry-configuration)]
      (try
        (with-redefs [email/send-email! (tu/works-after 1 mt/fake-inbox-email-fn)
                      subscription/retry-configuration (constantly (assoc retry-config
                                                                            :max-attempts 2
...
(deftest slack-notification-retry-test
  (testing "post slack message succeeds w/o retry"
    (with-redefs [slack/post-chat-message! (constantly nil)]
      (reset-retry)
      (#'subscription/send-notifications! [fake-slack-notification])
      (is (= {:numberOfSuccessfulCallsWithoutRetryAttempt 1}
             (pos-metrics (reset-retry))))))
