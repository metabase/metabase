...
export const NODE_SERIALIZERS: MarkdownSerializer["nodes"] = {
  ...
  [Paragraph.name]: (state, node, parent, index) => {
    ...
    if (parent.type.name === "doc" && node.content.size === 0) {
      ...
      if (isFirstInSequence) {
        state.write(formatSpacer(emptyCount));
  ...
  // Card embed nodes
  [CardEmbedNode.name]: (state, node) => {
    state.write(formatCardEmbed(node.attrs));
    state.closeBlock(node);
  },

  // Smart link nodes
  [SmartLinkNode.name]: (state, node) => {
    state.write(formatSmartLink(node.attrs));
  },
};
