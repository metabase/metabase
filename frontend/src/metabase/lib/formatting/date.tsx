...
interface DateRangeFormatSpec {
  same: null | string; // Like 'year', 'month', 'day', etc. for dayjs
...
export function formatDateToRangeForParameter(
  ...
  const start = m.clone().startOf(unit as any);
  const end = m.clone().endOf(unit as any);
...
export function normalizeDateTimeRangeWithUnit(
  ...
  const dayjsUnit = unit.split("-")[0] as any;
...
export function formatDateTimeRangeWithUnit(
  ...
  if (shift === undefined) {
    ...
    return formatWeek(dayjs.isDayjs(start) ? start : dayjs(start), options);
  }
  ...
  const matchSpec =
    specs.find(
      (spec) =>
        dayjs.isDayjs(start) &&
        dayjs.isDayjs(end) &&
        start.isSame(end, spec.same as any),
    ) ?? defaultSpec;
