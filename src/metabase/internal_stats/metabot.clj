...
(defn- usage-by-model
  ...
  [date-utc]
  (let [rows (internal-stats.queries/proxied-ai-usage-tokens-by-model date-utc)]
...
(defn metabot-stats
  ...
  (let [today-utc     (t/offset-date-time (t/zone-offset "+00"))
        ...
        tokens        (or (internal-stats.queries/proxied-ai-usage-tokens-on yesterday-utc) 0)
    ...
    (when (or (pos? tokens) (seq rolling-usage))
      (cond-> {}
        ...
        (merge {:metabot-tokens     (long tokens)
                ...
                :metabot-queries    (internal-stats.queries/proxied-metabot-user-message-count-on yesterday-utc)
                ...
                :metabot-users      (:cnt (internal-stats.queries/proxied-metabot-user-count-on yesterday-utc))
