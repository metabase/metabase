...
- `graph.goal_value`, `progress.goal`, `gauge.segments`, and `scalar.segments` are now reserved setting ids: they can point at another saved question and make Metabase run it. The `Settings` type rejects them and Metabase ignores setting definitions that use them. Runtime writes through `onChangeSettings`, `writeDependencies`, and `eraseDependencies` are limited (and now typed) to the plugin's own setting ids plus `card.title`, `card.description`, `card.hide_empty`, and `click_behavior`; other keys are dropped with a console warning.
- `ClickObject` is no longer generic and no longer has a `settings` field. Metabase attaches the visualization's current settings to every click itself, so a plugin cannot point dashboard click behavior elsewhere; anything passed under `settings` is ignored.
