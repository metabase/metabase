...
;; Re-export pure functions from transforms-base for backwards compatibility.
(shared.ns/import-fns
 [metabase.transforms-base.util
  ...
  source->checkpoint-filter-unique-key
...
;;; ------------------------------------------------- Feature gating (depends on transforms module) ---------------------
...
(defn normalize-source-tables
  ...
  (let [int-table-ids    (into #{} (filter int?) (vals source-tables))
        ...
        refs-needing-id  (filter transforms-base.util/missing-table-id? (vals source-tables))
        ref-lookup       (or (transforms-base.util/batch-lookup-table-ids refs-needing-id) {})]
    ...
    (update-vals source-tables
                 (fn [v]
                   (cond
                     ...
                     (:table_id v) v
                     :else        (assoc v :table_id (ref-lookup (transforms-base.util/source-table-ref->key v))))))))
  ...
  (let [{:keys [checkpoint-filter checkpoint-filter-unique-key]} source-incremental-strategy]
    (when-some [{column-name :name
                ...
                (cond
                  ...
                  (transforms-base.util/source->checkpoint-filter-unique-key query source-incremental-strategy)
                  checkpoint-filter
                  (when-some [field-id (t2/select-one-pk :model/Field
                                                         :table_id (:id table)
                                                         :name checkpoint-filter)]
                    (lib.metadata/field metadata-provider field-id)))]
      (when-not (transforms-base.util/supported-incremental-filter-type? base-type)
...
(defn- next-checkpoint-value
  ...
  [checkpoint]
  (transforms-base.util/next-checkpoint-value checkpoint))
    ...
    (case (keyword query-type)
      ...
      (let [checkpoint (next-checkpoint transform)
        ...
        (binding [driver/*compile-with-inline-parameters*
          ...
          (-> query
              (transforms-base.util/preprocess-incremental-query source-incremental-strategy checkpoint)
...
(defn execute-secondary-index-ddl-if-required!
  ...
  See [[metabase.transforms-base.util/decide-secondary-index-ddl]] for details."
...
(mu/defn handle-transform-complete!
  ...
  See [[metabase.transforms-base.util/decide-secondary-index-ddl]] for details on the index handling."
