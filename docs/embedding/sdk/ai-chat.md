...
- **Strip links returned by the agent**. The agent text may include links pointing back to the host Metabase (like  alink to a chart it created). Those links require an authenticated Metabase session, so people won't be able to view the links.
- **The chart components accept different props depending on `drills`.** Both the `Chart` on each chart message and the `CurrentChart` returned by the hook are pre-wired with `questionId`, `token`, and `query`, so you shouldn't pass those props. With the default `drills={false}`, the component renders [`StaticQuestion`](./questions.md#staticquestion) and accepts its remaining props. With `drills={true}`, the component renders [`InteractiveQuestion`](./questions.md#interactivequestion) and accepts its remaining props.
