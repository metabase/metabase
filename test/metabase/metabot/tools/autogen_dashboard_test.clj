...
(deftest create-autogenerated-dashboard-entity-usage-test
  (testing "table_id seed populates :input as a table ref"
    (mt/with-temp [:model/Table {table-id :id} {:name (mt/random-name) :db_id (mt/id)}]
      (let [result (binding [shared/*memory-atom* (atom {:state {}})]
                     (autogen-dashboard/create-autogenerated-dashboard-tool
                      {:source {:table_id table-id}}))
            eu     (get-in result [:structured-output :entity-usage])]
        (is (nil? (mr/explain entity-usage/entity-usage-schema eu)))
        (is (= [{:type "table" :id table-id}] (:input eu)))
        (is (= [] (:output eu))))))
  (testing "report_id seed populates :input as a question ref"
    (mt/with-temp [:model/Card {card-id :id} {:type :question :name "test_report"}]
      (let [result (binding [shared/*memory-atom* (atom {:state {}})]
                     (autogen-dashboard/create-autogenerated-dashboard-tool
                      {:source {:report_id card-id}}))
            eu     (get-in result [:structured-output :entity-usage])]
        (is (= [{:type "question" :id card-id}] (:input eu))))))
  (testing "query_id seed (memory-only) leaves :input empty"
    (let [query       {:lib/type :mbql/query :database 1 :stages []}
