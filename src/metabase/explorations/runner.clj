...
(defn- oldest-pending-age-seconds
  ...
  []
  (if-let [oldest (explorations.db/oldest-pending-query-created-at)]
...
(defn- runnable-query
  ...
  [query-id]
  (explorations.db/runnable-query query-id))
...
(defn- claim-analysis-if-ready!
  ...
  [thread-id]
  (pos? (explorations.db/claim-thread-analysis! thread-id (OffsetDateTime/now))))
...
(defn- slicing-note
  ...
  (let [segment-name (when-let [sid (:segment_id row)]
                       (explorations.db/segment-name sid))
...
(defn- build-score-context
  ...
  {:card-description (when-let [card-id (:card_id exploration-query)]
                       (some-> (explorations.db/card-description card-id)
                               str/trim
                               not-empty))
   :sql              (contextual-interestingness/dataset-query->sql (:dataset_query exploration-query))})

(defn- safe-score+describe
  "Best-effort combined contextual scorer + describer for one chart, returning

      {:score :chart-description :metric-description}

  `:metric-description` is always the *effective* description: the Card's own when it has one,
  the generated one otherwise.

  `stats` are the deep stats [[safe-deep-stats]] already computed for this chart. Nil-safe — the
  scorer computes its own when we have none.

  Returns nil whenever scoring isn't applicable (no prompt, no chart-config, no creator) or
  anything throws, so a scoring failure can never break the query lifecycle."
  [exploration-query chart-config stats creator-id]
  ;; Bind the creator so the LLM call is attributed, permission-checked, and usage-limited as them
  ;; rather than as whoever triggered this run.
  (try
...
(defn- persist-query-result!
  ...
  (try
    (t2/with-transaction [_conn]
      (let [sr-id (explorations.db/insert-stored-result! {:result_data       bytes
                                                          :creator_id        creator-id
                                                          :database_id       db-id
                                                          :dataset_query     (:dataset_query row)
                                                          :row_count         row-count
                                                          :data_access_token token})]
        (explorations.db/insert-query-result!
