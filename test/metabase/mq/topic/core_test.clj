(ns metabase.mq.topic.core-test
  (:require
   ...
   [metabase.mq.topic.core :as topics]
...
(deftest ^:parallel e2e-publish-subscribe-test
  (tpt/with-memory-topics [recent]
    (let [received (atom [])]
      (topics/subscribe! :topic/e2e "test-handler"
                         (fn [{:keys [messages]}]
                           (swap! received into messages)))

      (testing "Messages are received by subscriber"
        (topics/publish! :topic/e2e ["message-1"])
        (topics/publish! :topic/e2e ["message-2"])
        (Thread/sleep 200)
      ...
      (testing "Unsubscribe stops delivery"
        (topics/unsubscribe! :topic/e2e "test-handler")
        (topics/publish! :topic/e2e ["message-3"])
        (Thread/sleep 200)
        (is (= ["message-1" "message-2"] @received))))))
...
(deftest ^:parallel batch-publish-e2e-test
  (tpt/with-memory-topics [_recent]
    (let [received (atom [])]
      (topics/subscribe! :topic/batch "batch-handler"
                         (fn [{:keys [messages]}]
                           (swap! received into messages)))

      (topics/publish! :topic/batch ["a" "b" "c"])
      (Thread/sleep 200)

      (testing "Batch of messages delivered together"
        (is (= ["a" "b" "c"] @received)))

      (topics/unsubscribe! :topic/batch "batch-handler"))))

(deftest ^:parallel fan-out-e2e-test
  (tpt/with-memory-topics [_recent]
    (let [received-a (atom [])
          received-b (atom [])]
      (topics/subscribe! :topic/fan-out "handler-a"
                         (fn [{:keys [messages]}]
                           (swap! received-a into messages)))
      (topics/subscribe! :topic/fan-out "handler-b"
                         (fn [{:keys [messages]}]
                           (swap! received-b into messages)))

      (topics/publish! :topic/fan-out ["broadcast-msg"])
      (Thread/sleep 200)
      ...
      (topics/unsubscribe! :topic/fan-out "handler-a")
      (topics/unsubscribe! :topic/fan-out "handler-b"))))

(deftest ^:parallel error-handling-e2e-test
  (tpt/with-memory-topics [recent]
    (let [received (atom [])]
      (topics/subscribe! :topic/errors "error-handler"
                         (fn [{:keys [messages]}]
                           (when (= ["fail"] messages)
                             (throw (ex-info "Handler error" {})))
                           (swap! received into messages)))

      (topics/publish! :topic/errors ["ok-1"])
      (topics/publish! :topic/errors ["fail"])
      (topics/publish! :topic/errors ["ok-2"])
      (Thread/sleep 200)
      ...
      (topics/unsubscribe! :topic/errors "error-handler"))))

(deftest ^:parallel late-subscriber-test
  (tpt/with-memory-topics [_recent]
    (topics/publish! :topic/late ["old-message"])

    (let [received (atom [])]
      (topics/subscribe! :topic/late "late-handler"
                         (fn [{:keys [messages]}]
                           (swap! received into messages)))

      (topics/publish! :topic/late ["new-message"])
      (Thread/sleep 200)

      (testing "Late subscriber only sees new messages"
        (is (= ["new-message"] @received)))

      (topics/unsubscribe! :topic/late "late-handler"))))
