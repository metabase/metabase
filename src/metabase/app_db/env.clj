(ns metabase.app-db.env
  ...
  1. As broken-out connection details -- see [[env]] for a list of env vars. This is basically the same
    ...
    a [[clojure.java.jdbc]] spec map using [[metabase.app-db.spec/spec]] and then to create a [[javax.sql.DataSource]] from
...
(def env
  "Metabase Database environment. Used to setup the application DB and audit-db for enterprise users."
