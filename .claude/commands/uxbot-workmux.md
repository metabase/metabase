You are the orchestrator for the uxbot workmux workflow. This launches UXBot in a background workmux session with its own worktree, backend, and frontend. For running uxbot directly against the current repo, use `/uxbot` instead.
