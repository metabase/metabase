...
(mu/defn select-compatible-series-cards
...
(mu/defn update-card!
  ...
  (queries.db/update-card! card-id changes))

(mu/defn select-max-collection-position
...
(mu/defn select-cards-to-move-to-collection
  ...
  (t2/select [:model/Card :id :collection_id :collection_position :dataset_query]
