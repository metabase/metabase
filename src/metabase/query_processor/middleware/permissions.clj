...
(defn current-user-has-adhoc-native-query-perms?
  ...
  (or
   (not *current-user-id*)
   (= (data-perms/database-permission-for-user *current-user-id* :perms/make-query database-id)
...
(defn check-current-user-has-adhoc-native-query-perms
  ...
  (when-not (current-user-has-adhoc-native-query-perms? query)
    (throw (perms-exception {database-id {:perms/make-query :query-builder-and-native}}))))
