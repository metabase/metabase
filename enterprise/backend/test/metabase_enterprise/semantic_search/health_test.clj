...
;; ^{:parallel false}: redefs the process-global `get-configured-model` / `get-embedding` via with-redefs, so it
;; must not run concurrently with other tests (e.g. entity-retrieval-available? reads get-configured-model).
(deftest ^{:parallel false} probe-embedding-service-test
...
(deftest ^{:parallel false} breaker-transition-probe-cache-test
...
(deftest ^{:parallel false} circuit-recovery-probe-test
...
(deftest ^{:parallel false} report-repair-metrics!-test
...
(deftest ^{:parallel false} refresh-clears-garbage-when-inactive-test
