...
This skill runs mutation testing end-to-end: generates a coverage report, groups functions, shells out to `Codex -p` to write tests, verifies mutations are killed, and creates draft PRs with Linear issues.
...
4. For each group: create branch → invoke Codex to write tests → verify → retry if needed → commit & push → create Linear issue → create draft PR
...
To inspect what Codex will see without invoking it:
