...
(deftest ^:parallel normalize-visualization-settings-preserves-column-names-test
  (testing "table.columns: field refs are normalized, column names are preserved"
    (is (= {:table.columns [{:name "expression"  :enabled true  :field_ref [:field 1 nil]}
  ...
  (testing "table.column_formatting: legacy field refs in :columns are normalized"
    (is (= {:table.column_formatting [{:columns [[:field 1 nil]]
                                       :type    "single"
                                       :value   10}]}
           (mi/normalize-visualization-settings
            {"table.column_formatting" [{"columns" [["field" 1 nil]]
                                         "type"    "single"
                                         "value"   10}]}))))

  (testing "pivot_table.column_split: column names are preserved as strings"
    (is (= {:pivot_table.column_split {:rows    ["expression" "CREATED_AT"]
  ...
  (testing "pivot_table.collapsed_rows: column names are preserved as strings"
    (is (= {:pivot_table.collapsed_rows {:rows  ["aggregation" "expression"]
                                         :value ["some-id"]}}
           (mi/normalize-visualization-settings
            {"pivot_table.collapsed_rows" {"rows"  ["aggregation" "expression"]
                                           "value" ["some-id"]}}))))

  (testing "graph.metrics: column names are preserved as strings"
    (is (= {:graph.metrics ["expression" "aggregation"]}
           (mi/normalize-visualization-settings
            {"graph.metrics" ["expression" "aggregation"]}))))

  (testing "click_behavior: dimension refs are normalized"
    (is (= {:click_behavior {:parameterMapping
                             {:key {:target {:dimension [:dimension
                                                         [:field 1 nil]
                                                         {:stage-number 1}]}}}}}
           (mi/normalize-visualization-settings
  ...
  (testing "click_behavior in column_settings: dimension refs are normalized"
    (is (= {:column_settings
            {"[\"name\",\"ID\"]"
             {:click_behavior
              {:parameterMapping
               {:key {:target {:dimension [:dimension [:field 2 nil]]}}}}}}}
           (mi/normalize-visualization-settings
            {"column_settings"
             {"[\"name\",\"ID\"]"
              {"click_behavior"
