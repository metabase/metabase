...
import { FieldOrderPicker2 } from "./FieldOrderPicker";
...
function setup({ value = "smart" }: SetupOpts = {}) {
  ...
  const { rerender } = renderWithProviders(
    <FieldOrderPicker2 value={value} onChange={onChange} />,
  );

  return { rerender, onChange };
}

describe("FieldOrderPicker2", () => {
  ...
  it("updates selected value when value prop changes", () => {
    const { rerender } = renderWithProviders(
      <FieldOrderPicker2 value="smart" onChange={jest.fn()} />,
    );

    expect(screen.getByRole("radio", { name: "Auto order" })).toBeChecked();
    rerender(<FieldOrderPicker2 value="custom" onChange={jest.fn()} />);
