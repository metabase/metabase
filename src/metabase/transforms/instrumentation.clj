...
(mu/defn record-stage-start!
  ...
  (analytics/inc! :metabase-transforms/stage-started
...
(mu/defn record-stage-completion!
  ...
  (let [labels {:stage-type  (name stage-type)
                :stage-label (name stage-label)}]
    (analytics/inc! :metabase-transforms/stage-completed labels)
    (analytics/observe! :metabase-transforms/stage-duration-ms labels duration-ms)))
...
(mu/defn record-stage-failure!
  ...
  (let [labels {:stage-type  (name stage-type)
                :stage-label (name stage-label)}]
    (analytics/inc! :metabase-transforms/stage-failed labels)
    (when duration-ms
      (analytics/observe! :metabase-transforms/stage-duration-ms labels duration-ms))))
...
(mu/defn record-data-transfer!
  ...
  (let [labels {:stage-label (name stage-label)}]
    (when bytes
      (analytics/observe! :metabase-transforms/data-transfer-bytes labels bytes))
    (when rows
...
(defn record-job-start!
  ...
  (analytics/inc! :metabase-transforms/job-runs-total
...
(defn record-job-completion!
  ...
  (let [labels {:run-method (name run-method)}]
    (analytics/inc! :metabase-transforms/job-runs-completed labels)
    (analytics/observe! :metabase-transforms/job-run-duration-ms labels duration-ms)))
...
(defn record-job-failure!
  ...
  (let [labels {:run-method (name run-method)}]
    (analytics/inc! :metabase-transforms/job-runs-failed labels)
    (analytics/observe! :metabase-transforms/job-run-duration-ms labels duration-ms)))
