...
> **Refreshing the cache automatically doesn't apply when [data sandboxing](../permissions/data-sandboxes.md) or [connection impersonation](../permissions/impersonation.md) permissions are in effect.** In those permissions setups, Metabase ignores the cache and runs a fresh query (which it will then cache), as those permissions filter data specific to each viewer. So _caching_ still works for sandboxing and impersonation, it's just that preemptive caching (automatic caching) doesn't apply in these cases.
