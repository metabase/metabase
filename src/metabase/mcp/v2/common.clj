...
(defn success-content
  "Assemble the two MCP response channels deliberately. The text block is the only channel
   guaranteed to reach the model, so `text` must be self-sufficient: everything the model needs
   to reason or make its next call. `structured` is emitted only when a concrete programmatic
   consumer needs a typed shape, and must be a mirror or strict subset of the text — never its
   sole home. Read tools default to text-only (omit `structured`)."
