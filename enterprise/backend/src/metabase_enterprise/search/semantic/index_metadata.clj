(ns metabase-enterprise.search.semantic.index-metadata
  ...
  without data loss. Each immutable embedding space gets its own index table,
...
(def app-db-index-metadata
  ...
  [[metabase-enterprise.search.semantic.db.migration.impl]]) is structurally incapable of touching
  application tables.
  Table names are stored and threaded around schema-qualified; derive identifiers and query catalogs via
  [[metabase-enterprise.search.semantic.util/table-name-part]]."
...
(defn- row->index [{:keys [provider
                           ...
                           table_name
                           index_version]}]
  {:embedding-model  (cond-> {:provider          provider
                              :model-name        model_name
                              :vector-dimensions vector_dimensions
                              :embedding-space-id embedding_space_id
                              :embedding-spi-version embeddings.provider/embedding-spi-version}
                       model_revision (assoc :model-revision model_revision))
...
(comment
  ...
  (def embedding-model ((requiring-resolve 'metabase-enterprise.search.semantic.env/get-configured-embedding-model)))
