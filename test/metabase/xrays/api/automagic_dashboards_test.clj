...
(deftest xray-dashboard-parameter-types-test
  (testing "Should set correct parameter types with temporal filters first"
    (mt/dataset test-data
      (let [products-params (:parameters (mt/user-http-request :crowberto :get 200
                                                               (format "automagic-dashboards/table/%d" (mt/id :products))))
            people-params   (:parameters (mt/user-http-request :crowberto :get 200
                                                               (format "automagic-dashboards/table/%d" (mt/id :people))))]
        (testing "Products: Created At should be first, followed by Category"
          (is (= [{:name "Created At" :type "date/all-options" :sectionId "date"}
                  {:name "Category"   :type "string/="         :sectionId "string"}]
                 (mapv #(select-keys % [:name :type :sectionId]) products-params))))
        (testing "People: Created At should be first, followed by other filters"
          (is (= "Created At" (:name (first people-params))))
          (is (= #{{:name "Created At" :type "date/all-options" :sectionId "date"}
                   {:name "Source"     :type "string/="         :sectionId "string"}
                   {:name "State"      :type "string/="         :sectionId "location"}
                   {:name "City"       :type "string/="         :sectionId "location"}}
                 (set (map #(select-keys % [:name :type :sectionId]) people-params)))))))))
