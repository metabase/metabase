...
(deftest test-get-provider
  (testing "get-active-model returns based on setting"
    ...
    (testing "a named connection selects the provider: its type is the embedding API"
      (llm.tu/with-connections [{:key "embeddings" :type "openai" :name "Embeddings" :config {:api-key "sk-embed"}}]
        ...
        (testing "a connection that no longer exists embeds through nothing, rather than falling back"
          (mt/with-temporary-setting-values [ee-embedding-connection "missing"
                                             ee-embedding-provider   "ai-service"]
...
(deftest embedding-supported?-test
  ...
  (testing "openai: supported iff the connection semantic search embeds through has an API key"
