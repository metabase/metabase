...
describe("scenarios > embedding-sdk > URL", () => {
  describe("dashboard-click-behavior", () => {
    beforeEach(() => {
  ...
  describe("URL column", () => {
    const CYPRESS_URL = Cypress.config("baseUrl");
    beforeEach(() => {
      signInAsAdminAndEnableEmbeddingSdk();
      H.createNativeQuestion(
        {
          name: "A column with an optional URL value",
          native: {
            query: `select * from
                  values
                    ('Hello world', 'not a URL'),
                    ('${CYPRESS_URL}/question/999', 'internal URL'),
                    ('https://www.metabase.com/docs/latest/embedding/sdk/introduction', 'external URL')
                  as x(url, description)`,
          },
        },
    ...
    it("whatever", () => {
      cy.get("@questionId").then((questionId) => {
        ...
        tableInteractiveBody().within(() => {
          cy.log("Non-URL shouldn't be rendered as a link");
          cy.findByText("Hello world").should("not.have.attr", "href");

          cy.log("Internal links shouldn't be rendered as a link");
          cy.findByText(`${CYPRESS_URL}/question/999`).should(
            "not.have.attr",
            "href",
          );

          cy.log("Only external links are rendered as links");
          const getExternalUrl = () =>
            cy.findByText(
              "https://www.metabase.com/docs/latest/embedding/sdk/introduction",
            );
          /**
           * Chaining assertion doesn't work for some reason.
           * After the first assertion, it yields non chainable object.
           * That's why I need to call `cy.findByXxx` separately.
           */
          getExternalUrl().should("have.attr", "href");
          getExternalUrl().should("have.attr", "target", "_blank");
        });
      });
    });
  });
