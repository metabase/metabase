...
const getTimezoneWithOffset = (offsetString: string) => {
  ...
  const matchingZones = moment.tz.names().filter(zone => {
    const tzOffset = moment.tz(zone).utcOffset();
    return tzOffset === totalOffsetMinutes;
  });

  const etcZone = matchingZones.find(zone => zone.startsWith("Etc/GMT"));
  return etcZone || matchingZones[0];
};

export const getIanaTimezone = (timezone: string) => {
  if (!isTimezoneInOffsetFormat(timezone)) {
    return timezone;
  }

  try {
    const matchingTimezone = getTimezoneWithOffset(timezone);

    if (!matchingTimezone) {
      console.warn(`Could not find a time zone with offset ${timezone}`);
      return timezone;
    }
