...
describe("formatNumber with data measure", () => {
  describe("binary units (default)", () => {
    it("should format bytes correctly", () => {
      expect(formatNumber(0, { number_style: "datasize" })).toBe("0 B");
      expect(formatNumber(512, { number_style: "datasize" })).toBe("512 B");
      expect(formatNumber(1023, { number_style: "datasize" })).toBe("1023 B");
    });

    it("should format KiB correctly", () => {
      expect(formatNumber(1024, { number_style: "datasize" })).toBe("1 KiB");
      expect(formatNumber(1536, { number_style: "datasize" })).toBe("1.5 KiB");
      expect(formatNumber(1048575, { number_style: "datasize" })).toBe(
        "1024 KiB",
      );
    });

    it("should format MiB correctly", () => {
      expect(formatNumber(1048576, { number_style: "datasize" })).toBe("1 MiB");
      expect(formatNumber(1572864, { number_style: "datasize" })).toBe(
        "1.5 MiB",
      );
    });

    it("should format GiB correctly", () => {
      expect(formatNumber(1073741824, { number_style: "datasize" })).toBe(
        "1 GiB",
      );
      expect(formatNumber(1610612736, { number_style: "datasize" })).toBe(
        "1.5 GiB",
      );
    });

    it("should format TiB correctly", () => {
      expect(formatNumber(1099511627776, { number_style: "datasize" })).toBe(
        "1 TiB",
      );
      expect(formatNumber(1649267441664, { number_style: "datasize" })).toBe(
        "1.5 TiB",
      );
    });

    it("should format PiB correctly", () => {
      expect(formatNumber(1125899906842624, { number_style: "datasize" })).toBe(
        "1 PiB",
      );
      expect(formatNumber(1688849860263936, { number_style: "datasize" })).toBe(
        "1.5 PiB",
      );
    });
  });
  ...
  describe("decimal units", () => {
    it("should format with decimal units when specified", () => {
      const options: FormatNumberOptions = {
        number_style: "datasize",
        datasize_unit_system: "decimal",
      };
  ...
  describe("header unit display", () => {
    it("should omit unit when datasize_unit_in_header is true", () => {
      const options = {
        number_style: "datasize",
        datasize_unit_in_header: true,
        type: "cell",
      };
    ...
    it("should include unit when datasize_unit_in_header is false", () => {
      const options = {
        number_style: "datasize",
        datasize_unit_in_header: false,
        type: "cell",
      };
  ...
  describe("negative values", () => {
    it("should handle negative data measures", () => {
      expect(formatNumber(-1024, { number_style: "datasize" })).toBe("-1 KiB");
      expect(formatNumber(-1536, { number_style: "datasize" })).toBe(
        "-1.5 KiB",
      );
      expect(formatNumber(-1048576, { number_style: "datasize" })).toBe(
        "-1 MiB",
      );
    });
  });
  ...
  describe("edge cases", () => {
    it("should handle very large numbers", () => {
      expect(
        formatNumber(1125899906842624 * 1000, { number_style: "datasize" }),
      ).toBe("1000 PiB");
    });

    it("should handle fractional bytes", () => {
      expect(formatNumber(0.5, { number_style: "datasize" })).toBe("1 B"); // Rounds up
      expect(formatNumber(0.1, { number_style: "datasize" })).toBe("0 B");
    });
  });
});
