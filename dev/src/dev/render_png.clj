...
(defn render-card-to-png
  ...
  [card-id]
  (let [{:keys [dataset_query result_metadata], card-type :type, :as card} (t2/select-one card/:model/Card :id card-id)
...
(defn render-pulse-card
  ...
  [card-id]
  (let [{:keys [dataset_query] :as card} (t2/select-one card/:model/Card :id card-id)
