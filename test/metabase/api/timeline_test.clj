  ...
  (testing "GET /api/timeline"
      ...
      (let [id        (u/the-id collection)
            events-of (fn [tls]
                        (into #{} (comp (filter (comp #{id} :collection_id))
                                        (map :name))
                              tls))]
        (mt/with-temp [:model/Timeline _ {:name "Timeline A" :collection_id id}
                       :model/Timeline _ {:name "Timeline B" :collection_id id}
                       :model/Timeline _ {:name "Timeline C" :collection_id id :archived true}]
    ...
    (testing "checks permissions"
      (mt/with-temp [:model/Collection    {coll-id :id} {:name "private collection"}
                     ...
                     :model/Timeline      tl-b {:name "Timeline B" :collection_id coll-id}
...
(deftest timelines-range-test
  (testing "GET /api/timeline/:id?include=events&start=TIME&end=TIME"
    (mt/with-temp [:model/Collection    collection {:name "Collection"}
      ...
      (mt/with-temp [:model/TimelineEvent _event-a2 {:name         "event-a2"
                                                     :timeline_id  (u/the-id tl-a)
                                                     :timestamp    #t "2020-01-01T10:00:00.0Z"
                                                     :time_matters false}]
...
(deftest update-timeline-test
  (testing "PUT /api/timeline/:id"
    (mt/with-temp [:model/Collection _ {:name "Important Data"}
  ...
  (testing "GET /api/timeline/:id?include=events"
    (mt/with-temp [:model/Collection collection {:name "Important Data"}
                   :model/Timeline empty-tl {:name          "Empty TL"
                                             :collection_id (u/the-id collection)}
                   :model/Timeline unarchived-tl {:name          "Un-archived Events TL"
                                                  :collection_id (u/the-id collection)}
                   :model/Timeline archived-tl {:name          "Archived Events TL"
                                                :collection_id (u/the-id collection)}
                   :model/Timeline timeline {:name          "All Events TL"
                                             :collection_id (u/the-id collection)}
                   :model/TimelineEvent _ {:name        "event-a"
  ...
  (when config/ee-available?
    (mt/with-premium-features #{:advanced-permissions}
      (mt/with-temp [:model/Worktree {wt-id :id} {}
                     ...
                     :model/Timeline {main-id :id} {:name "main timeline"}
                     :model/Timeline {wt-content-id :id} {:name "worktree timeline" :collection_id (:id wt-coll) :worktree_id wt-id}
        ...
        (testing "the main-app list leaves worktree content out"
          (let [ids (into #{} (map :id) (mt/user-http-request :crowberto :get 200 "timeline"))]
            (is (contains? ids main-id))
            (is (not (contains? ids wt-content-id)))))
        (testing "worktree-id returns only that worktree's content"
          (is (= [wt-content-id] (mapv :id (mt/user-http-request :crowberto :get 200 "timeline" :worktree-id wt-id)))))
        (testing "worktree-id requires the remote-sync permission"
          (is (= "You don't have permissions to do that."
                 (mt/user-http-request :rasta :get 403 "timeline" :worktree-id wt-id))))
        (testing "a non-admin holding the remote-sync permission sees that worktree's content too"
          ...
          (is (= [wt-content-id] (mapv :id (mt/user-http-request :rasta :get 200 "timeline" :worktree-id wt-id)))))))))
