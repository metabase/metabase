(ns metabase.test-runner.effects-test
  ...
  (testing "Partial sequence match"
    ...
    (testing "actual missing element"
      (is (= {:only-in-actual   nil
              ...
              :pass?            false}
             (#'effects/partial=-diff {:a [1 2 3]} {:a [1 2]})))
      (is (= {:only-in-actual   nil
              :only-in-expected [nil nil 3]
              :pass?            false}
             (#'effects/partial=-diff [1 2 3] [1 2])))
      (is (= []
             (#'effects/remove-keys-not-in-expected ["A"] [])))
      (is (= {:only-in-actual   nil
              :only-in-expected ["A"]
              :pass?            false}
             (#'effects/partial=-diff ["A"] []))))
    (testing "actual has wrong element"
      (is (= {:only-in-actual   {:a [nil nil 4]}
              :only-in-expected {:a [nil nil 3]}
              :pass?            false}
             (#'effects/partial=-diff {:a [1 2 3]} {:a [1 2 4]})))
      (is (= {:only-in-actual   [nil nil 4]
              :only-in-expected [nil nil 3]
              :pass?            false}
             (#'effects/partial=-diff [1 2 3] [1 2 4])))))

  (testing "Empty sequence match"
    (is (= {:a 1, :b []}
           (#'effects/remove-keys-not-in-expected
            {:a 1, :b []}
            {:a 1, :b [], :c [1]})))
    (is (partial= {:a 1, :b []}
                  {:a 1, :b []}))))
