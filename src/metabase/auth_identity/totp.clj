...
(defn generate-code
  "Current TOTP code for `secret` (mainly for tests / enrollment confirmation helpers)."
  ^String [^String secret]
  (code-for-unix-time secret (quot (System/currentTimeMillis) 1000)))

(defn valid-code?
  "True if `code` matches the TOTP for `secret` within ±1 time step (clock-skew tolerance)."
  [^String secret code]
  (boolean
   (when (and secret (string? code) (re-matches #"\d{6}" code))
     (let [now (quot (quot (System/currentTimeMillis) 1000) time-step-seconds)]
       (some (fn [^long step] (= (format-code (hotp secret step)) code))
             [(dec now) now (inc now)])))))
