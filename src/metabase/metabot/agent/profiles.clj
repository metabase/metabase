...
(mu/defn ^:private register-profile!
  ...
  - :system-prompt-context - Optional fn of the request context returning a map of extra,
    ...
    opt in.
  - :routing? - Optional boolean (default false). When true, Jev picks each turn's tools and skills (see
    [[metabase.metabot.agent.routing]]) and the chosen skills are inlined instead of loaded via `load_skill`.
  - :prefetch-data-sources? - Optional boolean (default false). When true, the data sources Jev judges the latest
    prompt to need are found before the turn and described in its context (see
    [[metabase.metabot.agent.data-sources]]).
  - :fast? - Optional boolean (default false). When true, the agent's model calls request the provider's faster
    serving where the model supports it, whatever the `llm-fast-mode` setting says.
  ...
  [profile :- [:map {:closed true}
               ...
               [:prefetch-data-sources? {:optional true} :boolean]
               [:fast? {:optional true} :boolean]]]
...
(register-profile!
 (-> nlq-profile
     (assoc :name :nlq-old)
     (dissoc :routing? :prefetch-data-sources? :fast?)
     (update :tools (partial filterv #(not= #'tools/explore-table-tool %)))))
  ...
  (if-let [profile (get @*profiles profile-id)]
    (let [profile (if (nlq-fallback? profile-id)
                    (if-let [fb (get @*profiles :nlq-fallback)]
                      (assoc profile
                             :tools (mapv #(if (= #'tools/retrieve-library-entities-tool %) #'tools/nlq-search-tool %)
                                          (:tools profile))
                             :prompt-template (:prompt-template fb))
