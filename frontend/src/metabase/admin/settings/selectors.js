...
function updateSectionsWithPlugins(sections) {
    if (PLUGIN_ADMIN_SETTINGS_UPDATES.length > 0) {
        ...
        const sortByOrder = (
            [, { order: order1 = Number.MAX_VALUE }], [, { order: order2 = Number.MAX_VALUE }],
        ) => order1 - order2;
    ...
    email: {
        ...
        settings: [{
            ...
            },
            {
                ...
                required: true,
                validations: [
                    ["integer", t `That's not a valid port number`]
                ],
            },
            {
            ...
            },
            {
                ...
                required: true,
                validations: [
                    ["email", t `That's not a valid email address`]
                ],
            },
        ],
    },
...
export const getSettings = createSelector(
    ...
    settings.map(setting =>
        warnings[setting.key] ? {...setting, warning: warnings[setting.key] } :
        setting,
    ),
);
