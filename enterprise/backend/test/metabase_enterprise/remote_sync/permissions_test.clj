  ...
  (testing "can_write for non-remote-synced collection items should be unaffected by git-sync-allow-edit setting"
    (mt/with-current-user (mt/user->id :rasta)
      (mt/with-temp [:model/Collection {regular-coll-id :id} {:name "Regular Collection"
        ...
        (doseq [git-sync-setting [true false]]
          (testing (str "When git-sync-allow-edit is " git-sync-setting)
            (mt/with-temporary-setting-values [library.settings/git-sync-allow-edit git-sync-setting]
              ...
              (testing "Cards in regular collections have can_write=true"
                (mt/with-temp [:model/Card {card-id :id} {:name "Regular Card"
                                                          ...
                                                          :dataset_query (mt/native-query {:query "SELECT 1"})}]
                  (is (true? (mi/can-write? (t2/select-one :model/Card :id card-id)))
                      "Card in regular collection should always be writable regardless of git-sync-allow-edit")))
              ...
              (testing "Dashboards in regular collections have can_write=true"
                (mt/with-temp [:model/Dashboard {dashboard-id :id} {:name "Regular Dashboard"
                                                                    :collection_id regular-coll-id}]
                  (is (true? (mi/can-write? (t2/select-one :model/Dashboard :id dashboard-id)))
                      "Dashboard in regular collection should always be writable regardless of git-sync-allow-edit")))
...
(deftest remote-synced-collection-itself-permissions-test
  (testing "can_write for remote-synced collections themselves should respect git-sync-allow-edit"
    ...
    (testing "Regular collections are always writable regardless of git-sync-allow-edit"
      (doseq [git-sync-setting [true false]]
        (mt/with-temporary-setting-values [library.settings/git-sync-allow-edit git-sync-setting]
          (mt/with-temp [:model/Collection {regular-coll-id :id} {:name "Regular Collection"
                                                                  :type nil}]
            (mt/with-current-user (mt/user->id :rasta)
              (is (true? (mi/can-write? (t2/select-one :model/Collection :id regular-coll-id)))
                  (str "Regular collection should always be writable when git-sync-allow-edit is " git-sync-setting)))))))))
