...
(defn ordered-cards
  ...
  [{:keys [id]}]
  (db/sel DashboardCard, :dashboard_id id, {:order-by [[:created_at :asc]]}))
...
(defn revert-dashboard
  ...
  (let [serialized-cards    (:cards serialized-dashboard)
        ...
        current-cards       (db/sel [DashboardCard :sizeX :sizeY :row :col :id :card_id], :dashboard_id dashboard-id)
