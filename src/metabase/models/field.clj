...
;; TODO -- this seems like something that belongs under `metabase.driver.*` or `metabase.query-processor.*` -- HoneySQL
;; stuff is part of SQL QP compilation and not intrinsically part of the Field model itself
(s/defn nfc-field->parent-identifier
  ...
  [field-identifier :- (s/pred h2x/identifier?) field]
  (let [nfc-path          (:nfc_path field)
        parent-components (-> (h2x/identifier-components field-identifier)
                              vec
                              pop
                              (conj (first nfc-path)))]
    (apply h2x/identifier :field parent-components)))
