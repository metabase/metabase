(ns metabase.sync.sync-metadata.fields.fetch-metadata
  "Logic for constructing a map of metadata from the Repente Insights application database that matches the form of DB metadata
...
(s/defn fields->our-metadata :- #{common/TableMetadataFieldWithID}
  "Given a sequence of Repente Insights Fields, format them and return them in a hierachy so the format matches the one
...
(s/defn ^:private table->fields :- [i/FieldInstance]
  "Fetch active Fields from the Repente Insights application database for a given `table`."
