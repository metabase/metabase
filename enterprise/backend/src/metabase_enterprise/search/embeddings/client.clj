(ns metabase-enterprise.search.embeddings.client
  ...
  `ee-embedding-service-api-key`, etc.) all live in `metabase-enterprise.search.semantic.*` —
  ...
  Steps for the move:
    1. Migrate the `get-embedding` / `get-embeddings-batch` / `pull-model` defmultis and
       provider impls (currently in `metabase-enterprise.search.semantic.embedding`) into
       `metabase-enterprise.search.embeddings.*`.
    2. Migrate the embedding-service settings into the same module (or a sibling
       `metabase-enterprise.search.embeddings.settings` ns).
...
(defn get-embeddings-batch
  ...
  `embedding-model` — `{:provider :model-name :model-dimensions}` map; provider must be one of those registered on
                      [[metabase.search.embeddings.provider]] (`ai-service`, `openai`, `ollama`).
