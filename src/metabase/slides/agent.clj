...
(defmethod run-tool "search_metabase"
  ...
  (let [accepted (set (or models ["card" "dashboard"]))
    ...
    {:results
     (cond-> []
       (accepted "card")
       (into (->> (t2/select [:model/Card :id :name :description :display :collection_id]
                             {:where [:and
                                      [:= :archived false]
                                      [:or
    ...
    (if (or (nil? dash) (try (api/read-check dash) false (catch Exception _ true)))
      ...
      (let [card-ids (->> (t2/select [:model/DashboardCard :card_id]
            ...
            cards    (when (seq card-ids)
                       (->> (t2/select [:model/Card :id :name :description :display]
...
(defmethod run-tool "get_card"
  [_ {:keys [card_id]}]
  (let [card (t2/select-one [:model/Card :id :name :description :display :result_metadata]
