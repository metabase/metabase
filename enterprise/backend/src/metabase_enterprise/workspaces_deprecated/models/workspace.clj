(ns metabase-enterprise.workspaces-deprecated.models.workspace
  (:require
...
(comment metabase-enterprise.workspaces-deprecated.models.workspace-database/keep-me)

(methodical/defmethod t2/table-name :model/WorkspaceDeprecated [_model] :workspace_deprecated)

(doto :model/WorkspaceDeprecated
...
(defmethod mi/can-read? :model/WorkspaceDeprecated
  ([_instance] api/*is-superuser?*)
  ([_model _pk] api/*is-superuser?*))

(defmethod mi/can-create? :model/WorkspaceDeprecated
  [_model _instance]
  api/*is-superuser?*)

(defmethod mi/can-write? :model/WorkspaceDeprecated
  ([_instance] api/*is-superuser?*)
  ([_model _pk] api/*is-superuser?*))

(methodical/defmethod t2/batched-hydrate [:model/WorkspaceDeprecated :databases]
  [_model k workspaces]
   ...
   (fn []
     (when-let [ids (seq (map :id workspaces))]
       (group-by :workspace_id
                 (t2/select :model/WorkspaceDatabaseDeprecated :workspace_id [:in ids]))))
   :id
   {:default []}))

(methodical/defmethod t2/batched-hydrate [:model/WorkspaceDeprecated :creator]
  [_model k workspaces]
  (mi/instances-with-hydrated-data
   workspaces k
   (fn []
...
(defn list-workspaces
  ...
  `:creator` hydrated."
  []
  (t2/hydrate (t2/select :model/WorkspaceDeprecated {:order-by [[:id :asc]]}) :creator [:databases :database]))

(defn get-workspace
  "Return the Workspace with the given id and its `:databases` (each with its
  `:database`) + `:creator` hydrated, or nil if none exists."
  [id]
  (when-let [workspace (t2/select-one :model/WorkspaceDeprecated :id id)]
...
(defn get-workspace-by-name
  ...
  [workspace-name]
  (when-let [workspace (t2/select-one :model/WorkspaceDeprecated :name workspace-name {:order-by [[:id :asc]]})]
...
(defn create-workspace!
  ...
  (t2/with-transaction [_conn]
    (let [workspace-id (t2/insert-returning-pk! :model/WorkspaceDeprecated
                                                {:name       name
                                                 :creator_id creator_id})]
      (when (seq databases)
        (t2/insert! :model/WorkspaceDatabaseDeprecated
...
(defn delete-workspace!
  ...
  (when (t2/exists? :model/WorkspaceDatabaseDeprecated
    ...
    (throw (ex-info "Cannot delete a workspace with databases that are not :unprovisioned; deprovision them first"
                    {:status-code 409
                     :workspace_id id})))
  (t2/delete! :model/WorkspaceDeprecated :id id))
  ...
  (t2/with-transaction [_conn]
    ...
    (when (contains? params :databases)
      (let [databases         (:databases params)
            existing          (t2/select :model/WorkspaceDatabaseDeprecated :workspace_id id)
        ...
        (when-let [to-delete-ids (seq (map :id unprovisioned))]
          (t2/delete! :model/WorkspaceDatabaseDeprecated :id [:in to-delete-ids]))
