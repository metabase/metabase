(ns metabase-enterprise.workspaces-deprecated.models.workspace-test
  (:require
  ...
  (testing "create-workspace! creates a Workspace with no databases"
    (mt/with-model-cleanup [:model/WorkspaceDeprecated]
      (let [created (create-ws! {:name "Solo" :databases []})]
        ...
        (is (t2/exists? :model/WorkspaceDeprecated :id (:id created)))))))
      ...
      (mt/with-temp [:model/Database {db2-id :id} {:engine :h2 :details {}}]
        (let [{id :id} (create-ws!
          ...
          (testing "the removed workspace_database is gone from the database"
            (is (not (t2/exists? :model/WorkspaceDatabaseDeprecated
                                 :workspace_id id
                                 :output_namespace "drop_out")))))))))
  ...
  (testing "update-workspace! with empty :databases removes all children"
    (mt/with-model-cleanup [:model/WorkspaceDeprecated]
      (let [{id :id} (create-ws!
        ...
        (is (not (t2/exists? :model/WorkspaceDatabaseDeprecated :workspace_id id)))))))
      ...
      (mt/with-temp [:model/Database {db2-id :id} {:engine :h2 :details {}}]
        (let [{id :id} (create-ws!
                        {:name      "Doomed"
                         :databases [(ws-db-attrs)
                                     (ws-db-attrs {:database_id db2-id})]})]
          (is (= 2 (t2/count :model/WorkspaceDatabaseDeprecated :workspace_id id)))
          (t2/delete! :model/WorkspaceDeprecated :id id)
          (is (zero? (t2/count :model/WorkspaceDatabaseDeprecated :workspace_id id))))))))
...
(deftest update-preserves-initialized-rows-test
  (testing "update-workspace! leaves :provisioned rows untouched when the PUT keeps their database_id + input"
    (mt/with-temp [:model/WorkspaceDeprecated {ws-id :id} {:name "WS"}
                   :model/WorkspaceDatabaseDeprecated {init-id :id}
                   {:workspace_id     ws-id
                    :database_id      (mt/id)
      ...
      (let [updated (workspace/update-workspace!
            ...
            row     (t2/select-one :model/WorkspaceDatabaseDeprecated :id init-id)]
...
(deftest update-rejects-dropping-initialized-test
  (testing "update-workspace! refuses to drop an :provisioned row"
    (mt/with-temp [:model/Database {db2-id :id} {:engine :h2 :details {}}
                   :model/WorkspaceDeprecated {ws-id :id} {:name "WS"}
    ...
    (mt/with-model-cleanup [:model/WorkspaceDeprecated]
      (mt/with-temp [:model/Database {db-id :id} {:engine :h2 :details {}}]
        (let [{ws-id :id} (create-ws!
          ...
          (is (t2/exists? :model/WorkspaceDatabaseDeprecated :workspace_id ws-id))
          (t2/delete! :model/Database :id db-id)
          (is (not (t2/exists? :model/WorkspaceDatabaseDeprecated :workspace_id ws-id))))))))
...
(deftest can-read?-test
  (testing "Workspace `mi/can-read?`"
    (mt/with-temp [:model/WorkspaceDeprecated ws {}]
...
(deftest can-create?-test
  (testing "Workspace `mi/can-create?`"
    (testing "non-superuser — false"
      (with-normal-user (is (false? (mi/can-create? :model/WorkspaceDeprecated {})))))
    (testing "Data Analyst (non-superuser) — false"
      (with-data-analyst (is (false? (mi/can-create? :model/WorkspaceDeprecated {})))))
    (testing "superuser — true"
      (with-admin (is (true? (mi/can-create? :model/WorkspaceDeprecated {})))))))
...
(deftest can-write?-test
  (testing "Workspace `mi/can-write?`"
    ...
    (mt/with-temp [:model/Database          {db1-id :id}       {}
                   ...
                   :model/WorkspaceDeprecated         {ws-id :id :as ws} {}
                   :model/WorkspaceDatabaseDeprecated _                  {:workspace_id ws-id, :database_id db1-id}
                   :model/WorkspaceDatabaseDeprecated _                  {:workspace_id ws-id, :database_id db2-id}]
