...
(deftest ^:parallel address-allowed-for-network-policy?-test
  ...
  (testing "allow-loopback re-admits loopback on top of everything allow-private admits"
    (doseq [ip ["127.0.0.1" "127.0.1.5" "::1" "10.1.2.3" "192.168.0.1" "100.64.0.1" "fc00::1"]]
      (is (true? (http/address-allowed-for-network-policy? :allow-loopback (InetAddress/getByName ip))) ip)))
  (testing "allow-loopback still refuses link-local -- the cloud metadata service is the address that matters"
    (doseq [ip ["169.254.169.254" "fe80::1" "0.0.0.0" "224.0.0.1" "ff02::1"]]
      (is (false? (http/address-allowed-for-network-policy? :allow-loopback (InetAddress/getByName ip))) ip)))
