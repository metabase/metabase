(ns metabase.metabot.capabilities
  ...
  Capabilities arrive from the API as strings (e.g. \"frontend:navigate_user_v1\")
  while tool/skill metadata uses keywords (e.g. :frontend-navigate-user-v1). Some
  capabilities are inferred from backend state rather than sent by the frontend."
  (:require
...
(def ^:private api-string->capability-keyword
  ...
  used in tool/skill :capabilities metadata. Keywords pass through unchanged."
...
(defn- feature-capabilities
  "Return set of capabilities inferred from BE state, not sent from FE. Generated by function to avoid staleness."
  []
  (into #{}
        (remove nil?)
...
(defn capability-set
  "Build the full set of capability keywords for a request: the backend-inferred
  feature capabilities unioned with the (normalized) capabilities from the API."
  [capabilities]
