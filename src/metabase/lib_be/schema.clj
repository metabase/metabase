...
(mr/def ::internal-query.scope
  ...
  [:or
   [:map {:closed true, :probe/id "src/metabase/lib_be/schema.clj:41"} [:type :keyword] [:dashboard-id ms/PositiveInt]]
   [:map {:closed true, :probe/id "src/metabase/lib_be/schema.clj:42"} [:type :keyword] [:dashcard-id ms/PositiveInt]]
   [:map {:closed true, :probe/id "src/metabase/lib_be/schema.clj:43"} [:type :keyword] [:card-id ms/PositiveInt]]
   [:map {:closed true, :probe/id "src/metabase/lib_be/schema.clj:44"} [:type :keyword] [:model-id ms/PositiveInt]]
   [:map {:closed true, :probe/id "src/metabase/lib_be/schema.clj:45"} [:type :keyword] [:table-id ms/PositiveInt]]
   [:map {:closed true, :probe/id "src/metabase/lib_be/schema.clj:46"} [:type :keyword] [:webhook-id ms/PositiveInt]]
   [:map {:closed true, :probe/id "src/metabase/lib_be/schema.clj:47"} [:type :keyword] [:unknown [:enum :model-action]]]])
