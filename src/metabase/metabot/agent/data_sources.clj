...
(defn enabled?
  ...
  [profile]
  (boolean (and (:prefetch-data-sources? profile) (jev/key-present?))))
...
(defn- candidate-question [i]
  (jev/noul (str "`candidates[" i "]` would be used in the query or content that answers `user_prompt`, which was "
...
(defn- likely-candidates
  ...
  [prompt search-args results]
  (let [candidates (->> results
                        (keep #(when-let [uri (candidate-uri %)] (assoc % ::uri uri)))
                        (take max-candidates)
                        vec)]
    (when (seq candidates)
      (let [answers (:answers (jev/ask {:user_prompt    prompt
...
(defn describe
  ...
  with into the next turn."
  [search prompt read-resource-fn known]
                   ...
                   (try
                     (let [{:keys [args result]} @search
                           ...
                           candidates (when (and read-resource-fn (seq results))
                                        (likely-candidates prompt args results))
