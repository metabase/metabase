...
describe("scenarios > question > snippets", () => {
  ...
  it("should let you edit snippet", () => {
    ...
    // 1. select
    cy.openNativeEditor().type("select ");
    // 2. snippet
...
cy.describeEE("scenarios > question > snippets (EE)", () => {
  beforeEach(() => {
    cy.restore();
    cy.signInAsAdmin();
  ...
  it("should let you create a snippet folder and move a snippet into it", () => {
    ...
    // eslint-disable-next-line no-unscoped-text-selectors -- deprecated usage
    cy.popover().within(() => cy.findByText("New folder").click());
    cy.modal().within(() => {
    ...
    // eslint-disable-next-line no-unscoped-text-selectors -- deprecated usage
    cy.modal().within(() => cy.findByText("Top folder").click());
    ...
    // eslint-disable-next-line no-unscoped-text-selectors -- deprecated usage
    cy.modal().within(() => cy.findByText("Save").click());
  ...
  describe("existing snippet folder", () => {
    ...
    it("should not allow you to move a snippet collection into a itself or a child (metabase#44930)", () => {
      ...
      cy.popover().findByText("Edit folder details").click();
      cy.modal().findByTestId("collection-picker-button").click();
    ...
    it("shouldn't update root permissions when changing permissions on a created folder (metabase#17268)", () => {
      ...
      cy.popover().contains("View").click();
      cy.button("Save").click();
