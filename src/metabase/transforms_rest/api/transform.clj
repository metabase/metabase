...
(api.macros/defendpoint :get "/:id/dependencies" :- [:sequential TransformResponse]
  ...
  (let [transform     (api/read-check :model/Transform id)
        ;; a transform only depends on transforms in its own remote-sync worktree (nil is the main app)
        id->transform (transforms-rest.db/transforms-by-id (:worktree_id transform))
