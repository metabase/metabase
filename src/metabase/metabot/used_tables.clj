(ns metabase.metabot.used-tables
  ...
  Both query paths feed a single recursive collector that, for each MBQL 5
  query (converted from the raw tool-output payload via [[metabase.lib.core/query]]),
  attaches an application-DB metadata provider and:
  - Collects table and card ids via [[metabase.lib.core/all-referenced-entity-ids]],
    ...
    shared across the whole extraction via [[metabase.lib-be.core/with-metadata-provider-cache]],
...
(defn- query-tables-and-cards
  "Walk one MBQL 5 query and return
...
(defn- collect-tables
  "BFS over a queue of MBQL 5 queries, accumulating underlying table ids.
  Implicit-join targets are recovered by [[query-tables-and-cards]] via the
  attached metadata provider's `bulk-metadata`. Tracks visited card ids to
  break cycles. Never throws."
  [initial-queries]
  (loop [tables  #{}
...
(defn- notebook-starting-query
  "Starting MBQL 5 query for a `construct_notebook_query` tool output."
  [structured]
...
(defn- sql-starting-query
  "Starting MBQL 5 query for a SQL tool output. Prefers the structured
  output's full `:query` map; falls back to building a native query from the
  structured output's `:database` field plus the tool's raw SQL text so the
  recursive collector can still run macaw + walk template tags."
  [tool-name arguments structured]
...
(defn- transform-sql-starting-query
  "Starting MBQL 5 query for a `write_transform_sql` tool output. The
