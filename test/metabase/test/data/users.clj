...
(defmacro with-test-user
  "Execute `body` with the various API dynamic variables like `*current-user*` and `*current-user-permissions-set*`
  bound to their values for the test User defined by `user-kwd`.

    (with-test-user :lucky
      ;; Everything is bound! Now it's just like we were calling this from the API
      ...)"
  {:style/indent 1}
  [user-kwd & body]
