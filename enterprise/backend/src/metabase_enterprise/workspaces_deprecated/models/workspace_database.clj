(ns metabase-enterprise.workspaces-deprecated.models.workspace-database
  (:require
   [metabase-enterprise.workspaces-deprecated.settings :as ws.settings]
...
(methodical/defmethod t2/table-name :model/WorkspaceDatabaseDeprecated [_model] :workspace_database_deprecated)

(doto :model/WorkspaceDatabaseDeprecated
  (derive :metabase/model)
  (derive :hook/timestamped?))

(t2/deftransforms :model/WorkspaceDatabaseDeprecated
...
(defmethod mi/can-read? :model/WorkspaceDatabaseDeprecated
  ([_instance] api/*is-superuser?*)
  ([_model _pk] api/*is-superuser?*))

(defmethod mi/can-write? :model/WorkspaceDatabaseDeprecated
  ([_instance]
   api/*is-superuser?*)
  ([_model _pk]
   api/*is-superuser?*))

(defmethod mi/can-create? :model/WorkspaceDatabaseDeprecated
  [_model _instance]
  api/*is-superuser?*)

(methodical/defmethod t2/batched-hydrate [:model/WorkspaceDatabaseDeprecated :database]
...
(defenterprise reconcile-workspace-database-refs-before-delete!
  ...
  :feature :none
  [db-id]
  (when (t2/exists? :model/WorkspaceDatabaseDeprecated
                    :database_id db-id
    ...
    (throw (ex-info "Cannot delete a Database with active workspace_database rows; deprovision them first"
                    {:status-code 409
                     :database_id db-id})))
  (t2/delete! :model/WorkspaceDatabaseDeprecated :database_id db-id :status :unprovisioned))
