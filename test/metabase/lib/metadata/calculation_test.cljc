  ...
  (let [query (-> (lib/query meta/metadata-provider (meta/table-metadata :orders))
    ...
    (testing "with implicit joins"
      (is (=? (concat orders-cols
                      joined-cols
                      ;; First set of implicit joins - FKs from main source
                      (implicitly-joined :people)
                      (implicitly-joined :products)
                      ;; Second set of implicit joins - FKs from join
                      (implicitly-joined :people   (:alias join-clause))
                      (implicitly-joined :products (:alias join-clause)))
...
(deftest ^:parallel visible-columns-fk-to-self-test
  (testing "query with FK to self"
    ;; Implicit self-joins currently not supported.
    (check-visible-columns
