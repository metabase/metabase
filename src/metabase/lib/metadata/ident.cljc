...
(defn attach-idents
  ...
  3-arity version takes two functions:

  - `(db-fn 12)` returns the `:name` of the database with ID 12.
  - `(table-fn 31)` returns a maplike with `:db-id`, `:name` and maybe `:schema` for the table with ID 31.

  If you want to memoize or cache the calls to `prefix-fn`, `db-fn` or `table-fn`, you need to handle that! Some
  callers just have a map or constant value, so caching here is redundant and wasteful."
  ([prefix-fn]
