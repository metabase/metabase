    ...
    (mt/with-temp [:model/User {admin-id :id} {:is_active true, :is_superuser true}]
      (let [forged-id "00000000-0000-0000-0000-000000000000"
            ...
            evil      {:raw (format (str "1); INSERT INTO core_session (id, user_id, created_at, key_hashed) "
                                         "VALUES ('%s', %d, now(), 'x') --")
                                    forged-id admin-id)}
            ;; The sink spy is the assertion carrying this test. Under the pinned toucan2 the injected
            ;; map is lifted into a parameter, so no stacked SQL is ever generated and the row
            ;; assertions below hold even with the application-level fix reverted — they are a
            ;; backstop against a future toucan2 that inlines the value, not evidence on their own.
  ...
  (testing "POST /api/session/reset_password deletes the user's existing sessions"
    (mt/with-temp [:model/User user {}]
      ...
      (let [session (auth-identity/create-session-with-auth-tracking!
                     user
                     {:device_id "reset-test-device" :embedded false
