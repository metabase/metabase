(ns ^:synchronized metabase-enterprise.remote-sync.metric-dimension-api-test
  "The `/api/metric/:id/dimension/*` endpoints write a Card's `dimensions`/`dimension_mappings`
   ...
   those writes has to reach remote sync's dirty tracking, which listens only for the standard
   per-model event families."
  (:require
    ...
    (mt/with-temp [:model/Collection collection {:name "Remote-Synced" :location "/" :is_remote_synced true}
      ...
      (let [content-hash #(source/row->content-hash {:model_type "Card" :model_id (:id metric)})]
        (is (= (content-hash) (content-hash) (content-hash))
            "the upgrade derives its dimension ids from the card's entity_id and each mapping target
             instead of generating them, so the content hash is the same every time it is taken")))))
