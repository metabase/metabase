...
(deftest distinct-values-test
  (mt/with-dynamic-redefs [metadata-queries/field-distinct-values (constantly [[1] [2] [3] [4]])]
    (is (= {:values          [[1] [2] [3] [4]]
            :has_more_values false}
           (#'field-values/distinct-values {}))))

  (testing "(#2332) check that if field values are long we only store a subset of it"
    (mt/with-dynamic-redefs [metadata-queries/field-distinct-values (constantly [["AAAA"] [(str/join (repeat (+ 100 field-values/*total-max-length*) "A"))]])]
