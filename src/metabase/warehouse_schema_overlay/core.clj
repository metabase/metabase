...
(defenterprise-schema workspace-schemas :- [:sequential WorkspaceSchema]
  ...
  Read from the databases rather than from the remappings on hand: another worktree, or another instance pointed at
...
(mu/defn- workspace-table-filter
  "Honey SQL predicate keeping the rows a reader should see. Two jobs, and only one of them is per-worktree.

  Hiding the workspace schemas is unconditional: they are where transform runs write, not part of any database
  anyone browses, so their tables are dropped -- except the ones standing in for a canonical table in the reader's
...
(mu/defn table-query :- [:tuple :any :keyword]
  ...
  [[field-query]]. While workspaces are on, a workspace table also answers to the name of the canonical table it
  stands in for in the worktree being worked in, and every other table in a workspace schema is left out;
  `{:workspace-remapping? false}` gives the Tables as sync recorded them."
  ([]
   (table-query nil))
        ...
        (cond-> ^:allow-subquery
         {:select (mapv (fn [column]
                          (cond
                            ...
                            (and schemas (workspace-remapped-table-columns column))
