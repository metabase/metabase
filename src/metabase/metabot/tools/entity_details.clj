...
(defn metric-details
  ...
  ([card metadata-provider {:keys [field-values-fn with-default-temporal-breakout? with-queryable-dimensions?
   ...
   (let [id (:id card)
         ...
         ;; below; segments come straight off the Table, so gate them on it being queryable —
         ;; `mi/can-read?` passes on a bare `manage-table-metadata` grant with `view-data`
         ;; still `:blocked`.
...
(defn related-tables
  ...
  (let [all-main-cols    (lib/visible-columns query)
        ...
        contextual-index (into {}
                               ...
                               all-main-cols)
        ;; Permission-filter the FK columns because their target tables get named and expanded
        ;; below; a table the caller cannot query must not surface as a related table.
