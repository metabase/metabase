...
(defn normalize-name
  ...
  [driver name-str]
  (let [;; quoting the empty string yields the dialect's delimiters -- `` for backtick dialects, "" for ANSI ones
        quote-char (when-let [quote-fn (:quote (sql/get-dialect (sql.qp/quote-style driver)))]
                     (first (quote-fn "")))]
