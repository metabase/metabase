...
(def ^:private automagic-dashboards-collection
  (delay ; delay so that we don't needlessly create it
   (create-collection! (tru "Questions for autogenerated dashboards") "#509EE3" nil)))
...
(defn- add-card
  ...
  (let [card (-> {:creator_id    api/*current-user-id*
                  ...
                  :description   description
                  :name          title
                  :collection_id (:id @automagic-dashboards-collection)
                  :id            (gensym)}
