...
The overlap already shows why identity should follow the trap, not the agent or mining pass. [Codex's CI helper case](../papercuts/codex/chris.codex.ci-repository-default.md) and its later added Claude occurrence describe the same repository-default bug. [Codex's merge-preview case](../papercuts/codex/chris.codex.merge-preview-compile.md) mentions lost comments, but [Claude's comment-loss case](../papercuts/claude/chris.claude.comments-dropped-when-moving-code.md) is a separate mechanism and should remain its own record. [Codex's cluster-lock case](../papercuts/codex/chris.codex.cluster-lock-transaction.md) touches both a transaction-guard near miss and a wrong local app-DB diagnosis; Claude's [ambient-DDL case](../papercuts/claude/chris.claude.search-temp-index-ddl-commits-enclosing-with-temp.md) and [local app-DB case](../papercuts/claude/chris.claude.local-test-appdb-differs-per-worktree-lein-env.md) give those different mechanisms better homes. Neither collection's filename prefix reliably identifies the transcript provider: some `chris.codex.*` cases came from Claude sessions.
...
1. **Do not turn a symptom into a root cause by wording alone.** Store which observation established each step. A user's diagnosis, an agent's theory, a tool result, and a source check are different evidence types. For instance, the apparent H2 result in the [cluster-lock case](../papercuts/codex/chris.codex.cluster-lock-transaction.md) was actually a Postgres run until the environment was checked.
...
- Clear positives: [MCP scope tests that never reached authorization](../papercuts/codex/chris.codex.mcp-session-tests.md), [nested fields silently deduplicated](../papercuts/codex/chris.codex.nested-field-paths.md), and [git-spice dropping 17 commits while exiting 0](../papercuts/claude/chris.claude.git-spice-stale-base-hash-silently-drops-commits.md).
- Hard negatives: style corrections, normal review iterations, an ordinary bug caught by the agent's own test without a misleading code/tool contract, and the two [Codex negative controls](../papercuts/codex/chris.codex.negative-controls.md).
