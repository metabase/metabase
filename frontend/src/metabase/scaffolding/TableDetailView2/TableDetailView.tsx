...
import { Box, Card, Flex, Group, Stack, Text } from "metabase/ui/components";
...
export function TableDetailViewInner({
  ...
  const handleEditClick = useCallback(() => {
    dispatch(push(`/table2/${tableId}/detail/${rowId}/edit`));
  }, [tableId, rowId, dispatch]);

  const handleCloseClick = useCallback(() => {
    dispatch(push(`/table2/${tableId}/detail/${rowId}`));
  }, [tableId, rowId, dispatch]);
  ...
  const handleViewPreviousObjectDetail = useCallback(() => {
    setCurrentRowIndex((i) => {
      ...
      if (rowId !== undefined) {
        dispatch(
          push(`/table2/${tableId}/detail/${rowId}${isEdit ? "/edit" : ""}`),
        );
      }
      return newIndex;
    });
              ...
              <Card
                ...
                to={
                  pkIndex !== undefined && pkIndex >= 0
                    ? isEdit
                      ? `/table2/${tableId}/detail/${row[pkIndex]}/edit`
                      : `/table2/${tableId}/detail/${row[pkIndex]}`
                    : ""
                }
              >
                <Stack gap="xs">
                  <Group gap="md" justify="space-between">
                    <Group gap="md">
                      ...
                      <Text size="lg">{row[4]}</Text>
                    </Group>
                  </Group>

                  <Group justify="space-between" pl={48}>
                    <Text c="text-secondary">{row[2]}</Text>
                  </Group>
                </Stack>
              </Card>
            );
