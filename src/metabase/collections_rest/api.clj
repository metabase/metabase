  ...
  [_route-params
   {:keys [models archived namespace pinned_state sort_column sort_direction official_collections_first
           ...
           show_dashboard_questions]} :- [:map
                                          [:models                      {:optional true} [:maybe collection-items/Models]]
                                          [:collection_type             {:optional true} collection-items/CollectionType]
                                          [:include_can_run_adhoc_query {:default false} [:maybe ms/BooleanValue]]
                                          [:archived                    {:default false} [:maybe ms/BooleanValue]]
                                          [:namespace                   {:optional true} [:maybe ms/NonBlankString]]
                                          [:include_library             {:default false} [:maybe ms/BooleanValue]]
                                          [:pinned_state                {:optional true} [:maybe (into [:enum] collection-items/valid-pinned-state-values)]]
                                          [:sort_column                 {:optional true} [:maybe (into [:enum] collection-items/valid-sort-columns)]]
                                          [:sort_direction              {:optional true} [:maybe (into [:enum] collection-items/valid-sort-directions)]]
  ...
  (let [root-collection (assoc collection/root-collection :namespace namespace)
    ...
    (collection-items/collection-children
     ...
     {:archived?                   (boolean archived)
      ...
      :sort-info                   {:sort-column                 (or (some-> sort_column collection-items/normalize-sort-choice) :name)
                                    :sort-direction              (or (some-> sort_direction collection-items/normalize-sort-choice) :asc)
...
(api.macros/defendpoint :put "/:id"
  ...
  [{:keys [id]} :- [:map
   ...
   {authority-level :authority_level, :as collection-updates} :- [:map
                                                                  ...
                                                                  [:archived         {:default false} [:maybe ms/BooleanValue]]
                                                                  [:parent_id        {:optional true} [:maybe ms/PositiveInt]]
                                                                  [:type             {:optional true} [:maybe collection-items/CollectionType]]
                                                                  [:authority_level  {:optional true} [:maybe collection/AuthorityLevel]]]]
...
(api.macros/defendpoint :get "/:id/items"
  ...
  *  `include_can_run_adhoc_query` - when this is true hydrates the `can_run_adhoc_query` flag on card models
  ...
  (let [resolved-id (eid-translation/->id-or-404 :collection id)
    ...
    (u/prog1 (collection-items/collection-children collection
