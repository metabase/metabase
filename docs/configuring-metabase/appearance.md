...
People can display their Metabase in dark mode in their [account settings](../people-and-groups/account-settings.md#theme). Options are to use the system default (which is also the default setting), always use dark mode, or always use light mode. When using the system default, Metabase will switch between light and dark mode when the system switches without having to reload the page.
