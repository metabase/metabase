...
(defn- check-display-names [tests]
  (let [metadata-provider (lib/composed-metadata-provider
    ...
    (doseq [tst tests
            :let [exp (get tst "name")
                  expression (get tst "clause")
                  options (update-keys (get tst "options") keyword)]]
      (testing exp
                                        ...
                                        (fn [node]
                                          (let [node-options (when (= node expression)
                                            ...
                                            (if (and (vector? node) (not (map-entry? node)))
                                              (lib/expression-clause (first node) (rest node) node-options)
                                              node)))
                                        expression))))))))
