...
If the [connection is failing](./db-connection.md) or the [database privileges are wrong](./data-permissions.md#granting-database-privileges), the sync query won't be able to run. If Metabase can't sync with your database after you first set it up, then the initial scan and fingerprinting queries won't run either.
