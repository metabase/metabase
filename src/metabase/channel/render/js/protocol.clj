...
(defprotocol StaticVizRenderer
  ...
  (chart [renderer input]
    ...
    plus that kind's fields. An optional `:customVizBundles [{:identifier .. :source ..} ..]` carries
    untrusted custom-viz plugin JS to evaluate and register before rendering (the renderer must sandbox it).
    Returns a keywordized `{:type .. :content ..}` map.")
