...
(defn update-settings-last-updated!
  ...
  (let [current-timestamp-as-string-honeysql (h2x/cast (if (= (mdb.connection/db-type) :mysql) :char :text)
    ...
    ;; attempt to UPDATE the existing row. If no row exists, t2/update!` will return false...
    (or (t2/update! 'Setting {:key settings-last-updated-key} {:value current-timestamp-as-string-honeysql})
