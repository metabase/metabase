...
import { useListMeasuresQuery } from "metabase/api";
...
const EMPTY_MEASURES: Measure[] = [];
const getNodeId = (measure: Measure) => String(measure.id);
...
export function MeasureListPage() {
  ...
  const { data: measures, error, isLoading } = useListMeasuresQuery();

  const columnDefs = useMemo<TreeTableColumnDef<Measure>[]>(
    () => [
      {
        id: "name",
        accessorKey: "name",
      ...
      {
        id: "table",
        accessorFn: (measure) => measure.table?.display_name ?? "",
        header: t`Table`,
        minWidth: 160,
        enableSorting: true,
        cell: ({ row }) =>
  ...
  const getRowHref = useCallback(
    (row: Row<Measure>) =>
      Urls.dataStudioPublishedTableMeasure(
  ...
  const hasNoData = !measures?.length;
  ...
  return (
    <PageContainer data-testid="measures-list" gap={0}>
      <PaneHeader
        breadcrumbs={
          <DataStudioBreadcrumbs>{t`Measures`}</DataStudioBreadcrumbs>
        }
        py={0}
      />
      ...
      {isTablePickerOpen && (
        <EntityPickerModal
          title={t`Select a table for the new measure`}
          models={["table"]}
          options={{
            hasLibrary: false,
            hasRecents: false,
          ...
          onChange={(item: OmniPickerItem) => {
            if (item.model === "table") {
              ...
              dispatch(push(Urls.dataStudioPublishedTableMeasureNew(item.id)));
            }
          }}
