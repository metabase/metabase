...
(deftest ^{:parallel false} build-hnsw-index-async-deduplicates-local-builds-test
...
(deftest ^{:parallel false} repair-snapshot-precedes-canonical-document-read-test
  (let [events       (atom [])
