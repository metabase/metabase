(ns metabase.driver.googleanalytics
  ...
  (:import (java.util Collections Date List Map)
           ...
           (com.google.api.services.analytics.model Account Accounts Columns Column GaData Profile Profiles Webproperty Webproperties)))
  ...
  (try
    (let [table            (field/table field)
          ...
          query            {:database database
                            :native {:query {:ids                 (str "ga:" (:name table))
                                             :start-date          "2005-01-01"
                                             :end-date            "today"
                                             :metrics             "ga:users"
                                             :dimensions          (:name field)
                                             :include-empty-rows  true
                                             :max-results         (int (inc low-cardinality-threshold))}}}
          ^GaData response (google/execute-no-auto-retry (mbql-query->request query))
          values           (for [row (.getRows response)]
                             (first row))]
      (log/info (str "FIELD VALUES FOR " (:name field) " = " (vec values)))
      values)
  (catch Throwable e
    nil)))
...
(defn test:cardinality-and-extract-field-values
  ...
  (let [non-nil-values  (filter identity (field-distinct-values field (inc low-cardinality-threshold)))
...
(u/strict-extend GoogleAnalyticsDriver
  ...
  (merge driver/IDriverDefaultsMixin
         {:analyze-table              (fn [driver table new-field-ids]
                                        {:fields (for [{:keys [id] :as field} (table/fields table)]
                                                   (test:cardinality-and-extract-field-values field {:id id}))})
