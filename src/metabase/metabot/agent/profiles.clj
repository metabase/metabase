...
;; HACKATHON PROTOTYPE: :internal carries the megabot tool set on top of its own, so the in-app Metabot
;; can run a query and read the rows, call the REST API, and keep notes — while keeping everything that
;; makes :internal itself: its capability-driven prompt, its scoped tools, and the viewing context the
;; agent loop injects for every profile. The megabot tools carry no `:scope`, so they pass the scope
;; filter untouched; `run_warehouse_sql` still carries `:permission-write-sql-queries`, so a user who
;; may not write SQL never sees it. This is for evaluation, not for shipping: `call_api` gives the agent
;; the product's whole write surface, bounded only by the caller's own permissions and the
;; `megabot-api-read-only?` setting.
(register-profile!
 {:name                  :internal
  :prompt-template       "internal.selmer"
  ;; 15 is enough to build one query and hand it to the frontend. Running queries, reading rows and
  ;; correcting them takes more steps, so the budget follows the tools; 1000 (megabot's) is a research
  ;; budget, not one to give the in-app assistant.
  :max-iterations        60
  ;; The structured-query dialect and the discovery guidance are needed on nearly every turn that uses
  ;; run_warehouse_query, so they are inlined rather than left to a load_skill the model might skip.
  ;; Both are static, so they stay in the cached prompt prefix. The operator catalog and the API skill
  ;; stay on demand.
  :always-on-skills      [:megabot-discovery :megabot-query]
  ;; Loop hygiene for the longer budget: cap each turn's output, and compact old tool outputs when the
  ;; replayed history is rebuilt.
  ...
  :tools                 [;; :internal's own tools
                          ...
                          #'tools/save-entity-tool
                          ;; the megabot tool set
                          #'tools/run-warehouse-sql-tool
