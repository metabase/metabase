...
Metabase always generates TypeScript declaration files for ClojureScript modules that form a JavaScript runtime boundary. The existing Shadow `app` build hook uses ordinary Malli schemas as its primary input and writes `.d.ts` files beside runtime JavaScript during the `:flush` stage: development output goes to `target/cljs_dev` and release output goes to `target/cljs_release`.

The main TypeScript codebase actively imports these declarations through the existing `cljs/*` path mapping. The declarations describe runtime modules that TypeScript can actually import; they are not a catalog of every schema or public var in the ClojureScript analyzer. There is no separate declaration build.
...
Run the generator tests with the repository test runner and its `:cljs` option:
