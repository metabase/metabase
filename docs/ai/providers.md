...
To power [Metabot](./metabot.md), you can connect Metabase to one of the providers below with your own API key, or let Metabase manage the AI for you. For how to set up a connection, check out [AI settings](./settings.md).
...
- **API key** (required). [Where do I find this?](https://console.anthropic.com/settings/keys) Set it with `MB_LLM_ANTHROPIC_API_KEY`.
- **API base URL** (advanced). Defaults to `https://api.anthropic.com`. Set it with `MB_LLM_ANTHROPIC_API_BASE_URL`.
...
- **API key** (required). [Where do I find this?](https://platform.openai.com/api-keys) Set it with `MB_LLM_OPENAI_API_KEY`.
- **API base URL** (advanced). Defaults to `https://api.openai.com`. Set it with `MB_LLM_OPENAI_API_BASE_URL`.
...
- **API key** (required). [Where do I find this?](https://openrouter.ai/keys) Set it with `MB_LLM_OPENROUTER_API_KEY`.
- **API base URL** (advanced). Defaults to `https://openrouter.ai/api`. Set it with `MB_LLM_OPENROUTER_API_BASE_URL`.
...
- **API key** (required). [Where do I find this?](https://console.mistral.ai/api-keys) Set it with `MB_LLM_MISTRAL_API_KEY`.
- **API base URL** (advanced). Defaults to `https://api.mistral.ai/v1`. Set it with `MB_LLM_MISTRAL_API_BASE_URL`.
...
- **API key** (required). [Where do I find this?](https://z.ai/manage-apikey/apikey-list) Set it with `MB_LLM_ZAI_API_KEY`.
- **API base URL** (advanced). Defaults to `https://api.z.ai/api/paas/v4`. Set it with `MB_LLM_ZAI_API_BASE_URL`.
...
- **API key** (required). [Where do I find this?](https://platform.kimi.ai/console/api-keys) Set it with `MB_LLM_MOONSHOT_API_KEY`.
- **API base URL** (advanced). Point this at the .cn platform to use it instead; keys are not interchangeable between the two. Defaults to `https://api.moonshot.ai/v1`. Set it with `MB_LLM_MOONSHOT_API_BASE_URL`.
...
- **API key** (required). [Where do I find this?](https://platform.deepseek.com/api_keys) Set it with `MB_LLM_DEEPSEEK_API_KEY`.
- **API base URL** (advanced). The root both surfaces hang off; leave off any /anthropic or /v1 path. Defaults to `https://api.deepseek.com`. Set it with `MB_LLM_DEEPSEEK_API_BASE_URL`.
...
- **Project ID**. The Google Cloud project to use. Optional if the service account key provides it. [Where do I find this?](https://docs.cloud.google.com/resource-manager/docs/creating-managing-projects) Set it with `MB_LLM_GOOGLE_PROJECT_ID`.
- **Location**. Optional. Defaults to global. Set it with `MB_LLM_GOOGLE_LOCATION`.
...
- **Service account key file**. Only when **Authentication method** is **Service account key**. Upload a service account key file to authenticate with. [Where do I find this?](https://docs.cloud.google.com/iam/docs/keys-create-delete) Set it with `MB_LLM_GOOGLE_SERVICE_ACCOUNT_KEY`.
- **OAuth access token**. Only when **Authentication method** is **OAuth token**. A short-lived token, e.g. the output of gcloud auth print-access-token. Useful for testing. Set it with `MB_LLM_GOOGLE_OAUTH_ACCESS_TOKEN`.
- **API base URL** (advanced). Derived from the location when left at the global host. Defaults to `https://aiplatform.googleapis.com`. Set it with `MB_LLM_GOOGLE_API_BASE_URL`.
...
- **API key** (required). [Where do I find this?](https://ai.azure.com) Set it with `MB_LLM_AZURE_API_KEY`.
- **API base URL** (required). Set it with `MB_LLM_AZURE_API_BASE_URL`.
- **Model provider** (required). Whether your deployment serves an Anthropic or an OpenAI model. One of: `OpenAI`, `Anthropic`. Defaults to `OpenAI`. Set it with `MB_LLM_AZURE_MODEL_FAMILY`.
- **Deployment name** (required). The name of the model deployment on your Azure resource. We recommend naming deployments after the model they serve. Set it with `MB_LLM_AZURE_DEPLOYMENT_NAME`.
...
- **Access key ID** (required). [Where do I find this?](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html) Set it with `MB_LLM_BEDROCK_ACCESS_KEY_ID`.
- **Secret access key** (required). Set it with `MB_LLM_BEDROCK_SECRET_ACCESS_KEY`.
- **Region**. Pick one from the dropdown in **Admin > AI**. Defaults to `us-east-1`. Set it with `MB_LLM_BEDROCK_REGION`.
- **Session token** (advanced). Only needed for temporary credentials. Set it with `MB_LLM_BEDROCK_SESSION_TOKEN`.
...
- **API base URL** (required). Your server's OpenAI-compatible API. It should end in /v1. Set it with `MB_LLM_VLLM_API_BASE_URL`.
- **API key**. Only needed if you started your server with --api-key. Set it with `MB_LLM_VLLM_API_KEY`.

## Metabase AI service

- Provider key: `metabase`
- Default model: `anthropic/claude-sonnet-4-6`
- Managed by Metabase, so there's nothing to configure. You can only connect one. See [Metabase AI Service](./settings.md#metabase-ai-service).
