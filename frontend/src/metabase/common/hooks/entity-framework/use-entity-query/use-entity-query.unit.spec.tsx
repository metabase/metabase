...
describe("useEntityQuery", () => {
  ...
  it("should initially load data only once the reload flag", async () => {
    ...
    expect(fetchMock.callHistory.calls(`path:/api/database/${TEST_DB.id}`)).toHaveLength(1);
    expect(fetchMock.callHistory.calls(`path:/api/table/${TEST_TABLE.id}`)).toHaveLength(1);
  });
  ...
  it("should reload data only for calls with the reload flag when re-mounted", async () => {
    ...
    expect(screen.getByText(TEST_TABLE.name)).toBeInTheDocument();
    expect(fetchMock.callHistory.calls(`path:/api/database/${TEST_DB.id}`)).toHaveLength(1);
    expect(fetchMock.callHistory.calls(`path:/api/table/${TEST_TABLE.id}`)).toHaveLength(2);
  });
});
