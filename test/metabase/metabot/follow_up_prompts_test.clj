...
(deftest clean-prompts-test
  ...
  (testing "the pool is capped, since Jev picks from it afterwards"
    (is (= 30 (count (#'follow-up-prompts/clean-prompts (map str (range 50))))))))
...
(deftest pending-finalization-test
  (let [message-id (str (random-uuid))
        message {:external_id message-id :role :assistant :finished nil}]
    (doseq [[final-row expected] [[(assoc message :finished true) ["Show the trend"]]
                                  [(assoc message :finished false) []]
                                  [(assoc message :finished true :error "failed") []]
                                  [{:external_id "next-message" :role :user} []]]]
      (let [reads (atom 0)
            calls (atom 0)]
        (mt/with-dynamic-fn-redefs [metabot.db/recent-messages
                                    (fn [_ _] [(if (= 1 (swap! reads inc)) message final-row)])
                                    self/call-llm-structured
                                    (fn [& _]
                                      (swap! calls inc)
                                      {:prompts ["Show the trend"]})]
          (is (= expected (follow-up-prompts/generate "conversation" message-id)))
          (is (= 2 @reads))
          (is (= (if (seq expected) 1 0) @calls)))))))
