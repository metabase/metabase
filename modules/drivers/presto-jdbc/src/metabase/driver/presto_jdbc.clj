...
(defmethod sql.qp/honey-sql-version :presto-jdbc
  [_driver]
  2)
...
(defmethod sql.qp/date [:presto-jdbc :week]
  [_ _ expr]
  (sql.qp/adjust-start-of-week :presto-jdbc (partial into [:date_trunc (h2x/literal :week)]) (in-report-zone expr)))
...
(defmethod sql.qp/unix-timestamp->honeysql [:presto-jdbc :milliseconds]
  ...
  (let [report-zone (qp.timezone/report-timezone-id-if-supported :presto-jdbc)
        millis      [(u/qualified-name ::mod) expr 1000]]
    [:date_add
