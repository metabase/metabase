...
-- Schema v5's dispatches (which reference assessments) and assessments go first. SQLite has no DELETE IF EXISTS, so
-- these deletes are written out, then read back, only for the tables this database has.
.once reset-metabot.tmp.sql
SELECT printf('DELETE FROM %s WHERE papercut_id IN (SELECT id FROM gone);', name) FROM sqlite_master
 WHERE type = 'table' AND name IN ('dispatches', 'assessments') ORDER BY name = 'assessments';
