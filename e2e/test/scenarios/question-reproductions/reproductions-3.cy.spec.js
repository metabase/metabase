...
describe("issue 32625, issue 31635", () => {
  ...
  it("should remove dependent clauses when a clause is removed (metabase#32625, metabase#31635)", () => {
    cy.visitQuestionAdhoc(QUESTION, { mode: "notebook" });
    ...
    cy.getNotebookStep("expression").should("not.exist");
    cy.getNotebookStep("summarize").findByText(CC_NAME).should("not.exist");
...
describe("issue 33079", () => {
  ...
  it("underlying records drill should work in a non-English locale (metabase#33079)", () => {
    ...
    cy.cartesianChartCircle().eq(1).click({ force: true });
    cy.popover()
...
describe("issue 38176", () => {
  ...
  it("restoring a question to a previous version should preserve the variables (metabase#38176)", () => {
    ...
    cy.tableInteractive().should("contain", "NL");
  });
});
...
describe("issue 30056", () => {
  ...
  it("should show table breadcrumbs for questions with post-aggregation filters (metabase#30056)", () => {
    ...
    cy.queryBuilderHeader().findByText("People").should("exist");
  });
});
...
describe("issue 39102", () => {
  ...
  it("should be able to preview a multi-stage query (metabase#39102)", () => {
    ...
    cy.getNotebookStep("data", { stage: 0 }).icon("play").click();
    cy.wait("@dataset");
    ...
    cy.getNotebookStep("summarize", { stage: 0 }).icon("play").click();
    cy.wait("@dataset");
    cy.findByTestId("preview-root").within(() => {
    ...
    cy.getNotebookStep("filter", { stage: 1 }).icon("play").click();
    cy.wait("@dataset");
    cy.findByTestId("preview-root").within(() => {
      cy.findByText("Count").should("be.visible");
      cy.findByText("3,610").should("be.visible");
    ...
    cy.getNotebookStep("summarize", { stage: 1 }).icon("play").click();
    cy.wait("@dataset");
    cy.findByTestId("preview-root").within(() => {
      cy.findByText("Count").should("be.visible");
...
describe("issue 13814", () => {
  ...
  it("should support specifying a field in 'count' MBQL clause even if the UI doesn't support it (metabase#13814)", () => {
    ...
    cy.getNotebookStep("summarize").findByText("Count").should("be.visible");
  });
});
...
describe("issue 39795", () => {
  ...
  it("should allow me to re-order even when a field is set with a different display value (metabase#39795)", () => {
    ...
    cy.openVizSettingsSidebar();
    cy.moveColumnDown(cy.getDraggableElements().first(), 2);
    ...
    cy.getDraggableElements().eq(2).should("contain.text", "ID");
  });
});
...
describe("issue 40176", () => {
  ...
  beforeEach(() => {
    ...
    cy.signInAsAdmin();
    cy.resetTestTable({ type: DIALECT, table: TABLE });
    cy.resyncDatabase({
      dbId: WRITABLE_DB_ID,
      tableName: TABLE,
    });
  ...
  it(
    ...
    { tags: "@external" },
    () => {
      cy.getTable({ name: TABLE }).then(({ id: tableId }) => {
        cy.visitQuestionAdhoc({
          display: "table",
          dataset_query: {
...
describe("issue 40435", () => {
  ...
  it("should make new query columns visible by default (metabase#40435)", () => {
    ...
    cy.modal().last().findByText("Save").click();
    cy.wait("@updateCard");
    cy.visualize();
...
describe("issue 41381", () => {
  ...
  it("should show an error message when adding a constant-only custom expression (metabase#41381)", () => {
    ...
    cy.addCustomColumn();
    cy.enterCustomColumnDetails({ formula: "'Test'", name: "Constant" });
    cy.popover().within(() => {
...
describe("issue 42244", () => {
  ...
  it("should allow to change the temporal bucket when the column name is long (metabase#42244)", () => {
    ...
    cy.popover().last().findByText("Year").click();
    cy.getNotebookStep("summarize")
...
describe("issue 40064", () => {
  ...
  it("should be able to edit a custom column with the same name as one of the columns used in the expression (metabase#40064)", () => {
    ...
    cy.tableInteractive().findByText("4.14").should("be.visible");

    cy.log("update the expression and check the value");
    cy.openNotebook();
    cy.getNotebookStep("expression").findByText("Tax").click();
    cy.enterCustomColumnDetails({ formula: "[Tax] * 3" });
    cy.popover().button("Update").click();
    cy.visualize();
    cy.tableInteractive().findByText("6.21").should("be.visible");

    cy.log("rename the expression and make sure you cannot create a cycle");
    cy.openNotebook();
    cy.getNotebookStep("expression").findByText("Tax").click();
    cy.enterCustomColumnDetails({ formula: "[Tax] * 3", name: "Tax3" });
    cy.popover().button("Update").click();
    cy.getNotebookStep("expression").findByText("Tax3").click();
    cy.enterCustomColumnDetails({ formula: "[Tax3] * 3", name: "Tax3" });
    cy.popover().within(() => {
...
describe("issue 32020", () => {
  ...
  beforeEach(() => {
    cy.restore();
    cy.signInAsNormalUser();
    cy.createQuestion(question1Details);
    cy.createQuestion(question2Details);
  });
...
describe("issue 44415", () => {
  ...
  it("should be able to edit a table question in the notebook editor before running its query (metabase#44415)", () => {
    ...
    cy.getNotebookStep("filter").should("not.exist");
...
describe("issue 37374", () => {
  ...
  beforeEach(() => {
    cy.restore();
    cy.signInAsNormalUser();
    cy.createQuestion(questionDetails, { wrapId: true });
    cy.signIn("nodata");
  });
...
describe("issue 31960", () => {
  ...
  it("should apply a date range filter for a query broken out by week (metabase#31960)", () => {
    ...
    cy.getDashboardCard().within(() => {
      cy.cartesianChartCircle().eq(dotIndex).realHover();
    });
    ...
    cy.getDashboardCard().within(() => {
      cy.cartesianChartCircle().eq(dotIndex).click({ force: true });
    });
    ...
    cy.findByTestId("qb-filters-panel")
      ...
      .should("be.visible");
    cy.assertQueryBuilderRowCount(rowCount);
  });
});
...
describe("issue 43294", () => {
  ...
  it("should not overwrite viz settings with click actions in raw data mode (metabase#43294)", () => {
    cy.createQuestion(questionDetails, { visitQuestion: true });
    cy.queryBuilderFooter().findByLabelText("Switch to data").click();
    ...
    cy.queryBuilderFooter().findByLabelText("Switch to visualization").click();
    cy.echartsContainer().within(() => {
      cy.findByText("Count").should("be.visible");
...
describe("issue 40399", () => {
  ...
  it("should not show results from other stages in a stages preview (metabase#40399)", () => {
    ...
    cy.getNotebookStep("filter", { stage: 0 }).within(() => {
      cy.icon("play").click();
      cy.findByTestId("preview-root")
    ...
    cy.getNotebookStep("join", { stage: 0 }).within(() => {
      cy.icon("play").click();
      cy.findByTestId("preview-root")
        .findAllByText("Gizmo")
        .should("be.visible");
    ...
    cy.getNotebookStep("data", { stage: 0 }).within(() => {
      cy.icon("play").click();
      cy.findByTestId("preview-root")
        .findAllByText("Gizmo")
        .should("be.visible");
...
describe("issue 43057", () => {
  ...
  it("should differentiate between date and datetime filters with 00:00 time (metabase#43057)", () => {
    ...
    cy.wait("@dataset");
    cy.assertQueryBuilderRowCount(16);
    cy.findByTestId("qb-filters-panel")
...
describe("issue 19894", () => {
  ...
  it("should show all columns when using the join column selecter (metabase#19894)", () => {
    ...
    cy.entityPickerModalTab("Collections").click();
    cy.entityPickerModalItem(1, "Q1").click();

    cy.button("Join data").click();

    cy.entityPickerModalTab("Collections").click();
    cy.entityPickerModalItem(1, "Q2").click();
    ...
    cy.entityPickerModalTab("Collections").click();
    cy.entityPickerModalItem(1, "Q3").click();

    cy.popover().findByText("Category").should("be.visible");
    cy.popover().findByText("Count").should("be.visible");

    cy.popover().findByText("Q1").click();
    cy.popover().findByText("Q2").click();

    cy.popover().findByText("Category").should("be.visible");
    cy.popover().findByText("Sum of Price").should("be.visible");
...
describe("issue 44637", () => {
  ...
  it("should not crash when rendering a line/bar chart with empty results (metabase#44637)", () => {
    ...
    cy.queryBuilderMain().findByText("No results!").should("exist");
    cy.openVizTypeSidebar();
    cy.leftSidebar().icon("bar").click();
    cy.queryBuilderMain().within(() => {
      cy.findByText("No results!").should("exist");
      cy.findByText("Something's gone wrong").should("not.exist");
    });

    cy.queryBuilderFooter().icon("calendar").click();
    cy.rightSidebar().findByText("Add an event");
  });
});
...
describe("issue 44668", () => {
  ...
  it("should not drop graph.metrics after adding a new query stage (metabase#44668)", () => {
    ...
    cy.queryBuilderMain().findByLabelText("Legend").should("not.exist");

    cy.openVizSettingsSidebar();

    // Ensure can use Custom Number as series
    cy.leftSidebar().findByText("Add another series").click();
    cy.queryBuilderMain()
...
describe("issue 44974", () => {
  ...
  it("entity picker should not offer to join with a table or a question from a different database (metabase#44974)", () => {
    cy.withDatabase(PG_DB_ID, ({ PEOPLE_ID }) => {
      const questionDetails = {
      ...
      cy.entityPickerModal().within(() => {
        cy.entityPickerModalTab("Recents").should("not.exist");
        cy.entityPickerModalTab("Collections").click();
...
describe("issue 39771", () => {
  ...
  it("should show tooltip for ellipsified text (metabase#39771)", () => {
    ...
    cy.popover().findByText("by quarter of year").realHover();
...
describe("issue 45063", () => {
  ...
  function createGuiModel({ sourceTableId }) {
    ...
    cy.createQuestion(mbqlModelDetails, { wrapId: true, idAlias: "modelId" });
  }
  ...
  function createNativeModel({
    ...
    cy.createNativeQuestion(nativeModelDetails, {
      wrapId: true,
      idAlias: "modelId",
    }).then(({ body: model }) => {
  ...
  function verifyListFilter({ fieldDisplayName, fieldValue, fieldValueLabel }) {
    cy.tableHeaderClick(fieldDisplayName);
    ...
    cy.findByTestId("qb-filters-panel")
      ...
      .click();
    cy.popover().findByLabelText(fieldValueLabel).should("be.checked");
  }
  ...
  function verifySearchFilter({
    ...
    cy.selectDropdown().findByText(fieldValueLabel).click();
  ...
  describe("type/PK -> type/Name remapping (metabase#45063)", () => {
    it("should work with questions", () => {
      ...
      verifyRemappedFilter({
        visitCard: () => cy.visitQuestion("@questionId"),
        fieldId: PEOPLE.ID,
        fieldDisplayName: "ID",
        fieldValue: 1,
    ...
    it("should work with models", () => {
      ...
      verifyRemappedFilter({
        visitCard: () => cy.get("@modelId").then(cy.visitModel),
        fieldId: PEOPLE.ID,
        fieldDisplayName: "ID",
        fieldValue: 1,
        fieldValueLabel: "Hudson Borer",
...
describe("issue 36027", () => {
  beforeEach(() => {
    ...
    cy.createQuestion({ query: BASE_QUERY }, { wrapId: true }).then(
      baseQuestionId => {
        cy.createQuestion(
          {
            display: "waterfall",
...
describe("issue 48829", () => {
  ...
  it("should not show the unsaved changes warning when switching back to chill mode from the notebook editor after adding a filter from headers (metabase#48829)", () => {
    ...
    cy.queryBuilderHeader().button("Show Editor").click();
    cy.getNotebookStep("filter")
      .findAllByTestId("notebook-cell-item")
      .icon("close")
      .should("be.visible")
  ...
  it("should not show the unsaved changes warning when switching back to chill mode from the notebook editor after visiting a filtered question from a dashboard click action (metabase#48829)", () => {
    ...
    cy.entityPickerModal().findByText(questionDetails.name).click();
    cy.sidebar().findByTestId("click-mappings").findByText("Title").click();
    cy.popover().findByText("Title").click();
    cy.saveDashboard();

    // Navigate to question using click action in dashboard
    cy.main().findByText("Rustic Paper Wallet").click();
...
describe("issue 50038", () => {
  ...
  beforeEach(() => {
    ...
    cy.createQuestion(QUESTION, { wrapId: true, idAlias: "questionId" });
    cy.createQuestion(OTHER_QUESTION, {
      wrapId: true,
...
describe("issue 47940", () => {
  ...
  it("should be able to convert a question with date casting to a model", () => {
    ...
    cy.queryBuilderHeader().findByTestId("run-button").click();
    cy.wait("@cardQuery");
    cy.queryBuilderHeader().button("Save").click();
    cy.modal().button("Save").click();
    cy.wait("@updateCard");
