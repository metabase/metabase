...
cy.describeEE(
  "scenarios > dashboard > dashboard cards > click behavior",
  () => {
    beforeEach(() => {
      cy.restore();
      cy.signInAsAdmin();
    ...
    describe("dashcards without click behavior", () => {
      it("does not allow to set click behavior for virtual dashcards", () => {
        const textCard = cy.getTextCardDetails({ size_y: 1 });
        const headingCard = cy.getHeadingCardDetails({ text: "Heading card" });
        const actionCard = cy.getActionCardDetails();
        const linkCard = cy.getLinkCardDetails();
        const cards = [textCard, headingCard, actionCard, linkCard];

        cy.createDashboard().then(({ body: dashboard }) => {
          cy.updateDashboardCards({ dashboard_id: dashboard.id, cards });
      ...
      it("does not allow to set click behavior for object detail dashcard", () => {
        ...
        cy.getDashboardCard().realHover().icon("click").should("not.exist");
      });
    });
    ...
    describe("line chart", () => {
      ...
      it("sets non-specified parameters to default values when accessed from a click action", () => {
        ...
        cy.filterWidget().contains("Hello").click();
        cy.popover().within(() => {
          cy.fieldValuesInput().type("{backspace}World{enter}");
      ...
      it("sets parameters with default values to the correct value when accessed via click action", () => {
        ...
        cy.findAllByTestId("field-set")
          ...
          .click();
        cy.popover().within(() => {
          cy.fieldValuesInput().type("John Doe{enter}");
      ...
      it("does not allow setting dashboard as custom destination if user has no permissions to it", () => {
        ...
        cy.modal().findByText(RESTRICTED_COLLECTION_NAME).should("not.exist");
      });
      ...
      it("allows setting URL with parameters as custom destination", () => {
        ...
        cy.createQuestionAndDashboard({
          ...
          cy.visitDashboard(dashcard.dashboard_id);
        });
    ...
    describe("table", () => {
      ...
      it("should open drill-through menu as a default click-behavior", () => {
        ...
        cy.popover().should("contain.text", "Filter by this value");

        getTableCell(COLUMN_INDEX.CREATED_AT).click();
        cy.popover().should("contain.text", "Filter by this date and time");
    ...
    describe("interactive embedding", () => {
      ...
      it("allows opening custom URL destination that is not a Metabase instance URL using link (metabase#33379)", () => {
        cy.updateSetting("site-url", "https://localhost:4000/subpath");
        const dashboardDetails = {
          enable_embedding: true,
        };
    ...
    describe("multi-stage questions as target destination", () => {
      ...
      it("should allow navigating to questions with filters applied in every stage", () => {
        cy.createQuestion(targetQuestion);
        cy.createQuestionAndDashboard({ questionDetails }).then(
          ({ body: card }) => {
            cy.visitDashboard(card.dashboard_id);
          },
        ...
        cy.popover().findByText("User → Longitude: 10°").click();
    ...
    it("should navigate to a different tab on the same dashboard when configured (metabase#39319)", () => {
      ...
      cy.popover().findByText(FILTER_MAPPING_COLUMN).click();
    ...
    it("should allow click behavior on left/top header rows on a pivot table (metabase#25203)", () => {
      ...
      // test top header row
      cy.getDashboardCard().findByText("Doohickey").click();
      cy.get("@targetDashboardId").then(targetDashboardId => {
        cy.location().should(({ pathname, search }) => {
          expect(pathname).to.equal(`/dashboard/${targetDashboardId}`);
      ...
      cy.getDashboardCard().findByText("Affiliate").click();
      cy.get("@targetDashboardId").then(targetDashboardId => {
        cy.location().should(({ pathname, search }) => {
          expect(pathname).to.equal(`/dashboard/${targetDashboardId}`);
    ...
    it("should allow click through on the pivot column of a regular table that has been pivoted (metabase#25203)", () => {
      ...
      // test pivoted column
      cy.getDashboardCard().findByText("Organic").click();
      cy.get("@targetDashboardId").then(targetDashboardId => {
        cy.location().should(({ pathname, search }) => {
          expect(pathname).to.equal(`/dashboard/${targetDashboardId}`);
    ...
    it("should not pass through null values to filters in custom url click behavior (metabase#25203)", () => {
      ...
      cy.getDashboardCard().within(() => {
        cy.chartPathWithFillColor("#88BF4D").eq(2).click();
      });
      cy.get("@targetDashboardId").then(targetDashboardId => {
        cy.location().should(({ pathname, search }) => {
          expect(pathname).to.equal(`/dashboard/${targetDashboardId}`);
      ...
      cy.getDashboardCard().within(() => {
        cy.chartPathWithFillColor("#88BF4D").eq(1).click();
      });
      cy.get("@targetDashboardId").then(targetDashboardId => {
        cy.location().should(({ pathname, search }) => {
          expect(pathname).to.equal(`/dashboard/${targetDashboardId}`);
    ...
    it("should navigate to correct dashboard tab via custom destination click behavior (metabase#34447 metabase#44106)", () => {
      cy.createDashboardWithTabs({
        ...
        cy.createDashboard({
          ...
          cy.getDashboardCard(1).findByText("14").click();
          ...
          cy.getDashboardCard(0).findByText("14").click();
          cy.location("pathname").should(
            "eq",
            `/dashboard/${targetDashboard.id}`,
          );
...
const addDashboardDestination = () => {
  ...
  cy.entityPickerModal()
    ...
    .click();
  cy.entityPickerModal().findByText(TARGET_DASHBOARD.name).click();
};
...
const addSavedQuestionDestination = () => {
  ...
  cy.entityPickerModal()
    ...
    .click();
  cy.entityPickerModal().findByText(TARGET_QUESTION.name).click();
};
