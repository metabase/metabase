...
(api.macros/defendpoint :post "/"
  ...
  (let [timeline (timeline.db/select-one-timeline {:id timeline_id})]
    (when-not timeline
    ...
    (let [parsed   (if (nil? timestamp)
          ...
          tl-event (merge (dissoc body :source :question_id)
                          {:creator_id api/*current-user-id*
                           :timestamp  parsed}
                          (when-not icon
                            {:icon (:icon (timeline.db/select-one-timeline {:id timeline_id :columns [:icon]}))}))]
      ...
      (u/prog1 (timeline.db/insert-timeline-event! tl-event)
        (events/publish-event! :event/timeline-create {:object (timeline.db/select-one-timeline {:id (:timeline_id <>)}) :user-id api/*current-user-id*})))))
...
(api.macros/defendpoint :put "/:id"
  ...
  (let [existing (api/write-check :model/TimelineEvent id)
    ...
    (timeline.db/update-timeline-events! {:id id}
                                         (u/select-keys-when timeline-event-updates
                                                             :present #{:description :timestamp :time_matters :timezone :icon :timeline_id :archived}
                                                             :non-nil #{:name}))
    (u/prog1 (timeline.db/select-one-timeline-event {:id id})
      (events/publish-event! :event/timeline-update {:object (timeline.db/select-one-timeline {:id (:timeline_id <>)}) :user-id api/*current-user-id*}))))
...
(api.macros/defendpoint :delete "/:id"
  ...
  (let [timeline-event (api/write-check :model/TimelineEvent id)]
    (timeline.db/delete-timeline-events! {:id id})
    (events/publish-event! :event/timeline-delete {:object (timeline.db/select-one-timeline {:id (:timeline_id timeline-event)}) :user-id api/*current-user-id*}))
