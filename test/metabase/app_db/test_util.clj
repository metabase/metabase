...
(defn all-migration-files
  ...
  (into (if include-legacy?
        ...
        (concat
         ...
         ;; Directory-based migration files: version-numbered directories (v60-v65, `060/...`) and the year-based
         ;; directories of version-less changesets (`2026/...`)
           ...
           (when migrations-dir
             (->> (io/file migrations-dir)
                  ...
                  (filter (fn [^java.io.File f]
                            (and (.isFile f)
                                 (re-matches #".*(\d{3}|\d{4})/\d{8}_.+\.yaml$" (str f)))))
                  sort
