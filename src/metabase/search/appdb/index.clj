    ...
    (let [upsert! (fn [t]
      ...
      (try
        ...
        (catch Exception e
          ...
          (if (and (transient-error/table-not-found? e) (not (exists? table-name)))
              ...
              (if (= table-name refreshed-table-name)
                ...
                (try
                  ...
                  (catch InterruptedException ie
                    ...
                    (throw ie))
                  (catch Exception e2
                    (if (transient-error/table-not-found? e2)
         ...
         (keep (fn [table-name]
                 (when table-name
                   ...
                   {search-model (try (isolate-write!
                                      ...
                                      (catch Exception e (if (transient-error/table-not-found? e) 0 (throw e))))})))
         (apply merge-with +)
         (into {}))))
