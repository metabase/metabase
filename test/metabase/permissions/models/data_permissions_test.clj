...
;; NOTE: upstream's `set-database-permission!-implied-rows-are-not-duplicated-test` is omitted here.
;; It asserts on `:perms/transforms`, which does not exist on this branch, so the duplicate-row path it
;; guards (view-data :blocked implying transforms both directly and via create-queries) cannot occur.
