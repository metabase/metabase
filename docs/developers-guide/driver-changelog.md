...
- Drivers that use `metabase.driver.sql.query-processor/->honeysql` can implement
  ...
  fields can be referenced with `<table>.<record>.<record-field>`. See `bigquery-cloud-sdk` for an example. Defaults
  to `nil` to indicate that the path should not be part of the identifier.

- `metabase.driver.sql-jdbc.execute/connection-with-timezone`, deprecated in `0.47.0`, has been removed. Implement
  `metabase.driver.sql-jdbc.execute/do-with-connection-with-options` instead.
