...
(defendpoint POST "/"
  ...
  (let [user (db/sel-1 :fields [User :id :password_salt :password :last_login], :email email (k/where {:is_active true}))]
...
(defendpoint DELETE "/"
  ...
  (delete-and-return-204! Session :id session_id))
...
(defendpoint POST "/forgot_password"
  ...
  (when-let [user-id (db/sel-1 :id User :email email)]
...
(defn- valid-reset-token->user
  ...
  (when-let [[_ user-id] (re-matches #"(^\d+)_.+$" token)]
    (let [user-id (Integer/parseInt user-id)]
      (when-let [{:keys [reset_token reset_triggered], :as user} (db/sel-1 :fields [User :id :last_login :reset_triggered :reset_token] :id user-id)]
