...
- **Jetty threads:** `Jetty threads: 3/50 (4 idle, 0 queued)`. Lists the number of active threads, and the total pool of threads available. The `(4 idle, 0 queued)` are the spare hot threads, and the number of threads queued. If you find you're maxing out the number of threads in your pool, check out [Metabase at scale][scale].
...
[log-level]: ../monitor/application-logs.md
