(ns ^:mb/driver-tests metabase.transforms.api.transform-test
...
(mt/deftest-oss update-transform-feature-flag-test
  (mt/test-drivers (mt/normal-drivers-with-feature :transforms/table)
    (mt/dataset transforms-dataset/transforms-test
      (mt/with-db-perm-for-group! (perms-group/all-users) (mt/id) :perms/transforms :yes
        (mt/with-data-analyst-role! (mt/user->id :lucky)
          (testing "Updating a query transform works without :transforms feature in OSS"
        ...
        (let [schema (get-test-schema)]
          (testing "Accepts any column if they were not extractable"
            (with-transform-cleanup! [table-name "fallback_test_unextracted"]
              (with-redefs [metabase.transforms.api.transform/extract-all-columns-from-query
