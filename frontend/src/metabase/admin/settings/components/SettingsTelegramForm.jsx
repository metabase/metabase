...
export default class SettingsTelegramForm extends Component {
    ...
    updateTelegramSettings(e) {
        ...
        if (valid) {
            this.props.updateTelegramSettings(formData).then(() => {
                this.setState({
                    submitting: "success"
                });

                MetabaseAnalytics.trackEvent("Telegram Settings", "Update", "success");
            ...
            <form noValidate>
                <div className="px2" style={{maxWidth: "585px"}}>
                    <h1>
                        ...
                        <RetinaImage
                            className="mx1"
                            src="/app/img/telegram_emoji.png"
                            width={79}
                            forceOriginalDimensions={false /* broken in React v0.13 */}
                        />
                        Telegram
                    </h1>
                    <h3 className="text-grey-1">Answers sent right to your Telegram #channels</h3>

                    <div className="pt3">
                        <a href="https://my.telegram.com/services/new/bot" target="_blank" className="Button Button--primary" style={{padding:0}}>
                            <div className="float-left py2 pl2">Create a Telegram Bot User for Metabot</div>
                            <Icon className="float-right p2 text-white cursor-pointer" style={{opacity:0.6}} name="external" size={18}/>
                        </a>
                    </div>
                    <div className="py2">
                        Once you're there, give it a name and click <strong>"Add bot integration"</strong>. Then copy and paste the Bot API Token into the field below. Once you are done, create a "metabase_files" channel in Telegram. Metabase needs this to upload graphs.
                ...
                <ul>
                    {settings}
                    <li className="m2 mb4">
                        <button className={cx("Button mr2", {"Button--primary": !disabled}, {"Button--success-new": submitting === "success"})} disabled={disabled} onClick={this.updateTelegramSettings.bind(this)}>
                            {saveButtonText}
                        </button>
