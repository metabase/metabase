...
(deftest ^:parallel instrumented-fn-form-test
  (are [form expected] (= expected
    ...
    '(let* [&uninstrumented (fn* ([x y]))]
       (fn* ([a b]
             (metabase.util.malli.fn/validate-input {} :int a)
             (metabase.util.malli.fn/validate-output {} :int (&uninstrumented a b)))))

    '(:- :int [x :- :int y] (+ x y))
    '(let* [&uninstrumented (fn* ([x y] (+ x y)))]
       (fn* ([a b]
             (metabase.util.malli.fn/validate-input {} :int a)
             (metabase.util.malli.fn/validate-output {} :int (&uninstrumented a b)))))
    ...
    '(:- :int
         ([x] (inc x))
         ([x :- :int y] (+ x y)))
    '(let* [&uninstrumented (fn* ([x]
                                  (inc x))
                                 ([x y]
                                  (+ x y)))]
       (fn*
        ([a]
         (metabase.util.malli.fn/validate-output {} :int (&uninstrumented a)))
        ([a b]
         (metabase.util.malli.fn/validate-input {} :int a)
         (metabase.util.malli.fn/validate-output {} :int (&uninstrumented a b)))))))
...
(deftest ^:parallel varargs-test
  (let [form '(metabase.util.malli.fn/fn my-fn
    ...
    (is (= '(let* [&uninstrumented (clojure.core/fn &my-fn
              ...
              (clojure.core/fn &my-fn
                ([a b & more]
                 ...
                 (clojure.core/apply &uninstrumented a b more))))
           (macroexpand form)))
    (is (= [:=>
            [:cat :any :map [:* :any]]
            :any]
