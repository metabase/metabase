...
describe("api", () => {
  ...
  it("should use _status from body when HTTP status is 200 (streaming error)", async () => {
    fetchMock.get("path:/streaming-error", {
      status: 200,
      body: {
        _status: 400,
        error: "query error",
    ...
    const streamingError = GET("/streaming-error");
    await expect(streamingError()).rejects.toEqual(
      expect.objectContaining({
        status: 400,
        data: expect.objectContaining({ error: "query error" }),
      }),
    );
  });

  it("should use _status from body when HTTP status is 4xx (new streaming behavior)", async () => {
    fetchMock.get("path:/new-streaming-error", {
      status: 400,
      body: {
        _status: 403,
    ...
    const newStreamingError = GET("/new-streaming-error");
    await expect(newStreamingError()).rejects.toEqual(
      expect.objectContaining({
        status: 403,
  ...
  it("should use HTTP status when no _status in body (4xx without override)", async () => {
    fetchMock.get("path:/plain-error", {
      status: 400,
      body: {
        error: "bad request",
      },
    });
    const plainError = GET("/plain-error");
    await expect(plainError()).rejects.toEqual(
      expect.objectContaining({
        status: 400,
        data: expect.objectContaining({ error: "bad request" }),
      }),
    );
  });
});
