...
(defn- with-library-synced
  ...
  (if-let [library (collection/library-collection)]
    ...
    (mt/with-temp [:model/Collection library {:name "Library" :type collection/library-collection-type :is_remote_synced true :location "/"}]
      (f))))

(defn- with-library-not-synced
  "Helper to ensure Library collection is not remote-synced for tests."
  [f]
  (if-let [library (collection/library-collection)]
    (let [original-synced (:is_remote_synced library)]
      (try
