...
(mr/def ::action-value
  ...
  What it must never be is a map: downstream a map is indistinguishable from Metabase's HoneySQL DSL and is spliced
  in as query structure rather than a bound value."
...
(mr/def ::action-value-map
  ...
  action that writes would store NULL instead of rejecting the request."
  [:and
   [:map-of :keyword :any]
