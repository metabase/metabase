...
export async function getReleaseBranchCommits({
  ...
  const { stdout: branchCommit } =
    await $`git merge-base origin/release-x.${lastMajorVersion}.x master`;
  const { stdout: commitMessages } =
    await $`git log ${branchCommit.trim()}..origin/release-x.${versionNumber}.x --pretty='format:%s'`;
