(ns metabase.cmd.reset-password
  (:require [metabase.db :as mdb]
            ...
            [metabase.util.i18n :refer [lazy-trs trs]]
            [toucan.db :as db]))
...
(defn- set-reset-token!
  ...
  (let [user-id (or (db/select-one-id User, :email email-address)
                    (throw (Exception. (str (lazy-trs "No user found with email address ''{0}''. " email-address)
                                            (lazy-trs "Please check the spelling and try again.")))))]
...
(defn reset-password!
  ...
  [email-address]
  (mdb/setup-db!)
  (println (str (lazy-trs "Resetting password for {0}..." email-address)
                "\n"))
  (try
