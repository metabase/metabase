...
(defn validate-configuration
  ...
  [config]
  (if (mc/validate ::oidc-configuration config)
    {:valid? true}
    {:valid? false
     :errors (mc/explain ::oidc-configuration config)}))

(defn validate-id-token-claims
  "Validates ID token claims against the schema.
   Returns a map with :valid? boolean and :errors (if any)."
  [claims]
  (if (mc/validate ::id-token-claims claims)
    {:valid? true}
    {:valid? false
     :errors (mc/explain ::id-token-claims claims)}))
