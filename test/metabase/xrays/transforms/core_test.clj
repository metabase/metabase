(ns metabase.xrays.transforms.core-test
  (:require
   [clojure.test :refer :all]
   [medley.core :as m]
   [metabase.lib.test-metadata :as meta]
   [metabase.models.interface :as mi]
   [metabase.query-processor :as qp]
   [metabase.test :as mt]
   [metabase.util :as u]
   [metabase.xrays.domain-entities.specs :as de.specs]
   [metabase.xrays.test-util.domain-entities :refer [with-test-domain-entity-specs]]
   [metabase.xrays.test-util.transforms :refer [test-transform-spec with-test-transform-specs]]
   [metabase.xrays.transforms.core :as tf]
   [metabase.xrays.transforms.specs :as tf.specs]
   [toucan2.core :as t2]))

(use-fixtures :each (fn [thunk]
                      (t2/with-transaction [_conn nil {:rollback-only true}]
                        (thunk))))

(def ^:private test-bindings
  (delay
    (with-test-domain-entity-specs
      (let [table (m/find-first (comp #{(meta/id :venues)} u/the-id) (#'tf/tableset meta/metadata-provider "PUBLIC"))]
        {"Venues" {:transform.binding/dimensions (get-in table [:xrays/domain-entity :domain-entity/dimensions])
                   :transform.binding/entity     table}}))))

(deftest ^:parallel add-bindings-test
  (testing "Can we accure bindings?"
    (let [new-bindings {"D2" [:sum [:field 4 nil]]
                        "D3" [:field 5 nil]}]
      (is (= (update-in @test-bindings ["Venues" :dimensions] merge new-bindings)
             (#'tf/add-bindings @test-bindings "Venues" new-bindings))))))

(deftest ^:parallel add-bindings-test-2
  (testing "Gracefully handle nil"
    (is (= @test-bindings
           (#'tf/add-bindings @test-bindings "Venues" nil)))))

(deftest ^:parallel tableset-test
  (testing "Can we get a tableset for a given schema?"
    (is (= (t2/select-pks-set :model/Table :db_id (meta/id))
           (set (map u/the-id (#'tf/tableset meta/metadata-provider "PUBLIC")))))))

(deftest ^:parallel find-tables-with-domain-entity-test
  (with-test-domain-entity-specs
    (testing "Can we filter a tableset by domain entity?"
      (is (= [(meta/id :venues)]
             (map u/the-id (#'tf/find-tables-with-domain-entity (#'tf/tableset meta/metadata-provider "PUBLIC")
                                                                ((de.specs/domain-entity-specs) "Venues"))))))))

(deftest ^:parallel find-tables-with-domain-entity-test-2
  (with-test-domain-entity-specs
    (testing "Gracefully handle no-match"
      (with-test-domain-entity-specs
        (is (empty? (#'tf/find-tables-with-domain-entity [] ((de.specs/domain-entity-specs) "Venues"))))))))

(deftest ^:parallel resulting-entities-test
  (testing "Can we extract results from the final bindings?"
    (with-test-transform-specs
      (is (= [(meta/id :venues)]
             (map u/the-id (#'tf/resulting-cards {"VenuesEnhanced" {:transform.binding/entity     (meta/table-metadata :venues)
                                                                    :transform.binding/dimensions {"D1" (meta/field-metadata :venues :id)}}}
                                                 (first (tf.specs/transform-specs)))))))))

(deftest ^:parallel tables-matching-requirements-test
  (testing "Can we find a table set matching requirements of a given spec?"
    (with-test-transform-specs
      (with-test-domain-entity-specs
        (is (= [(meta/id :venues)]
               (map u/the-id (#'tf/tables-matching-requirements (#'tf/tableset meta/metadata-provider "PUBLIC")
                                                                (first (tf.specs/transform-specs))))))))))

(deftest ^:parallel tableset->bindings-test
  (testing "Can we turn a tableset into corresponding bindings?"
    (with-test-domain-entity-specs
      (is (= @test-bindings
             (#'tf/tableset->bindings (filter (comp #{(meta/id :venues)} u/the-id) (#'tf/tableset meta/metadata-provider "PUBLIC"))))))))

(deftest ^:parallel validation-test
  (with-test-domain-entity-specs
    (with-test-transform-specs
      (testing "Is the validation of results working?"
        (is (#'tf/validate-results meta/metadata-provider
                                   {"VenuesEnhanced" {:transform.binding/entity     (mi/instance
                                                                                     :model/Card
                                                                                     {:result_metadata [{:name "AvgPrice"}
                                                                                                        {:name "MaxPrice"}
                                                                                                        {:name "MinPrice"}]})
                                                      :transform.binding/dimensions {"D1" [:field 1 nil]}}}
                                   (first (tf.specs/transform-specs))))))))

(deftest ^:parallel validation-test-2
  (with-test-domain-entity-specs
    (with-test-transform-specs
      (testing "... and do we throw if we didn't get what we expected?"
        (is (thrown?
             java.lang.AssertionError
             (#'tf/validate-results
              meta/metadata-provider
              {"VenuesEnhanced" {:transform.binding/entity     (meta/table-metadata :venues)
                                 :transform.binding/dimensions {"D1" (meta/field-metadata :venues :id)}}}
              (first (tf.specs/transform-specs)))))))))

(deftest transform-test
  (testing "Run the transform and make sure it produces the correct result"
    (mt/with-full-data-perms-for-all-users!
      (mt/with-test-user :rasta
        (with-test-domain-entity-specs
          (let [cards (tf/apply-transform! (mt/metadata-provider) "PUBLIC" test-transform-spec)]
            (assert (seq cards))
            (is (= [[1 "Red Medicine"           4 10.065 -165.374 3 1.5  4 3 2 1]
                    [2 "Stout Burgers & Beers" 11 34.1   -118.329 2 1.1 11 2 1 1]
                    [3 "The Apple Pan"         11 34.041 -118.428 2 1.1 11 2 1 1]]
                   (mt/formatted-rows
                    [int str int 3.0 3.0 int 1.0 int int int int]
                    (-> cards
                        first
                        :dataset-query
                        qp/process-query))))))))))

(deftest ^:parallel correct-transforms-for-table-test
  (testing "Can we find the right transform(s) for a given table"
    (with-test-transform-specs
      (with-test-domain-entity-specs
        (is (= "Test transform"
               (-> (tf/candidates meta/metadata-provider (meta/table-metadata :venues))
                   first
                   :transform/name)))))))
