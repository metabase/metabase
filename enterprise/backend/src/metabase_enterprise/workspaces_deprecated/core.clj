(ns metabase-enterprise.workspaces-deprecated.core
  "Programmatic API for workspaces.
   ...
   - **Manager-side state.** Workspaces and their per-database configs live in the
     `:model/WorkspaceDeprecated` and `:model/WorkspaceDatabaseDeprecated` rows. Mutations go through
  ...
  (:require
   ...
   [metabase-enterprise.workspaces-deprecated.models.workspace :as workspace]
   [metabase-enterprise.workspaces-deprecated.models.workspace-database :as workspace-database]
...
(defn- assert-workspace-exists [workspace-id]
  (when-not (t2/exists? :model/WorkspaceDeprecated :id workspace-id)
  ...
  (let [failures (into []
                       (keep (fn [wsd]
                               (try
                                 ...
                                 nil
                                 (catch Throwable t t))))
                       (t2/select :model/WorkspaceDatabaseDeprecated :workspace_id workspace-id))]
    (when (seq failures)
