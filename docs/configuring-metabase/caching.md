...
> **Refreshing the cache automatically does not apply when [data sandboxing](../permissions/data-sandboxes.md) or [connection impersonation](../permissions/impersonation.md) permissions are in effect.** In those permissions setups, Metabase ignores the cache and runs a fresh query, as those permissions filter data specific to each viewer.
