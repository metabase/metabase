(ns metabase.metabot.tools.autogen-dashboard-test
  (:require
   [clojure.string :as str]
   [clojure.test :refer :all]
   [metabase.metabot.tools.autogen-dashboard :as autogen-dashboard]
   [metabase.test :as mt]))
...
(deftest response-structure-test
  (testing "returns instructions for LLM"
  ...
  (testing "returns a dashboard generated_entity data part"
    (mt/with-temp [:model/Table table {:name (mt/random-name) :db_id (mt/id)}]
      (let [result (autogen-dashboard/create-autogenerated-dashboard
                    {:source {:table_id (:id table)}
                     :memory-atom (atom {:state {}})})
        ...
        (is (= "dashboard" (:type entity)))
        (is (str/starts-with? (:url entity) "/auto/dashboard/"))
        (is (= (or (:display_name table) (:name table)) (:title entity))))))
...
(deftest create-autogenerated-dashboard-tool-errors-test
  (testing "a query missing from memory goes back to the agent as output"
    (is (= {:output "Query not found in memory with ID: q-9. Available queries: []"}
           (autogen-dashboard/create-autogenerated-dashboard-tool {:source {:query_id "q-9"}}))))
  (testing "an unexpected error propagates to the agent loop"
    (mt/with-dynamic-fn-redefs [autogen-dashboard/create-autogenerated-dashboard (fn [_] (throw (ex-info "boom" {})))]
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"boom"
                            (autogen-dashboard/create-autogenerated-dashboard-tool {:source {:table_id 1}}))))))
