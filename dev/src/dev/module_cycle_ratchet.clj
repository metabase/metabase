...
(defn resolve-clusters
  ...
  - `:status` — one of
    ...
    - `:merged`, joins two recorded clusters, named after the one it takes most from and nameless when they
      tie
    - `:ambiguous`, two components of one recorded cluster inherit it equally, so continuity would be a guess
                          ...
                          (let [o (get shared i)]
                            (merge cluster
                                   ...
                                   (cond
                                     ...
                                     ;; Drawing from two recorded clusters is a merge whether or not the
                                     ;; shares tie. Deciding that first keeps an even merge out of the
                                     ;; ambiguous case, which is for a cluster splitting within itself.
...
(defn pr-summary
  ...
  (when-let [lines (seq (changes before after))]
    (str "### Module cycles\n\n"
         ...
         ;; In :observe mode it records a cluster growing as readily as one shrinking, so the ratchet's
         ;; promise would be a lie on that PR.
         (when (enforce? after) " It only ever records a cluster shrinking, splitting or dissolving.")
         "\n\n```text\n" (str/join "\n" lines) "\n```\n")))
