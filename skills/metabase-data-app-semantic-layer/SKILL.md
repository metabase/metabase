...
- Keep runtime state out of the query object itself. A clause whose value comes from a control — a selected plan, a date range, a search box — belongs in the second argument to `useMetabaseQuery`/`useMetabaseQueryObject`, not in the query. See "Static and dynamic query parts".
...
For end-to-end prototypes, swap the source manually. After synchronization injects `savedQuestionSourceId`, build a separate card-source query for interactive filters, aggregations, breakouts, ordering, or limits:
