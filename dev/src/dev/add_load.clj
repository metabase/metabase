...
(def BindingForm [:or [:= :?] logical-kw :map])

(def NormalizedTuple [:tuple :keyword BindingForm :map])
...
(mu/defn from-script*
  ...
  (let [ids (atom {})]
    (doseq [[modelable bindings attrs] script]
      (prn ["Inserting: "
            [modelable bindings attrs]
            (fill-attrs @ids attrs)])
...
;; One random card
;; (add-load/from-script [[:model/Card]])

;; ignore the return value:
;; (add-load/from-script [[:model/Card {}]])

;; Two random cards
;; (add-load/from-script [[:model/Card] [:model/Card]])
...
;; (add-load/from-script [[:model/Card :?/my-card {:name "My Card 2"}]])

;; Desctructure card id
;; (add-load/from-script [[:model/Card {:?/card-id :id} {:name "My Card 4"}]])

;; reuse ids from previous steps
;; (add-load/from-script [[:model/Card {:?/card-id :id} {:name "My Card (w/ dashboard)"}]
...
;; TODO: batching
;; (time (add-load/from-script (repeat 1000 [:model/Card {}])))
