...
describe("issue 34955", () => {
  ...
  beforeEach(() => {
    ...
    H.createQuestionAndDashboard({
      ...
      H.tableInteractiveBody()
        ...
        .filter(":contains(May 15, 2030, 8:04 AM)")
  ...
  it("should connect specific date filter (`Between`) to the temporal custom column (metabase#34955)", () => {
    cy.get("@dashboardId").then((dashboard_id) => {
      ...
      cy.visit(`/dashboard/${dashboard_id}?on=&between=2030-01-01~2030-03-01`);
      checkAppliedFilter("Between", "January 1, 2030 - March 1, 2030");

      cy.findAllByTestId("cell-data")
        .filter(":contains(January 1, 2030, 7:26 AM)")
        .should("have.length", 2);
    });

    cy.get("@dashboardId").then((dashboard_id) => {
      // Apply filter through URL to prevent the typing flakes
      cy.visit(`/dashboard/${dashboard_id}?on=2030-01-01&between=`);
      checkAppliedFilter("On", "January 1, 2030");
...
describe("issue 54236", () => {
  ...
  it("should show correct date range in the date picker (metabase#54236)", () => {
    ...
    H.popover().within(() => {
      ...
      cy.findByText("Jul 1 – Sep 30, 2031").should("be.visible");
      cy.findByText("Apr 1 – Jun 30, 2031").should("not.exist");
    });
  });
});
...
describe("issue 55678", () => {
  ...
  it("should ignore parameters mapped to post-aggregation stages when doing query drills (metabase#55678)", () => {
    ...
    H.queryBuilderFiltersPanel()
      .findByText("Created At: Month is Apr 1–30, 2028")
      .should("be.visible");
