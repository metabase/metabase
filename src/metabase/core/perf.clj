    ...
    (.scheduleAtFixedRate
     ...
     ^Runnable (fn []
                 (try
                   ...
                   (catch Throwable e
                     (log/warn "Error during rolling performance dump:" (ex-message e)))))
     interval-minutes
     interval-minutes
     TimeUnit/MINUTES)
    (reset! dump-executor-atom executor)))
...
(defn maybe-enable-monitoring!
  ...
  (try
    ...
    (catch Throwable e
      (log/warn "Failed to start performance monitoring:" (ex-message e)))))
...
(defn stop-monitoring!
  ...
  (when-let [^Recording recording @recording-atom]
    (try
      ...
      (catch Throwable e
        (log/warn "Error stopping performance monitoring:" (ex-message e))))))
