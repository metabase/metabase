(ns metabase.subscription.test-util
  (:require
...
(defn send-pulse-created-by-user!
  ...
  (t2.with-temp/with-temp [Pulse     pulse {:creator_id (test.users/user->id user-kw)}
    ...
    (with-redefs [subscription/send-notifications!    identity
                  subscription/parts->notifications (fn [_ results]
                                                     (vec results))]
      (let [[{:keys [result]}] (subscription/send-pulse! pulse)]
...
(defmacro slack-test-setup
  ...
  [& body]
  `(with-redefs [subscription/send-notifications! realize-lazy-seqs
