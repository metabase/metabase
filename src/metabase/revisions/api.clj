...
(api.macros/defendpoint :post "/revert"
  ...
  (let [[model instance] (model-and-instance entity id)
    ...
    ;; for Segments `table_id` is re-derived from `definition` on update, so when the restored definition specifies a
    ;; source table, check write perms against that table rather than the revision's stored `table_id`
    (when (= model :model/Segment)
