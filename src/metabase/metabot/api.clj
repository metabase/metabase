...
(defn- request-credentials
  ...
  Azure — resolves to a credentials map whose key material is nil: an explicit clear. (Blank *fields inside* the
  Bedrock/Azure credentials map are not a clear; they mean \"keep the saved value\", so e.g. a region-only edit
  can't wipe the keys.) Throws a 400 when non-nil Bedrock/Azure credentials don't resolve to a complete set."
