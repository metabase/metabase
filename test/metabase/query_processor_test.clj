...
(defn do-with-bigquery-fks [driver thunk]
  {:pre [(keyword? driver)]}
  (if-not (= driver/*driver* driver)
    (thunk)
    (let [supports? driver/supports?]
      (with-redefs [driver/supports? (fn [a-driver feature]
                                       (if (= [a-driver feature] [driver :foreign-keys])
                                         true
                                         (supports? a-driver feature)))]
        (let [thunk (reduce
...
(defmacro with-bigquery-fks
  ...
  when testing against BigQuery, for the BigQuery based driver `driver`. BigQuery does not support Foreign Key
  ...
  [driver & body]
  `(do-with-bigquery-fks ~driver (fn [] ~@body)))
