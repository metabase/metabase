...
(defn collection-readable?
  "Returns if a collection can be read. The read counterpart of [[collection-editable?]], and unlike it not gated on
  remote sync being enabled at all: a worktree's collections are admin-only, always. Main-app collections
  (`:worktree_id` `nil`) are not restricted here."
  [collection]
  (worktree-accessible? collection))

(defn model-readable?
  "Returns if a model instance can be read. The read counterpart of [[model-editable?]], and unlike it not gated on
  remote sync being enabled at all: content checked out into a worktree is admin-only, always. Main-app content
  (`:worktree_id` `nil`) is not restricted here. AND this into a worktree-scoped model's `can-read?` / `can-write?`
  / `can-create?`."
  [instance]
  (worktree-accessible? instance))
