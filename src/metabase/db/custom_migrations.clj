...
(defn- get-metric-migration-collection
  "Get or create the collection used to store the questions created from
  the v1 metrics.
  The collection is not created unless the option `create?` is truthy."
  ([]
   (get-metric-migration-collection nil))
  ([{:keys [create?]}]
   (let [coll-name "Metabase v50 Migrated Metric Queries"
     ...
     (or (t2/select-one :collection :name coll-name)
         (when create?
...
(defn- split-metric-id
  "Parse the metric v1 ID from `description` created by [[add-metric-id]]."
  [description]
  (when-let [[_ desc metric-id] (re-matches #"(.*) \(Migrated from metric (\d+)\.\)" description)]
    {:id (parse-long metric-id)
     :description desc}))

;; TODO handle metric :show_in_getting_started, :caveats, :how_is_this_calculated, :points_of_interest
;;      card: :initially_published_at, :made_public_by_id, :visualization_settings
;; TODO copy entity_id?
(defn- convert-metric-v2
  "Convert a metric v2 question from `metric-v1`.
  ...
  (let [definition (json/parse-string (:definition metric-v1) true)
        dataset-query {:type :query
                       :database (:database_id metric-v1)
                       :query definition}]
    (-> metric-v1
        ...
        (assoc :dataset_query (json/generate-string dataset-query)
               ...
               :query_type "query" ; TODO or "metric" or something else?
               :type "metric"
               :parameters "[]"
               :parameter_mappings "[]"
...
;; TODO prevent inserting duplicates if a partial migration is rerried
...
(defn- convert-metric-v1
  "Create a v1 metric from the v2 metric question `metric-v2-card`."
  [metric-v2-card]
  (let [dataset-query (json/parse-string (:dataset_query metric-v2-card) true)
        definition (:query dataset-query)
        {:keys [id description]} (-> metric-v2-card :description split-metric-id)]
    (-> metric-v2-card
        (select-keys [:archived :created_at :creator_id :name :table_id])
        (assoc :id id
               :description description
               :definition (json/generate-string definition)))))

(defn- create-metric-v1
  "Create a v1 metric from the v2 metric question `metric-v2-card`."
  [metric-v2-card]
  (let [metric (convert-metric-v1 metric-v2-card)]
    (t2/insert-returning-instance! :metric metric)))

(defn- get-metric-v2-cards
  "Loads the v2 metric cards (aka questions) created by the migration.
  These are the questions we need to convert back to v1 metrics during rollback."
  [metric-v2-coll]
  (t2/select :report_card :collection_id (:id metric-v2-coll)))
...
(defn- replace-metric-refs
  ...
  The `expr` is an MBQL aggregation expression or a part of it.
  The mapping is either from metric IDs to v2 metric cards (forward migration)
  or v2 metric card IDs to v1 metrics (rollback).
  ...
  (if-not (vector? expr)
    ...
    (if-let [id (metric-ref->id expr)]
      [(assoc expr 1 (-> id id->entity :id)) #{id}]
...
;; TODO handle questions consuming multiple metrics
(defn- rewrite-metric-consumption
  "Rewrite the question `card` replacing references to v1 metrics with references
  to the corresponding v2 metric question as specified by the mapping `metric-id->v2-query`."
  [card metric-id->v2-query]
  (let [dataset-query (json/parse-string (:dataset_query card) true)
        aggregation (-> dataset-query :query :aggregation)
        [rewritten metric-ids] (replace-metric-refs aggregation metric-id->v2-query)]
    (when (not= rewritten aggregation)
      (assoc card
             :dataset_query (-> dataset-query
                                (update :query
                                        assoc
                                        :aggregation rewritten
                                        :source-table (str "card__" (-> metric-ids first metric-id->v2-query :id)))
                                json/generate-string)
             :updated_at (Instant/now)))))

;; TODO See if what remains common with rewrite-metric-consumption after adding
;; support for multiple consumed metrics. Delete them before the retry or try to
;; resue them?
(defn- revert-metric-consumption
  "Rewrite the question `card` replacing references to v2 metric questions with references
  to the corresponding v1 metrics as specified by the mapping `v2-card-id->metric`."
  [card v2-card-id->metric]
  (let [dataset-query (json/parse-string (:dataset_query card) true)
        aggregation (-> dataset-query :query :aggregation)
        [rewritten card-ids] (replace-metric-refs aggregation v2-card-id->metric)]
    (when (not= rewritten aggregation)
      (assoc card
             :dataset_query (-> dataset-query
                                (update :query
                                        assoc
                                        :aggregation rewritten
                                        :source-table (-> card-ids first v2-card-id->metric :table_id))
                                json/generate-string)
             :updated_at (Instant/now)))))

;; TODO SQL migration for dropping (and recreating) the metric table
  ...
  (when (t2/exists? :metric)
    (let [metric-v2-coll (get-metric-migration-collection {:create? true})
      ...
      (doseq [card (t2/select :report_card {:where [:like :dataset_query "%\"metric\"%"]})
              :let [rewritten (rewrite-metric-consumption card metric-id->v2-query)]
              :when (and rewritten (not= card rewritten))]
        ;; updating the dataset_query field only should be enough
        (t2/update! :report_card (:id card) rewritten))))
  (when-let [metric-v2-coll (get-metric-migration-collection)]
    (let [metric-v2-cards (get-metric-v2-cards metric-v2-coll)
          v2-card-id->metric (into {}
                                   (juxt :id create-metric-v1)
                                   metric-v2-cards)]
      (doseq [card (t2/select :report_card {:where [:like :dataset_query "%\"metric\"%"]})
              :let [rewritten (revert-metric-consumption card v2-card-id->metric)]
              :when (and rewritten (not= card rewritten))]
        ;; updating dataset_query should be enough
        (t2/update! :report_card (:id card) rewritten)
        (when (seq v2-card-id->metric)
          (t2/delete! :report_card :id [:in (keys v2-card-id->metric)]))
        (t2/delete! :collection :id (:id metric-v2-coll))))))

(comment
  (require 'metabase.db.env)
  (let [driver :postgres
        conn-uri "postgres://vacskamati/metabase?user=metabase&password=metasample123"
        ds-desc {:mb-db-connection-uri conn-uri}
        data-source (#'metabase.db.env/env->DataSource driver ds-desc)]
    (binding [mdb.connection/*application-db* (mdb.connection/application-db driver data-source)]
      ...
      (let [coll (get-metric-migration-collection {:create? true})
            ...
            card (t2/select-one :report_card :id 3691)
            converted-card (rewrite-metric-consumption card {46 v2-query})]
