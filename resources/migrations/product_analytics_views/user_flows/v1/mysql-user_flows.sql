...
CREATE OR REPLACE SQL SECURITY INVOKER VIEW V_PA_USER_FLOWS AS
SELECT
  FLOWS.SOURCE_URL AS SOURCE,
  FLOWS.TARGET_URL AS TARGET,
  COUNT(*)         AS VALUE
FROM (
  SELECT
    STEP_NUM,
...
WHERE FLOWS.TARGET_URL IS NOT NULL
  AND FLOWS.SOURCE_URL != FLOWS.TARGET_URL
  AND NOT EXISTS (
    SELECT 1
    FROM (
      SELECT
...
GROUP BY FLOWS.STEP_NUM, FLOWS.SOURCE_URL, FLOWS.TARGET_URL;
