...
(def ^:private ^:const sample-dataset-filename
  (str (System/getProperty "user.dir") "/resources/sample-database.db"))
...
(defn- relative-date [unit n]
  (u.date/add unit n))

(defn- to-millis [date-time]
  (.. date-time toInstant toEpochMilli))

(defn- maybe-sql-date-time [v]
...
(defn- random-date-between [^ZonedDateTime min-date ^ZonedDateTime max-date]
  (let [min-ms (to-millis min-date)
        max-ms (to-millis max-date)
        ms-range (- max-ms min-ms)]
    (ZonedDateTime/ofInstant (Instant/ofEpochMilli
                               (+ (long (rand ms-range)) min-ms))
...
(defn- random-person []
  (let [first (name/first-name)
    ...
    {:name       (format "%s %s" first last)
     ...
     :birth_date (random-date-between (relative-date :year -60) (relative-date :year -18))
     ...
     :created_at (random-date-between (relative-date :year -2) (relative-date :year 1))}))
...
(defn- max-date [& dates]
  {:pre [(every? (partial instance? ZonedDateTime) dates)]
   :post [(instance? ZonedDateTime %)]}
  (last (sort-by to-millis dates)))

(defn- min-date [& dates]
  {:pre [(every? (partial instance? ZonedDateTime) dates)]
   :post [(instance? ZonedDateTime %)]}
  (first (sort-by to-millis dates)))
...
(defn- random-order [{:keys [state], :as person} {:keys [price], :as product}]
  {:pre [(string? state)
         (number? price)]
   :post [(map? %)]}
  (let [tax-rate   (state->tax-rate state)
        ...
        created-at (random-date-between (min-date (:created_at person)
                                                  (:created_at product))
                                        (relative-date :year 2))
        price      (if (t/after? (t/zoned-date-time) (t/zoned-date-time 2018 1 1))
                     (* 1.5 price)
                     price)
...
(defn- random-review [product]
  {:product_id (:id product)
   ...
   :created_at (random-date-between (:created_at product) (relative-date :year 2))})
    ...
    (let [products (for [product (create-randoms products random-product)]
      ...
      {:people   (map #(dissoc % :orders) people)
       ...
       :orders   (->> people
                      ...
                      (add-seasonality #(.getMonthValue (:created_at %))
                                       :quantity {1  0.6
                                                  2  0.5
                                                  3  0.3
                                                  4  0.9
...
(defn- create-h2-db
  ...
  ([filename data]
   ...
   (println "Creating db...")
   (let [db (mdb.spec/spec :h2 {:db (format (str "file:%s;UNDO_LOG=0;CACHE_SIZE=131072;QUERY_CACHE_SIZE=128;COMPRESS=TRUE;"
       ...
       (let [table-name (str/upper-case (name table))]
         ...
         (jdbc/insert-multi! db table-name (for [row rows]
                                             (into {} (for [[k v] (seq row)]
                                                        {(str/upper-case (name k)) (maybe-sql-date-time v)}))))))
...
(defn -main
  "Generates a sample db at filename"
  [& [filename]]
