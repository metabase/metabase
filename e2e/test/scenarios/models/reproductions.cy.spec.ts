...
describe("model reproductions", () => {
  ...
  describe("issue 39150", { viewportWidth: 1600 }, () => {
    ...
    it("allows custom columns with the same name as the aggregation column from the souce model (metabase#39150-2)", () => {
      ...
      H.saveQuestion("Nested Model", {
        wrapId: true,
        idAlias: "nestedModelId",
      });
  ...
  // needs own restore because it modifies a shared card via cy.request("PUT")
  ...
  // needs own restore because it uses H.activateToken
  ...
  describe("issue 34574", () => {
    ...
    function assertMarkdownPreview() {
      cy.findByRole("heading", { level: 1, name: "Hello" }).should(
        "be.visible",
      );
      cy.findByRole("heading", { level: 2, name: "World" }).should(
        "be.visible",
      );
  ...
  describe("issue 51925", () => {
    function setLinkDisplayType() {
      cy.findByTestId("chart-settings-widget-view_as")
        .findByText("Link")
        .click();
    }
  ...
  // needs own restore because it modifies field visibility via cy.request("PUT", "/api/field/...")
  describe("issue 45919", () => {
    ...
    it("should allow to query a model with a hidden column", () => {
      ...
      cy.log(
        "the query should succeed, and the Password field should be hidden",
      );
  ...
  // needs own restore because it uses H.activateToken and cy.updatePermissionsGraph
