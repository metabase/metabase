...
DROP TABLE IF EXISTS ` test_data `.` users `

CREATE EXTERNAL TABLE ` test_data `.` users ` (
  ` id ` INT,
  ` name ` STRING,
  ` last_login ` TIMESTAMP,
  ` password ` STRING
) LOCATION 's3://metabase-ci-athena-results/test_data/users/';

DROP TABLE IF EXISTS ` test_data `.` categories `

CREATE EXTERNAL TABLE ` test_data `.` categories ` (` id ` INT, ` name ` STRING) LOCATION 's3://metabase-ci-athena-results/test_data/categories/';

DROP TABLE IF EXISTS ` test_data `.` venues `

CREATE EXTERNAL TABLE ` test_data `.` venues ` (
  ` id ` INT,
  ` name ` STRING,
  ` category_id ` INT,
  ` latitude ` DOUBLE,
  ` longitude ` DOUBLE,
  ` price ` INT
) LOCATION 's3://metabase-ci-athena-results/test_data/venues/';

DROP TABLE IF EXISTS ` test_data `.` checkins `

CREATE EXTERNAL TABLE ` test_data `.` checkins ` (
  ` id ` INT,
  ` date ` TIMESTAMP,
  ` user_id ` INT,
  ` venue_id ` INT
) LOCATION 's3://metabase-ci-athena-results/test_data/checkins/';

-- 15 rows
INSERT INTO "test_data"."users" ("name", "last_login", "password")
VALUES
...
-- 75 rows
INSERT INTO "test_data"."categories" ("name")
VALUES
('African'),
('American'),
('Artisan'),
...
-- 100 rows
INSERT INTO "test_data"."venues" ("name", "category_id", "latitude", "longitude", "price")
VALUES
...
-- 1000 rows
INSERT INTO "test_data"."checkins" ("date", "user_id", "venue_id")
VALUES
