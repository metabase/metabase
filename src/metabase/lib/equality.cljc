...
(defmethod = :metadata/column
  ...
  (and
   ...
   ;; same join alias
   (columns-equal-by-fn :metabase.lib.join/join-alias col-1 col-2)
   ;; same FK Field (for implicitly joined columns)
   (columns-equal-by-fn-when-non-nil-in-both :fk-field-id col-1 col-2)
   (columns-equal-by-fn :fk-join-alias col-1 col-2)
   ...
   (columns-equal-by-fn-when-non-nil-in-both :fk-field-name col-1 col-2)
   ;;
...
(defn- find-matching-column-by-=
  ...
  (when-let [col (if (map? ref-or-col)
                   ref-or-col
                   (lib.metadata.calculation/metadata query stage-number ref-or-col))]
    (let [col (find-matching-column-update-col-for-options col options)]
      (m/find-first #(= (find-matching-column-update-col-for-options % options) col)
                    columns))))
