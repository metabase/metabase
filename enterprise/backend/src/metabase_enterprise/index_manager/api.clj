...
(api.macros/defendpoint :get "/" :- [:map [:data [:sequential IndexRequest]]]
  ...
  (api/check-superuser)
  {:data (mapv present (impl/requests-for-transform transform-id))})
...
(api.macros/defendpoint :post "/" :- IndexRequest
  ...
  [_route-params
   ...
   {:keys [transform_id structured]} :- [:map
                                         ...
                                         [:structured :map]]]
  (api/check-superuser)
  (present (impl/create-request! transform_id structured :created-by api/*current-user-id*)))
...
(api.macros/defendpoint :put "/:id" :- IndexRequest
  ...
  (api/check-superuser)
  (api/check-404 (present (impl/update-request! id structured))))
...
(api.macros/defendpoint :delete "/:id"
  ...
  [{:keys [id]} :- [:map [:id ms/PositiveInt]]]
  (api/check-superuser)
  (api/check-404 (impl/delete-request! id))
