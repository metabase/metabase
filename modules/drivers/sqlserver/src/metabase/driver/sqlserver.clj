...
(defn- update-aggregations [inner]
  ...
  ;; Remove `percentile-aggregations` from query, in case :aggregations are left empty, dissoc the key.

  (let [non-percentile-aggregations
        (filterv (complement percentile-aggregation?) (:aggregation inner))]
    (if (not-empty non-percentile-aggregations)
      (assoc inner :aggregation non-percentile-aggregations)
      (dissoc inner :aggregation))))

(defn- percentile-source-query [original-query]

  ;; Generate :source-query computing percentile with window function.

  (as-> (select-keys original-query [:source-query :source-table :joins :filter]) under-construction
    ;; this query computes only percentile "aggregations"
    (assoc under-construction :aggregation (->> (:aggregation original-query)
                                                (filterv percentile-aggregation?)))
    ;; fields to be used for further real aggregations in parent query, ie. generated by nest-expressions
    (assoc under-construction :fields (-> (select-keys original-query [:aggregation :breakout :order-by :page :limit])
                                          (update :aggregation #(filter (complement percentile-aggregation?) %))
                                          (mbql.u/match :field)
                                          distinct
                                          vec))
    ;; following used in ->honeysql [:sqlserver :percentile] in over clause
    (assoc under-construction ::partition-by (:breakout original-query))))

(defn- aggregation->field [aggregation]

  ;; Hardcoding base-type magically works, but I'm not sure, if I just haven't encounter
  ;; app state where it would cause problems.
...
(defn update-order-by-references [order-by-clause original-query]
  ...
  ;;    aggregations transform to fields from modified query.

  (let [prior-perc-count (reductions (fn [acc aggregation]
                                       (if (percentile-aggregation? aggregation)
                                         (+ 1 acc)
                                         acc))
                                     0
                                     (:aggregation original-query))]
    (mapv (fn [[order [field-type index] :as order-by-elm]]
            (if (= field-type :aggregation)
              (let [cur-perc-count (nth prior-perc-count index)
                    next-perc-count (nth prior-perc-count (+ 1 index))]
                ;; if true, current index points to percentile aggregation
                (if (< cur-perc-count next-perc-count)
                  ;; [:aggregation 1] -> [:field "some percentile aggregation" {...}]
                  [order (aggregation->field (nth (:aggregation original-query) index))]
                  ;; decrease aggregation index by count of preceding percentile aggregations
                  [order [field-type (- index cur-perc-count)]]))
              order-by-elm))
          order-by-clause)))
...
(defn rewrite-percentile-aggregations--current-query [original-inner]
  ...
  ;; EXPRESSIONS
  ;; Function is coded in a way that it expects nest-expressions to be called first in preprocess, hence 

  ;; HOW IT WORKS
  ;; 1. Original query has removed source, joins and filter.
  ;;    Ie. everything which is used to create source for aggregation. `:expressions` ignored here
  ;;    because of nest-expressions application prior to percentile aggregations rewrite transforms them to fields.
  ;; 2. __Fields generated from window percentile are added to breakout__, percentile aggregations are removed
  ;;    from modified query, aggregation references in order-by are updated.
  ;; 3. Subquery ([[percentile-source-query]]) containing:
  ;;      - fields for computation of different aggregations (eg from expressions),
  ...
  ;;    so it would appear first in query results without this reordering.

  ;; 4.
  {:fields       (into (vec (:breakout original-inner))
                       (map aggregation->field (:aggregation original-inner)))
   :source-query (->
                  ;; 1.
                  ...
                  (m/update-existing :order-by update-order-by-references original-inner)
                  ;; 3.
...
(defn rewrite-percentile-aggregations
  "Modify original query to compute percentile aggregation with use of window function `PERCENTILE_CONT`
   ...
   - Alias info is updated because of newly created fields.

   This function is ignoring :source-query in joins as it is meant to be called in [[sql.qp/preprocess]] after
   [[nest-query/nest-expressions]].
   "
  [original-query]
