...
(defn- add-alias-to-join-refs [metadata-providerable form join-alias join-cols]
  (mbql.u.match/replace form
    (field :guard (fn [field-clause]
                    (and (lib.util/field-clause? field-clause)
                         (boolean (lib.equality/closest-matching-metadata metadata-providerable field-clause join-cols)))))
    (with-join-alias field join-alias)))

(defn- add-alias-to-condition
  [metadata-providerable condition join-alias home-cols join-cols]
  (let [condition (add-alias-to-join-refs metadata-providerable condition join-alias join-cols)]
    ...
    (mbql.u.match/replace condition
      ...
      (let [lhs-alias (current-join-alias lhs)
        ...
        (cond
          ...
          (if (lib.equality/closest-matching-metadata metadata-providerable rhs home-cols)
          ...
          (let [bare-lhs (lib.options/update-options lhs dissoc :join-alias)
            ...
            (if (and (nil? (lib.equality/closest-matching-metadata metadata-providerable bare-lhs home-cols))
                     (lib.equality/index-of-closest-matching-metadata metadata-providerable bare-rhs home-cols))
  ...
  (if (contains? a-join :alias)
    ...
    (let [stage       (lib.util/query-stage query stage-number)
      ...
      (-> a-join
          (update :conditions
                  (fn [conditions]
                    (mapv #(add-alias-to-condition query % join-alias home-cols join-cols)
                          conditions)))
          (with-join-alias join-alias)))))
