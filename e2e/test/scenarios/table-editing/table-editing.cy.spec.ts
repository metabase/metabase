...
const EDITABLE_TABLE_NAME = "many_data_types";
const EDITABLE_TABLE_NAME_REGEX = new RegExp("Many Data Types", "i");
...
describe("scenarios > table-editing", () => {
  ...
  it("should show edit icon on table browser", () => {
    openTableBrowser();
    getTableEditIcon(EDITABLE_TABLE_NAME_REGEX).should("be.visible");
  });
  ...
  it("should not show edit icon on table browser if user is not admin", () => {
    ...
    openTableBrowser();
    getTableEditIcon(EDITABLE_TABLE_NAME_REGEX).should("not.exist");
  });

  it("should allow to open table data edit mode", () => {
    openTableBrowser();
    openTableEdit(EDITABLE_TABLE_NAME_REGEX);

    H.getTableId({
      name: EDITABLE_TABLE_NAME,
    }).then((tableId) => {
    ...
    cy.findByTestId("edit-table-data-root")
      .should("be.visible")
      .within(() => {
        cy.findByText(EDITABLE_TABLE_NAME_REGEX).should("be.visible");
    ...
    cy.findByTestId("head-crumbs-container")
      .findByText(EDITABLE_TABLE_NAME_REGEX)
      .click();
  ...
  describe("table edit mode", () => {
    ...
    describe("inline cell editing", () => {
      /**
       * We test only a couple of columns which are visible on initial table rendering
       * to avoid flakiness, since other columns are rendered lazily and are not
       * present in the DOM right away. We'd have to scroll and look for them.
       * Overall, testing different cell types editing should be done in scope
       * of unit tests.
       */
      const cases = [
        {
