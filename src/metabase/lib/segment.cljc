...
(mu/defn available-segments :- [:maybe [:sequential {:min 1} ::lib.schema.metadata/segment]]
  ...
  ([query :- ::lib.schema/query
   ...
   (when (zero? (lib.util/canonical-stage-index query stage-number))
     (let [source-table-id (lib.util/source-table-id query)
           source-card-id (lib.util/source-card-id query)
           segments (if source-card-id
                      (lib.metadata.protocols/metadatas-for-card (lib.metadata/->metadata-provider query) :metadata/segment source-card-id)
                      (lib.metadata.protocols/metadatas-for-table (lib.metadata/->metadata-provider query) :metadata/segment source-table-id))
