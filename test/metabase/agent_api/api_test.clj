...
(deftest get-table-details-field-types-test
  (testing "Field metadata (base_type, effective_type, semantic_type, coercion_strategy) is returned correctly"
    (testing "base_type is always set, semantic_type is returned when set"
    ...
    (testing "semantic_type is omitted when not set, effective_type is omitted when it equals base_type"
      (mt/with-temp [:model/Database {db-id :id}    {}
                     :model/Table    {table-id :id} {:db_id db-id, :name "t", :active true}
                     :model/Field    _              {:table_id table-id, :name "name"
                                                     :base_type :type/Text}]
        (let [[field :as fields] (:fields (mt/user-http-request :rasta :get 200 (str "agent/v1/table/" table-id)))]
          (is (= 1 (count fields)))
          (is (=? {:name      "name"
                   :base_type "type/Text"}
                  field))
          (is (not (contains? field :semantic_type)))
          (is (not (contains? field :effective_type))))))
