...
(defn- allowed-clause-key?
  "Whether `k` may appear in an MBQL clause's options. Only the `:lib/*` keys MBQL is made of; everything else
  namespaced is put there by middleware that runs after this one."
  [k]
  (case (namespace k)
    (nil "lib") true
    false))
