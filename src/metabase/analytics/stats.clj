  ...
  (let [thirty-days-ago (str "cast('" (t/minus (t/offset-date-time) (t/days 30)) "' AS TIMESTAMP)")]
    (str/join
     ...
     ["WITH user_executions AS ("
      ...
      "    SELECT"
      "        COUNT(*) AS executions,"
      "        SUM(CASE WHEN error IS NULL OR error = '' THEN 1 ELSE 0 END) AS by_status__completed,"
      "        SUM(CASE WHEN error IS NOT NULL AND error != '' THEN 1 ELSE 0 END) AS by_status__failed,"
