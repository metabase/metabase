...
(def ^:private root-content-workspace-column
  "The content table's own `workspace_id` column, qualified with its `search-query-for-model` FROM alias, for every
  model whose `add-collection-join-and-where-clauses` call needs workspace isolation on top of collection
  permissions. `permitted-collections-clause`'s root-collection branch admits root-level rows from every workspace,
  so root content additionally needs this per-row check; non-root content is already scoped transitively through
  its (workspace-scoped) collection."
  {"card"      :card.workspace_id
   "dataset"   :card.workspace_id
   "metric"    :card.workspace_id
   "action"    :action.workspace_id
   "document"  :document.workspace_id
   "dashboard" :dashboard.workspace_id})
...
(mu/defn add-collection-join-and-where-clauses
  ...
  (let [collection-id-col      (case model
        ...
        workspace-column        (root-content-workspace-column model)]
    (-> honeysql-query
