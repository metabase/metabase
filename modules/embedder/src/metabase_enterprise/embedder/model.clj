...
(defn- built-in-model-url?
  "Whether `url` was generated by [[model-source]], rather than supplied as an override."
  [url]
  (or (= url default-model-zoo-url)
      (str/starts-with? url "jar:///metabase-embedder/")))

(defn- sanitized-url
  "Safe endpoint for a model-source log. Paths are retained only for URLs generated by [[model-source]];
  arbitrary overrides may carry credentials in any component, including the path."
  [url include-path?]
  (try
...
(defn- source-log-summary
  ...
  [{:keys [type url path]}]
  (let [built-in-url? (and (= type :url) (built-in-model-url? url))]
    (cond-> {:type type
             :origin (if built-in-url? :built-in :override)}
      (= type :path) (assoc :path path)
      (= type :url)  (assoc :url (sanitized-url url built-in-url?)))))
