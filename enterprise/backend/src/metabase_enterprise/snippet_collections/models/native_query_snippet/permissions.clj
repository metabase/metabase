...
(defenterprise can-read?
  ...
  ([snippet]
   (and
    (remote-sync/workspace-accessible? snippet)
    (not (perms/sandboxed-user?))
    (snippets/has-any-native-permissions?)
    (has-parent-collection-perms? snippet :read)))
  ([model id]
   (can-read? (t2/select-one [model :collection_id :workspace_id] :id id))))
...
(defenterprise can-write?
  ...
  ([model id]
   (can-write? (t2/select-one [model :collection_id :workspace_id] :id id))))
...
(defenterprise can-create?
  ...
  [_model m]
  (and
   (remote-sync/workspace-accessible? m)
   (not (perms/sandboxed-user?))
   (snippets/has-any-native-permissions?)
