...
select
    f.id as entity_id,
    'field_' || f.id as entity_qualified_id, -- should match idx_field_entity_qualified_id
    f.created_at,
    f.updated_at,
    f.name,
...
left join metabase_field_user_settings u on u.field_id = f.id and u.worktree_id is null;
