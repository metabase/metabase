...
(deftest ^:parallel collection-in-value-clause-test
  (let [evil-values {"map inside a vector"   [{:raw "1) UNION SELECT 1 -- "}]
    ...
    ;; The upstream test also covers the MBQL 5 `[:value <opts> <value>]` order, which reaches a `:sql-mbql5`
    ;; reordering method. That MBQL 5 dispatch does not exist on release-x.58.x -- `:value` here is legacy-order only --
