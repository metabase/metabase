...
describe("scenarios > dashboard > parameters", () => {
  ...
  it("one filter should search across multiple fields", () => {
    ...
    H.popover().find("input").type("Ga");
    cy.wait("@dashboard");
    H.popover().contains("Gabrielle Considine");

    // Continue typing a "d" and you see "Gadget"
    H.popover().find("input").type("d");
    cy.wait("@dashboard");
  ...
  describe("when parameters are (dis)connected to dashcards", () => {
    beforeEach(() => {
      ...
      H.popover().contains("Past 7 days").click({ force: true });
      H.saveDashboard();
    ...
    it("should fetch dashcard data when parameter mapping is removed", () => {
      ...
      cy.findByTestId("edit-dashboard-parameters-widget-container")
        ...
        .click();
      H.selectDashboardFilter(H.getDashboardCard(0), "Created At");
      H.disconnectDashboardFilter(H.getDashboardCard(1));
      H.saveDashboard();
