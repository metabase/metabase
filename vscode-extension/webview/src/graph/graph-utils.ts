import type { DependentsCount, GraphNodeModel, GraphViewEdge, GraphViewNode } from '../../../src/shared-types'
...
export function getDependencyGroupTitle(
  ...
  if (groups.length === 0)
    return 'Nothing uses this'
  if (node.model === 'transform')
    return 'Generates'
  return 'Used by'
}
...
export function getFieldIconName(semanticType: string | null): IconName {
  if (!semanticType)
    return 'hash'
  if (semanticType.includes('PK') || semanticType === 'type/PK')
    return 'key'
  if (semanticType.includes('FK') || semanticType === 'type/FK')
    return 'connections'
  return 'hash'
}
