...
describe("sandbox", () => {
  ...
  it("confines custom viz and custom viz setting widget to its container (GDGT-2400)", () => {
    const timeout = 100;
    const payload = `
    ...
    H.createQuestion(
      ...
      { wrapId: true, idAlias: "componentCardId" },
    );

    H.visitQuestion("@componentCardId");
    ...
    H.vizSettingsSidebar()
      ...
      .should(($element) => {
        ...
        expect(rect.width, "setting widget width vs viewport").to.be.lessThan(
          viewportWidth,
        );
        expect(rect.height, "setting widget height vs viewport").to.be.eq(0);
      });
    ...
    H.queryVisualizationRoot()
      ...
      .should(($element) => {
        ...
        expect(rect.width, "container width vs viewport").to.be.lessThan(
          viewportWidth,
        );
        expect(rect.height, "container height vs viewport").to.be.lessThan(
          viewportHeight,
        );
      });
  });
