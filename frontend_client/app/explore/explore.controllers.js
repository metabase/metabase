...
ExploreControllers.controller('ExploreTableMetadata', ['$scope', '$routeParams', 'Metabase', function($scope, $routeParams, Metabase) {
    Metabase.table_get({
        'tableId': $routeParams.tableId
        ...
        // get the views for this table
        Metabase.table_views({
            'tableId': $routeParams.tableId
        }, function(result) {
            $scope.views = result;
        });
    ...
    //not used yet
    $scope.getFieldSpecialTypeNameById = function(id) {
        for (var type in $scope.util.field_special_types) {
            console.log(type);
            if (type.id == id) {
                return type.name;
            }
