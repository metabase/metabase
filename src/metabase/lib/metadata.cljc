...
(mu/defn fields :- [:sequential ::lib.schema.metadata/column]
  ...
  [metadata-providerable :- ::lib.schema.metadata/metadata-providerable
   table-id              :- ::lib.schema.id/table]
  (let [db     (database metadata-providerable)
        tbl    (table metadata-providerable table-id)
        prefix (str/join "__" ["field" (:name db) (:schema tbl "!noschema!") (:name tbl)])]
    (for [field (lib.metadata.protocols/fields (->metadata-provider metadata-providerable) table-id)]
      (assoc field :ident (str prefix "__" (:name field))))))
...
(mu/defn field :- [:maybe ::lib.schema.metadata/column]
  ...
  (let [fieldd (lib.metadata.protocols/field (->metadata-provider metadata-providerable) field-id)]
    (when (and (not (:ident fieldd))
               *enforce-idents*)
      (throw (ex-info "Returning a field with no :ident" {:metadata-providerable metadata-providerable
                                                          :id                    field-id
                                                          :field                 fieldd})))
    fieldd))
