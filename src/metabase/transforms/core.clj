...
(mu/defn ^:private add-bindings :- Bindings
  [bindings :- Bindings, source :- SourceName, new-bindings :- [:maybe DimensionBindings]]
...
(mu/defn ^:private tableset :- Tableset
  [db-id :- ms/PositiveInt, schema :- [:maybe :string]]
  (table/with-fields
...
(mu/defn apply-transform!
  ...
  [db-id :- ms/PositiveInt, schema :- [:maybe :string], spec :- TransformSpec]
