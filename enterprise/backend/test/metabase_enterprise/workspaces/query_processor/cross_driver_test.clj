(ns ^:mb/driver-tests metabase-enterprise.workspaces.query-processor.cross-driver-test
  "End-to-end read-path coverage for workspace table remapping against real
   ...
   Why this matters: the H2-only string-SQL tests in `middleware_test.clj`
   prove the rewriter emits the right strings. The SQLGlot corpus runner
   proves the grammar layer across 7 dialects. This namespace fills the third
   layer — does the warehouse actually accept and execute the rewritten SQL?

   Driver-specific risks this catches that H2 cannot:
   ...
   Locally with only H2 configured these tests are no-ops (skipped — H2 doesn't
   claim `:transforms/table`); CI exercises them against the configured driver
   matrix."
  (:require
