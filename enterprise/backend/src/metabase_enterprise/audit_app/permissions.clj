(ns metabase-enterprise.audit-app.permissions
  (:require    [metabase-enterprise.audit-db :refer [default-audit-collection-entity-id
                                                     default-audit-db-id]]
        ...
        (when (= change-id audit-collection-id)
          (let [change-permissions! (case type
                                      ...
                                      :write (throw (ex-info (tru
                                                             ...
                                                             {:status-code 400})))
                view-tables         (t2/select Table :db_id (default-audit-db-id) :name [:in (audit-db-view-names)])]
