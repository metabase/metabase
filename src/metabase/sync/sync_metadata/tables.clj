...
(mu/defn ^:private update-table-metadata-if-needed!
  ...
  (let [to-update-keys [:description :database_require_filter :properties]
...
(mu/defn ^:private db->our-metadata :- [:set i/DatabaseMetadataTable]
  ...
  [database :- i/DatabaseInstance]
  (set (t2/select [:model/Table :id :name :schema :description :database_require_filter :properties]
                  :db_id  (u/the-id database)
                  :active true)))
