...
(defn get-or-generate-uuid-nonce!
  "If the setting has been set, returns that - otherwise, generates a random UUID and stores/returns it."
  [setting-name]
  (fn []
    (or (setting/get-parsed-value setting-name)
        (u/prog1 (str (random-uuid))
          (setting/set-parsed-value! setting-name <>)))))
...
(defsetting site-uuid
  ...
  :getter     (get-or-generate-uuid-nonce! :site-uuid)
  :type       :string
  :doc        false)
...
(defsetting site-uuid-for-premium-features-token-checks
  ...
  :setter     :none
  :type       :string
  :getter     (get-or-generate-uuid-nonce! :site-uuid-for-premium-features-token-checks)
  :doc        false)
...
(defsetting site-uuid-for-version-info-fetching
  ...
  :setter     :none
  :type       :string
  :getter     (get-or-generate-uuid-nonce! :site-uuid-for-version-info-fetching))
...
(defsetting site-uuid-for-unsubscribing-url
  ...
  :setter     :none
  :type       :string
  :getter     (get-or-generate-uuid-nonce! :site-uuid-for-unsubscribing-url))
...
(defsetting site-url
  ...
  :getter (fn []
            (try
              (some-> (setting/get-parsed-value :site-url) normalize-site-url)
  ...
  :setter (fn [new-value]
            (let [new-value (some-> new-value normalize-site-url)
              ...
              (setting/set-parsed-value! :site-url new-value))))
...
(defsetting site-locale
  ...
  :visibility :public
  :getter     (fn []
                (let [value (setting/get-parsed-value :site-locale)]
  ...
  :setter     (fn [new-value]
                ...
                (setting/set-parsed-value! :site-locale (some-> new-value i18n/normalized-locale-string))))
...
(defsetting query-caching-max-kb
  ...
  :setter  (fn [new-value]
             ...
             (setting/set-parsed-value! :query-caching-max-kb new-value)))
...
(defsetting application-font
  ...
  :setter (fn [new-value]
              ...
              (setting/set-parsed-value! :application-font new-value)))
...
(defsetting enable-password-login
  ...
  :getter     (fn []
                ...
                (let [v (setting/get-parsed-value :enable-password-login)]
                  (if (and (some? v)
                           (sso-enabled?))
                    v
                    true))))
...
(defsetting source-address-header
  ...
  :getter  (fn [] (some-> (setting/get-parsed-value :source-address-header)
                          u/lower-case-en)))
...
(defsetting redirect-all-requests-to-https
  ...
  :setter     (fn [new-value]
                ...
                (setting/set-parsed-value! :redirect-all-requests-to-https new-value)))
...
(defsetting show-database-syncing-modal
  ...
  :type       :boolean
  :getter     (fn []
                (let [v (setting/get-parsed-value :show-database-syncing-modal)]
                  (if (nil? v)
...
(defn set-uploads-database-id!
  ...
  (if (or (not-handling-api-request?)
    ...
    (setting/set-parsed-value! :uploads-database-id new-id)
    (api/throw-403)))
