...
(deftest publish-test
  (let [topic-name :topic/publish-test]
    (ps.backend/publish! :pubsub.backend/appdb topic-name ["test message"])
...
(deftest batch-publish-test
  (let [topic-name :topic/batch-publish-test]
    (ps.backend/publish! :pubsub.backend/appdb topic-name ["msg-1" "msg-2" "msg-3"])
...
(deftest subscribe-and-receive-test
  (let [topic-name :topic/sub-receive-test
        received   (atom [])]
    (ps.backend/subscribe! :pubsub.backend/appdb topic-name "test-subscriber"
                           (fn [{:keys [messages]}]
                             (swap! received into messages)))
    (ps.backend/publish! :pubsub.backend/appdb topic-name ["hello-appdb"])
    ...
    (ps.backend/unsubscribe! :pubsub.backend/appdb topic-name "test-subscriber")
    ;; cleanup
    (t2/delete! :topic_message :topic_name (name topic-name))))
...
(deftest fan-out-appdb-test
  (let [topic-name :topic/fan-out-appdb
        received-1 (atom [])
        received-2 (atom [])]
    (ps.backend/subscribe! :pubsub.backend/appdb topic-name "sub-1"
                           (fn [{:keys [messages]}]
                             (swap! received-1 into messages)))
    (ps.backend/subscribe! :pubsub.backend/appdb topic-name "sub-2"
                           (fn [{:keys [messages]}]
                             (swap! received-2 into messages)))

    (ps.backend/publish! :pubsub.backend/appdb topic-name ["broadcast"])
    (Thread/sleep 5000)
    ...
    (ps.backend/unsubscribe! :pubsub.backend/appdb topic-name "sub-1")
    (ps.backend/unsubscribe! :pubsub.backend/appdb topic-name "sub-2")
    (t2/delete! :topic_message :topic_name (name topic-name))))

(deftest cleanup-test
  (let [topic-name :topic/cleanup-test]
    (ps.backend/publish! :pubsub.backend/appdb topic-name ["old-message"])
    ...
    (ps.backend/cleanup! :pubsub.backend/appdb topic-name 0)
...
(deftest no-messages-without-subscribers-test
  (let [topic-name :topic/no-sub-test]
    (ps.backend/publish! :pubsub.backend/appdb topic-name ["orphan-msg"])
