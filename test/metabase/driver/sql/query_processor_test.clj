  ...
  (driver/with-driver :h2
    (mt/with-everything-store
      (testing "params from source queries should get passed in to the top-level. Semicolons should be removed"
        (is (= {:query ["SELECT \"source\".*"
                        "FROM (SELECT * FROM some_table WHERE name = ?) AS \"source\""
                        "WHERE (\"source\".\"name\" <> ?) OR (\"source\".\"name\" IS NULL)"]
...
(deftest compile-honeysql-test
  (testing "make sure the generated HoneySQL will compile to the correct SQL"
    (is (= ["INNER JOIN (SELECT * FROM VENUES) AS card ON \"PUBLIC\".\"CHECKINS\".\"VENUE_ID\" = \"card\".\"id\""]
  ...
  (driver/with-driver :h2
    (with-redefs [driver/db-start-of-week   (constantly :monday)
      ...
      (is (= (hsql/call :dateadd
                        (hx/literal "day")
                        (hx/with-database-type-info [::hx/numeric-literal -1] "long")
                        (hsql/call :week (hsql/call :dateadd
                                                    (hx/literal "day")
                                                    (hx/with-database-type-info [::hx/numeric-literal 1] "long")
                                                    :created_at)))
    ...
    (mt/dataset sample-dataset
      (is (= '{:select   [source.PRODUCTS__via__PRODUCT_ID__CATEGORY AS PRODUCTS__via__PRODUCT_ID__CATEGORY
               ...
               :where    [((source.PEOPLE__via__USER_ID__SOURCE = ?) OR (source.PEOPLE__via__USER_ID__SOURCE = ?))
                          ...
                          AND
                          (source.CREATED_AT >= PARSEDATETIME (YEAR (DATEADD ("year" -2 NOW ())) "yyyy"))
                          AND
