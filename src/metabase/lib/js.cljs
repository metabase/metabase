...
(defn ^:export source-table-or-card-id
  ...
  > **Code health:** Smelly. This is exposing too much about cards and sources. Its callers will likely have to be
...
(defn ^:export join-fields
  ...
  > **Code health:** Smelly. It's fine to call this, but it's one of the few places that returns field refs directly.
  These should probably be returned as columns instead."
  [a-join]
...
(defn ^:export with-join-fields
  ...
  This can either be a list of fields

  > **Code health:** Smelly. It's fine to call this, but it's one spot where field refs appear in the API. This should
  be refactored to accept columns rather than refs."
  [a-join new-fields]
...
(defn ^:export with-native-query
  ...
  > **Code health:** Deprecated. This is never called and should be removed if it's not needed."
  [a-query inner-query]
