...
(mr/def ::DatabaseMetadata
  [:map
   [:tables [:set-of DatabaseMetadataTable]]
...
(mr/def ::TableMetadataField
  [:map
   ...
   [:database-position ::lib.schema.common/positive-int]
   [:position                   {:optional true} ::lib.schema.common/positive-int]
   ...
   [:nested-fields              {:optional true} [:set-of [:ref ::TableMetadataField]]]
   [:json-unfolding             {:optional true} :boolean]
   [:nfc-path                   {:optional true} [:any]]
   [:custom                     {:optional true} {:any :any}]
...
(mr/def ::TableMetadata
  [:map
   [:name                         ::lib.schema.common/non-blank-string]
   [:schema                       [:maybe ::lib.schema.common/non-blank-string]]
   [:fields                       [:set-of [:ref ::TableMetadataField]]]
   [:description {:optional true} [:maybe :string]]])
...
(comment
  (def NestedFCMetadata
    ...
    [:maybe [:set-of [:ref ::TableMetadataField]]]))
...
(mr/def ::FKMetadata
  [:maybe [:set-of [:ref ::FKMetadataEntry]]])
...
(mr/def ::GlobalFingerprint
  [:map
   [:distinct-count {:optional true} :integer]
