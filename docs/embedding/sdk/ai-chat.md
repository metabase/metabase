...
- **Strip links returned by the agent**. The agent text may include links pointing back to the host Metabase. Those links require an authenticated Metabase session, so people won't be able to view the links.
- **The chart components accept different props depending on `drills`.** Both the `Chart` on each chart message and the `CurrentChart` returned by the hook are pre-wired with `questionId`, `token`, and `query`, so you shouldn't pass those props. With the default `drills={false}`, the component renders [`StaticQuestion`](./questions.md#staticquestion) and accepts its remaining props. With `drills={true}`, the component renders [`InteractiveQuestion`](./questions.md#interactivequestion) and accepts its remaining props. Most apps will set `drills` and `height`. `height="100%"` only works if the parent has a defined height; the dedicated-chart snippet below sets `height: 600` on the outer flex container for that reason.
