...
(deftest ^:parallel prevent-metabase-transform-data-source-change-via-api-test
  (testing "PUT /api/table/:id"
    (testing "Cannot change data_source from metabase-transform"
      (mt/with-temp [:model/Table table {:data_source "metabase-transform"}]
        (testing "to another value"
          (is (= "Cannot change data_source from metabase-transform"
                 (mt/user-http-request :crowberto :put 400 (format "table/%d" (u/the-id table))
                                       {:data_source "some-other-source"}))))
        (testing "to nil"
          (is (= "Cannot change data_source from metabase-transform"
                 (mt/user-http-request :crowberto :put 400 (format "table/%d" (u/the-id table))
                                       {:data_source nil}))))
        (testing "but can update other fields"
          ...
          (is (= "metabase-transform" (t2/select-one-fn :data_source :model/Table :id (:id table)))))))

    (testing "Cannot change data_source to metabase-transform"
      (mt/with-temp [:model/Table table {:data_source "some-source"}]
        (testing "from another value"
          (is (= "Cannot set data_source to metabase-transform"
                 (mt/user-http-request :crowberto :put 400 (format "table/%d" (u/the-id table))
                                       {:data_source "metabase-transform"}))))
        (testing "but can change to other non-metabase-transform values"
          (mt/user-http-request :crowberto :put 200 (format "table/%d" (u/the-id table))
                                {:data_source "source-b"})
          (is (= "source-b" (t2/select-one-fn :data_source :model/Table :id (:id table)))))))

    (testing "Cannot set data_source to metabase-transform from nil"
      (mt/with-temp [:model/Table table {:data_source nil}]
        (is (= "Cannot set data_source to metabase-transform"
               (mt/user-http-request :crowberto :put 400 (format "table/%d" (u/the-id table))
                                     {:data_source "metabase-transform"})))))))

#_(deftest ^:parallel prevent-metabase-transform-data-source-via-bulk-update-test
    (testing "PUT /api/table/"
      (testing "Cannot change data_source to metabase-transform via bulk update"
        (mt/with-temp [:model/Table table1 {:data_source "source-a"}
                       :model/Table table2 {:data_source "source-b"}]
          (is (= "Cannot set data_source to metabase-transform"
                 (mt/user-http-request :crowberto :put 400 "table"
                                       {:ids [(:id table1) (:id table2)]
                                        :data_source "metabase-transform"})))))

      (testing "Cannot change data_source from metabase-transform via bulk update"
        (mt/with-temp [:model/Table table1 {:data_source "metabase-transform"}
                       :model/Table table2 {:data_source "metabase-transform"}]
          (is (= "Cannot change data_source from metabase-transform"
                 (mt/user-http-request :crowberto :put 400 "table"
                                       {:ids [(:id table1) (:id table2)]
                                        :data_source "some-other-source"})))))))

#_(deftest ^:parallel prevent-metabase-transform-data-source-via-bulk-edit-test
    (testing "POST /api/table/edit"
      (testing "Cannot change data_source to metabase-transform via bulk edit"
        (mt/with-temp [:model/Table table1 {:data_source "source-a"}
                       :model/Table table2 {:data_source "source-b"}]
          (is (= "Cannot set data_source to metabase-transform"
                 (mt/user-http-request :crowberto :post 400 "table/edit"
                                       {:table_ids [(:id table1) (:id table2)]
                                        :data_source "metabase-transform"})))))

      (testing "Cannot change data_source from metabase-transform via bulk edit"
        (mt/with-temp [:model/Table table1 {:data_source "metabase-transform"}
                       :model/Table table2 {:data_source "metabase-transform"}]
          (is (= "Cannot change data_source from metabase-transform"
                 (mt/user-http-request :crowberto :post 400 "table/edit"
