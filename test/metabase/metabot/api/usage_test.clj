(ns metabase.metabot.api.usage-test
  (:require
...
(deftest get-instance-limit-test
  (testing "GET /api/metabot/usage/instance"
    (testing "requires superuser"
      (is (= "You don't have permissions to do that."
             (mt/user-http-request :rasta :get 403 "metabot/usage/instance"))))
    (testing "returns max_usage: null when no limit is set"
      (t2/delete! :model/MetabotInstanceLimit :tenant_id nil)
      (is (= {:max_usage nil}
             (mt/user-http-request :crowberto :get 200 "metabot/usage/instance"))))
...
(deftest put-instance-limit-test
  (testing "PUT /api/metabot/usage/instance"
    (testing "requires superuser"
      (is (= "You don't have permissions to do that."
             (mt/user-http-request :rasta :put 403 "metabot/usage/instance" {:max_usage 500}))))
    (testing "creates a new limit when none exists"
      (t2/delete! :model/MetabotInstanceLimit :tenant_id nil)
      (is (= {:max_usage 500}
             (mt/user-http-request :crowberto :put 200 "metabot/usage/instance" {:max_usage 500})))
      (t2/delete! :model/MetabotInstanceLimit :tenant_id nil))
    (testing "updates an existing limit"
      (mt/with-temp [:model/MetabotInstanceLimit _ {:tenant_id nil :max_usage 1000}]
        (is (= {:max_usage 2000}
               (mt/user-http-request :crowberto :put 200 "metabot/usage/instance" {:max_usage 2000})))))
    (testing "removes the limit when max_usage is null"
      (mt/with-temp [:model/MetabotInstanceLimit _ {:tenant_id nil :max_usage 1000}]
        (is (= {:max_usage nil}
               (mt/user-http-request :crowberto :put 200 "metabot/usage/instance" {:max_usage nil})))
        (is (nil? (t2/select-one :model/MetabotInstanceLimit :tenant_id nil)))))))

;;; ------------------------------------------------- Tenant limits --------------------------------------------------

(deftest get-tenant-limits-test
  (testing "GET /api/metabot/usage/tenant"
    (testing "requires superuser"
      (is (= "You don't have permissions to do that."
             (mt/user-http-request :rasta :get 403 "metabot/usage/tenant"))))
    (testing "returns all tenant limits as a flat list"
      (mt/with-temp [:model/Tenant {tenant-a :id} {}
        ...
        (let [limits    (mt/user-http-request :crowberto :get 200 "metabot/usage/tenant")
...
(deftest get-tenant-limit-test
  (testing "GET /api/metabot/usage/tenant/:tenant-id"
    (testing "requires superuser"
      (is (= "You don't have permissions to do that."
             (mt/user-http-request :rasta :get 403 "metabot/usage/tenant/1"))))
      ...
      (mt/with-temp [:model/Tenant {tenant-id :id} {}]
        (is (= {:tenant_id tenant-id :max_usage nil}
               (mt/user-http-request :crowberto :get 200
                                     (format "metabot/usage/tenant/%d" tenant-id))))))
...
(deftest put-tenant-limit-test
  (testing "PUT /api/metabot/usage/tenant/:tenant-id"
    (testing "requires superuser"
      (is (= "You don't have permissions to do that."
             (mt/user-http-request :rasta :put 403 "metabot/usage/tenant/1" {:max_usage 500}))))
...
(deftest get-group-limits-test
  (testing "GET /api/metabot/usage/group"
    (testing "requires superuser"
      (is (= "You don't have permissions to do that."
             (mt/user-http-request :rasta :get 403 "metabot/usage/group"))))
    (testing "returns all group limits as a flat list"
      (mt/with-temp [:model/PermissionsGroup {group-a :id} {:name "Limit Group A"}
        ...
        (let [limits   (mt/user-http-request :crowberto :get 200 "metabot/usage/group")
...
(deftest get-group-limit-test
  (testing "GET /api/metabot/usage/group/:group-id"
    (testing "requires superuser"
      (is (= "You don't have permissions to do that."
             (mt/user-http-request :rasta :get 403 "metabot/usage/group/1"))))
    (testing "returns group_id and max_usage: null when no limit is set"
      (mt/with-temp [:model/PermissionsGroup {group-id :id} {:name "No Limit Group"}]
        (is (= {:group_id group-id :max_usage nil}
               (mt/user-http-request :crowberto :get 200
                                     (format "metabot/usage/group/%d" group-id))))))
...
(deftest put-group-limit-test
  (testing "PUT /api/metabot/usage/group/:group-id"
    (testing "requires superuser"
      (is (= "You don't have permissions to do that."
             (mt/user-http-request :rasta :put 403 "metabot/usage/group/1" {:max_usage 500}))))
    (testing "creates a new limit"
