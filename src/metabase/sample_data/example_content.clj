(ns metabase.sample-data.example-content
  ...
  reference still lines up. When we recreate the content on an existing instance the tables are not
  empty, so we insert the same rows but remap each id: simple FK columns by lookup, and the
  Database/Table/Field/Card ids embedded in card and dashcard query blobs via the serdes export walkers
  bound to numeric id maps."
  (:require
   [clojure.edn :as edn]
