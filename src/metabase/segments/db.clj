(ns metabase.segments.db
  ...
  The queries below follow [[::opts]]; queries that do not fit it live in the segments-only section at the bottom of
  this namespace."
  (:require
...
(mr/def ::filters
  "Which Segments a query applies to. Keys mirror the columns of `segment`: a scalar matches that value and a set
  matches any of its values."
  [:map {:closed true}
   [:id       {:optional true} [:or ::lib.schema.id/segment [:set ::lib.schema.id/segment]]]
...
(mr/def ::opts
  ...
  [:merge
   ::filters
   [:map {:closed true}
    [:columns  {:optional true} [:sequential ::segments.schema/segment.column]]
    [:order-by {:optional true} [:sequential [:or
                                              ::segments.schema/segment.column
                                              [:tuple ::segments.schema/segment.column [:enum :asc :desc]]]]]
...
(defn- ->model
  [columns]
  (u.query/model-with-columns :model/Segment columns))
...
(mu/defn select-segments :- [:sequential ::segments.schema/segment.partial]
  "The Segments matching `opts`."
  ([]
   (select-segments nil))
  ([{:keys [columns] :as opts} :- [:maybe ::opts]]
   (apply t2/select (->model columns) (->args opts))))

(mu/defn select-one-segment :- [:maybe ::segments.schema/segment.partial]
  "The first Segment matching `opts`, or nil."
  ([]
   (select-one-segment nil))
  ([{:keys [columns] :as opts} :- [:maybe ::opts]]
   (apply t2/select-one (->model columns) (->args opts))))

(mu/defn select-segment-pks :- [:set ::lib.schema.id/segment]
  "The ids of the Segments matching `opts`."
  ([]
   (select-segment-pks nil))
  ([opts :- [:maybe ::opts]]
   (or (apply t2/select-pks-set :model/Segment (->args opts)) #{})))

;;; ------------------------------------------------ Writes -------------------------------------------------

(mu/defn insert-segment! :- ::segments.schema/segment
  "Insert the Segment `row` and return the inserted instance."
  [row :- ::segments.schema/segment.create]
  (t2/insert-returning-instance! :model/Segment row))

(mu/defn update-segments! :- :int
  "Apply `changes` to every Segment matching `opts`, returning the number updated."
  [opts    :- [:maybe ::opts]
   changes :- ::segments.schema/segment.update]
  (apply t2/update! :model/Segment (conj (->kv-args opts) changes)))

;;; ------------------------------------- Queries used only by the segments module -------------------------------------
...
(mu/defn select-table-database-id :- [:maybe ::lib.schema.id/database]
