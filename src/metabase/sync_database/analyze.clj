...
(defn analyze-table-data-shape!
  ...
  (let [new-field-ids (set (db/sel-idfield/Field, :table_id table-id, :visibility_type [not= "retired"], :last_analyzed nil))]
...
(defn analyze-data-shape-for-tables!
  ...
  (let [start-time-ns         (System/nanoTime)
        tables                (db/sel table/Table, :db_id database-id, :active true)
